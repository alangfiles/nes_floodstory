;
; File generated by cc65 v 2.18 - N/A
;
	.fopt		compiler,"cc65 v 2.18 - N/A"
	.setcpu		"6502"
	.smart		on
	.autoimport	on
	.case		on
	.debuginfo	off
	.importzp	sp, sreg, regsave, regbank
	.importzp	tmp1, tmp2, tmp3, tmp4, ptr1, ptr2, ptr3, ptr4
	.macpack	longbranch
	.forceimport	__STARTUP__
	.import		_pal_bg
	.import		_pal_spr
	.import		_ppu_wait_nmi
	.import		_ppu_off
	.import		_ppu_on_all
	.import		_oam_clear
	.import		_oam_meta_spr
	.import		_pad_poll
	.import		_bank_spr
	.import		_bank_bg
	.import		_vram_adr
	.import		_vram_put
	.import		_memcpy
	.import		_set_vram_buffer
	.import		_multi_vram_buffer_horz
	.import		_clear_vram_buffer
	.import		_get_pad_new
	.import		_set_scroll_x
	.import		_set_scroll_y
	.import		_get_ppu_addr
	.import		_set_data_pointer
	.import		_set_mt_pointer
	.import		_buffer_4_mt
	.import		_flush_vram_update2
	.export		_bankLevel
	.export		_bankBuffer
	.export		_banked_call
	.export		_bank_push
	.export		_bank_pop
	.import		_set_prg_bank
	.import		_get_prg_bank
	.import		_set_chr_bank_0
	.import		_set_chr_bank_1
	.import		_set_mirroring
	.export		_direction
	.export		_direction_y
	.export		_arg1
	.export		_arg2
	.export		_pad1
	.export		_pad1_new
	.export		_pad1_state
	.export		_char_state
	.export		_song
	.export		_sound
	.export		_game_mode
	.export		_selected_stage
	.export		_temp_x
	.export		_temp_y
	.export		_temp_sprite
	.export		_old_x
	.export		_old_y
	.export		_player_in_air
	.export		_player_is_sliding
	.export		_player_is_running
	.export		_player_on_ladder
	.export		_player_on_ladder_pose
	.export		_player_on_ladder_top
	.export		_player_in_hitstun
	.export		_falling_down
	.export		_multi_jump
	.export		_multi_jump_max
	.export		_short_jump_count
	.export		_player_shooting
	.export		_projectile_cooldown
	.export		_projectile_index
	.export		_hit_direction
	.export		_animation_frame_counter
	.export		_current_animation_frame
	.export		_current_animation_ptr
	.export		_projectiles_list
	.export		_projectiles_x
	.export		_projectiles_y
	.export		_eject_L
	.export		_eject_R
	.export		_eject_D
	.export		_eject_U
	.export		_collision_L
	.export		_collision_R
	.export		_collision_U
	.export		_collision_D
	.export		_coordinates
	.export		_location_with_scroll
	.export		_collision
	.export		_map_loaded
	.export		_scrolling_direction
	.export		_metatile_colision_map
	.export		_nametable_to_load
	.export		_offset
	.export		_address
	.export		_x
	.export		_y
	.export		_nt
	.export		_index
	.export		_map
	.export		_scroll_x
	.export		_scroll_y
	.export		_max_rooms
	.export		_max_scroll
	.export		_room_to_load
	.export		_temp_room
	.export		_current_stage
	.export		_current_level
	.export		_scroll_count
	.export		_pseudo_scroll_x
	.export		_c_map
	.export		_c_map2
	.export		_temp1
	.export		_temp2
	.export		_temp3
	.export		_l_scroll_frames
	.export		_r_scroll_frames
	.export		_Player1
	.export		_Generic
	.export		_Generic2
	.export		_wram_array
	.export		_noah_box
	.export		_noah_slide
	.export		_projectile
	.export		_topleft
	.export		_bottomleft
	.export		_animate_playerstandleft_data
	.export		_animate_playerstandright_data
	.export		_animate_playerrun1left_data
	.export		_animate_playerrun2left_data
	.export		_animate_playerrun3left_data
	.export		_animate_playerrun1right_data
	.export		_animate_playerrun2right_data
	.export		_animate_playerrun3right_data
	.export		_animate_playerjumpleft_data
	.export		_animate_playerjumpright_data
	.export		_animate_slideleft_data
	.export		_animate_slideright_data
	.export		_animate_playerclimb1_data
	.export		_animate_playerclimb2_data
	.export		_animate_laddertop_data
	.export		_animate_playerstandshootleft_data
	.export		_animate_playerstandshootright_data
	.export		_bg_collision_sub
	.export		_bg_collision_fast
	.export		_bg_coll_L
	.export		_bg_coll_R
	.export		_bg_coll_ladder
	.export		_bg_coll_ladder_top_at_player
	.export		_bg_coll_ladder_top_under_player
	.export		_bg_coll_U
	.export		_bg_coll_D
	.export		_movement
	.export		_check_player_colision
	.export		_update_player_animation
	.export		_bank0_player_movement
	.export		_bank0_draw_player_sprite
	.export		_projectile_movement
	.export		_function_bank1
	.export		_stage1_level0_0
	.export		_stage1_level0_1
	.export		_stage1_level0_2
	.export		_stage1_level0_3
	.export		_stage1_level0_4
	.export		_stage1_level0_5
	.export		_stage1_level0_6
	.export		_stage1_level0_7
	.export		_stage1_levels_list
	.export		_stage1_levels
	.export		_stage1_max_rooms
	.export		_stage1_offsets
	.export		_stage1_bg_palette
	.export		_stage1_metatiles
	.export		_starting_x_place
	.export		_starting_y_place
	.export		_stage1_level_0_enemies
	.export		_stage1_Enemy_list
	.export		_stage1_level_0_entities
	.export		_stage1_entity_list
	.export		_stage2_level0_0
	.export		_stage2_level0_1
	.export		_stage2_levels_list
	.export		_stage2_levels
	.export		_stage2_max_rooms
	.export		_stage2_offsets
	.export		_stage2_bg_palette
	.export		_stage2_metatiles
	.export		_stage2_palette_sp
	.export		_metatile
	.export		_stage2_starting_x_place
	.export		_stage2_starting_y_place
	.export		_stage2_level_0_enemies
	.export		_stage2_Enemy_list
	.export		_stage2_level_0_entities
	.export		_stage2_entity_list
	.export		_stage3_levels
	.export		_stage3_max_rooms
	.export		_stage3_offsets
	.export		_stage3_bg_palette
	.export		_stage3_palette_sp
	.export		_stage3_metatiles
	.export		_stage4_levels
	.export		_stage4_max_rooms
	.export		_stage4_offsets
	.export		_stage4_bg_palette
	.export		_stage4_palette_sp
	.export		_stage4_metatiles
	.export		_stage5_levels
	.export		_stage5_max_rooms
	.export		_stage5_offsets
	.export		_stage5_bg_palette
	.export		_stage5_palette_sp
	.export		_stage5_metatiles
	.export		_stage_table
	.export		_stage_bg_palettes
	.export		_stage_metatiles
	.export		_levels_per_stage
	.export		_function_bank2
	.export		_drawMetatileBlock
	.export		_draw_screen_L
	.export		_draw_screen_R
	.export		_new_cmap
	.export		_prep_scroll_screen
	.export		_handle_scrolling
	.export		_titletiled_0
	.export		_bank2_load_title
	.export		_gameovertiled_0
	.export		_bank2_load_gameover
	.export		_bank2_load_room
	.export		_bank2_scroll_screen
	.export		_iceworld_0
	.export		_iceworld_1
	.export		_iceworld_2
	.export		_iceworld_3
	.export		_iceworld_4
	.export		_iceworld_5
	.export		_iceworld_6
	.export		_iceworld_7
	.export		_iceworld_8
	.export		_iceworld_9
	.export		_iceworld_10
	.export		_iceworld_11
	.export		_iceworld_12
	.export		_iceworld_13
	.export		_iceworld_14
	.export		_iceworld_15
	.export		_iceworld_16
	.export		_iceworld_17
	.export		_iceworld_18
	.export		_iceworld_19
	.export		_stage3_levels_list
	.export		_stage3_starting_x_place
	.export		_stage3_starting_y_place
	.export		_stage3_level_0_enemies
	.export		_stage3_Enemy_list
	.export		_stage3_level_0_entities
	.export		_stage3_entity_list
	.export		_camelworld_0
	.export		_camelworld_1
	.export		_camelworld_2
	.export		_camelworld_3
	.export		_camelworld_4
	.export		_camelworld_5
	.export		_camelworld_6
	.export		_camelworld_7
	.export		_camelworld_8
	.export		_camelworld_9
	.export		_camelworld_10
	.export		_camelworld_11
	.export		_camelworld_12
	.export		_camelworld_13
	.export		_camelworld_14
	.export		_camelworld_15
	.export		_camelworld_16
	.export		_camelworld_17
	.export		_camelworld_18
	.export		_camelworld_19
	.export		_camelworld_20
	.export		_camelworld_21
	.export		_camelworld_22
	.export		_camelworld_23
	.export		_stage4_levels_list
	.export		_stage4_starting_x_place
	.export		_stage4_starting_y_place
	.export		_stage4_level_0_enemies
	.export		_stage4_Enemy_list
	.export		_stage4_level_0_entities
	.export		_stage4_entity_list
	.export		_function_bank3
	.export		_leviathanworld_0
	.export		_leviathanworld_1
	.export		_leviathanworld_2
	.export		_leviathanworld_3
	.export		_leviathanworld_4
	.export		_leviathanworld_5
	.export		_leviathanworld_6
	.export		_leviathanworld_7
	.export		_leviathanworld_8
	.export		_leviathanworld_9
	.export		_leviathanworld_10
	.export		_leviathanworld_11
	.export		_leviathanworld_12
	.export		_leviathanworld_13
	.export		_leviathanworld_14
	.export		_leviathanworld_15
	.export		_leviathanworld_16
	.export		_leviathanworld_17
	.export		_leviathanworld_18
	.export		_leviathanworld_19
	.export		_leviathanworld_20
	.export		_leviathanworld_21
	.export		_leviathanworld_22
	.export		_stage5_levels_list
	.export		_stage5_starting_x_place
	.export		_stage5_starting_y_place
	.export		_stage5_level_0_enemies
	.export		_stage5_Enemy_list
	.export		_stage5_level_0_entities
	.export		_stage5_entity_list
	.export		_function_bank4
	.export		_function_bank5
	.export		_function_bank6
	.export		_palette_bg
	.export		_palette_sp
	.export		_load_level_select
	.export		_update_level_select_display
	.export		_main

.segment	"DATA"

_projectiles_list:
	.byte	$FF
	.byte	$FF
	.byte	$FF
	.byte	$FF
_projectiles_x:
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
_projectiles_y:
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
_room_to_load:
	.byte	$00
_current_stage:
	.byte	$00
_current_level:
	.byte	$00
_Player1:
	.word	$4000
	.word	$9400
	.res	6,$00
_stage1_levels:
	.addr	_stage1_level0_0
	.addr	_stage1_level0_1
	.addr	_stage1_level0_2
	.addr	_stage1_level0_3
	.addr	_stage1_level0_4
	.addr	_stage1_level0_5
	.addr	_stage1_level0_6
	.addr	_stage1_level0_7
_stage2_levels:
	.addr	_stage2_level0_0
	.addr	_stage2_level0_1
_stage3_levels:
	.addr	_iceworld_0
	.addr	_iceworld_1
	.addr	_iceworld_2
	.addr	_iceworld_3
	.addr	_iceworld_4
	.addr	_iceworld_5
	.addr	_iceworld_6
	.addr	_iceworld_7
	.addr	_iceworld_8
	.addr	_iceworld_9
	.addr	_iceworld_10
	.addr	_iceworld_11
	.addr	_iceworld_12
	.addr	_iceworld_13
	.addr	_iceworld_14
	.addr	_iceworld_15
	.addr	_iceworld_16
	.addr	_iceworld_17
	.addr	_iceworld_18
	.addr	_iceworld_19
_stage4_levels:
	.addr	_camelworld_0
	.addr	_camelworld_1
	.addr	_camelworld_2
	.addr	_camelworld_3
	.addr	_camelworld_4
	.addr	_camelworld_5
	.addr	_camelworld_6
	.addr	_camelworld_7
	.addr	_camelworld_8
	.addr	_camelworld_9
	.addr	_camelworld_10
	.addr	_camelworld_11
	.addr	_camelworld_12
	.addr	_camelworld_13
	.addr	_camelworld_14
	.addr	_camelworld_15
	.addr	_camelworld_16
	.addr	_camelworld_17
	.addr	_camelworld_18
	.addr	_camelworld_19
	.addr	_camelworld_20
	.addr	_camelworld_21
	.addr	_camelworld_22
	.addr	_camelworld_23
_stage5_levels:
	.addr	_leviathanworld_0
	.addr	_leviathanworld_1
	.addr	_leviathanworld_2
	.addr	_leviathanworld_3
	.addr	_leviathanworld_4
	.addr	_leviathanworld_5
	.addr	_leviathanworld_6
	.addr	_leviathanworld_7
	.addr	_leviathanworld_8
	.addr	_leviathanworld_9
	.addr	_leviathanworld_10
	.addr	_leviathanworld_11
	.addr	_leviathanworld_12
	.addr	_leviathanworld_13
	.addr	_leviathanworld_14
	.addr	_leviathanworld_15
	.addr	_leviathanworld_16
	.addr	_leviathanworld_17
	.addr	_leviathanworld_18
	.addr	_leviathanworld_19
	.addr	_leviathanworld_20
	.addr	_leviathanworld_21
	.addr	_leviathanworld_22
_stage_table:
	.addr	_stage1_levels
	.addr	_stage2_levels
	.addr	_stage3_levels
	.addr	_stage4_levels
	.addr	_stage5_levels
_stage_bg_palettes:
	.addr	_stage1_bg_palette
	.addr	_stage2_bg_palette
	.addr	_stage3_bg_palette
	.addr	_stage4_bg_palette
	.addr	_stage5_bg_palette
_stage_metatiles:
	.addr	_stage1_metatiles
	.addr	_stage2_metatiles
	.addr	_stage3_metatiles
	.addr	_stage4_metatiles
	.addr	_stage5_metatiles

.segment	"RODATA"

_metatile_colision_map:
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$00
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$01
	.byte	$02
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
.segment	"BANK0"
_noah_box:
	.byte	$00
	.byte	$00
	.byte	$4F
	.byte	$00
	.byte	$00
	.byte	$08
	.byte	$4F
	.byte	$00
	.byte	$00
	.byte	$10
	.byte	$3F
	.byte	$80
	.byte	$06
	.byte	$00
	.byte	$4F
	.byte	$00
	.byte	$06
	.byte	$08
	.byte	$4F
	.byte	$00
	.byte	$06
	.byte	$10
	.byte	$3F
	.byte	$80
	.byte	$80
_noah_slide:
	.byte	$00
	.byte	$0C
	.byte	$4F
	.byte	$00
	.byte	$06
	.byte	$0C
	.byte	$4F
	.byte	$00
	.byte	$0C
	.byte	$0C
	.byte	$4F
	.byte	$00
	.byte	$12
	.byte	$0C
	.byte	$4F
	.byte	$00
	.byte	$80
_projectile:
	.byte	$00
	.byte	$00
	.byte	$5F
	.byte	$00
	.byte	$80
_topleft:
	.byte	$00
	.byte	$00
	.byte	$50
	.byte	$00
	.byte	$80
_bottomleft:
	.byte	$00
	.byte	$00
	.byte	$51
	.byte	$00
	.byte	$80
_animate_playerstandleft_data:
	.byte	$FE
	.byte	$FE
	.byte	$00
	.byte	$00
	.byte	$06
	.byte	$FE
	.byte	$01
	.byte	$00
	.byte	$FE
	.byte	$06
	.byte	$10
	.byte	$00
	.byte	$06
	.byte	$06
	.byte	$11
	.byte	$00
	.byte	$FE
	.byte	$0E
	.byte	$20
	.byte	$00
	.byte	$06
	.byte	$0E
	.byte	$21
	.byte	$00
	.byte	$02
	.byte	$FF
	.byte	$0B
	.byte	$02
	.byte	$80
_animate_playerstandright_data:
	.byte	$06
	.byte	$FE
	.byte	$00
	.byte	$40
	.byte	$FE
	.byte	$FE
	.byte	$01
	.byte	$40
	.byte	$06
	.byte	$06
	.byte	$10
	.byte	$40
	.byte	$FE
	.byte	$06
	.byte	$11
	.byte	$40
	.byte	$06
	.byte	$0E
	.byte	$20
	.byte	$40
	.byte	$FE
	.byte	$0E
	.byte	$21
	.byte	$40
	.byte	$02
	.byte	$FF
	.byte	$0B
	.byte	$42
	.byte	$80
_animate_playerrun1left_data:
	.byte	$0E
	.byte	$0E
	.byte	$46
	.byte	$00
	.byte	$FE
	.byte	$FE
	.byte	$1A
	.byte	$00
	.byte	$06
	.byte	$FE
	.byte	$1B
	.byte	$00
	.byte	$FE
	.byte	$06
	.byte	$32
	.byte	$00
	.byte	$06
	.byte	$06
	.byte	$33
	.byte	$00
	.byte	$FE
	.byte	$0E
	.byte	$42
	.byte	$00
	.byte	$06
	.byte	$0E
	.byte	$43
	.byte	$00
	.byte	$02
	.byte	$FF
	.byte	$0B
	.byte	$02
	.byte	$80
_animate_playerrun2left_data:
	.byte	$FE
	.byte	$FE
	.byte	$00
	.byte	$00
	.byte	$06
	.byte	$FE
	.byte	$01
	.byte	$00
	.byte	$FE
	.byte	$06
	.byte	$30
	.byte	$00
	.byte	$06
	.byte	$06
	.byte	$31
	.byte	$00
	.byte	$FE
	.byte	$0E
	.byte	$40
	.byte	$00
	.byte	$06
	.byte	$0E
	.byte	$41
	.byte	$00
	.byte	$02
	.byte	$FF
	.byte	$0B
	.byte	$02
	.byte	$80
_animate_playerrun3left_data:
	.byte	$0E
	.byte	$0D
	.byte	$46
	.byte	$00
	.byte	$FE
	.byte	$FE
	.byte	$1A
	.byte	$00
	.byte	$06
	.byte	$FE
	.byte	$1B
	.byte	$00
	.byte	$FE
	.byte	$06
	.byte	$34
	.byte	$00
	.byte	$06
	.byte	$06
	.byte	$35
	.byte	$00
	.byte	$FE
	.byte	$0E
	.byte	$44
	.byte	$00
	.byte	$06
	.byte	$0E
	.byte	$45
	.byte	$00
	.byte	$02
	.byte	$FF
	.byte	$0B
	.byte	$02
	.byte	$80
_animate_playerrun1right_data:
	.byte	$06
	.byte	$FE
	.byte	$1A
	.byte	$40
	.byte	$FE
	.byte	$FE
	.byte	$1B
	.byte	$40
	.byte	$06
	.byte	$06
	.byte	$32
	.byte	$40
	.byte	$FE
	.byte	$06
	.byte	$33
	.byte	$40
	.byte	$06
	.byte	$0E
	.byte	$42
	.byte	$40
	.byte	$FE
	.byte	$0E
	.byte	$43
	.byte	$40
	.byte	$F6
	.byte	$0E
	.byte	$46
	.byte	$40
	.byte	$02
	.byte	$FF
	.byte	$0B
	.byte	$42
	.byte	$80
_animate_playerrun2right_data:
	.byte	$06
	.byte	$FE
	.byte	$00
	.byte	$40
	.byte	$FE
	.byte	$FE
	.byte	$01
	.byte	$40
	.byte	$06
	.byte	$06
	.byte	$30
	.byte	$40
	.byte	$FE
	.byte	$06
	.byte	$31
	.byte	$40
	.byte	$06
	.byte	$0E
	.byte	$40
	.byte	$40
	.byte	$FE
	.byte	$0E
	.byte	$41
	.byte	$40
	.byte	$02
	.byte	$FF
	.byte	$0B
	.byte	$42
	.byte	$80
_animate_playerrun3right_data:
	.byte	$06
	.byte	$0D
	.byte	$46
	.byte	$40
	.byte	$FE
	.byte	$FE
	.byte	$1A
	.byte	$00
	.byte	$06
	.byte	$FE
	.byte	$1B
	.byte	$00
	.byte	$06
	.byte	$06
	.byte	$34
	.byte	$40
	.byte	$FE
	.byte	$06
	.byte	$35
	.byte	$40
	.byte	$06
	.byte	$0E
	.byte	$44
	.byte	$40
	.byte	$FE
	.byte	$0E
	.byte	$45
	.byte	$40
	.byte	$02
	.byte	$FF
	.byte	$0B
	.byte	$42
	.byte	$80
_animate_playerjumpleft_data:
	.byte	$FE
	.byte	$FE
	.byte	$27
	.byte	$00
	.byte	$06
	.byte	$FE
	.byte	$28
	.byte	$00
	.byte	$FE
	.byte	$06
	.byte	$37
	.byte	$00
	.byte	$06
	.byte	$06
	.byte	$38
	.byte	$00
	.byte	$FE
	.byte	$0E
	.byte	$47
	.byte	$00
	.byte	$06
	.byte	$0E
	.byte	$48
	.byte	$00
	.byte	$06
	.byte	$16
	.byte	$58
	.byte	$00
	.byte	$02
	.byte	$FF
	.byte	$0B
	.byte	$02
	.byte	$80
_animate_playerjumpright_data:
	.byte	$06
	.byte	$FE
	.byte	$27
	.byte	$40
	.byte	$FE
	.byte	$FE
	.byte	$28
	.byte	$40
	.byte	$06
	.byte	$06
	.byte	$37
	.byte	$40
	.byte	$FE
	.byte	$06
	.byte	$38
	.byte	$40
	.byte	$06
	.byte	$0E
	.byte	$47
	.byte	$40
	.byte	$FE
	.byte	$0E
	.byte	$48
	.byte	$40
	.byte	$FE
	.byte	$16
	.byte	$58
	.byte	$40
	.byte	$02
	.byte	$FF
	.byte	$0B
	.byte	$42
	.byte	$80
_animate_slideleft_data:
	.byte	$EE
	.byte	$13
	.byte	$E6
	.byte	$00
	.byte	$F6
	.byte	$0B
	.byte	$A6
	.byte	$00
	.byte	$FE
	.byte	$0B
	.byte	$B6
	.byte	$00
	.byte	$06
	.byte	$0C
	.byte	$C6
	.byte	$00
	.byte	$0E
	.byte	$0D
	.byte	$D6
	.byte	$00
	.byte	$F6
	.byte	$13
	.byte	$F6
	.byte	$00
	.byte	$FE
	.byte	$13
	.byte	$3E
	.byte	$00
	.byte	$06
	.byte	$14
	.byte	$4E
	.byte	$00
	.byte	$0E
	.byte	$15
	.byte	$4F
	.byte	$00
	.byte	$F7
	.byte	$0C
	.byte	$0B
	.byte	$02
	.byte	$80
_animate_slideright_data:
	.byte	$16
	.byte	$13
	.byte	$E6
	.byte	$40
	.byte	$0E
	.byte	$0B
	.byte	$A6
	.byte	$40
	.byte	$06
	.byte	$0B
	.byte	$B6
	.byte	$40
	.byte	$FE
	.byte	$0C
	.byte	$C6
	.byte	$40
	.byte	$F6
	.byte	$0D
	.byte	$D6
	.byte	$40
	.byte	$0E
	.byte	$13
	.byte	$F6
	.byte	$40
	.byte	$06
	.byte	$13
	.byte	$3E
	.byte	$40
	.byte	$FE
	.byte	$14
	.byte	$4E
	.byte	$40
	.byte	$F6
	.byte	$15
	.byte	$4F
	.byte	$40
	.byte	$0D
	.byte	$0C
	.byte	$0B
	.byte	$42
	.byte	$80
_animate_playerclimb1_data:
	.byte	$FE
	.byte	$FE
	.byte	$04
	.byte	$00
	.byte	$06
	.byte	$FE
	.byte	$03
	.byte	$00
	.byte	$FE
	.byte	$06
	.byte	$14
	.byte	$00
	.byte	$06
	.byte	$06
	.byte	$13
	.byte	$00
	.byte	$FE
	.byte	$0E
	.byte	$22
	.byte	$00
	.byte	$06
	.byte	$0E
	.byte	$25
	.byte	$00
	.byte	$80
_animate_playerclimb2_data:
	.byte	$FE
	.byte	$FE
	.byte	$02
	.byte	$00
	.byte	$06
	.byte	$FE
	.byte	$05
	.byte	$00
	.byte	$FE
	.byte	$06
	.byte	$12
	.byte	$00
	.byte	$06
	.byte	$06
	.byte	$15
	.byte	$00
	.byte	$FE
	.byte	$0E
	.byte	$24
	.byte	$00
	.byte	$06
	.byte	$0E
	.byte	$23
	.byte	$00
	.byte	$80
_animate_laddertop_data:
	.byte	$FE
	.byte	$06
	.byte	$57
	.byte	$00
	.byte	$06
	.byte	$06
	.byte	$57
	.byte	$40
	.byte	$FE
	.byte	$0E
	.byte	$26
	.byte	$00
	.byte	$06
	.byte	$0E
	.byte	$36
	.byte	$00
	.byte	$FE
	.byte	$FE
	.byte	$55
	.byte	$00
	.byte	$06
	.byte	$FE
	.byte	$56
	.byte	$00
	.byte	$80
_animate_playerstandshootleft_data:
	.byte	$FE
	.byte	$FE
	.byte	$00
	.byte	$00
	.byte	$06
	.byte	$FE
	.byte	$01
	.byte	$00
	.byte	$FE
	.byte	$06
	.byte	$09
	.byte	$00
	.byte	$06
	.byte	$06
	.byte	$11
	.byte	$00
	.byte	$FE
	.byte	$0E
	.byte	$20
	.byte	$00
	.byte	$06
	.byte	$0E
	.byte	$21
	.byte	$00
	.byte	$02
	.byte	$FF
	.byte	$0B
	.byte	$02
	.byte	$F6
	.byte	$06
	.byte	$08
	.byte	$00
	.byte	$80
_animate_playerstandshootright_data:
	.byte	$06
	.byte	$FE
	.byte	$00
	.byte	$40
	.byte	$FE
	.byte	$FE
	.byte	$01
	.byte	$40
	.byte	$06
	.byte	$06
	.byte	$09
	.byte	$40
	.byte	$FE
	.byte	$06
	.byte	$11
	.byte	$40
	.byte	$06
	.byte	$0E
	.byte	$20
	.byte	$40
	.byte	$FE
	.byte	$0E
	.byte	$21
	.byte	$40
	.byte	$02
	.byte	$FF
	.byte	$0B
	.byte	$42
	.byte	$0E
	.byte	$06
	.byte	$08
	.byte	$40
	.byte	$80
.segment	"BANK1"
.segment	"BANK2"
_stage1_level0_0:
	.byte	$3C
	.byte	$1E
	.byte	$1F
	.byte	$50
	.byte	$8B
	.byte	$86
	.byte	$2D
	.byte	$87
	.byte	$7B
	.byte	$86
	.byte	$2D
	.byte	$2D
	.byte	$92
	.byte	$94
	.byte	$89
	.byte	$94
	.byte	$00
	.byte	$01
	.byte	$1E
	.byte	$1F
	.byte	$86
	.byte	$93
	.byte	$2D
	.byte	$2D
	.byte	$87
	.byte	$93
	.byte	$2E
	.byte	$93
	.byte	$2D
	.byte	$2D
	.byte	$89
	.byte	$91
	.byte	$02
	.byte	$03
	.byte	$00
	.byte	$1E
	.byte	$93
	.byte	$92
	.byte	$2E
	.byte	$2D
	.byte	$92
	.byte	$91
	.byte	$2E
	.byte	$2E
	.byte	$93
	.byte	$2D
	.byte	$94
	.byte	$2D
	.byte	$00
	.byte	$01
	.byte	$02
	.byte	$02
	.byte	$91
	.byte	$93
	.byte	$2D
	.byte	$2D
	.byte	$2D
	.byte	$91
	.byte	$89
	.byte	$2D
	.byte	$2D
	.byte	$92
	.byte	$94
	.byte	$91
	.byte	$02
	.byte	$03
	.byte	$15
	.byte	$1B
	.byte	$91
	.byte	$2D
	.byte	$2D
	.byte	$2D
	.byte	$89
	.byte	$90
	.byte	$88
	.byte	$2E
	.byte	$2D
	.byte	$2E
	.byte	$88
	.byte	$91
	.byte	$00
	.byte	$15
	.byte	$CC
	.byte	$CD
	.byte	$29
	.byte	$89
	.byte	$2D
	.byte	$88
	.byte	$2A
	.byte	$46
	.byte	$90
	.byte	$2A
	.byte	$2B
	.byte	$2F
	.byte	$90
	.byte	$88
	.byte	$14
	.byte	$CE
	.byte	$BF
	.byte	$9A
	.byte	$49
	.byte	$90
	.byte	$2C
	.byte	$2A
	.byte	$47
	.byte	$73
	.byte	$48
	.byte	$49
	.byte	$6A
	.byte	$9F
	.byte	$49
	.byte	$2B
	.byte	$CA
	.byte	$C9
	.byte	$CD
	.byte	$50
	.byte	$50
	.byte	$49
	.byte	$62
	.byte	$48
	.byte	$50
	.byte	$71
	.byte	$77
	.byte	$50
	.byte	$69
	.byte	$50
	.byte	$50
	.byte	$9F
	.byte	$C9
	.byte	$CB
	.byte	$51
	.byte	$50
	.byte	$50
	.byte	$66
	.byte	$60
	.byte	$67
	.byte	$50
	.byte	$71
	.byte	$50
	.byte	$6E
	.byte	$69
	.byte	$50
	.byte	$50
	.byte	$76
	.byte	$C9
	.byte	$CB
	.byte	$CD
	.byte	$E2
	.byte	$59
	.byte	$59
	.byte	$61
	.byte	$58
	.byte	$59
	.byte	$72
	.byte	$58
	.byte	$59
	.byte	$68
	.byte	$58
	.byte	$59
	.byte	$58
	.byte	$C9
	.byte	$C9
	.byte	$CB
	.byte	$51
	.byte	$54
	.byte	$63
	.byte	$64
	.byte	$65
	.byte	$74
	.byte	$55
	.byte	$75
	.byte	$52
	.byte	$69
	.byte	$54
	.byte	$51
	.byte	$74
	.byte	$0F
	.byte	$04
	.byte	$04
	.byte	$0C
	.byte	$42
	.byte	$51
	.byte	$55
	.byte	$42
	.byte	$54
	.byte	$55
	.byte	$53
	.byte	$6B
	.byte	$6C
	.byte	$6D
	.byte	$53
	.byte	$51
	.byte	$02
	.byte	$3A
	.byte	$3B
	.byte	$00
	.byte	$0F
	.byte	$06
	.byte	$08
	.byte	$0C
	.byte	$51
	.byte	$40
	.byte	$41
	.byte	$56
	.byte	$55
	.byte	$57
	.byte	$56
	.byte	$56
	.byte	$00
	.byte	$3C
	.byte	$10
	.byte	$02
	.byte	$00
	.byte	$03
	.byte	$00
	.byte	$01
	.byte	$0E
	.byte	$07
	.byte	$05
	.byte	$06
	.byte	$07
	.byte	$08
	.byte	$09
	.byte	$0A
	.byte	$37
	.byte	$37
	.byte	$37
	.byte	$38
	.byte	$38
	.byte	$37
	.byte	$37
	.byte	$38
	.byte	$38
	.byte	$37
	.byte	$38
	.byte	$38
	.byte	$37
	.byte	$37
	.byte	$38
	.byte	$37
_stage1_level0_1:
	.byte	$2D
	.byte	$2E
	.byte	$89
	.byte	$88
	.byte	$89
	.byte	$2E
	.byte	$2E
	.byte	$89
	.byte	$88
	.byte	$89
	.byte	$2E
	.byte	$2E
	.byte	$89
	.byte	$87
	.byte	$50
	.byte	$50
	.byte	$2D
	.byte	$2E
	.byte	$94
	.byte	$93
	.byte	$93
	.byte	$2D
	.byte	$92
	.byte	$2E
	.byte	$94
	.byte	$2E
	.byte	$2E
	.byte	$2E
	.byte	$2E
	.byte	$2E
	.byte	$50
	.byte	$50
	.byte	$2E
	.byte	$2E
	.byte	$2E
	.byte	$91
	.byte	$2D
	.byte	$2E
	.byte	$2D
	.byte	$2E
	.byte	$92
	.byte	$2D
	.byte	$2E
	.byte	$2E
	.byte	$92
	.byte	$2A
	.byte	$50
	.byte	$50
	.byte	$2D
	.byte	$2E
	.byte	$92
	.byte	$88
	.byte	$94
	.byte	$2D
	.byte	$2E
	.byte	$2E
	.byte	$92
	.byte	$93
	.byte	$2E
	.byte	$2E
	.byte	$94
	.byte	$48
	.byte	$50
	.byte	$50
	.byte	$2D
	.byte	$2D
	.byte	$94
	.byte	$89
	.byte	$88
	.byte	$2D
	.byte	$2D
	.byte	$89
	.byte	$2A
	.byte	$2B
	.byte	$2C
	.byte	$2E
	.byte	$2A
	.byte	$DC
	.byte	$DE
	.byte	$50
	.byte	$90
	.byte	$2B
	.byte	$2C
	.byte	$2A
	.byte	$90
	.byte	$2D
	.byte	$2C
	.byte	$2A
	.byte	$46
	.byte	$62
	.byte	$47
	.byte	$2B
	.byte	$48
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$46
	.byte	$62
	.byte	$48
	.byte	$49
	.byte	$46
	.byte	$2F
	.byte	$46
	.byte	$46
	.byte	$48
	.byte	$61
	.byte	$67
	.byte	$9F
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$73
	.byte	$61
	.byte	$DC
	.byte	$DE
	.byte	$9F
	.byte	$6A
	.byte	$73
	.byte	$9E
	.byte	$50
	.byte	$61
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$71
	.byte	$60
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$69
	.byte	$71
	.byte	$77
	.byte	$50
	.byte	$61
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$72
	.byte	$60
	.byte	$58
	.byte	$58
	.byte	$59
	.byte	$68
	.byte	$72
	.byte	$58
	.byte	$59
	.byte	$61
	.byte	$58
	.byte	$59
	.byte	$58
	.byte	$58
	.byte	$58
	.byte	$58
	.byte	$63
	.byte	$64
	.byte	$65
	.byte	$54
	.byte	$52
	.byte	$69
	.byte	$72
	.byte	$52
	.byte	$63
	.byte	$64
	.byte	$65
	.byte	$54
	.byte	$55
	.byte	$54
	.byte	$51
	.byte	$53
	.byte	$55
	.byte	$55
	.byte	$52
	.byte	$42
	.byte	$6B
	.byte	$6C
	.byte	$6D
	.byte	$75
	.byte	$56
	.byte	$55
	.byte	$57
	.byte	$52
	.byte	$51
	.byte	$56
	.byte	$51
	.byte	$52
	.byte	$51
	.byte	$54
	.byte	$55
	.byte	$57
	.byte	$56
	.byte	$41
	.byte	$57
	.byte	$51
	.byte	$51
	.byte	$53
	.byte	$41
	.byte	$40
	.byte	$42
	.byte	$0F
	.byte	$04
	.byte	$0C
	.byte	$0B
	.byte	$04
	.byte	$05
	.byte	$06
	.byte	$07
	.byte	$04
	.byte	$08
	.byte	$06
	.byte	$07
	.byte	$04
	.byte	$05
	.byte	$06
	.byte	$0D
	.byte	$00
	.byte	$10
	.byte	$11
	.byte	$37
	.byte	$38
	.byte	$38
	.byte	$37
	.byte	$38
	.byte	$37
	.byte	$37
	.byte	$38
	.byte	$37
	.byte	$38
	.byte	$37
	.byte	$37
	.byte	$37
	.byte	$37
	.byte	$37
	.byte	$38
_stage1_level0_2:
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$DD
	.byte	$DC
	.byte	$DC
	.byte	$DC
	.byte	$DD
	.byte	$DC
	.byte	$DC
	.byte	$DE
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$DD
	.byte	$DC
	.byte	$DE
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$AC
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$AC
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$AD
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$35
	.byte	$34
	.byte	$A4
	.byte	$A1
	.byte	$BB
	.byte	$BD
	.byte	$B6
	.byte	$BD
	.byte	$BA
	.byte	$B0
	.byte	$BC
	.byte	$B0
	.byte	$BD
	.byte	$BC
	.byte	$35
	.byte	$36
	.byte	$25
	.byte	$21
	.byte	$51
	.byte	$A0
	.byte	$B8
	.byte	$B0
	.byte	$BC
	.byte	$B0
	.byte	$B8
	.byte	$B0
	.byte	$B7
	.byte	$B9
	.byte	$B0
	.byte	$B8
	.byte	$25
	.byte	$21
	.byte	$20
	.byte	$20
	.byte	$A3
	.byte	$A2
	.byte	$AA
	.byte	$A9
	.byte	$B0
	.byte	$A8
	.byte	$AB
	.byte	$B0
	.byte	$A8
	.byte	$A9
	.byte	$35
	.byte	$36
	.byte	$23
	.byte	$20
	.byte	$20
	.byte	$20
	.byte	$A5
	.byte	$A6
	.byte	$A6
	.byte	$35
	.byte	$34
	.byte	$34
	.byte	$34
	.byte	$33
	.byte	$34
	.byte	$36
	.byte	$25
	.byte	$21
	.byte	$20
	.byte	$20
	.byte	$20
	.byte	$20
	.byte	$51
	.byte	$51
	.byte	$51
	.byte	$38
	.byte	$37
	.byte	$38
	.byte	$37
	.byte	$37
	.byte	$38
	.byte	$38
	.byte	$23
	.byte	$20
	.byte	$20
	.byte	$20
	.byte	$20
	.byte	$20
_stage1_level0_3:
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$8B
	.byte	$86
	.byte	$89
	.byte	$2D
	.byte	$89
	.byte	$87
	.byte	$7B
	.byte	$50
	.byte	$DE
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$86
	.byte	$89
	.byte	$2E
	.byte	$88
	.byte	$88
	.byte	$89
	.byte	$87
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$DD
	.byte	$DC
	.byte	$89
	.byte	$93
	.byte	$88
	.byte	$2D
	.byte	$91
	.byte	$2E
	.byte	$94
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$91
	.byte	$89
	.byte	$2E
	.byte	$2D
	.byte	$92
	.byte	$2E
	.byte	$89
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$2C
	.byte	$88
	.byte	$91
	.byte	$2E
	.byte	$88
	.byte	$89
	.byte	$2A
	.byte	$50
	.byte	$50
	.byte	$DD
	.byte	$DC
	.byte	$DC
	.byte	$DE
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$49
	.byte	$2C
	.byte	$90
	.byte	$2B
	.byte	$90
	.byte	$2A
	.byte	$48
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$9F
	.byte	$49
	.byte	$6A
	.byte	$47
	.byte	$48
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$6E
	.byte	$69
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$AD
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$68
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$33
	.byte	$36
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$AC
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$69
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$21
	.byte	$26
	.byte	$35
	.byte	$36
	.byte	$BC
	.byte	$B0
	.byte	$BD
	.byte	$BC
	.byte	$B0
	.byte	$4D
	.byte	$6B
	.byte	$6C
	.byte	$6D
	.byte	$53
	.byte	$4F
	.byte	$BE
	.byte	$20
	.byte	$20
	.byte	$21
	.byte	$26
	.byte	$BD
	.byte	$B8
	.byte	$B0
	.byte	$B7
	.byte	$B6
	.byte	$4A
	.byte	$4B
	.byte	$5A
	.byte	$4B
	.byte	$5B
	.byte	$4C
	.byte	$B6
	.byte	$20
	.byte	$20
	.byte	$20
	.byte	$24
	.byte	$35
	.byte	$34
	.byte	$36
	.byte	$B0
	.byte	$A8
	.byte	$B0
	.byte	$B0
	.byte	$B9
	.byte	$B0
	.byte	$A8
	.byte	$AB
	.byte	$A8
	.byte	$20
	.byte	$20
	.byte	$20
	.byte	$20
	.byte	$21
	.byte	$21
	.byte	$26
	.byte	$35
	.byte	$33
	.byte	$34
	.byte	$33
	.byte	$34
	.byte	$33
	.byte	$33
	.byte	$33
	.byte	$36
	.byte	$20
	.byte	$20
	.byte	$20
	.byte	$20
	.byte	$20
	.byte	$20
	.byte	$24
	.byte	$38
	.byte	$37
	.byte	$38
	.byte	$37
	.byte	$37
	.byte	$37
	.byte	$37
	.byte	$37
	.byte	$38
_stage1_level0_4:
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$DD
	.byte	$DC
	.byte	$DC
	.byte	$DE
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$DD
	.byte	$DC
	.byte	$DC
	.byte	$DE
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$DD
	.byte	$DC
	.byte	$DE
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$D1
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$D1
	.byte	$D2
	.byte	$D6
	.byte	$50
	.byte	$50
	.byte	$AC
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$AD
	.byte	$50
	.byte	$D0
	.byte	$D5
	.byte	$D4
	.byte	$D4
	.byte	$B0
	.byte	$B6
	.byte	$B0
	.byte	$BA
	.byte	$B0
	.byte	$BB
	.byte	$7E
	.byte	$78
	.byte	$5F
	.byte	$BA
	.byte	$B0
	.byte	$BA
	.byte	$BD
	.byte	$BE
	.byte	$BA
	.byte	$B0
	.byte	$B8
	.byte	$B0
	.byte	$B9
	.byte	$B0
	.byte	$B7
	.byte	$BC
	.byte	$5C
	.byte	$5D
	.byte	$5E
	.byte	$B6
	.byte	$B7
	.byte	$A9
	.byte	$AB
	.byte	$B7
	.byte	$0E
	.byte	$0D
	.byte	$B0
	.byte	$B9
	.byte	$B0
	.byte	$AA
	.byte	$B0
	.byte	$B7
	.byte	$B0
	.byte	$A8
	.byte	$B7
	.byte	$B0
	.byte	$A9
	.byte	$B0
	.byte	$0E
	.byte	$0D
	.byte	$00
	.byte	$3C
	.byte	$A6
	.byte	$A6
	.byte	$A6
	.byte	$0E
	.byte	$07
	.byte	$08
	.byte	$09
	.byte	$04
	.byte	$05
	.byte	$06
	.byte	$07
	.byte	$0C
	.byte	$3A
	.byte	$3B
	.byte	$02
	.byte	$01
	.byte	$51
	.byte	$51
	.byte	$51
	.byte	$38
	.byte	$37
	.byte	$37
	.byte	$38
	.byte	$38
	.byte	$38
	.byte	$38
	.byte	$37
	.byte	$38
	.byte	$38
	.byte	$37
	.byte	$37
	.byte	$38
_stage1_level0_5:
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$DD
	.byte	$DC
	.byte	$DC
	.byte	$DC
	.byte	$DE
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$DD
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$DD
	.byte	$DC
	.byte	$DE
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$D1
	.byte	$D3
	.byte	$50
	.byte	$50
	.byte	$C5
	.byte	$D1
	.byte	$D3
	.byte	$E0
	.byte	$E1
	.byte	$E2
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$D1
	.byte	$D2
	.byte	$D5
	.byte	$0E
	.byte	$50
	.byte	$D0
	.byte	$D4
	.byte	$D6
	.byte	$D7
	.byte	$EC
	.byte	$EB
	.byte	$EA
	.byte	$E3
	.byte	$50
	.byte	$50
	.byte	$D0
	.byte	$D5
	.byte	$D4
	.byte	$D4
	.byte	$00
	.byte	$D2
	.byte	$D4
	.byte	$D4
	.byte	$D4
	.byte	$D9
	.byte	$DA
	.byte	$EE
	.byte	$E9
	.byte	$E9
	.byte	$E4
	.byte	$E5
	.byte	$E6
	.byte	$D4
	.byte	$0F
	.byte	$0C
	.byte	$02
	.byte	$D6
	.byte	$D4
	.byte	$D6
	.byte	$D6
	.byte	$D4
	.byte	$D8
	.byte	$DA
	.byte	$EF
	.byte	$E9
	.byte	$E9
	.byte	$EB
	.byte	$EA
	.byte	$E7
	.byte	$3C
	.byte	$12
	.byte	$3A
	.byte	$0E
	.byte	$07
	.byte	$04
	.byte	$06
	.byte	$04
	.byte	$0C
	.byte	$D8
	.byte	$DB
	.byte	$EE
	.byte	$0E
	.byte	$0C
	.byte	$E9
	.byte	$EB
	.byte	$12
	.byte	$01
	.byte	$12
	.byte	$00
	.byte	$3C
	.byte	$12
	.byte	$02
	.byte	$10
	.byte	$01
	.byte	$BC
	.byte	$B0
	.byte	$BD
	.byte	$00
	.byte	$01
	.byte	$BB
	.byte	$BD
	.byte	$10
	.byte	$03
	.byte	$3C
	.byte	$02
	.byte	$3A
	.byte	$3B
	.byte	$10
	.byte	$12
	.byte	$11
	.byte	$B9
	.byte	$B0
	.byte	$B9
	.byte	$02
	.byte	$03
	.byte	$B8
	.byte	$BC
	.byte	$3A
	.byte	$3B
	.byte	$3C
	.byte	$00
	.byte	$01
	.byte	$3C
	.byte	$3A
	.byte	$3B
	.byte	$01
	.byte	$A9
	.byte	$B6
	.byte	$A9
	.byte	$00
	.byte	$01
	.byte	$B0
	.byte	$A7
	.byte	$00
	.byte	$3C
	.byte	$00
	.byte	$02
	.byte	$03
	.byte	$10
	.byte	$3C
	.byte	$10
	.byte	$03
	.byte	$A6
	.byte	$A6
	.byte	$A6
	.byte	$02
	.byte	$03
	.byte	$A6
	.byte	$A6
	.byte	$02
	.byte	$01
	.byte	$02
	.byte	$38
	.byte	$37
	.byte	$37
	.byte	$38
	.byte	$37
	.byte	$38
	.byte	$51
	.byte	$51
	.byte	$51
	.byte	$38
	.byte	$38
	.byte	$51
	.byte	$51
	.byte	$38
	.byte	$38
	.byte	$37
_stage1_level0_6:
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$DD
	.byte	$DC
	.byte	$DC
	.byte	$DC
	.byte	$DE
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$DC
	.byte	$DC
	.byte	$DC
	.byte	$DE
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$DD
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$E3
	.byte	$AC
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$DD
	.byte	$DC
	.byte	$DC
	.byte	$DE
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$0A
	.byte	$06
	.byte	$0C
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$D1
	.byte	$D3
	.byte	$E0
	.byte	$01
	.byte	$02
	.byte	$03
	.byte	$E4
	.byte	$E2
	.byte	$50
	.byte	$AC
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$C5
	.byte	$D0
	.byte	$D4
	.byte	$D9
	.byte	$EC
	.byte	$02
	.byte	$00
	.byte	$01
	.byte	$0E
	.byte	$08
	.byte	$06
	.byte	$0D
	.byte	$50
	.byte	$50
	.byte	$D1
	.byte	$D2
	.byte	$D4
	.byte	$D6
	.byte	$D4
	.byte	$D5
	.byte	$DB
	.byte	$3B
	.byte	$02
	.byte	$03
	.byte	$00
	.byte	$01
	.byte	$02
	.byte	$11
	.byte	$E4
	.byte	$E5
	.byte	$E8
	.byte	$D6
	.byte	$D6
	.byte	$D4
	.byte	$D6
	.byte	$D4
	.byte	$D8
	.byte	$12
	.byte	$3C
	.byte	$01
	.byte	$02
	.byte	$03
	.byte	$00
	.byte	$3C
	.byte	$E9
	.byte	$EB
	.byte	$EB
	.byte	$E7
	.byte	$DF
	.byte	$D6
	.byte	$D4
	.byte	$D6
	.byte	$D9
	.byte	$01
	.byte	$3A
	.byte	$3B
	.byte	$00
	.byte	$3C
	.byte	$02
	.byte	$01
	.byte	$0E
	.byte	$0A
	.byte	$0C
	.byte	$B9
	.byte	$BA
	.byte	$BA
	.byte	$B0
	.byte	$BC
	.byte	$B0
	.byte	$03
	.byte	$00
	.byte	$00
	.byte	$02
	.byte	$10
	.byte	$01
	.byte	$02
	.byte	$3A
	.byte	$3B
	.byte	$3C
	.byte	$B8
	.byte	$B0
	.byte	$B8
	.byte	$AB
	.byte	$B9
	.byte	$BA
	.byte	$01
	.byte	$02
	.byte	$03
	.byte	$3C
	.byte	$3C
	.byte	$03
	.byte	$00
	.byte	$3C
	.byte	$10
	.byte	$10
	.byte	$A9
	.byte	$AB
	.byte	$A7
	.byte	$B8
	.byte	$B0
	.byte	$AA
	.byte	$03
	.byte	$01
	.byte	$00
	.byte	$01
	.byte	$3A
	.byte	$3B
	.byte	$02
	.byte	$10
	.byte	$3C
	.byte	$12
	.byte	$A6
	.byte	$A6
	.byte	$A6
	.byte	$A6
	.byte	$0E
	.byte	$09
	.byte	$37
	.byte	$38
	.byte	$38
	.byte	$37
	.byte	$38
	.byte	$38
	.byte	$37
	.byte	$37
	.byte	$38
	.byte	$37
	.byte	$51
	.byte	$51
	.byte	$51
	.byte	$51
	.byte	$10
	.byte	$00
_stage1_level0_7:
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$43
	.byte	$44
	.byte	$19
	.byte	$18
	.byte	$10
	.byte	$3D
	.byte	$00
	.byte	$3C
	.byte	$02
	.byte	$03
	.byte	$02
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$9B
	.byte	$44
	.byte	$19
	.byte	$3D
	.byte	$02
	.byte	$10
	.byte	$10
	.byte	$01
	.byte	$00
	.byte	$DC
	.byte	$DC
	.byte	$DE
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$9F
	.byte	$3D
	.byte	$00
	.byte	$3A
	.byte	$3B
	.byte	$03
	.byte	$02
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$3D
	.byte	$02
	.byte	$3C
	.byte	$00
	.byte	$01
	.byte	$00
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$AD
	.byte	$50
	.byte	$50
	.byte	$3D
	.byte	$00
	.byte	$10
	.byte	$03
	.byte	$02
	.byte	$02
	.byte	$E1
	.byte	$E2
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$3E
	.byte	$0E
	.byte	$05
	.byte	$0A
	.byte	$0D
	.byte	$02
	.byte	$01
	.byte	$00
	.byte	$3A
	.byte	$3B
	.byte	$E9
	.byte	$E9
	.byte	$E3
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$3D
	.byte	$3C
	.byte	$03
	.byte	$00
	.byte	$01
	.byte	$10
	.byte	$12
	.byte	$02
	.byte	$3C
	.byte	$00
	.byte	$ED
	.byte	$EF
	.byte	$EB
	.byte	$E3
	.byte	$50
	.byte	$50
	.byte	$3D
	.byte	$00
	.byte	$01
	.byte	$02
	.byte	$03
	.byte	$00
	.byte	$01
	.byte	$11
	.byte	$10
	.byte	$02
	.byte	$DB
	.byte	$DB
	.byte	$E9
	.byte	$EB
	.byte	$E3
	.byte	$AC
	.byte	$3D
	.byte	$02
	.byte	$03
	.byte	$3C
	.byte	$01
	.byte	$02
	.byte	$03
	.byte	$3A
	.byte	$3B
	.byte	$00
	.byte	$DA
	.byte	$DB
	.byte	$EF
	.byte	$EB
	.byte	$0F
	.byte	$07
	.byte	$0D
	.byte	$00
	.byte	$01
	.byte	$10
	.byte	$02
	.byte	$00
	.byte	$01
	.byte	$3C
	.byte	$01
	.byte	$02
	.byte	$BC
	.byte	$B0
	.byte	$BD
	.byte	$BC
	.byte	$3C
	.byte	$10
	.byte	$10
	.byte	$02
	.byte	$03
	.byte	$3C
	.byte	$10
	.byte	$02
	.byte	$03
	.byte	$00
	.byte	$02
	.byte	$25
	.byte	$B9
	.byte	$0F
	.byte	$08
	.byte	$0C
	.byte	$00
	.byte	$12
	.byte	$3C
	.byte	$00
	.byte	$01
	.byte	$10
	.byte	$00
	.byte	$3C
	.byte	$01
	.byte	$02
	.byte	$25
	.byte	$20
	.byte	$A9
	.byte	$00
	.byte	$03
	.byte	$00
	.byte	$02
	.byte	$10
	.byte	$3C
	.byte	$02
	.byte	$03
	.byte	$00
	.byte	$02
	.byte	$10
	.byte	$02
	.byte	$25
	.byte	$20
	.byte	$20
	.byte	$0D
	.byte	$02
	.byte	$01
	.byte	$02
	.byte	$11
	.byte	$3C
	.byte	$10
	.byte	$00
	.byte	$10
	.byte	$02
	.byte	$25
	.byte	$21
	.byte	$21
	.byte	$20
	.byte	$20
	.byte	$20
	.byte	$01
	.byte	$10
	.byte	$01
	.byte	$3A
	.byte	$3B
	.byte	$10
	.byte	$01
	.byte	$02
	.byte	$25
	.byte	$21
	.byte	$20
	.byte	$20
	.byte	$20
	.byte	$20
	.byte	$20
	.byte	$20
_stage1_levels_list:
	.addr	_stage1_level0_0
	.addr	_stage1_level0_1
	.addr	_stage1_level0_2
	.addr	_stage1_level0_3
	.addr	_stage1_level0_4
	.addr	_stage1_level0_5
	.addr	_stage1_level0_6
_stage1_max_rooms:
	.byte	$01
_stage1_offsets:
	.byte	$00
_stage1_bg_palette:
	.byte	$21
	.byte	$0F
	.byte	$00
	.byte	$10
	.byte	$21
	.byte	$0F
	.byte	$30
	.byte	$08
	.byte	$21
	.byte	$0F
	.byte	$17
	.byte	$06
	.byte	$21
	.byte	$0F
	.byte	$19
	.byte	$29
_stage1_metatiles:
	.byte	$7B
	.byte	$7C
	.byte	$8B
	.byte	$8C
	.byte	$00
	.byte	$7D
	.byte	$7E
	.byte	$8D
	.byte	$8E
	.byte	$00
	.byte	$9B
	.byte	$9C
	.byte	$AB
	.byte	$AC
	.byte	$00
	.byte	$9D
	.byte	$9E
	.byte	$AD
	.byte	$AE
	.byte	$00
	.byte	$BE
	.byte	$AF
	.byte	$7B
	.byte	$7C
	.byte	$00
	.byte	$BE
	.byte	$BE
	.byte	$8B
	.byte	$8C
	.byte	$00
	.byte	$BE
	.byte	$BF
	.byte	$8D
	.byte	$8E
	.byte	$00
	.byte	$BE
	.byte	$BE
	.byte	$AD
	.byte	$AE
	.byte	$00
	.byte	$BE
	.byte	$B9
	.byte	$9B
	.byte	$9C
	.byte	$00
	.byte	$BE
	.byte	$BE
	.byte	$9D
	.byte	$9E
	.byte	$00
	.byte	$BE
	.byte	$BB
	.byte	$AB
	.byte	$AC
	.byte	$00
	.byte	$BE
	.byte	$AF
	.byte	$AD
	.byte	$AE
	.byte	$00
	.byte	$BE
	.byte	$BD
	.byte	$7B
	.byte	$7C
	.byte	$00
	.byte	$BE
	.byte	$BD
	.byte	$7D
	.byte	$7E
	.byte	$00
	.byte	$BA
	.byte	$BF
	.byte	$9B
	.byte	$9C
	.byte	$00
	.byte	$BA
	.byte	$B8
	.byte	$9D
	.byte	$9E
	.byte	$00
	.byte	$7B
	.byte	$AE
	.byte	$7C
	.byte	$7B
	.byte	$00
	.byte	$8B
	.byte	$8C
	.byte	$9B
	.byte	$9C
	.byte	$00
	.byte	$8D
	.byte	$8E
	.byte	$9D
	.byte	$9E
	.byte	$00
	.byte	$7B
	.byte	$7C
	.byte	$8B
	.byte	$01
	.byte	$00
	.byte	$AD
	.byte	$8E
	.byte	$AE
	.byte	$01
	.byte	$00
	.byte	$AD
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$00
	.byte	$7D
	.byte	$7E
	.byte	$01
	.byte	$01
	.byte	$00
	.byte	$9D
	.byte	$9E
	.byte	$01
	.byte	$01
	.byte	$00
	.byte	$AB
	.byte	$AC
	.byte	$01
	.byte	$01
	.byte	$00
	.byte	$01
	.byte	$7B
	.byte	$01
	.byte	$01
	.byte	$00
	.byte	$7B
	.byte	$AD
	.byte	$01
	.byte	$AE
	.byte	$00
	.byte	$7B
	.byte	$7C
	.byte	$01
	.byte	$7B
	.byte	$00
	.byte	$7A
	.byte	$7C
	.byte	$8B
	.byte	$8C
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$7A
	.byte	$00
	.byte	$7B
	.byte	$6A
	.byte	$7C
	.byte	$7B
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$6A
	.byte	$00
	.byte	$00
	.byte	$75
	.byte	$76
	.byte	$85
	.byte	$86
	.byte	$02
	.byte	$65
	.byte	$66
	.byte	$85
	.byte	$86
	.byte	$02
	.byte	$75
	.byte	$76
	.byte	$95
	.byte	$96
	.byte	$02
	.byte	$74
	.byte	$76
	.byte	$84
	.byte	$86
	.byte	$02
	.byte	$75
	.byte	$77
	.byte	$85
	.byte	$87
	.byte	$02
	.byte	$64
	.byte	$66
	.byte	$84
	.byte	$86
	.byte	$02
	.byte	$65
	.byte	$67
	.byte	$85
	.byte	$87
	.byte	$02
	.byte	$74
	.byte	$76
	.byte	$94
	.byte	$96
	.byte	$02
	.byte	$75
	.byte	$77
	.byte	$95
	.byte	$97
	.byte	$02
	.byte	$DF
	.byte	$DF
	.byte	$DC
	.byte	$CD
	.byte	$03
	.byte	$DF
	.byte	$CE
	.byte	$CE
	.byte	$DB
	.byte	$03
	.byte	$CD
	.byte	$CE
	.byte	$EE
	.byte	$EE
	.byte	$03
	.byte	$DF
	.byte	$CF
	.byte	$DC
	.byte	$CC
	.byte	$03
	.byte	$34
	.byte	$CF
	.byte	$DF
	.byte	$33
	.byte	$03
	.byte	$DF
	.byte	$CF
	.byte	$33
	.byte	$CF
	.byte	$03
	.byte	$CB
	.byte	$CE
	.byte	$EF
	.byte	$EE
	.byte	$03
	.byte	$65
	.byte	$66
	.byte	$95
	.byte	$96
	.byte	$02
	.byte	$64
	.byte	$66
	.byte	$94
	.byte	$96
	.byte	$02
	.byte	$65
	.byte	$67
	.byte	$95
	.byte	$97
	.byte	$02
	.byte	$EB
	.byte	$EB
	.byte	$FB
	.byte	$FC
	.byte	$03
	.byte	$EB
	.byte	$EA
	.byte	$FC
	.byte	$FB
	.byte	$03
	.byte	$FA
	.byte	$EB
	.byte	$FC
	.byte	$FC
	.byte	$03
	.byte	$EB
	.byte	$EC
	.byte	$FB
	.byte	$FC
	.byte	$03
	.byte	$98
	.byte	$99
	.byte	$A8
	.byte	$A9
	.byte	$03
	.byte	$9A
	.byte	$98
	.byte	$AA
	.byte	$A8
	.byte	$03
	.byte	$01
	.byte	$01
	.byte	$9A
	.byte	$98
	.byte	$03
	.byte	$7B
	.byte	$8B
	.byte	$AD
	.byte	$9B
	.byte	$00
	.byte	$8E
	.byte	$7B
	.byte	$9C
	.byte	$AE
	.byte	$00
	.byte	$8B
	.byte	$7C
	.byte	$9B
	.byte	$AD
	.byte	$00
	.byte	$0A
	.byte	$0B
	.byte	$0A
	.byte	$0B
	.byte	$02
	.byte	$0A
	.byte	$0B
	.byte	$0A
	.byte	$0B
	.byte	$02
	.byte	$CA
	.byte	$CA
	.byte	$DA
	.byte	$DA
	.byte	$00
	.byte	$01
	.byte	$01
	.byte	$63
	.byte	$73
	.byte	$03
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$63
	.byte	$03
	.byte	$01
	.byte	$01
	.byte	$73
	.byte	$01
	.byte	$03
	.byte	$12
	.byte	$13
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$01
	.byte	$12
	.byte	$13
	.byte	$00
	.byte	$EF
	.byte	$9F
	.byte	$01
	.byte	$01
	.byte	$03
	.byte	$EF
	.byte	$EF
	.byte	$EE
	.byte	$EF
	.byte	$03
	.byte	$ED
	.byte	$ED
	.byte	$36
	.byte	$37
	.byte	$03
	.byte	$ED
	.byte	$38
	.byte	$37
	.byte	$00
	.byte	$03
	.byte	$39
	.byte	$ED
	.byte	$00
	.byte	$39
	.byte	$03
	.byte	$02
	.byte	$0D
	.byte	$0E
	.byte	$02
	.byte	$03
	.byte	$01
	.byte	$01
	.byte	$02
	.byte	$02
	.byte	$03
	.byte	$1F
	.byte	$02
	.byte	$02
	.byte	$2F
	.byte	$03
	.byte	$1B
	.byte	$0C
	.byte	$02
	.byte	$01
	.byte	$03
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$0F
	.byte	$3F
	.byte	$01
	.byte	$02
	.byte	$03
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$00
	.byte	$01
	.byte	$01
	.byte	$2A
	.byte	$01
	.byte	$03
	.byte	$1A
	.byte	$01
	.byte	$01
	.byte	$2A
	.byte	$03
	.byte	$1A
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$03
	.byte	$F2
	.byte	$F2
	.byte	$01
	.byte	$01
	.byte	$02
	.byte	$01
	.byte	$F3
	.byte	$01
	.byte	$01
	.byte	$02
	.byte	$D4
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$02
	.byte	$00
	.byte	$00
	.byte	$80
	.byte	$81
	.byte	$03
	.byte	$00
	.byte	$00
	.byte	$82
	.byte	$82
	.byte	$03
	.byte	$01
	.byte	$2A
	.byte	$02
	.byte	$02
	.byte	$03
	.byte	$1A
	.byte	$01
	.byte	$02
	.byte	$02
	.byte	$03
	.byte	$03
	.byte	$AA
	.byte	$03
	.byte	$E8
	.byte	$03
	.byte	$A8
	.byte	$A9
	.byte	$68
	.byte	$FC
	.byte	$03
	.byte	$8A
	.byte	$03
	.byte	$F8
	.byte	$03
	.byte	$03
	.byte	$03
	.byte	$03
	.byte	$E9
	.byte	$03
	.byte	$03
	.byte	$08
	.byte	$19
	.byte	$09
	.byte	$18
	.byte	$02
	.byte	$08
	.byte	$19
	.byte	$08
	.byte	$18
	.byte	$02
	.byte	$B1
	.byte	$B2
	.byte	$08
	.byte	$18
	.byte	$02
	.byte	$D4
	.byte	$B0
	.byte	$F3
	.byte	$C0
	.byte	$02
	.byte	$08
	.byte	$19
	.byte	$C1
	.byte	$C2
	.byte	$02
	.byte	$B3
	.byte	$F3
	.byte	$C3
	.byte	$D4
	.byte	$02
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$F0
	.byte	$02
	.byte	$F1
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$02
	.byte	$14
	.byte	$04
	.byte	$15
	.byte	$04
	.byte	$02
	.byte	$15
	.byte	$04
	.byte	$15
	.byte	$05
	.byte	$02
	.byte	$E5
	.byte	$E6
	.byte	$14
	.byte	$04
	.byte	$02
	.byte	$D4
	.byte	$E4
	.byte	$F3
	.byte	$F4
	.byte	$02
	.byte	$14
	.byte	$04
	.byte	$F5
	.byte	$F6
	.byte	$02
	.byte	$E7
	.byte	$F3
	.byte	$F7
	.byte	$D4
	.byte	$02
	.byte	$00
	.byte	$E2
	.byte	$00
	.byte	$00
	.byte	$02
	.byte	$E3
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$02
	.byte	$06
	.byte	$17
	.byte	$06
	.byte	$17
	.byte	$02
	.byte	$07
	.byte	$16
	.byte	$07
	.byte	$17
	.byte	$02
	.byte	$06
	.byte	$17
	.byte	$07
	.byte	$17
	.byte	$02
	.byte	$E0
	.byte	$E1
	.byte	$06
	.byte	$16
	.byte	$02
	.byte	$D4
	.byte	$D0
	.byte	$F2
	.byte	$F2
	.byte	$02
	.byte	$D1
	.byte	$F3
	.byte	$F2
	.byte	$F2
	.byte	$02
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$D2
	.byte	$02
	.byte	$00
	.byte	$00
	.byte	$D3
	.byte	$00
	.byte	$02
	.byte	$03
	.byte	$03
	.byte	$AA
	.byte	$AA
	.byte	$03
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$A9
	.byte	$A9
	.byte	$78
	.byte	$69
	.byte	$03
	.byte	$00
	.byte	$00
	.byte	$89
	.byte	$00
	.byte	$03
	.byte	$8A
	.byte	$00
	.byte	$8A
	.byte	$00
	.byte	$03
	.byte	$8A
	.byte	$00
	.byte	$79
	.byte	$00
	.byte	$03
	.byte	$03
	.byte	$03
	.byte	$03
	.byte	$F9
	.byte	$03
	.byte	$F9
	.byte	$E9
	.byte	$AA
	.byte	$AA
	.byte	$03
	.byte	$90
	.byte	$90
	.byte	$00
	.byte	$00
	.byte	$02
	.byte	$91
	.byte	$92
	.byte	$00
	.byte	$00
	.byte	$02
	.byte	$01
	.byte	$01
	.byte	$92
	.byte	$92
	.byte	$02
	.byte	$92
	.byte	$90
	.byte	$00
	.byte	$00
	.byte	$02
	.byte	$01
	.byte	$01
	.byte	$92
	.byte	$90
	.byte	$02
	.byte	$01
	.byte	$92
	.byte	$90
	.byte	$00
	.byte	$02
	.byte	$7A
	.byte	$32
	.byte	$32
	.byte	$DF
	.byte	$03
	.byte	$CF
	.byte	$89
	.byte	$DF
	.byte	$CF
	.byte	$03
	.byte	$DF
	.byte	$33
	.byte	$DE
	.byte	$DF
	.byte	$03
	.byte	$DD
	.byte	$CF
	.byte	$DF
	.byte	$34
	.byte	$03
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$7A
	.byte	$03
	.byte	$AA
	.byte	$A8
	.byte	$68
	.byte	$69
	.byte	$03
	.byte	$AA
	.byte	$A8
	.byte	$68
	.byte	$68
	.byte	$03
	.byte	$00
	.byte	$00
	.byte	$D7
	.byte	$A7
	.byte	$00
	.byte	$D7
	.byte	$A7
	.byte	$01
	.byte	$01
	.byte	$00
	.byte	$DF
	.byte	$CF
	.byte	$DC
	.byte	$DB
	.byte	$03
	.byte	$32
	.byte	$33
	.byte	$DE
	.byte	$DF
	.byte	$03
	.byte	$DF
	.byte	$35
	.byte	$CF
	.byte	$DD
	.byte	$03
	.byte	$32
	.byte	$34
	.byte	$32
	.byte	$33
	.byte	$03
	.byte	$33
	.byte	$35
	.byte	$34
	.byte	$35
	.byte	$03
	.byte	$00
	.byte	$7A
	.byte	$88
	.byte	$DF
	.byte	$03
	.byte	$00
	.byte	$88
	.byte	$88
	.byte	$CF
	.byte	$03
	.byte	$6A
	.byte	$00
	.byte	$CF
	.byte	$6A
	.byte	$03
	.byte	$89
	.byte	$00
	.byte	$A8
	.byte	$89
	.byte	$03
	.byte	$A9
	.byte	$6A
	.byte	$A9
	.byte	$AA
	.byte	$03
	.byte	$01
	.byte	$FF
	.byte	$FF
	.byte	$00
	.byte	$03
	.byte	$FF
	.byte	$01
	.byte	$00
	.byte	$FE
	.byte	$03
	.byte	$EF
	.byte	$EF
	.byte	$FE
	.byte	$FD
	.byte	$03
	.byte	$ED
	.byte	$EE
	.byte	$FE
	.byte	$FF
	.byte	$03
	.byte	$FD
	.byte	$FE
	.byte	$00
	.byte	$00
	.byte	$03
	.byte	$FF
	.byte	$FD
	.byte	$00
	.byte	$00
	.byte	$03
	.byte	$05
	.byte	$03
	.byte	$05
	.byte	$03
	.byte	$03
	.byte	$3F
	.byte	$03
	.byte	$05
	.byte	$03
	.byte	$03
	.byte	$05
	.byte	$03
	.byte	$2F
	.byte	$03
	.byte	$03
	.byte	$01
	.byte	$1F
	.byte	$1F
	.byte	$02
	.byte	$03
	.byte	$0F
	.byte	$02
	.byte	$01
	.byte	$0F
	.byte	$03
	.byte	$02
	.byte	$2F
	.byte	$11
	.byte	$11
	.byte	$03
	.byte	$03
	.byte	$03
	.byte	$11
	.byte	$11
	.byte	$03
	.byte	$03
	.byte	$61
	.byte	$60
	.byte	$03
	.byte	$03
	.byte	$03
	.byte	$62
	.byte	$03
	.byte	$03
	.byte	$03
	.byte	$03
	.byte	$03
	.byte	$61
	.byte	$03
	.byte	$03
	.byte	$60
	.byte	$03
	.byte	$03
	.byte	$03
	.byte	$03
	.byte	$03
	.byte	$03
	.byte	$72
	.byte	$03
	.byte	$03
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$71
	.byte	$03
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$70
	.byte	$03
	.byte	$C8
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$C8
	.byte	$C9
	.byte	$01
	.byte	$D9
	.byte	$01
	.byte	$03
	.byte	$03
	.byte	$03
	.byte	$03
	.byte	$03
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$3F
	.byte	$03
	.byte	$02
	.byte	$3F
	.byte	$03
	.byte	$01
	.byte	$01
	.byte	$0D
	.byte	$01
	.byte	$03
	.byte	$03
	.byte	$0E
	.byte	$03
	.byte	$03
	.byte	$03
	.byte	$A1
	.byte	$03
	.byte	$03
	.byte	$03
	.byte	$03
	.byte	$03
	.byte	$03
	.byte	$A2
	.byte	$03
	.byte	$03
	.byte	$A2
	.byte	$03
	.byte	$03
	.byte	$A1
	.byte	$03
	.byte	$A0
	.byte	$03
	.byte	$03
	.byte	$A1
	.byte	$03
	.byte	$83
	.byte	$03
	.byte	$03
	.byte	$83
	.byte	$03
	.byte	$83
	.byte	$93
	.byte	$03
	.byte	$03
	.byte	$03
	.byte	$83
	.byte	$03
	.byte	$03
	.byte	$93
	.byte	$03
	.byte	$03
	.byte	$03
	.byte	$83
	.byte	$03
	.byte	$03
	.byte	$83
	.byte	$03
	.byte	$03
	.byte	$03
	.byte	$03
	.byte	$C9
	.byte	$D9
	.byte	$D8
	.byte	$01
	.byte	$01
	.byte	$B5
	.byte	$B6
	.byte	$C5
	.byte	$C6
	.byte	$02
	.byte	$01
	.byte	$B4
	.byte	$01
	.byte	$C4
	.byte	$02
	.byte	$B7
	.byte	$01
	.byte	$C7
	.byte	$01
	.byte	$02
	.byte	$01
	.byte	$01
	.byte	$A5
	.byte	$A6
	.byte	$02
	.byte	$D5
	.byte	$D6
	.byte	$01
	.byte	$01
	.byte	$02
	.byte	$00
	.byte	$00
	.byte	$6B
	.byte	$6B
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$10
	.byte	$10
	.byte	$01
	.byte	$01
	.byte	$00
	.byte	$01
	.byte	$9F
	.byte	$01
	.byte	$01
	.byte	$00
	.byte	$C8
	.byte	$C9
	.byte	$D8
	.byte	$D9
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$D8
	.byte	$D9
	.byte	$01
	.byte	$C8
	.byte	$01
	.byte	$D8
	.byte	$D9
	.byte	$01
	.byte	$01
	.byte	$C9
	.byte	$D8
	.byte	$D9
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$D8
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$D9
	.byte	$01
	.byte	$01
	.byte	$D9
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$00
	.byte	$5E
	.byte	$6C
	.byte	$03
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$6B
	.byte	$6C
	.byte	$00
	.byte	$6B
	.byte	$5E
	.byte	$03
	.byte	$6D
	.byte	$00
	.byte	$6B
	.byte	$5E
	.byte	$6D
	.byte	$5D
	.byte	$00
	.byte	$6D
	.byte	$03
	.byte	$03
	.byte	$6D
	.byte	$00
	.byte	$6D
	.byte	$6D
	.byte	$03
	.byte	$03
	.byte	$00
	.byte	$6D
	.byte	$03
	.byte	$03
	.byte	$03
	.byte	$00
	.byte	$6D
	.byte	$40
	.byte	$6D
	.byte	$40
	.byte	$00
	.byte	$40
	.byte	$6E
	.byte	$6F
	.byte	$40
	.byte	$00
	.byte	$6D
	.byte	$14
	.byte	$03
	.byte	$6F
	.byte	$00
	.byte	$40
	.byte	$6E
	.byte	$40
	.byte	$40
	.byte	$00
	.byte	$6E
	.byte	$6E
	.byte	$40
	.byte	$6E
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$1E
	.byte	$1E
	.byte	$01
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$1C
	.byte	$01
	.byte	$00
	.byte	$00
	.byte	$1D
	.byte	$00
	.byte	$01
	.byte	$6D
	.byte	$6D
	.byte	$A3
	.byte	$A4
	.byte	$00
	.byte	$5D
	.byte	$5B
	.byte	$40
	.byte	$9F
	.byte	$00
	.byte	$5C
	.byte	$80
	.byte	$9F
	.byte	$01
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$5B
	.byte	$5C
	.byte	$00
	.byte	$5F
	.byte	$00
	.byte	$9F
	.byte	$5F
	.byte	$00
	.byte	$5B
	.byte	$5C
	.byte	$9F
	.byte	$01
	.byte	$00
	.byte	$6B
	.byte	$6C
	.byte	$01
	.byte	$A3
	.byte	$00
	.byte	$6D
	.byte	$03
	.byte	$A4
	.byte	$03
	.byte	$00
	.byte	$A3
	.byte	$A4
	.byte	$9F
	.byte	$9F
	.byte	$00
	.byte	$03
	.byte	$6D
	.byte	$A4
	.byte	$03
	.byte	$00
	.byte	$9F
	.byte	$9F
	.byte	$9F
	.byte	$9F
	.byte	$00
	.byte	$01
	.byte	$01
	.byte	$9F
	.byte	$9F
	.byte	$00
	.byte	$9F
	.byte	$01
	.byte	$9F
	.byte	$9F
	.byte	$00
	.byte	$6E
	.byte	$9F
	.byte	$6E
	.byte	$8F
	.byte	$00
	.byte	$8F
	.byte	$7F
	.byte	$6E
	.byte	$6E
	.byte	$00
	.byte	$9F
	.byte	$9F
	.byte	$8F
	.byte	$7F
	.byte	$00
	.byte	$7F
	.byte	$9F
	.byte	$6E
	.byte	$8F
	.byte	$00
_starting_x_place:
	.byte	$1E
_starting_y_place:
	.byte	$A0
_stage1_level_0_enemies:
	.byte	$FF
_stage1_Enemy_list:
	.addr	_stage1_level_0_enemies
_stage1_level_0_entities:
	.byte	$FF
_stage1_entity_list:
	.addr	_stage1_level_0_entities
_stage2_level0_0:
	.byte	$00
	.byte	$04
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$00
	.byte	$04
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$00
	.byte	$04
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$70
	.byte	$71
	.byte	$71
	.byte	$71
	.byte	$72
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$00
	.byte	$04
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$00
	.byte	$04
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$70
	.byte	$71
	.byte	$00
	.byte	$04
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$00
	.byte	$04
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$00
	.byte	$04
	.byte	$71
	.byte	$72
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$3D
	.byte	$00
	.byte	$04
	.byte	$40
	.byte	$3D
	.byte	$40
	.byte	$40
	.byte	$3C
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$3C
	.byte	$40
	.byte	$40
	.byte	$07
	.byte	$09
	.byte	$09
	.byte	$00
	.byte	$04
	.byte	$3B
	.byte	$38
	.byte	$39
	.byte	$38
	.byte	$3B
	.byte	$39
	.byte	$38
	.byte	$3A
	.byte	$38
	.byte	$3A
	.byte	$3B
	.byte	$05
	.byte	$01
	.byte	$01
	.byte	$00
	.byte	$04
	.byte	$07
	.byte	$08
	.byte	$0A
	.byte	$38
	.byte	$3A
	.byte	$38
	.byte	$38
	.byte	$38
	.byte	$07
	.byte	$08
	.byte	$0A
	.byte	$03
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$01
	.byte	$06
	.byte	$39
	.byte	$38
	.byte	$3A
	.byte	$38
	.byte	$39
	.byte	$05
	.byte	$01
	.byte	$01
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$04
	.byte	$07
	.byte	$08
	.byte	$08
	.byte	$08
	.byte	$0A
	.byte	$03
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
_stage2_level0_1:
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$34
	.byte	$40
	.byte	$0B
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$03
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$34
	.byte	$40
	.byte	$0C
	.byte	$40
	.byte	$40
	.byte	$70
	.byte	$03
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$34
	.byte	$40
	.byte	$0D
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$03
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$34
	.byte	$20
	.byte	$21
	.byte	$22
	.byte	$23
	.byte	$40
	.byte	$03
	.byte	$71
	.byte	$72
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$34
	.byte	$24
	.byte	$25
	.byte	$26
	.byte	$27
	.byte	$40
	.byte	$03
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$34
	.byte	$28
	.byte	$29
	.byte	$CA
	.byte	$2A
	.byte	$C9
	.byte	$03
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$40
	.byte	$3C
	.byte	$40
	.byte	$3D
	.byte	$40
	.byte	$40
	.byte	$34
	.byte	$2C
	.byte	$2D
	.byte	$C7
	.byte	$2B
	.byte	$3D
	.byte	$03
	.byte	$3C
	.byte	$40
	.byte	$40
	.byte	$07
	.byte	$09
	.byte	$09
	.byte	$09
	.byte	$09
	.byte	$09
	.byte	$09
	.byte	$09
	.byte	$09
	.byte	$09
	.byte	$09
	.byte	$0A
	.byte	$03
	.byte	$08
	.byte	$09
	.byte	$0A
	.byte	$05
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$00
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
_stage2_levels_list:
_stage2_max_rooms:
	.byte	$01
_stage2_offsets:
	.byte	$00
_stage2_bg_palette:
	.byte	$21
	.byte	$07
	.byte	$17
	.byte	$37
	.byte	$21
	.byte	$30
	.byte	$32
	.byte	$02
	.byte	$21
	.byte	$0F
	.byte	$17
	.byte	$06
	.byte	$21
	.byte	$0F
	.byte	$18
	.byte	$28
_stage2_metatiles:
	.byte	$6D
	.byte	$6E
	.byte	$7D
	.byte	$7E
	.byte	$02
	.byte	$5D
	.byte	$5E
	.byte	$7D
	.byte	$7E
	.byte	$02
	.byte	$6D
	.byte	$6E
	.byte	$8D
	.byte	$8E
	.byte	$02
	.byte	$6C
	.byte	$6E
	.byte	$7C
	.byte	$7E
	.byte	$02
	.byte	$6D
	.byte	$6F
	.byte	$7D
	.byte	$7F
	.byte	$02
	.byte	$5C
	.byte	$5E
	.byte	$7C
	.byte	$7E
	.byte	$02
	.byte	$5D
	.byte	$5F
	.byte	$7D
	.byte	$7F
	.byte	$02
	.byte	$AD
	.byte	$9E
	.byte	$AE
	.byte	$AF
	.byte	$03
	.byte	$9D
	.byte	$9E
	.byte	$AE
	.byte	$AF
	.byte	$03
	.byte	$9E
	.byte	$9D
	.byte	$AF
	.byte	$AF
	.byte	$03
	.byte	$9D
	.byte	$9F
	.byte	$AE
	.byte	$AF
	.byte	$03
	.byte	$C2
	.byte	$C3
	.byte	$D2
	.byte	$D3
	.byte	$00
	.byte	$E2
	.byte	$E3
	.byte	$F2
	.byte	$F3
	.byte	$00
	.byte	$D4
	.byte	$E7
	.byte	$E4
	.byte	$F7
	.byte	$00
	.byte	$AA
	.byte	$AB
	.byte	$BA
	.byte	$BB
	.byte	$00
	.byte	$CA
	.byte	$CB
	.byte	$DA
	.byte	$DB
	.byte	$00
	.byte	$EA
	.byte	$EB
	.byte	$FA
	.byte	$FB
	.byte	$00
	.byte	$2F
	.byte	$CB
	.byte	$3F
	.byte	$DB
	.byte	$00
	.byte	$A8
	.byte	$A9
	.byte	$B8
	.byte	$B9
	.byte	$00
	.byte	$C8
	.byte	$C9
	.byte	$D8
	.byte	$D9
	.byte	$00
	.byte	$E8
	.byte	$E9
	.byte	$F8
	.byte	$F9
	.byte	$00
	.byte	$88
	.byte	$89
	.byte	$98
	.byte	$99
	.byte	$00
	.byte	$8A
	.byte	$8B
	.byte	$9A
	.byte	$9B
	.byte	$00
	.byte	$6A
	.byte	$6B
	.byte	$7A
	.byte	$7B
	.byte	$00
	.byte	$BD
	.byte	$BC
	.byte	$9C
	.byte	$AC
	.byte	$00
	.byte	$EE
	.byte	$EF
	.byte	$FE
	.byte	$FF
	.byte	$00
	.byte	$CE
	.byte	$CF
	.byte	$DE
	.byte	$DF
	.byte	$00
	.byte	$EC
	.byte	$ED
	.byte	$FC
	.byte	$FD
	.byte	$00
	.byte	$CC
	.byte	$CD
	.byte	$DC
	.byte	$DD
	.byte	$00
	.byte	$CA
	.byte	$BE
	.byte	$BF
	.byte	$F8
	.byte	$00
	.byte	$0F
	.byte	$0E
	.byte	$0D
	.byte	$0C
	.byte	$00
	.byte	$C7
	.byte	$D7
	.byte	$B7
	.byte	$A7
	.byte	$00
	.byte	$00
	.byte	$80
	.byte	$00
	.byte	$90
	.byte	$00
	.byte	$81
	.byte	$82
	.byte	$91
	.byte	$92
	.byte	$00
	.byte	$83
	.byte	$84
	.byte	$93
	.byte	$94
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$95
	.byte	$96
	.byte	$00
	.byte	$00
	.byte	$A0
	.byte	$00
	.byte	$B0
	.byte	$00
	.byte	$A1
	.byte	$A2
	.byte	$B1
	.byte	$B2
	.byte	$00
	.byte	$A3
	.byte	$A4
	.byte	$B3
	.byte	$B4
	.byte	$00
	.byte	$A5
	.byte	$A6
	.byte	$B5
	.byte	$B6
	.byte	$00
	.byte	$00
	.byte	$C0
	.byte	$00
	.byte	$D0
	.byte	$00
	.byte	$C1
	.byte	$00
	.byte	$D1
	.byte	$00
	.byte	$00
	.byte	$C5
	.byte	$C6
	.byte	$D5
	.byte	$D6
	.byte	$00
	.byte	$E5
	.byte	$E6
	.byte	$F5
	.byte	$F6
	.byte	$00
	.byte	$00
	.byte	$E0
	.byte	$00
	.byte	$F0
	.byte	$00
	.byte	$E1
	.byte	$00
	.byte	$F1
	.byte	$00
	.byte	$00
	.byte	$33
	.byte	$33
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$32
	.byte	$01
	.byte	$32
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$02
	.byte	$02
	.byte	$02
	.byte	$02
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$34
	.byte	$34
	.byte	$01
	.byte	$01
	.byte	$35
	.byte	$01
	.byte	$35
	.byte	$01
	.byte	$16
	.byte	$17
	.byte	$16
	.byte	$17
	.byte	$00
	.byte	$04
	.byte	$05
	.byte	$14
	.byte	$15
	.byte	$00
	.byte	$08
	.byte	$09
	.byte	$18
	.byte	$19
	.byte	$00
	.byte	$16
	.byte	$17
	.byte	$16
	.byte	$17
	.byte	$00
	.byte	$03
	.byte	$03
	.byte	$03
	.byte	$03
	.byte	$03
	.byte	$60
	.byte	$03
	.byte	$03
	.byte	$62
	.byte	$03
	.byte	$03
	.byte	$61
	.byte	$03
	.byte	$03
	.byte	$03
	.byte	$03
	.byte	$03
	.byte	$03
	.byte	$72
	.byte	$03
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$70
	.byte	$03
	.byte	$00
	.byte	$00
	.byte	$71
	.byte	$00
	.byte	$03
	.byte	$06
	.byte	$03
	.byte	$03
	.byte	$07
	.byte	$03
	.byte	$03
	.byte	$39
	.byte	$38
	.byte	$03
	.byte	$03
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$02
	.byte	$02
	.byte	$02
	.byte	$02
	.byte	$01
	.byte	$76
	.byte	$77
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$75
	.byte	$65
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$68
	.byte	$68
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$68
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$00
	.byte	$68
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$68
	.byte	$75
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$65
	.byte	$68
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$77
	.byte	$75
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$65
	.byte	$76
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$65
	.byte	$75
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$02
	.byte	$02
	.byte	$76
	.byte	$77
	.byte	$01
	.byte	$02
	.byte	$02
	.byte	$75
	.byte	$65
	.byte	$01
	.byte	$02
	.byte	$02
	.byte	$68
	.byte	$68
	.byte	$01
	.byte	$02
	.byte	$65
	.byte	$68
	.byte	$00
	.byte	$01
	.byte	$75
	.byte	$02
	.byte	$00
	.byte	$68
	.byte	$01
	.byte	$02
	.byte	$02
	.byte	$68
	.byte	$75
	.byte	$01
	.byte	$02
	.byte	$02
	.byte	$65
	.byte	$68
	.byte	$01
	.byte	$02
	.byte	$02
	.byte	$77
	.byte	$75
	.byte	$01
	.byte	$02
	.byte	$02
	.byte	$65
	.byte	$76
	.byte	$01
	.byte	$02
	.byte	$02
	.byte	$65
	.byte	$75
	.byte	$01
	.byte	$37
	.byte	$31
	.byte	$76
	.byte	$77
	.byte	$01
	.byte	$34
	.byte	$34
	.byte	$68
	.byte	$68
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$34
	.byte	$34
	.byte	$01
	.byte	$34
	.byte	$34
	.byte	$02
	.byte	$02
	.byte	$01
	.byte	$75
	.byte	$34
	.byte	$00
	.byte	$68
	.byte	$01
	.byte	$34
	.byte	$65
	.byte	$68
	.byte	$00
	.byte	$01
	.byte	$37
	.byte	$01
	.byte	$75
	.byte	$37
	.byte	$01
	.byte	$01
	.byte	$31
	.byte	$31
	.byte	$65
	.byte	$01
	.byte	$75
	.byte	$37
	.byte	$00
	.byte	$75
	.byte	$01
	.byte	$31
	.byte	$65
	.byte	$65
	.byte	$00
	.byte	$01
	.byte	$37
	.byte	$01
	.byte	$75
	.byte	$34
	.byte	$01
	.byte	$01
	.byte	$31
	.byte	$34
	.byte	$65
	.byte	$01
	.byte	$34
	.byte	$37
	.byte	$68
	.byte	$75
	.byte	$01
	.byte	$31
	.byte	$34
	.byte	$65
	.byte	$68
	.byte	$01
	.byte	$37
	.byte	$31
	.byte	$02
	.byte	$02
	.byte	$01
	.byte	$02
	.byte	$34
	.byte	$02
	.byte	$02
	.byte	$01
	.byte	$34
	.byte	$02
	.byte	$02
	.byte	$02
	.byte	$01
	.byte	$37
	.byte	$01
	.byte	$02
	.byte	$37
	.byte	$01
	.byte	$01
	.byte	$31
	.byte	$31
	.byte	$02
	.byte	$01
	.byte	$02
	.byte	$37
	.byte	$02
	.byte	$02
	.byte	$01
	.byte	$31
	.byte	$02
	.byte	$02
	.byte	$02
	.byte	$01
	.byte	$37
	.byte	$01
	.byte	$02
	.byte	$34
	.byte	$01
	.byte	$01
	.byte	$31
	.byte	$34
	.byte	$02
	.byte	$01
	.byte	$34
	.byte	$37
	.byte	$02
	.byte	$02
	.byte	$01
	.byte	$31
	.byte	$34
	.byte	$02
	.byte	$02
	.byte	$01
	.byte	$00
	.byte	$1C
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$1E
	.byte	$1E
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$1D
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$00
	.byte	$00
	.byte	$78
	.byte	$79
	.byte	$01
	.byte	$00
	.byte	$00
	.byte	$74
	.byte	$64
	.byte	$01
	.byte	$00
	.byte	$00
	.byte	$67
	.byte	$67
	.byte	$01
	.byte	$00
	.byte	$00
	.byte	$67
	.byte	$00
	.byte	$01
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$67
	.byte	$01
	.byte	$00
	.byte	$00
	.byte	$67
	.byte	$74
	.byte	$01
	.byte	$00
	.byte	$00
	.byte	$64
	.byte	$67
	.byte	$01
	.byte	$00
	.byte	$00
	.byte	$79
	.byte	$74
	.byte	$01
	.byte	$00
	.byte	$00
	.byte	$64
	.byte	$78
	.byte	$01
	.byte	$00
	.byte	$00
	.byte	$64
	.byte	$74
	.byte	$01
	.byte	$78
	.byte	$79
	.byte	$02
	.byte	$02
	.byte	$01
	.byte	$74
	.byte	$64
	.byte	$02
	.byte	$02
	.byte	$01
	.byte	$67
	.byte	$67
	.byte	$02
	.byte	$02
	.byte	$01
	.byte	$67
	.byte	$00
	.byte	$02
	.byte	$64
	.byte	$01
	.byte	$00
	.byte	$67
	.byte	$74
	.byte	$02
	.byte	$01
	.byte	$67
	.byte	$74
	.byte	$02
	.byte	$02
	.byte	$01
	.byte	$64
	.byte	$67
	.byte	$02
	.byte	$02
	.byte	$01
	.byte	$79
	.byte	$74
	.byte	$02
	.byte	$02
	.byte	$01
	.byte	$64
	.byte	$78
	.byte	$02
	.byte	$02
	.byte	$01
	.byte	$64
	.byte	$74
	.byte	$02
	.byte	$02
	.byte	$01
	.byte	$78
	.byte	$79
	.byte	$36
	.byte	$30
	.byte	$01
	.byte	$67
	.byte	$67
	.byte	$33
	.byte	$33
	.byte	$01
	.byte	$33
	.byte	$33
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$02
	.byte	$02
	.byte	$33
	.byte	$33
	.byte	$01
	.byte	$00
	.byte	$67
	.byte	$74
	.byte	$33
	.byte	$01
	.byte	$67
	.byte	$00
	.byte	$33
	.byte	$64
	.byte	$01
	.byte	$74
	.byte	$36
	.byte	$36
	.byte	$01
	.byte	$01
	.byte	$30
	.byte	$64
	.byte	$01
	.byte	$30
	.byte	$01
	.byte	$00
	.byte	$74
	.byte	$74
	.byte	$36
	.byte	$01
	.byte	$64
	.byte	$00
	.byte	$30
	.byte	$64
	.byte	$01
	.byte	$74
	.byte	$33
	.byte	$36
	.byte	$01
	.byte	$01
	.byte	$33
	.byte	$64
	.byte	$01
	.byte	$30
	.byte	$01
	.byte	$67
	.byte	$74
	.byte	$33
	.byte	$36
	.byte	$01
	.byte	$64
	.byte	$67
	.byte	$30
	.byte	$33
	.byte	$01
	.byte	$02
	.byte	$02
	.byte	$36
	.byte	$30
	.byte	$01
	.byte	$02
	.byte	$02
	.byte	$02
	.byte	$33
	.byte	$01
	.byte	$02
	.byte	$02
	.byte	$33
	.byte	$02
	.byte	$01
	.byte	$02
	.byte	$36
	.byte	$36
	.byte	$01
	.byte	$01
	.byte	$30
	.byte	$02
	.byte	$01
	.byte	$30
	.byte	$01
	.byte	$02
	.byte	$02
	.byte	$02
	.byte	$36
	.byte	$01
	.byte	$02
	.byte	$02
	.byte	$30
	.byte	$02
	.byte	$01
	.byte	$02
	.byte	$33
	.byte	$36
	.byte	$01
	.byte	$01
	.byte	$33
	.byte	$02
	.byte	$01
	.byte	$30
	.byte	$01
	.byte	$02
	.byte	$02
	.byte	$33
	.byte	$36
	.byte	$01
	.byte	$02
	.byte	$02
	.byte	$30
	.byte	$33
	.byte	$01
	.byte	$63
	.byte	$00
	.byte	$73
	.byte	$00
	.byte	$01
	.byte	$64
	.byte	$00
	.byte	$65
	.byte	$00
	.byte	$01
	.byte	$69
	.byte	$00
	.byte	$69
	.byte	$00
	.byte	$01
	.byte	$69
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$00
	.byte	$00
	.byte	$69
	.byte	$00
	.byte	$01
	.byte	$69
	.byte	$00
	.byte	$64
	.byte	$00
	.byte	$01
	.byte	$65
	.byte	$00
	.byte	$69
	.byte	$00
	.byte	$01
	.byte	$73
	.byte	$00
	.byte	$64
	.byte	$00
	.byte	$01
	.byte	$65
	.byte	$00
	.byte	$63
	.byte	$00
	.byte	$01
	.byte	$65
	.byte	$00
	.byte	$64
	.byte	$00
	.byte	$01
	.byte	$02
	.byte	$63
	.byte	$02
	.byte	$73
	.byte	$01
	.byte	$02
	.byte	$64
	.byte	$02
	.byte	$65
	.byte	$01
	.byte	$02
	.byte	$69
	.byte	$02
	.byte	$69
	.byte	$01
	.byte	$02
	.byte	$69
	.byte	$65
	.byte	$00
	.byte	$01
	.byte	$64
	.byte	$00
	.byte	$02
	.byte	$69
	.byte	$01
	.byte	$02
	.byte	$65
	.byte	$65
	.byte	$00
	.byte	$01
	.byte	$02
	.byte	$65
	.byte	$02
	.byte	$69
	.byte	$01
	.byte	$02
	.byte	$69
	.byte	$02
	.byte	$64
	.byte	$01
	.byte	$02
	.byte	$73
	.byte	$02
	.byte	$64
	.byte	$01
	.byte	$02
	.byte	$65
	.byte	$02
	.byte	$63
	.byte	$01
	.byte	$30
	.byte	$63
	.byte	$31
	.byte	$73
	.byte	$01
	.byte	$35
	.byte	$69
	.byte	$35
	.byte	$69
	.byte	$01
	.byte	$01
	.byte	$35
	.byte	$01
	.byte	$35
	.byte	$01
	.byte	$35
	.byte	$02
	.byte	$35
	.byte	$02
	.byte	$01
	.byte	$35
	.byte	$69
	.byte	$65
	.byte	$00
	.byte	$01
	.byte	$64
	.byte	$00
	.byte	$35
	.byte	$69
	.byte	$01
	.byte	$01
	.byte	$35
	.byte	$31
	.byte	$65
	.byte	$01
	.byte	$30
	.byte	$64
	.byte	$01
	.byte	$35
	.byte	$01
	.byte	$35
	.byte	$69
	.byte	$30
	.byte	$64
	.byte	$01
	.byte	$31
	.byte	$65
	.byte	$35
	.byte	$69
	.byte	$01
	.byte	$30
	.byte	$02
	.byte	$31
	.byte	$02
	.byte	$01
	.byte	$00
	.byte	$00
	.byte	$64
	.byte	$00
	.byte	$01
	.byte	$00
	.byte	$75
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$30
	.byte	$02
	.byte	$01
	.byte	$35
	.byte	$01
	.byte	$01
	.byte	$35
	.byte	$31
	.byte	$02
	.byte	$01
	.byte	$35
	.byte	$02
	.byte	$30
	.byte	$02
	.byte	$01
	.byte	$31
	.byte	$02
	.byte	$35
	.byte	$02
	.byte	$01
	.byte	$64
	.byte	$00
	.byte	$02
	.byte	$64
	.byte	$01
	.byte	$00
	.byte	$74
	.byte	$74
	.byte	$02
	.byte	$01
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$85
	.byte	$00
	.byte	$86
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$96
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$C4
	.byte	$00
	.byte	$00
	.byte	$02
	.byte	$02
	.byte	$32
	.byte	$02
	.byte	$36
	.byte	$01
	.byte	$02
	.byte	$37
	.byte	$02
	.byte	$32
	.byte	$01
	.byte	$02
	.byte	$36
	.byte	$32
	.byte	$01
	.byte	$01
	.byte	$32
	.byte	$01
	.byte	$02
	.byte	$37
	.byte	$01
	.byte	$65
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$00
	.byte	$87
	.byte	$00
	.byte	$97
	.byte	$01
	.byte	$00
	.byte	$74
	.byte	$00
	.byte	$75
	.byte	$01
	.byte	$00
	.byte	$66
	.byte	$00
	.byte	$66
	.byte	$01
	.byte	$00
	.byte	$66
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$66
	.byte	$01
	.byte	$00
	.byte	$66
	.byte	$00
	.byte	$74
	.byte	$01
	.byte	$00
	.byte	$75
	.byte	$00
	.byte	$66
	.byte	$01
	.byte	$00
	.byte	$75
	.byte	$00
	.byte	$74
	.byte	$01
	.byte	$75
	.byte	$02
	.byte	$00
	.byte	$75
	.byte	$01
	.byte	$87
	.byte	$36
	.byte	$97
	.byte	$37
	.byte	$01
	.byte	$66
	.byte	$32
	.byte	$66
	.byte	$32
	.byte	$01
	.byte	$32
	.byte	$01
	.byte	$32
	.byte	$01
	.byte	$01
	.byte	$02
	.byte	$32
	.byte	$02
	.byte	$32
	.byte	$01
	.byte	$32
	.byte	$01
	.byte	$75
	.byte	$37
	.byte	$01
	.byte	$74
	.byte	$36
	.byte	$32
	.byte	$01
	.byte	$01
	.byte	$66
	.byte	$32
	.byte	$74
	.byte	$36
	.byte	$01
	.byte	$75
	.byte	$37
	.byte	$66
	.byte	$32
	.byte	$01
	.byte	$02
	.byte	$36
	.byte	$02
	.byte	$37
	.byte	$01
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$00
	.byte	$10
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$12
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$13
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$1A
	.byte	$00
	.byte	$00
_stage2_palette_sp:
	.byte	$21
	.byte	$17
	.byte	$24
	.byte	$36
	.byte	$21
	.byte	$05
	.byte	$16
	.byte	$37
	.byte	$21
	.byte	$0F
	.byte	$11
	.byte	$30
	.byte	$21
	.byte	$17
	.byte	$3D
	.byte	$37
_metatile:
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
_stage2_starting_x_place:
	.byte	$1E
_stage2_starting_y_place:
	.byte	$A0
_stage2_level_0_enemies:
	.byte	$FF
_stage2_Enemy_list:
	.addr	_stage2_level_0_enemies
_stage2_level_0_entities:
	.byte	$FF
_stage2_entity_list:
	.addr	_stage2_level_0_entities
_levels_per_stage:
	.byte	$08
	.byte	$02
	.byte	$02
	.byte	$02
	.byte	$02
_titletiled_0:
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$DD
	.byte	$DC
	.byte	$DC
	.byte	$DE
	.byte	$50
	.byte	$DC
	.byte	$DE
	.byte	$95
	.byte	$91
	.byte	$97
	.byte	$50
	.byte	$DD
	.byte	$DC
	.byte	$DE
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$50
	.byte	$96
	.byte	$91
	.byte	$2D
	.byte	$89
	.byte	$97
	.byte	$C6
	.byte	$C6
	.byte	$C6
	.byte	$C6
	.byte	$C6
	.byte	$C6
	.byte	$C6
	.byte	$C6
	.byte	$C6
	.byte	$C6
	.byte	$50
	.byte	$91
	.byte	$89
	.byte	$94
	.byte	$89
	.byte	$94
	.byte	$C6
	.byte	$C6
	.byte	$C6
	.byte	$C6
	.byte	$C6
	.byte	$C6
	.byte	$C6
	.byte	$C6
	.byte	$C6
	.byte	$C6
	.byte	$50
	.byte	$2C
	.byte	$91
	.byte	$89
	.byte	$94
	.byte	$2A
	.byte	$C6
	.byte	$C6
	.byte	$C6
	.byte	$C6
	.byte	$C6
	.byte	$C6
	.byte	$C6
	.byte	$C6
	.byte	$C6
	.byte	$C6
	.byte	$50
	.byte	$49
	.byte	$2C
	.byte	$90
	.byte	$90
	.byte	$48
	.byte	$C6
	.byte	$C6
	.byte	$C6
	.byte	$C6
	.byte	$C6
	.byte	$C6
	.byte	$C6
	.byte	$C6
	.byte	$C6
	.byte	$C6
	.byte	$50
	.byte	$50
	.byte	$49
	.byte	$6A
	.byte	$48
	.byte	$50
	.byte	$50
	.byte	$D1
	.byte	$D3
	.byte	$E0
	.byte	$E1
	.byte	$E2
	.byte	$50
	.byte	$50
	.byte	$D1
	.byte	$D3
	.byte	$D1
	.byte	$D2
	.byte	$E4
	.byte	$68
	.byte	$58
	.byte	$59
	.byte	$D0
	.byte	$D4
	.byte	$D9
	.byte	$EC
	.byte	$EE
	.byte	$E9
	.byte	$E3
	.byte	$D0
	.byte	$D4
	.byte	$D9
	.byte	$BE
	.byte	$BE
	.byte	$BD
	.byte	$69
	.byte	$BD
	.byte	$BC
	.byte	$B0
	.byte	$BD
	.byte	$B0
	.byte	$BC
	.byte	$BD
	.byte	$BC
	.byte	$B0
	.byte	$BD
	.byte	$BE
	.byte	$BD
	.byte	$B0
	.byte	$B8
	.byte	$B0
	.byte	$68
	.byte	$B0
	.byte	$BE
	.byte	$BC
	.byte	$B0
	.byte	$BE
	.byte	$B6
	.byte	$7E
	.byte	$7F
	.byte	$5F
	.byte	$B9
	.byte	$B0
	.byte	$BC
	.byte	$BC
	.byte	$4D
	.byte	$6B
	.byte	$6C
	.byte	$6D
	.byte	$4F
	.byte	$B0
	.byte	$A7
	.byte	$B0
	.byte	$B8
	.byte	$5C
	.byte	$5D
	.byte	$5E
	.byte	$A8
	.byte	$A9
	.byte	$AA
	.byte	$B0
	.byte	$4A
	.byte	$4B
	.byte	$4B
	.byte	$4B
	.byte	$4C
	.byte	$B6
	.byte	$B7
	.byte	$B8
	.byte	$AB
	.byte	$A7
	.byte	$B0
	.byte	$A8
	.byte	$A9
	.byte	$B0
	.byte	$A9
	.byte	$B6
	.byte	$B7
	.byte	$B0
	.byte	$B0
	.byte	$B8
	.byte	$B0
	.byte	$B6
	.byte	$B0
	.byte	$B0
	.byte	$A9
	.byte	$B0
	.byte	$AA
	.byte	$B0
	.byte	$B7
	.byte	$B0
	.byte	$B9
	.byte	$A7
	.byte	$A8
	.byte	$AB
	.byte	$AA
	.byte	$B0
	.byte	$A7
	.byte	$A8
	.byte	$B0
	.byte	$B0
	.byte	$B0
	.byte	$AA
	.byte	$B0
	.byte	$A9
	.byte	$B0
	.byte	$B9
	.byte	$B0
	.byte	$B0
	.byte	$B6
	.byte	$B7
	.byte	$B0
	.byte	$B0
	.byte	$B0
	.byte	$B0
	.byte	$A9
	.byte	$B7
	.byte	$B0
	.byte	$B0
	.byte	$A9
	.byte	$AA
	.byte	$B0
	.byte	$B0
	.byte	$AB
_gameovertiled_0:
	.byte	$3A
	.byte	$3B
	.byte	$02
	.byte	$14
	.byte	$03
	.byte	$10
	.byte	$14
	.byte	$10
	.byte	$3C
	.byte	$10
	.byte	$01
	.byte	$10
	.byte	$10
	.byte	$01
	.byte	$02
	.byte	$03
	.byte	$00
	.byte	$01
	.byte	$10
	.byte	$10
	.byte	$01
	.byte	$14
	.byte	$51
	.byte	$1A
	.byte	$03
	.byte	$12
	.byte	$03
	.byte	$1A
	.byte	$00
	.byte	$10
	.byte	$3A
	.byte	$3B
	.byte	$02
	.byte	$03
	.byte	$14
	.byte	$51
	.byte	$17
	.byte	$CE
	.byte	$C9
	.byte	$CD
	.byte	$51
	.byte	$51
	.byte	$51
	.byte	$CD
	.byte	$19
	.byte	$18
	.byte	$10
	.byte	$10
	.byte	$14
	.byte	$51
	.byte	$51
	.byte	$CC
	.byte	$CA
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$CD
	.byte	$19
	.byte	$1B
	.byte	$51
	.byte	$CC
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$CB
	.byte	$51
	.byte	$CC
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$CB
	.byte	$C9
	.byte	$C9
	.byte	$BF
	.byte	$C3
	.byte	$AF
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$BF
	.byte	$C3
	.byte	$AF
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$C1
	.byte	$C0
	.byte	$C2
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$C1
	.byte	$C0
	.byte	$C2
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$CB
	.byte	$C4
	.byte	$CC
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$CB
	.byte	$C4
	.byte	$CC
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$BF
	.byte	$BF
	.byte	$51
	.byte	$AF
	.byte	$AF
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$BF
	.byte	$51
	.byte	$51
	.byte	$51
	.byte	$51
	.byte	$51
	.byte	$CF
	.byte	$AF
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$BF
	.byte	$51
	.byte	$51
	.byte	$51
	.byte	$51
	.byte	$51
	.byte	$51
	.byte	$51
	.byte	$CF
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$C9
	.byte	$04
	.byte	$05
	.byte	$07
	.byte	$07
	.byte	$08
	.byte	$09
	.byte	$06
	.byte	$04
	.byte	$06
	.byte	$06
	.byte	$07
	.byte	$08
	.byte	$09
	.byte	$0A
	.byte	$0B
	.byte	$04
	.byte	$00
	.byte	$02
	.byte	$3A
	.byte	$3B
	.byte	$10
	.byte	$00
	.byte	$01
	.byte	$00
	.byte	$03
	.byte	$02
	.byte	$10
	.byte	$00
	.byte	$3C
	.byte	$01
	.byte	$3A
	.byte	$3B
.segment	"BANK3"
_iceworld_0:
	.byte	$74
	.byte	$70
	.byte	$74
	.byte	$70
	.byte	$71
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$73
	.byte	$72
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$73
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$A0
	.byte	$A1
	.byte	$76
	.byte	$77
	.byte	$61
	.byte	$61
	.byte	$A0
	.byte	$96
	.byte	$AC
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$A0
	.byte	$96
	.byte	$97
	.byte	$B6
	.byte	$61
	.byte	$61
	.byte	$A0
	.byte	$96
	.byte	$97
	.byte	$A6
	.byte	$BA
	.byte	$AA
	.byte	$AB
	.byte	$61
	.byte	$A0
	.byte	$96
	.byte	$97
	.byte	$A6
	.byte	$A7
	.byte	$B8
	.byte	$A0
	.byte	$A1
	.byte	$A2
	.byte	$B6
	.byte	$B7
	.byte	$B8
	.byte	$B9
	.byte	$A9
	.byte	$92
	.byte	$93
	.byte	$94
	.byte	$B6
	.byte	$B8
	.byte	$B6
	.byte	$B8
	.byte	$B6
	.byte	$83
	.byte	$84
	.byte	$85
	.byte	$85
	.byte	$86
	.byte	$84
	.byte	$85
	.byte	$87
	.byte	$8A
	.byte	$88
	.byte	$8C
	.byte	$84
	.byte	$85
	.byte	$86
	.byte	$84
	.byte	$85
	.byte	$7E
	.byte	$7F
	.byte	$62
	.byte	$78
	.byte	$79
	.byte	$62
	.byte	$7C
	.byte	$7E
	.byte	$7F
	.byte	$7E
	.byte	$7F
	.byte	$62
	.byte	$7C
	.byte	$7A
	.byte	$7B
	.byte	$62
	.byte	$62
	.byte	$78
	.byte	$79
	.byte	$7E
	.byte	$7F
	.byte	$62
	.byte	$7C
	.byte	$62
	.byte	$78
	.byte	$79
	.byte	$62
	.byte	$78
	.byte	$79
	.byte	$62
	.byte	$62
	.byte	$7C
	.byte	$00
	.byte	$01
	.byte	$01
	.byte	$03
	.byte	$02
	.byte	$01
	.byte	$0E
	.byte	$0F
	.byte	$02
	.byte	$03
	.byte	$02
	.byte	$01
	.byte	$02
	.byte	$0D
	.byte	$01
	.byte	$09
	.byte	$47
	.byte	$12
	.byte	$16
	.byte	$18
	.byte	$13
	.byte	$14
	.byte	$1E
	.byte	$1F
	.byte	$19
	.byte	$12
	.byte	$13
	.byte	$31
	.byte	$44
	.byte	$1D
	.byte	$12
	.byte	$17
	.byte	$10
	.byte	$11
	.byte	$12
	.byte	$13
	.byte	$14
	.byte	$28
	.byte	$2E
	.byte	$2F
	.byte	$10
	.byte	$11
	.byte	$12
	.byte	$13
	.byte	$14
	.byte	$15
	.byte	$16
	.byte	$30
	.byte	$20
	.byte	$21
	.byte	$22
	.byte	$49
	.byte	$24
	.byte	$25
	.byte	$14
	.byte	$19
	.byte	$20
	.byte	$21
	.byte	$22
	.byte	$4A
	.byte	$24
	.byte	$25
	.byte	$26
	.byte	$32
_iceworld_1:
	.byte	$72
	.byte	$72
	.byte	$70
	.byte	$71
	.byte	$61
	.byte	$61
	.byte	$73
	.byte	$74
	.byte	$74
	.byte	$71
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$70
	.byte	$71
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$75
	.byte	$76
	.byte	$77
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$99
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$BA
	.byte	$AA
	.byte	$AB
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$75
	.byte	$76
	.byte	$76
	.byte	$77
	.byte	$61
	.byte	$A8
	.byte	$BB
	.byte	$92
	.byte	$99
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$B8
	.byte	$A9
	.byte	$BB
	.byte	$BB
	.byte	$AA
	.byte	$AB
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$86
	.byte	$87
	.byte	$88
	.byte	$89
	.byte	$8A
	.byte	$8B
	.byte	$8D
	.byte	$81
	.byte	$82
	.byte	$80
	.byte	$81
	.byte	$82
	.byte	$80
	.byte	$81
	.byte	$82
	.byte	$80
	.byte	$62
	.byte	$62
	.byte	$62
	.byte	$7C
	.byte	$7A
	.byte	$7B
	.byte	$62
	.byte	$7C
	.byte	$7E
	.byte	$7F
	.byte	$62
	.byte	$7E
	.byte	$7F
	.byte	$62
	.byte	$7A
	.byte	$7B
	.byte	$62
	.byte	$78
	.byte	$79
	.byte	$7A
	.byte	$7B
	.byte	$62
	.byte	$62
	.byte	$7A
	.byte	$7B
	.byte	$78
	.byte	$79
	.byte	$7A
	.byte	$7B
	.byte	$7C
	.byte	$62
	.byte	$78
	.byte	$62
	.byte	$62
	.byte	$62
	.byte	$62
	.byte	$62
	.byte	$62
	.byte	$7A
	.byte	$7B
	.byte	$62
	.byte	$7A
	.byte	$7B
	.byte	$62
	.byte	$62
	.byte	$62
	.byte	$7A
	.byte	$7B
	.byte	$0A
	.byte	$01
	.byte	$09
	.byte	$7C
	.byte	$7F
	.byte	$62
	.byte	$62
	.byte	$62
	.byte	$7C
	.byte	$7E
	.byte	$62
	.byte	$8E
	.byte	$42
	.byte	$8F
	.byte	$62
	.byte	$7E
	.byte	$31
	.byte	$48
	.byte	$4C
	.byte	$62
	.byte	$7A
	.byte	$7B
	.byte	$8E
	.byte	$40
	.byte	$41
	.byte	$8F
	.byte	$62
	.byte	$7A
	.byte	$7B
	.byte	$62
	.byte	$8E
	.byte	$40
	.byte	$24
	.byte	$25
	.byte	$3A
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
_iceworld_2:
	.byte	$61
	.byte	$73
	.byte	$74
	.byte	$70
	.byte	$70
	.byte	$71
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$73
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$75
	.byte	$76
	.byte	$77
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$90
	.byte	$AC
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$90
	.byte	$97
	.byte	$A9
	.byte	$AA
	.byte	$AB
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$A0
	.byte	$96
	.byte	$A2
	.byte	$A7
	.byte	$B9
	.byte	$BA
	.byte	$92
	.byte	$99
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$A0
	.byte	$96
	.byte	$97
	.byte	$A7
	.byte	$A7
	.byte	$A7
	.byte	$0C
	.byte	$02
	.byte	$01
	.byte	$00
	.byte	$0E
	.byte	$81
	.byte	$82
	.byte	$80
	.byte	$81
	.byte	$82
	.byte	$83
	.byte	$84
	.byte	$85
	.byte	$86
	.byte	$84
	.byte	$85
	.byte	$48
	.byte	$49
	.byte	$12
	.byte	$16
	.byte	$1E
	.byte	$62
	.byte	$62
	.byte	$7E
	.byte	$7F
	.byte	$62
	.byte	$62
	.byte	$7A
	.byte	$7B
	.byte	$7C
	.byte	$7D
	.byte	$62
	.byte	$45
	.byte	$47
	.byte	$26
	.byte	$39
	.byte	$2E
	.byte	$79
	.byte	$62
	.byte	$7A
	.byte	$7B
	.byte	$78
	.byte	$79
	.byte	$62
	.byte	$62
	.byte	$0A
	.byte	$0D
	.byte	$09
	.byte	$4A
	.byte	$48
	.byte	$12
	.byte	$13
	.byte	$14
	.byte	$62
	.byte	$62
	.byte	$62
	.byte	$62
	.byte	$7C
	.byte	$62
	.byte	$7E
	.byte	$7F
	.byte	$3A
	.byte	$1D
	.byte	$26
	.byte	$19
	.byte	$32
	.byte	$20
	.byte	$23
	.byte	$22
	.byte	$7F
	.byte	$78
	.byte	$79
	.byte	$0A
	.byte	$01
	.byte	$03
	.byte	$01
	.byte	$09
	.byte	$32
	.byte	$19
	.byte	$32
	.byte	$39
	.byte	$10
	.byte	$11
	.byte	$30
	.byte	$46
	.byte	$41
	.byte	$8F
	.byte	$62
	.byte	$12
	.byte	$13
	.byte	$11
	.byte	$49
	.byte	$10
	.byte	$13
	.byte	$14
	.byte	$33
	.byte	$32
	.byte	$20
	.byte	$21
	.byte	$10
	.byte	$11
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$22
	.byte	$20
	.byte	$21
	.byte	$32
	.byte	$20
	.byte	$23
	.byte	$24
	.byte	$16
	.byte	$46
	.byte	$33
	.byte	$46
	.byte	$20
	.byte	$21
_iceworld_3:
	.byte	$74
	.byte	$74
	.byte	$71
	.byte	$61
	.byte	$1B
	.byte	$1C
	.byte	$18
	.byte	$31
	.byte	$26
	.byte	$32
	.byte	$30
	.byte	$31
	.byte	$32
	.byte	$33
	.byte	$32
	.byte	$4C
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$1B
	.byte	$1C
	.byte	$19
	.byte	$15
	.byte	$16
	.byte	$49
	.byte	$45
	.byte	$32
	.byte	$45
	.byte	$15
	.byte	$16
	.byte	$48
	.byte	$49
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$2B
	.byte	$15
	.byte	$16
	.byte	$4D
	.byte	$36
	.byte	$12
	.byte	$13
	.byte	$2A
	.byte	$36
	.byte	$29
	.byte	$1A
	.byte	$AE
	.byte	$38
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$6F
	.byte	$64
	.byte	$67
	.byte	$6B
	.byte	$60
	.byte	$D5
	.byte	$D5
	.byte	$AD
	.byte	$D3
	.byte	$E3
	.byte	$E0
	.byte	$B1
	.byte	$D3
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$6A
	.byte	$C5
	.byte	$60
	.byte	$E0
	.byte	$E0
	.byte	$C4
	.byte	$C3
	.byte	$E0
	.byte	$E5
	.byte	$D0
	.byte	$61
	.byte	$75
	.byte	$76
	.byte	$77
	.byte	$61
	.byte	$61
	.byte	$6A
	.byte	$60
	.byte	$D1
	.byte	$60
	.byte	$B0
	.byte	$D2
	.byte	$D3
	.byte	$D4
	.byte	$E0
	.byte	$E0
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$6A
	.byte	$E4
	.byte	$60
	.byte	$D3
	.byte	$E0
	.byte	$C4
	.byte	$B0
	.byte	$B1
	.byte	$B2
	.byte	$B3
	.byte	$0F
	.byte	$05
	.byte	$04
	.byte	$0D
	.byte	$05
	.byte	$06
	.byte	$02
	.byte	$03
	.byte	$01
	.byte	$02
	.byte	$09
	.byte	$D1
	.byte	$D1
	.byte	$3E
	.byte	$50
	.byte	$53
	.byte	$1F
	.byte	$19
	.byte	$44
	.byte	$1D
	.byte	$28
	.byte	$30
	.byte	$14
	.byte	$22
	.byte	$22
	.byte	$32
	.byte	$3A
	.byte	$E1
	.byte	$B3
	.byte	$3D
	.byte	$22
	.byte	$15
	.byte	$2F
	.byte	$30
	.byte	$17
	.byte	$32
	.byte	$12
	.byte	$16
	.byte	$48
	.byte	$4D
	.byte	$36
	.byte	$38
	.byte	$3B
	.byte	$B0
	.byte	$E0
	.byte	$3D
	.byte	$C5
	.byte	$AE
	.byte	$18
	.byte	$14
	.byte	$24
	.byte	$25
	.byte	$30
	.byte	$31
	.byte	$28
	.byte	$E4
	.byte	$D3
	.byte	$60
	.byte	$E4
	.byte	$60
	.byte	$D3
	.byte	$3D
	.byte	$D2
	.byte	$D3
	.byte	$39
	.byte	$22
	.byte	$34
	.byte	$35
	.byte	$28
	.byte	$15
	.byte	$16
	.byte	$D4
	.byte	$C0
	.byte	$C0
	.byte	$C1
	.byte	$E4
	.byte	$B3
	.byte	$3D
	.byte	$C2
	.byte	$C3
	.byte	$22
	.byte	$39
	.byte	$14
	.byte	$18
	.byte	$14
	.byte	$33
	.byte	$3A
	.byte	$E0
	.byte	$D3
	.byte	$D0
	.byte	$D1
	.byte	$C2
	.byte	$C3
	.byte	$3D
	.byte	$D2
	.byte	$D3
	.byte	$19
	.byte	$30
	.byte	$31
	.byte	$15
	.byte	$16
	.byte	$12
	.byte	$11
	.byte	$50
	.byte	$5B
	.byte	$55
	.byte	$5C
	.byte	$54
	.byte	$53
	.byte	$54
	.byte	$59
	.byte	$5B
	.byte	$47
	.byte	$49
	.byte	$22
	.byte	$32
	.byte	$48
	.byte	$20
	.byte	$21
	.byte	$24
	.byte	$25
	.byte	$32
	.byte	$15
	.byte	$16
	.byte	$32
	.byte	$46
	.byte	$10
	.byte	$11
_iceworld_4:
	.byte	$3C
	.byte	$2C
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$73
	.byte	$74
	.byte	$70
	.byte	$70
	.byte	$71
	.byte	$61
	.byte	$61
	.byte	$3B
	.byte	$3C
	.byte	$2C
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$47
	.byte	$4B
	.byte	$3C
	.byte	$2C
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$36
	.byte	$38
	.byte	$29
	.byte	$2D
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$D2
	.byte	$E4
	.byte	$6D
	.byte	$6F
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$60
	.byte	$D5
	.byte	$6D
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$96
	.byte	$B4
	.byte	$C5
	.byte	$6D
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$75
	.byte	$76
	.byte	$77
	.byte	$61
	.byte	$A0
	.byte	$AB
	.byte	$61
	.byte	$A1
	.byte	$A6
	.byte	$54
	.byte	$55
	.byte	$51
	.byte	$0C
	.byte	$0B
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$96
	.byte	$97
	.byte	$98
	.byte	$A4
	.byte	$B5
	.byte	$A7
	.byte	$16
	.byte	$48
	.byte	$15
	.byte	$17
	.byte	$3A
	.byte	$82
	.byte	$80
	.byte	$81
	.byte	$82
	.byte	$83
	.byte	$84
	.byte	$85
	.byte	$87
	.byte	$8A
	.byte	$8B
	.byte	$86
	.byte	$D3
	.byte	$60
	.byte	$D3
	.byte	$AD
	.byte	$6E
	.byte	$7C
	.byte	$7A
	.byte	$7B
	.byte	$7E
	.byte	$7F
	.byte	$62
	.byte	$7E
	.byte	$7F
	.byte	$7C
	.byte	$7A
	.byte	$7B
	.byte	$C4
	.byte	$C1
	.byte	$E4
	.byte	$D5
	.byte	$6E
	.byte	$62
	.byte	$62
	.byte	$62
	.byte	$62
	.byte	$7C
	.byte	$7A
	.byte	$7B
	.byte	$62
	.byte	$7E
	.byte	$7F
	.byte	$7C
	.byte	$C1
	.byte	$C2
	.byte	$D5
	.byte	$60
	.byte	$6E
	.byte	$62
	.byte	$62
	.byte	$7A
	.byte	$7B
	.byte	$62
	.byte	$62
	.byte	$62
	.byte	$62
	.byte	$62
	.byte	$62
	.byte	$62
	.byte	$D1
	.byte	$C3
	.byte	$E0
	.byte	$D1
	.byte	$6E
	.byte	$62
	.byte	$62
	.byte	$62
	.byte	$62
	.byte	$7C
	.byte	$62
	.byte	$8E
	.byte	$40
	.byte	$43
	.byte	$43
	.byte	$43
	.byte	$54
	.byte	$56
	.byte	$53
	.byte	$54
	.byte	$51
	.byte	$0A
	.byte	$09
	.byte	$62
	.byte	$8E
	.byte	$42
	.byte	$8F
	.byte	$7A
	.byte	$7B
	.byte	$7E
	.byte	$7F
	.byte	$62
	.byte	$24
	.byte	$25
	.byte	$26
	.byte	$27
	.byte	$32
	.byte	$46
	.byte	$28
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
_iceworld_5:
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$73
	.byte	$70
	.byte	$70
	.byte	$74
	.byte	$74
	.byte	$71
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$75
	.byte	$76
	.byte	$77
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$75
	.byte	$76
	.byte	$76
	.byte	$77
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$A0
	.byte	$AB
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$90
	.byte	$AC
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$97
	.byte	$98
	.byte	$99
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$90
	.byte	$97
	.byte	$A9
	.byte	$AA
	.byte	$AB
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$B8
	.byte	$A8
	.byte	$BB
	.byte	$AA
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$96
	.byte	$A2
	.byte	$A7
	.byte	$A8
	.byte	$BB
	.byte	$92
	.byte	$99
	.byte	$61
	.byte	$61
	.byte	$B8
	.byte	$B7
	.byte	$A9
	.byte	$BB
	.byte	$AA
	.byte	$AB
	.byte	$A5
	.byte	$B5
	.byte	$A7
	.byte	$B8
	.byte	$B8
	.byte	$BA
	.byte	$BB
	.byte	$BB
	.byte	$AA
	.byte	$AB
	.byte	$84
	.byte	$85
	.byte	$87
	.byte	$8A
	.byte	$88
	.byte	$8C
	.byte	$84
	.byte	$85
	.byte	$86
	.byte	$84
	.byte	$85
	.byte	$87
	.byte	$8A
	.byte	$88
	.byte	$89
	.byte	$8A
	.byte	$62
	.byte	$78
	.byte	$79
	.byte	$62
	.byte	$62
	.byte	$62
	.byte	$7C
	.byte	$7D
	.byte	$7E
	.byte	$7F
	.byte	$62
	.byte	$62
	.byte	$7E
	.byte	$7F
	.byte	$78
	.byte	$79
	.byte	$62
	.byte	$62
	.byte	$7E
	.byte	$7F
	.byte	$78
	.byte	$79
	.byte	$7A
	.byte	$7B
	.byte	$62
	.byte	$62
	.byte	$78
	.byte	$79
	.byte	$62
	.byte	$62
	.byte	$62
	.byte	$62
	.byte	$78
	.byte	$79
	.byte	$62
	.byte	$62
	.byte	$7C
	.byte	$7D
	.byte	$62
	.byte	$62
	.byte	$62
	.byte	$7A
	.byte	$7B
	.byte	$62
	.byte	$7C
	.byte	$78
	.byte	$79
	.byte	$62
	.byte	$41
	.byte	$8F
	.byte	$78
	.byte	$79
	.byte	$62
	.byte	$78
	.byte	$79
	.byte	$7F
	.byte	$62
	.byte	$8E
	.byte	$40
	.byte	$41
	.byte	$8F
	.byte	$62
	.byte	$62
	.byte	$0A
	.byte	$62
	.byte	$62
	.byte	$8E
	.byte	$40
	.byte	$41
	.byte	$8F
	.byte	$8E
	.byte	$42
	.byte	$8F
	.byte	$62
	.byte	$62
	.byte	$7E
	.byte	$7F
	.byte	$78
	.byte	$79
	.byte	$22
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$4A
_iceworld_6:
	.byte	$61
	.byte	$73
	.byte	$74
	.byte	$74
	.byte	$74
	.byte	$70
	.byte	$74
	.byte	$72
	.byte	$72
	.byte	$72
	.byte	$72
	.byte	$72
	.byte	$72
	.byte	$72
	.byte	$72
	.byte	$72
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$73
	.byte	$74
	.byte	$74
	.byte	$72
	.byte	$72
	.byte	$72
	.byte	$72
	.byte	$72
	.byte	$74
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$75
	.byte	$76
	.byte	$76
	.byte	$77
	.byte	$61
	.byte	$73
	.byte	$74
	.byte	$74
	.byte	$74
	.byte	$71
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$8B
	.byte	$8D
	.byte	$81
	.byte	$82
	.byte	$80
	.byte	$81
	.byte	$82
	.byte	$80
	.byte	$81
	.byte	$82
	.byte	$0C
	.byte	$03
	.byte	$05
	.byte	$06
	.byte	$04
	.byte	$03
	.byte	$7C
	.byte	$62
	.byte	$62
	.byte	$78
	.byte	$79
	.byte	$7E
	.byte	$7C
	.byte	$7D
	.byte	$7E
	.byte	$7F
	.byte	$22
	.byte	$45
	.byte	$19
	.byte	$15
	.byte	$16
	.byte	$15
	.byte	$78
	.byte	$79
	.byte	$62
	.byte	$62
	.byte	$62
	.byte	$0A
	.byte	$01
	.byte	$01
	.byte	$0D
	.byte	$09
	.byte	$49
	.byte	$28
	.byte	$29
	.byte	$48
	.byte	$24
	.byte	$25
	.byte	$78
	.byte	$79
	.byte	$7E
	.byte	$7F
	.byte	$62
	.byte	$10
	.byte	$11
	.byte	$47
	.byte	$1D
	.byte	$26
	.byte	$48
	.byte	$38
	.byte	$39
	.byte	$49
	.byte	$34
	.byte	$35
	.byte	$01
	.byte	$01
	.byte	$00
	.byte	$01
	.byte	$09
	.byte	$20
	.byte	$21
	.byte	$49
	.byte	$4A
	.byte	$10
	.byte	$11
	.byte	$32
	.byte	$32
	.byte	$48
	.byte	$10
	.byte	$11
	.byte	$12
	.byte	$13
	.byte	$14
	.byte	$15
	.byte	$16
	.byte	$45
	.byte	$15
	.byte	$16
	.byte	$32
	.byte	$20
	.byte	$21
	.byte	$49
	.byte	$33
	.byte	$32
	.byte	$20
	.byte	$21
	.byte	$23
	.byte	$24
	.byte	$25
	.byte	$26
	.byte	$27
	.byte	$45
	.byte	$46
	.byte	$30
	.byte	$31
	.byte	$32
	.byte	$33
	.byte	$32
	.byte	$46
	.byte	$10
	.byte	$11
	.byte	$24
_iceworld_7:
	.byte	$72
	.byte	$74
	.byte	$74
	.byte	$74
	.byte	$71
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$1B
	.byte	$1C
	.byte	$15
	.byte	$17
	.byte	$22
	.byte	$44
	.byte	$45
	.byte	$20
	.byte	$71
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$1B
	.byte	$1C
	.byte	$30
	.byte	$31
	.byte	$27
	.byte	$14
	.byte	$15
	.byte	$16
	.byte	$30
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$1B
	.byte	$1C
	.byte	$45
	.byte	$32
	.byte	$49
	.byte	$46
	.byte	$24
	.byte	$25
	.byte	$26
	.byte	$4E
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$2B
	.byte	$15
	.byte	$16
	.byte	$4A
	.byte	$4D
	.byte	$AD
	.byte	$34
	.byte	$35
	.byte	$36
	.byte	$60
	.byte	$61
	.byte	$61
	.byte	$75
	.byte	$76
	.byte	$77
	.byte	$61
	.byte	$6F
	.byte	$64
	.byte	$67
	.byte	$68
	.byte	$6B
	.byte	$B0
	.byte	$60
	.byte	$B2
	.byte	$60
	.byte	$C5
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$6A
	.byte	$6A
	.byte	$E4
	.byte	$60
	.byte	$C2
	.byte	$C3
	.byte	$C4
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$6A
	.byte	$6A
	.byte	$60
	.byte	$C3
	.byte	$D2
	.byte	$D3
	.byte	$D4
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$6A
	.byte	$6A
	.byte	$C5
	.byte	$E1
	.byte	$E2
	.byte	$E3
	.byte	$E4
	.byte	$05
	.byte	$06
	.byte	$05
	.byte	$05
	.byte	$0D
	.byte	$0E
	.byte	$0F
	.byte	$05
	.byte	$06
	.byte	$05
	.byte	$04
	.byte	$08
	.byte	$07
	.byte	$09
	.byte	$60
	.byte	$D1
	.byte	$16
	.byte	$49
	.byte	$18
	.byte	$14
	.byte	$1D
	.byte	$1E
	.byte	$1F
	.byte	$19
	.byte	$10
	.byte	$11
	.byte	$45
	.byte	$17
	.byte	$22
	.byte	$29
	.byte	$60
	.byte	$E1
	.byte	$26
	.byte	$27
	.byte	$28
	.byte	$29
	.byte	$48
	.byte	$2E
	.byte	$2F
	.byte	$46
	.byte	$20
	.byte	$21
	.byte	$49
	.byte	$27
	.byte	$28
	.byte	$4A
	.byte	$50
	.byte	$53
	.byte	$10
	.byte	$11
	.byte	$38
	.byte	$39
	.byte	$30
	.byte	$14
	.byte	$18
	.byte	$31
	.byte	$47
	.byte	$18
	.byte	$2A
	.byte	$24
	.byte	$25
	.byte	$49
	.byte	$45
	.byte	$46
	.byte	$20
	.byte	$21
	.byte	$32
	.byte	$33
	.byte	$32
	.byte	$48
	.byte	$37
	.byte	$32
	.byte	$48
	.byte	$33
	.byte	$45
	.byte	$34
	.byte	$35
	.byte	$15
	.byte	$16
	.byte	$39
	.byte	$32
	.byte	$47
	.byte	$48
	.byte	$30
	.byte	$31
	.byte	$33
	.byte	$47
	.byte	$45
	.byte	$33
	.byte	$46
	.byte	$32
	.byte	$14
	.byte	$19
	.byte	$47
	.byte	$15
	.byte	$16
	.byte	$25
	.byte	$32
	.byte	$18
	.byte	$31
	.byte	$32
	.byte	$10
	.byte	$11
	.byte	$4A
	.byte	$32
	.byte	$30
	.byte	$31
	.byte	$47
	.byte	$33
	.byte	$46
	.byte	$30
	.byte	$31
_iceworld_8:
	.byte	$21
	.byte	$10
	.byte	$11
	.byte	$26
	.byte	$47
	.byte	$45
	.byte	$47
	.byte	$45
	.byte	$26
	.byte	$27
	.byte	$22
	.byte	$47
	.byte	$22
	.byte	$47
	.byte	$30
	.byte	$31
	.byte	$31
	.byte	$20
	.byte	$21
	.byte	$48
	.byte	$37
	.byte	$1A
	.byte	$AD
	.byte	$37
	.byte	$AE
	.byte	$60
	.byte	$36
	.byte	$38
	.byte	$30
	.byte	$31
	.byte	$32
	.byte	$26
	.byte	$60
	.byte	$60
	.byte	$37
	.byte	$E5
	.byte	$B0
	.byte	$B1
	.byte	$B2
	.byte	$B3
	.byte	$B0
	.byte	$B1
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$45
	.byte	$47
	.byte	$27
	.byte	$C1
	.byte	$C2
	.byte	$C3
	.byte	$C4
	.byte	$C0
	.byte	$C1
	.byte	$C2
	.byte	$C3
	.byte	$C0
	.byte	$B0
	.byte	$B1
	.byte	$B2
	.byte	$60
	.byte	$60
	.byte	$4F
	.byte	$10
	.byte	$B4
	.byte	$D2
	.byte	$D3
	.byte	$D4
	.byte	$D0
	.byte	$D1
	.byte	$D2
	.byte	$D3
	.byte	$D0
	.byte	$C0
	.byte	$C1
	.byte	$C2
	.byte	$C3
	.byte	$D5
	.byte	$60
	.byte	$20
	.byte	$C3
	.byte	$E2
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$E1
	.byte	$B0
	.byte	$B1
	.byte	$B2
	.byte	$B3
	.byte	$D1
	.byte	$60
	.byte	$60
	.byte	$D4
	.byte	$C4
	.byte	$AE
	.byte	$D4
	.byte	$B3
	.byte	$B4
	.byte	$D3
	.byte	$B2
	.byte	$B3
	.byte	$60
	.byte	$C1
	.byte	$C2
	.byte	$C3
	.byte	$60
	.byte	$E5
	.byte	$60
	.byte	$D5
	.byte	$D4
	.byte	$B0
	.byte	$E4
	.byte	$C3
	.byte	$C4
	.byte	$C1
	.byte	$C2
	.byte	$C3
	.byte	$C4
	.byte	$D1
	.byte	$D2
	.byte	$D3
	.byte	$D4
	.byte	$60
	.byte	$E4
	.byte	$60
	.byte	$B3
	.byte	$B4
	.byte	$D2
	.byte	$D3
	.byte	$D4
	.byte	$D1
	.byte	$D2
	.byte	$D3
	.byte	$D4
	.byte	$E1
	.byte	$E2
	.byte	$E3
	.byte	$E4
	.byte	$C0
	.byte	$C1
	.byte	$C2
	.byte	$C3
	.byte	$C4
	.byte	$60
	.byte	$E3
	.byte	$E4
	.byte	$E1
	.byte	$60
	.byte	$E3
	.byte	$E4
	.byte	$E3
	.byte	$E4
	.byte	$C3
	.byte	$C4
	.byte	$D0
	.byte	$D1
	.byte	$D2
	.byte	$D3
	.byte	$D4
	.byte	$52
	.byte	$55
	.byte	$51
	.byte	$60
	.byte	$D1
	.byte	$D2
	.byte	$D3
	.byte	$D4
	.byte	$D2
	.byte	$D3
	.byte	$D4
	.byte	$E0
	.byte	$E1
	.byte	$E2
	.byte	$E3
	.byte	$E4
	.byte	$47
	.byte	$48
	.byte	$3A
	.byte	$60
	.byte	$E1
	.byte	$E0
	.byte	$60
	.byte	$D1
	.byte	$E2
	.byte	$E3
	.byte	$E4
	.byte	$60
	.byte	$B0
	.byte	$B1
	.byte	$B2
	.byte	$B3
	.byte	$49
	.byte	$30
	.byte	$31
	.byte	$50
	.byte	$5B
	.byte	$5F
	.byte	$54
	.byte	$55
	.byte	$56
	.byte	$57
	.byte	$51
	.byte	$3E
	.byte	$D1
	.byte	$C1
	.byte	$C2
	.byte	$C3
	.byte	$33
	.byte	$45
	.byte	$4A
	.byte	$10
	.byte	$11
	.byte	$12
	.byte	$13
	.byte	$14
	.byte	$15
	.byte	$16
	.byte	$4E
	.byte	$3D
	.byte	$60
	.byte	$D1
	.byte	$60
	.byte	$60
	.byte	$47
	.byte	$32
	.byte	$46
	.byte	$20
	.byte	$21
	.byte	$22
	.byte	$47
	.byte	$45
	.byte	$4D
	.byte	$1A
	.byte	$D1
	.byte	$3D
	.byte	$D5
	.byte	$B0
	.byte	$E2
	.byte	$60
_iceworld_9:
	.byte	$45
	.byte	$46
	.byte	$47
	.byte	$32
	.byte	$15
	.byte	$16
	.byte	$48
	.byte	$45
	.byte	$1A
	.byte	$E4
	.byte	$B0
	.byte	$3D
	.byte	$B2
	.byte	$B3
	.byte	$B4
	.byte	$B0
	.byte	$15
	.byte	$16
	.byte	$30
	.byte	$31
	.byte	$46
	.byte	$48
	.byte	$45
	.byte	$4E
	.byte	$60
	.byte	$D2
	.byte	$C0
	.byte	$3D
	.byte	$C2
	.byte	$C3
	.byte	$C4
	.byte	$C0
	.byte	$25
	.byte	$4E
	.byte	$27
	.byte	$4D
	.byte	$C5
	.byte	$AE
	.byte	$D1
	.byte	$60
	.byte	$D2
	.byte	$D4
	.byte	$D0
	.byte	$3D
	.byte	$D2
	.byte	$D3
	.byte	$D4
	.byte	$D0
	.byte	$37
	.byte	$D3
	.byte	$AD
	.byte	$E4
	.byte	$C1
	.byte	$C2
	.byte	$E1
	.byte	$E2
	.byte	$E3
	.byte	$E4
	.byte	$E0
	.byte	$3D
	.byte	$E2
	.byte	$E3
	.byte	$E4
	.byte	$E0
	.byte	$60
	.byte	$D1
	.byte	$D2
	.byte	$D0
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$B2
	.byte	$B3
	.byte	$B4
	.byte	$B0
	.byte	$3D
	.byte	$B2
	.byte	$B3
	.byte	$60
	.byte	$B0
	.byte	$E0
	.byte	$E1
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$C1
	.byte	$C2
	.byte	$C3
	.byte	$C4
	.byte	$C0
	.byte	$C1
	.byte	$C2
	.byte	$C3
	.byte	$C4
	.byte	$C0
	.byte	$9B
	.byte	$9B
	.byte	$9B
	.byte	$9B
	.byte	$9C
	.byte	$60
	.byte	$60
	.byte	$D2
	.byte	$60
	.byte	$60
	.byte	$D0
	.byte	$D1
	.byte	$D2
	.byte	$D3
	.byte	$D4
	.byte	$D0
	.byte	$15
	.byte	$17
	.byte	$46
	.byte	$4C
	.byte	$9E
	.byte	$60
	.byte	$60
	.byte	$E2
	.byte	$E3
	.byte	$60
	.byte	$E0
	.byte	$E1
	.byte	$60
	.byte	$E3
	.byte	$E4
	.byte	$E0
	.byte	$26
	.byte	$27
	.byte	$47
	.byte	$4E
	.byte	$9E
	.byte	$60
	.byte	$B1
	.byte	$B2
	.byte	$B3
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$B2
	.byte	$B3
	.byte	$B4
	.byte	$B0
	.byte	$33
	.byte	$4D
	.byte	$E2
	.byte	$E3
	.byte	$9E
	.byte	$60
	.byte	$C1
	.byte	$C2
	.byte	$C3
	.byte	$60
	.byte	$50
	.byte	$58
	.byte	$52
	.byte	$57
	.byte	$54
	.byte	$56
	.byte	$D0
	.byte	$D1
	.byte	$D2
	.byte	$60
	.byte	$9E
	.byte	$D0
	.byte	$D1
	.byte	$D2
	.byte	$3F
	.byte	$3F
	.byte	$22
	.byte	$46
	.byte	$10
	.byte	$11
	.byte	$15
	.byte	$16
	.byte	$E0
	.byte	$E1
	.byte	$E2
	.byte	$60
	.byte	$9E
	.byte	$60
	.byte	$E1
	.byte	$E2
	.byte	$45
	.byte	$47
	.byte	$26
	.byte	$49
	.byte	$20
	.byte	$21
	.byte	$30
	.byte	$31
	.byte	$B0
	.byte	$B1
	.byte	$B2
	.byte	$60
	.byte	$9E
	.byte	$B0
	.byte	$60
	.byte	$B2
	.byte	$B3
	.byte	$36
	.byte	$37
	.byte	$48
	.byte	$1A
	.byte	$38
	.byte	$47
	.byte	$22
	.byte	$C0
	.byte	$C1
	.byte	$C2
	.byte	$60
	.byte	$9E
	.byte	$60
	.byte	$C1
	.byte	$C2
	.byte	$C3
	.byte	$60
	.byte	$C0
	.byte	$C1
	.byte	$C2
	.byte	$C3
	.byte	$60
	.byte	$37
	.byte	$D0
	.byte	$D1
	.byte	$60
	.byte	$60
	.byte	$9E
	.byte	$60
	.byte	$60
	.byte	$D2
	.byte	$D3
	.byte	$D4
	.byte	$D0
	.byte	$D1
	.byte	$D2
	.byte	$D3
	.byte	$D4
	.byte	$E5
_iceworld_10:
	.byte	$C3
	.byte	$C0
	.byte	$60
	.byte	$60
	.byte	$9E
	.byte	$60
	.byte	$60
	.byte	$B4
	.byte	$60
	.byte	$B1
	.byte	$B2
	.byte	$B0
	.byte	$B1
	.byte	$B2
	.byte	$B3
	.byte	$B4
	.byte	$D3
	.byte	$D0
	.byte	$D1
	.byte	$60
	.byte	$9E
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$C0
	.byte	$C1
	.byte	$C2
	.byte	$C0
	.byte	$C1
	.byte	$C2
	.byte	$60
	.byte	$C4
	.byte	$E3
	.byte	$E0
	.byte	$E1
	.byte	$60
	.byte	$9E
	.byte	$D1
	.byte	$D2
	.byte	$D4
	.byte	$D0
	.byte	$D1
	.byte	$D2
	.byte	$D0
	.byte	$60
	.byte	$D2
	.byte	$D3
	.byte	$D4
	.byte	$B1
	.byte	$B2
	.byte	$60
	.byte	$60
	.byte	$9E
	.byte	$E1
	.byte	$E2
	.byte	$60
	.byte	$E0
	.byte	$E1
	.byte	$E2
	.byte	$E0
	.byte	$E1
	.byte	$E2
	.byte	$E3
	.byte	$E4
	.byte	$C1
	.byte	$B0
	.byte	$B1
	.byte	$B2
	.byte	$9E
	.byte	$60
	.byte	$B3
	.byte	$B4
	.byte	$B2
	.byte	$B3
	.byte	$60
	.byte	$60
	.byte	$E0
	.byte	$E1
	.byte	$E2
	.byte	$E3
	.byte	$D1
	.byte	$C0
	.byte	$C1
	.byte	$60
	.byte	$9E
	.byte	$60
	.byte	$C3
	.byte	$C4
	.byte	$C2
	.byte	$C3
	.byte	$60
	.byte	$C4
	.byte	$E0
	.byte	$E1
	.byte	$E2
	.byte	$E3
	.byte	$E1
	.byte	$D0
	.byte	$D1
	.byte	$60
	.byte	$9E
	.byte	$60
	.byte	$60
	.byte	$D1
	.byte	$D2
	.byte	$D3
	.byte	$60
	.byte	$D4
	.byte	$C0
	.byte	$C1
	.byte	$50
	.byte	$53
	.byte	$B1
	.byte	$B2
	.byte	$60
	.byte	$60
	.byte	$9E
	.byte	$60
	.byte	$60
	.byte	$E1
	.byte	$E2
	.byte	$E3
	.byte	$E4
	.byte	$E4
	.byte	$B2
	.byte	$B3
	.byte	$D3
	.byte	$60
	.byte	$C1
	.byte	$B0
	.byte	$B1
	.byte	$60
	.byte	$9E
	.byte	$60
	.byte	$60
	.byte	$C1
	.byte	$C2
	.byte	$C3
	.byte	$C0
	.byte	$C1
	.byte	$C2
	.byte	$C3
	.byte	$C4
	.byte	$E0
	.byte	$D1
	.byte	$C0
	.byte	$60
	.byte	$60
	.byte	$9E
	.byte	$60
	.byte	$D4
	.byte	$D1
	.byte	$50
	.byte	$56
	.byte	$57
	.byte	$51
	.byte	$B3
	.byte	$D3
	.byte	$D4
	.byte	$D3
	.byte	$E1
	.byte	$D0
	.byte	$D1
	.byte	$60
	.byte	$9E
	.byte	$60
	.byte	$60
	.byte	$B0
	.byte	$48
	.byte	$24
	.byte	$25
	.byte	$46
	.byte	$3F
	.byte	$3F
	.byte	$3F
	.byte	$3F
	.byte	$3F
	.byte	$3F
	.byte	$3F
	.byte	$3F
	.byte	$9E
	.byte	$60
	.byte	$B2
	.byte	$C0
	.byte	$D0
	.byte	$34
	.byte	$35
	.byte	$37
	.byte	$39
	.byte	$49
	.byte	$30
	.byte	$31
	.byte	$32
	.byte	$15
	.byte	$16
	.byte	$3B
	.byte	$9E
	.byte	$60
	.byte	$60
	.byte	$D0
	.byte	$D1
	.byte	$D2
	.byte	$D3
	.byte	$60
	.byte	$E4
	.byte	$C2
	.byte	$38
	.byte	$47
	.byte	$45
	.byte	$49
	.byte	$2A
	.byte	$C5
	.byte	$9E
	.byte	$60
	.byte	$E0
	.byte	$E0
	.byte	$E1
	.byte	$E2
	.byte	$E3
	.byte	$60
	.byte	$D1
	.byte	$D2
	.byte	$D3
	.byte	$D4
	.byte	$C1
	.byte	$E3
	.byte	$C5
	.byte	$60
	.byte	$9E
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$D1
	.byte	$D2
	.byte	$D3
	.byte	$D4
	.byte	$E1
	.byte	$E2
	.byte	$E3
	.byte	$E4
_iceworld_11:
	.byte	$C3
	.byte	$C0
	.byte	$60
	.byte	$60
	.byte	$9E
	.byte	$60
	.byte	$60
	.byte	$B4
	.byte	$60
	.byte	$B1
	.byte	$B2
	.byte	$B0
	.byte	$B1
	.byte	$B2
	.byte	$B3
	.byte	$B4
	.byte	$D3
	.byte	$D0
	.byte	$D1
	.byte	$60
	.byte	$9E
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$C0
	.byte	$C1
	.byte	$C2
	.byte	$C0
	.byte	$C1
	.byte	$C2
	.byte	$60
	.byte	$C4
	.byte	$E3
	.byte	$E0
	.byte	$E1
	.byte	$60
	.byte	$9E
	.byte	$D1
	.byte	$D2
	.byte	$D4
	.byte	$D0
	.byte	$D1
	.byte	$D2
	.byte	$D0
	.byte	$60
	.byte	$D2
	.byte	$D3
	.byte	$D4
	.byte	$B1
	.byte	$B2
	.byte	$60
	.byte	$60
	.byte	$9E
	.byte	$E1
	.byte	$E2
	.byte	$60
	.byte	$E0
	.byte	$E1
	.byte	$E2
	.byte	$E0
	.byte	$E1
	.byte	$E2
	.byte	$E3
	.byte	$E4
	.byte	$C1
	.byte	$B0
	.byte	$B1
	.byte	$B2
	.byte	$9E
	.byte	$60
	.byte	$B3
	.byte	$B4
	.byte	$B2
	.byte	$B3
	.byte	$60
	.byte	$60
	.byte	$E0
	.byte	$E1
	.byte	$E2
	.byte	$E3
	.byte	$D1
	.byte	$C0
	.byte	$C1
	.byte	$60
	.byte	$9E
	.byte	$60
	.byte	$C3
	.byte	$C4
	.byte	$C2
	.byte	$C3
	.byte	$60
	.byte	$C4
	.byte	$E0
	.byte	$E1
	.byte	$E2
	.byte	$E3
	.byte	$E1
	.byte	$D0
	.byte	$D1
	.byte	$60
	.byte	$9E
	.byte	$60
	.byte	$60
	.byte	$D1
	.byte	$D2
	.byte	$D3
	.byte	$60
	.byte	$D4
	.byte	$C0
	.byte	$C1
	.byte	$50
	.byte	$53
	.byte	$B1
	.byte	$B2
	.byte	$60
	.byte	$60
	.byte	$9E
	.byte	$60
	.byte	$60
	.byte	$E1
	.byte	$E2
	.byte	$E3
	.byte	$E4
	.byte	$E4
	.byte	$B2
	.byte	$B3
	.byte	$D3
	.byte	$60
	.byte	$C1
	.byte	$B0
	.byte	$B1
	.byte	$60
	.byte	$9E
	.byte	$60
	.byte	$60
	.byte	$C1
	.byte	$C2
	.byte	$C3
	.byte	$C0
	.byte	$C1
	.byte	$C2
	.byte	$C3
	.byte	$C4
	.byte	$E0
	.byte	$D1
	.byte	$C0
	.byte	$60
	.byte	$60
	.byte	$9E
	.byte	$60
	.byte	$D4
	.byte	$D1
	.byte	$50
	.byte	$56
	.byte	$57
	.byte	$51
	.byte	$B3
	.byte	$D3
	.byte	$D4
	.byte	$D3
	.byte	$E1
	.byte	$D0
	.byte	$D1
	.byte	$60
	.byte	$9E
	.byte	$60
	.byte	$60
	.byte	$B0
	.byte	$48
	.byte	$24
	.byte	$25
	.byte	$46
	.byte	$3F
	.byte	$3F
	.byte	$3F
	.byte	$3F
	.byte	$3F
	.byte	$3F
	.byte	$3F
	.byte	$3F
	.byte	$9E
	.byte	$60
	.byte	$B2
	.byte	$C0
	.byte	$D0
	.byte	$34
	.byte	$35
	.byte	$37
	.byte	$39
	.byte	$49
	.byte	$30
	.byte	$31
	.byte	$32
	.byte	$15
	.byte	$16
	.byte	$3B
	.byte	$9E
	.byte	$60
	.byte	$60
	.byte	$D0
	.byte	$D1
	.byte	$D2
	.byte	$D3
	.byte	$60
	.byte	$E4
	.byte	$C2
	.byte	$38
	.byte	$47
	.byte	$45
	.byte	$49
	.byte	$2A
	.byte	$C5
	.byte	$9E
	.byte	$60
	.byte	$E0
	.byte	$E0
	.byte	$E1
	.byte	$E2
	.byte	$E3
	.byte	$60
	.byte	$D1
	.byte	$D2
	.byte	$D3
	.byte	$D4
	.byte	$C1
	.byte	$E3
	.byte	$C5
	.byte	$60
	.byte	$9E
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$D1
	.byte	$D2
	.byte	$D3
	.byte	$D4
	.byte	$E1
	.byte	$E2
	.byte	$E3
	.byte	$E4
_iceworld_12:
	.byte	$E4
	.byte	$60
	.byte	$9E
	.byte	$60
	.byte	$60
	.byte	$B2
	.byte	$B0
	.byte	$B1
	.byte	$B2
	.byte	$B3
	.byte	$C0
	.byte	$B0
	.byte	$B1
	.byte	$B2
	.byte	$B3
	.byte	$9E
	.byte	$C2
	.byte	$60
	.byte	$9E
	.byte	$60
	.byte	$C1
	.byte	$C2
	.byte	$C0
	.byte	$C1
	.byte	$C2
	.byte	$C3
	.byte	$D0
	.byte	$C0
	.byte	$C1
	.byte	$C2
	.byte	$C3
	.byte	$9E
	.byte	$B2
	.byte	$60
	.byte	$9E
	.byte	$B0
	.byte	$B1
	.byte	$B2
	.byte	$D0
	.byte	$B0
	.byte	$B1
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$D1
	.byte	$D2
	.byte	$60
	.byte	$9E
	.byte	$C2
	.byte	$C3
	.byte	$9E
	.byte	$C0
	.byte	$C1
	.byte	$C2
	.byte	$E0
	.byte	$C0
	.byte	$60
	.byte	$60
	.byte	$B0
	.byte	$60
	.byte	$B2
	.byte	$B3
	.byte	$60
	.byte	$9E
	.byte	$E4
	.byte	$60
	.byte	$9E
	.byte	$60
	.byte	$E0
	.byte	$D2
	.byte	$D3
	.byte	$D0
	.byte	$D1
	.byte	$B0
	.byte	$B1
	.byte	$60
	.byte	$60
	.byte	$B2
	.byte	$B3
	.byte	$9E
	.byte	$D5
	.byte	$60
	.byte	$9E
	.byte	$60
	.byte	$60
	.byte	$E2
	.byte	$E3
	.byte	$E0
	.byte	$E1
	.byte	$C0
	.byte	$C1
	.byte	$C0
	.byte	$60
	.byte	$C2
	.byte	$C3
	.byte	$9E
	.byte	$9B
	.byte	$9B
	.byte	$AF
	.byte	$9B
	.byte	$9B
	.byte	$9B
	.byte	$9B
	.byte	$9B
	.byte	$9C
	.byte	$60
	.byte	$D1
	.byte	$D0
	.byte	$D1
	.byte	$D2
	.byte	$60
	.byte	$9E
	.byte	$10
	.byte	$11
	.byte	$12
	.byte	$13
	.byte	$45
	.byte	$15
	.byte	$16
	.byte	$3A
	.byte	$9E
	.byte	$60
	.byte	$60
	.byte	$E0
	.byte	$E1
	.byte	$E2
	.byte	$60
	.byte	$9E
	.byte	$47
	.byte	$22
	.byte	$10
	.byte	$11
	.byte	$18
	.byte	$31
	.byte	$47
	.byte	$47
	.byte	$AF
	.byte	$9B
	.byte	$9B
	.byte	$9B
	.byte	$9B
	.byte	$9C
	.byte	$D5
	.byte	$9E
	.byte	$48
	.byte	$45
	.byte	$32
	.byte	$45
	.byte	$45
	.byte	$47
	.byte	$45
	.byte	$48
	.byte	$47
	.byte	$22
	.byte	$26
	.byte	$27
	.byte	$28
	.byte	$9E
	.byte	$E5
	.byte	$9E
	.byte	$48
	.byte	$49
	.byte	$4A
	.byte	$32
	.byte	$10
	.byte	$11
	.byte	$27
	.byte	$46
	.byte	$26
	.byte	$48
	.byte	$49
	.byte	$47
	.byte	$4C
	.byte	$9E
	.byte	$E0
	.byte	$9E
	.byte	$10
	.byte	$11
	.byte	$45
	.byte	$47
	.byte	$20
	.byte	$21
	.byte	$26
	.byte	$27
	.byte	$32
	.byte	$45
	.byte	$46
	.byte	$26
	.byte	$28
	.byte	$9E
	.byte	$9D
	.byte	$9E
	.byte	$20
	.byte	$21
	.byte	$47
	.byte	$45
	.byte	$48
	.byte	$30
	.byte	$31
	.byte	$15
	.byte	$16
	.byte	$17
	.byte	$47
	.byte	$4C
	.byte	$29
	.byte	$9F
	.byte	$9F
	.byte	$9F
	.byte	$49
	.byte	$4A
	.byte	$4B
	.byte	$45
	.byte	$46
	.byte	$47
	.byte	$4C
	.byte	$44
	.byte	$26
	.byte	$27
	.byte	$26
	.byte	$47
	.byte	$4B
	.byte	$62
	.byte	$62
	.byte	$62
	.byte	$24
	.byte	$25
	.byte	$4C
	.byte	$4A
	.byte	$45
	.byte	$46
	.byte	$24
	.byte	$25
	.byte	$19
	.byte	$4A
	.byte	$47
	.byte	$48
	.byte	$3B
	.byte	$63
	.byte	$63
	.byte	$63
_iceworld_13:
	.byte	$B0
	.byte	$B1
	.byte	$B0
	.byte	$60
	.byte	$B1
	.byte	$B2
	.byte	$B0
	.byte	$60
	.byte	$B2
	.byte	$B3
	.byte	$C0
	.byte	$B0
	.byte	$B1
	.byte	$B2
	.byte	$60
	.byte	$38
	.byte	$60
	.byte	$C1
	.byte	$C0
	.byte	$C0
	.byte	$60
	.byte	$C2
	.byte	$C0
	.byte	$C1
	.byte	$C2
	.byte	$C3
	.byte	$D0
	.byte	$C0
	.byte	$C1
	.byte	$C2
	.byte	$C3
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$D0
	.byte	$B0
	.byte	$B1
	.byte	$60
	.byte	$B0
	.byte	$B1
	.byte	$B2
	.byte	$B3
	.byte	$C0
	.byte	$B0
	.byte	$B1
	.byte	$B2
	.byte	$B3
	.byte	$B0
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$B1
	.byte	$B2
	.byte	$B3
	.byte	$C0
	.byte	$B0
	.byte	$B1
	.byte	$B2
	.byte	$B3
	.byte	$C1
	.byte	$C2
	.byte	$60
	.byte	$C0
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$C0
	.byte	$C1
	.byte	$C2
	.byte	$C3
	.byte	$D0
	.byte	$C0
	.byte	$60
	.byte	$E3
	.byte	$C3
	.byte	$D1
	.byte	$D2
	.byte	$D3
	.byte	$D0
	.byte	$B0
	.byte	$60
	.byte	$B2
	.byte	$D0
	.byte	$B0
	.byte	$B1
	.byte	$B2
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$D2
	.byte	$D3
	.byte	$B2
	.byte	$B3
	.byte	$E3
	.byte	$60
	.byte	$60
	.byte	$C1
	.byte	$60
	.byte	$E0
	.byte	$C0
	.byte	$B0
	.byte	$60
	.byte	$B2
	.byte	$B0
	.byte	$B1
	.byte	$B2
	.byte	$B3
	.byte	$60
	.byte	$B0
	.byte	$B1
	.byte	$B2
	.byte	$60
	.byte	$D1
	.byte	$D2
	.byte	$D3
	.byte	$D0
	.byte	$C0
	.byte	$C1
	.byte	$C2
	.byte	$C0
	.byte	$C1
	.byte	$C2
	.byte	$C3
	.byte	$D0
	.byte	$50
	.byte	$51
	.byte	$C2
	.byte	$E0
	.byte	$E1
	.byte	$E2
	.byte	$E3
	.byte	$E0
	.byte	$B0
	.byte	$B1
	.byte	$B2
	.byte	$50
	.byte	$54
	.byte	$51
	.byte	$60
	.byte	$E0
	.byte	$D0
	.byte	$C5
	.byte	$D2
	.byte	$C2
	.byte	$50
	.byte	$54
	.byte	$55
	.byte	$51
	.byte	$C0
	.byte	$B0
	.byte	$E0
	.byte	$60
	.byte	$D3
	.byte	$60
	.byte	$B2
	.byte	$B3
	.byte	$60
	.byte	$B0
	.byte	$B1
	.byte	$E3
	.byte	$48
	.byte	$22
	.byte	$32
	.byte	$3B
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$C2
	.byte	$C0
	.byte	$60
	.byte	$C2
	.byte	$C3
	.byte	$D0
	.byte	$C0
	.byte	$C1
	.byte	$9D
	.byte	$9D
	.byte	$9D
	.byte	$9D
	.byte	$9D
	.byte	$9D
	.byte	$9D
	.byte	$9D
	.byte	$9D
	.byte	$9D
	.byte	$9D
	.byte	$9D
	.byte	$9D
	.byte	$9D
	.byte	$9D
	.byte	$9D
	.byte	$CC
	.byte	$9F
	.byte	$9F
	.byte	$9F
	.byte	$9F
	.byte	$9F
	.byte	$9F
	.byte	$9F
	.byte	$9F
	.byte	$9F
	.byte	$9F
	.byte	$9F
	.byte	$9F
	.byte	$9F
	.byte	$9F
	.byte	$9F
	.byte	$BC
	.byte	$62
	.byte	$62
	.byte	$62
	.byte	$62
	.byte	$62
	.byte	$62
	.byte	$62
	.byte	$62
	.byte	$62
	.byte	$62
	.byte	$62
	.byte	$62
	.byte	$62
	.byte	$62
	.byte	$62
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
_iceworld_14:
	.byte	$49
	.byte	$26
	.byte	$45
	.byte	$10
	.byte	$16
	.byte	$45
	.byte	$49
	.byte	$2A
	.byte	$9E
	.byte	$60
	.byte	$B2
	.byte	$B0
	.byte	$B1
	.byte	$B2
	.byte	$B3
	.byte	$C0
	.byte	$AE
	.byte	$37
	.byte	$22
	.byte	$47
	.byte	$45
	.byte	$32
	.byte	$2A
	.byte	$C3
	.byte	$9E
	.byte	$E0
	.byte	$B1
	.byte	$B2
	.byte	$B0
	.byte	$B1
	.byte	$D4
	.byte	$D5
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$4F
	.byte	$1A
	.byte	$AD
	.byte	$D3
	.byte	$B2
	.byte	$9E
	.byte	$60
	.byte	$60
	.byte	$B1
	.byte	$60
	.byte	$B0
	.byte	$E2
	.byte	$E3
	.byte	$C1
	.byte	$C2
	.byte	$E0
	.byte	$60
	.byte	$60
	.byte	$C2
	.byte	$B0
	.byte	$60
	.byte	$9E
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$B3
	.byte	$C0
	.byte	$B0
	.byte	$B1
	.byte	$60
	.byte	$D2
	.byte	$D3
	.byte	$D0
	.byte	$D1
	.byte	$B0
	.byte	$C5
	.byte	$60
	.byte	$9E
	.byte	$60
	.byte	$C5
	.byte	$E1
	.byte	$C3
	.byte	$D0
	.byte	$C0
	.byte	$C1
	.byte	$60
	.byte	$B2
	.byte	$B0
	.byte	$B1
	.byte	$B2
	.byte	$B3
	.byte	$60
	.byte	$60
	.byte	$9E
	.byte	$60
	.byte	$60
	.byte	$C1
	.byte	$B2
	.byte	$E0
	.byte	$D0
	.byte	$D1
	.byte	$60
	.byte	$60
	.byte	$C0
	.byte	$C1
	.byte	$C2
	.byte	$C3
	.byte	$9A
	.byte	$9B
	.byte	$AF
	.byte	$9B
	.byte	$9C
	.byte	$D1
	.byte	$C2
	.byte	$B0
	.byte	$B1
	.byte	$60
	.byte	$C3
	.byte	$B2
	.byte	$B3
	.byte	$B0
	.byte	$29
	.byte	$B2
	.byte	$9E
	.byte	$30
	.byte	$31
	.byte	$29
	.byte	$9E
	.byte	$E1
	.byte	$B0
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$C2
	.byte	$29
	.byte	$C3
	.byte	$B0
	.byte	$B1
	.byte	$60
	.byte	$9E
	.byte	$39
	.byte	$46
	.byte	$4D
	.byte	$9E
	.byte	$60
	.byte	$60
	.byte	$50
	.byte	$51
	.byte	$C3
	.byte	$B2
	.byte	$D3
	.byte	$E4
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$9E
	.byte	$B0
	.byte	$B1
	.byte	$B2
	.byte	$9E
	.byte	$60
	.byte	$D5
	.byte	$60
	.byte	$60
	.byte	$E4
	.byte	$C2
	.byte	$C3
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$9E
	.byte	$60
	.byte	$B3
	.byte	$E4
	.byte	$9E
	.byte	$B1
	.byte	$C0
	.byte	$C1
	.byte	$B0
	.byte	$B1
	.byte	$9D
	.byte	$9D
	.byte	$9D
	.byte	$9D
	.byte	$9D
	.byte	$9D
	.byte	$9E
	.byte	$9D
	.byte	$9D
	.byte	$9D
	.byte	$9E
	.byte	$9D
	.byte	$9D
	.byte	$9D
	.byte	$9D
	.byte	$9D
	.byte	$CC
	.byte	$9F
	.byte	$9F
	.byte	$9F
	.byte	$9F
	.byte	$9F
	.byte	$9F
	.byte	$9F
	.byte	$9F
	.byte	$9F
	.byte	$9F
	.byte	$9F
	.byte	$9F
	.byte	$9F
	.byte	$9F
	.byte	$9F
	.byte	$BC
	.byte	$62
	.byte	$62
	.byte	$62
	.byte	$62
	.byte	$62
	.byte	$62
	.byte	$62
	.byte	$62
	.byte	$62
	.byte	$62
	.byte	$62
	.byte	$62
	.byte	$62
	.byte	$62
	.byte	$62
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
_iceworld_15:
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$C0
	.byte	$C1
	.byte	$C2
	.byte	$C3
	.byte	$C4
	.byte	$B1
	.byte	$B2
	.byte	$B3
	.byte	$60
	.byte	$60
	.byte	$B1
	.byte	$60
	.byte	$C0
	.byte	$60
	.byte	$60
	.byte	$C0
	.byte	$C1
	.byte	$D1
	.byte	$D2
	.byte	$D3
	.byte	$D4
	.byte	$C1
	.byte	$B2
	.byte	$B3
	.byte	$C0
	.byte	$60
	.byte	$60
	.byte	$B2
	.byte	$B3
	.byte	$60
	.byte	$D1
	.byte	$D0
	.byte	$D1
	.byte	$C3
	.byte	$E4
	.byte	$D4
	.byte	$D5
	.byte	$D1
	.byte	$B2
	.byte	$B3
	.byte	$C0
	.byte	$B0
	.byte	$60
	.byte	$B2
	.byte	$C3
	.byte	$C0
	.byte	$B0
	.byte	$B1
	.byte	$3E
	.byte	$5B
	.byte	$5C
	.byte	$5D
	.byte	$5E
	.byte	$5F
	.byte	$59
	.byte	$5A
	.byte	$59
	.byte	$5A
	.byte	$51
	.byte	$C3
	.byte	$B2
	.byte	$D0
	.byte	$C0
	.byte	$60
	.byte	$3D
	.byte	$38
	.byte	$29
	.byte	$1A
	.byte	$60
	.byte	$D3
	.byte	$60
	.byte	$37
	.byte	$15
	.byte	$16
	.byte	$4E
	.byte	$D3
	.byte	$60
	.byte	$E0
	.byte	$D0
	.byte	$60
	.byte	$3D
	.byte	$E4
	.byte	$E5
	.byte	$60
	.byte	$B2
	.byte	$D0
	.byte	$B0
	.byte	$B1
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$E3
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$C0
	.byte	$C0
	.byte	$C1
	.byte	$C2
	.byte	$60
	.byte	$E3
	.byte	$C1
	.byte	$C2
	.byte	$B0
	.byte	$B1
	.byte	$B1
	.byte	$60
	.byte	$50
	.byte	$54
	.byte	$51
	.byte	$B0
	.byte	$D0
	.byte	$C5
	.byte	$60
	.byte	$60
	.byte	$E5
	.byte	$60
	.byte	$D1
	.byte	$B0
	.byte	$B1
	.byte	$B0
	.byte	$C1
	.byte	$E5
	.byte	$60
	.byte	$60
	.byte	$E4
	.byte	$C0
	.byte	$E0
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$E1
	.byte	$60
	.byte	$C1
	.byte	$C0
	.byte	$D1
	.byte	$B0
	.byte	$B1
	.byte	$60
	.byte	$E0
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$B1
	.byte	$E1
	.byte	$60
	.byte	$B0
	.byte	$B1
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$50
	.byte	$5C
	.byte	$5D
	.byte	$51
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$50
	.byte	$9D
	.byte	$9D
	.byte	$9D
	.byte	$9D
	.byte	$9D
	.byte	$9D
	.byte	$9D
	.byte	$9D
	.byte	$9D
	.byte	$9D
	.byte	$9D
	.byte	$9D
	.byte	$9D
	.byte	$9D
	.byte	$9D
	.byte	$9D
	.byte	$9F
	.byte	$CC
	.byte	$9F
	.byte	$9F
	.byte	$9F
	.byte	$9F
	.byte	$9F
	.byte	$9F
	.byte	$9F
	.byte	$9F
	.byte	$9F
	.byte	$CC
	.byte	$9F
	.byte	$9F
	.byte	$9F
	.byte	$9F
	.byte	$62
	.byte	$BC
	.byte	$62
	.byte	$62
	.byte	$62
	.byte	$62
	.byte	$62
	.byte	$62
	.byte	$62
	.byte	$62
	.byte	$62
	.byte	$BC
	.byte	$62
	.byte	$62
	.byte	$62
	.byte	$62
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
_iceworld_16:
	.byte	$B3
	.byte	$B0
	.byte	$B0
	.byte	$C0
	.byte	$C1
	.byte	$C2
	.byte	$C0
	.byte	$C1
	.byte	$C2
	.byte	$C3
	.byte	$D0
	.byte	$C0
	.byte	$C1
	.byte	$C0
	.byte	$B0
	.byte	$B1
	.byte	$B3
	.byte	$C0
	.byte	$C0
	.byte	$B0
	.byte	$B1
	.byte	$B2
	.byte	$D0
	.byte	$B0
	.byte	$B1
	.byte	$B2
	.byte	$B0
	.byte	$60
	.byte	$B2
	.byte	$B3
	.byte	$C0
	.byte	$C1
	.byte	$B3
	.byte	$B0
	.byte	$D0
	.byte	$C0
	.byte	$C1
	.byte	$60
	.byte	$E0
	.byte	$C0
	.byte	$C1
	.byte	$C2
	.byte	$C0
	.byte	$C1
	.byte	$C2
	.byte	$C3
	.byte	$B0
	.byte	$B1
	.byte	$C4
	.byte	$C0
	.byte	$E0
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$D3
	.byte	$D0
	.byte	$60
	.byte	$B0
	.byte	$60
	.byte	$B0
	.byte	$B1
	.byte	$D0
	.byte	$60
	.byte	$60
	.byte	$D4
	.byte	$60
	.byte	$60
	.byte	$E0
	.byte	$E1
	.byte	$D4
	.byte	$E3
	.byte	$60
	.byte	$60
	.byte	$C0
	.byte	$C1
	.byte	$C0
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$D1
	.byte	$E4
	.byte	$E0
	.byte	$C2
	.byte	$D1
	.byte	$60
	.byte	$60
	.byte	$D0
	.byte	$D1
	.byte	$B0
	.byte	$60
	.byte	$B0
	.byte	$3E
	.byte	$50
	.byte	$58
	.byte	$59
	.byte	$5A
	.byte	$60
	.byte	$60
	.byte	$D2
	.byte	$E1
	.byte	$60
	.byte	$60
	.byte	$E0
	.byte	$E1
	.byte	$C0
	.byte	$C1
	.byte	$C0
	.byte	$3D
	.byte	$36
	.byte	$37
	.byte	$47
	.byte	$4D
	.byte	$C2
	.byte	$B3
	.byte	$C3
	.byte	$B2
	.byte	$B3
	.byte	$B0
	.byte	$60
	.byte	$B2
	.byte	$B0
	.byte	$B1
	.byte	$60
	.byte	$3D
	.byte	$C0
	.byte	$C0
	.byte	$60
	.byte	$D3
	.byte	$D2
	.byte	$C3
	.byte	$C1
	.byte	$60
	.byte	$60
	.byte	$C0
	.byte	$60
	.byte	$60
	.byte	$C0
	.byte	$60
	.byte	$C2
	.byte	$B2
	.byte	$B0
	.byte	$C0
	.byte	$B0
	.byte	$E2
	.byte	$E2
	.byte	$D0
	.byte	$D1
	.byte	$D2
	.byte	$D3
	.byte	$B0
	.byte	$B1
	.byte	$B2
	.byte	$D0
	.byte	$B0
	.byte	$B1
	.byte	$B2
	.byte	$C0
	.byte	$B0
	.byte	$C0
	.byte	$60
	.byte	$51
	.byte	$52
	.byte	$53
	.byte	$54
	.byte	$55
	.byte	$56
	.byte	$57
	.byte	$58
	.byte	$51
	.byte	$B0
	.byte	$B1
	.byte	$B2
	.byte	$D0
	.byte	$C0
	.byte	$B0
	.byte	$C0
	.byte	$9D
	.byte	$9D
	.byte	$9D
	.byte	$9D
	.byte	$9D
	.byte	$9D
	.byte	$9D
	.byte	$9D
	.byte	$9D
	.byte	$9D
	.byte	$9D
	.byte	$9D
	.byte	$9D
	.byte	$9D
	.byte	$9D
	.byte	$9D
	.byte	$9F
	.byte	$9F
	.byte	$9F
	.byte	$CC
	.byte	$9F
	.byte	$9F
	.byte	$9F
	.byte	$9F
	.byte	$9F
	.byte	$9F
	.byte	$9F
	.byte	$9F
	.byte	$9F
	.byte	$9F
	.byte	$CC
	.byte	$9F
	.byte	$62
	.byte	$62
	.byte	$62
	.byte	$BC
	.byte	$62
	.byte	$62
	.byte	$62
	.byte	$62
	.byte	$62
	.byte	$62
	.byte	$62
	.byte	$62
	.byte	$62
	.byte	$62
	.byte	$BC
	.byte	$62
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$63
_iceworld_17:
	.byte	$B2
	.byte	$B0
	.byte	$B1
	.byte	$B2
	.byte	$B3
	.byte	$C0
	.byte	$B0
	.byte	$9E
	.byte	$38
	.byte	$47
	.byte	$48
	.byte	$15
	.byte	$16
	.byte	$17
	.byte	$18
	.byte	$19
	.byte	$C2
	.byte	$60
	.byte	$C1
	.byte	$C2
	.byte	$C3
	.byte	$D0
	.byte	$C0
	.byte	$9E
	.byte	$60
	.byte	$37
	.byte	$4A
	.byte	$4B
	.byte	$4C
	.byte	$27
	.byte	$28
	.byte	$29
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$B2
	.byte	$E0
	.byte	$60
	.byte	$9E
	.byte	$E2
	.byte	$E3
	.byte	$AD
	.byte	$4F
	.byte	$36
	.byte	$39
	.byte	$49
	.byte	$39
	.byte	$60
	.byte	$60
	.byte	$B0
	.byte	$60
	.byte	$B2
	.byte	$60
	.byte	$60
	.byte	$9E
	.byte	$E1
	.byte	$D3
	.byte	$E3
	.byte	$60
	.byte	$60
	.byte	$AE
	.byte	$38
	.byte	$4A
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$B1
	.byte	$B2
	.byte	$B3
	.byte	$60
	.byte	$9E
	.byte	$60
	.byte	$D5
	.byte	$C1
	.byte	$C2
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$37
	.byte	$5B
	.byte	$5C
	.byte	$5D
	.byte	$51
	.byte	$C2
	.byte	$60
	.byte	$60
	.byte	$9E
	.byte	$60
	.byte	$B1
	.byte	$60
	.byte	$C0
	.byte	$C2
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$38
	.byte	$30
	.byte	$31
	.byte	$4E
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$9E
	.byte	$60
	.byte	$60
	.byte	$D5
	.byte	$D0
	.byte	$D0
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$B0
	.byte	$E0
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$9E
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$E0
	.byte	$D5
	.byte	$E1
	.byte	$60
	.byte	$60
	.byte	$C1
	.byte	$60
	.byte	$B1
	.byte	$60
	.byte	$9A
	.byte	$9B
	.byte	$9B
	.byte	$AF
	.byte	$9B
	.byte	$9B
	.byte	$9B
	.byte	$9B
	.byte	$9B
	.byte	$9C
	.byte	$E0
	.byte	$B0
	.byte	$D1
	.byte	$B2
	.byte	$60
	.byte	$60
	.byte	$9E
	.byte	$10
	.byte	$11
	.byte	$12
	.byte	$13
	.byte	$14
	.byte	$15
	.byte	$16
	.byte	$17
	.byte	$9E
	.byte	$60
	.byte	$60
	.byte	$E1
	.byte	$60
	.byte	$9A
	.byte	$9B
	.byte	$AF
	.byte	$22
	.byte	$12
	.byte	$13
	.byte	$14
	.byte	$4A
	.byte	$49
	.byte	$4C
	.byte	$4C
	.byte	$AF
	.byte	$9B
	.byte	$9B
	.byte	$9D
	.byte	$9D
	.byte	$9E
	.byte	$32
	.byte	$17
	.byte	$12
	.byte	$13
	.byte	$14
	.byte	$15
	.byte	$16
	.byte	$26
	.byte	$45
	.byte	$32
	.byte	$10
	.byte	$11
	.byte	$12
	.byte	$9F
	.byte	$9F
	.byte	$9F
	.byte	$45
	.byte	$15
	.byte	$16
	.byte	$32
	.byte	$24
	.byte	$49
	.byte	$26
	.byte	$27
	.byte	$47
	.byte	$14
	.byte	$15
	.byte	$16
	.byte	$17
	.byte	$62
	.byte	$62
	.byte	$62
	.byte	$15
	.byte	$16
	.byte	$44
	.byte	$45
	.byte	$46
	.byte	$47
	.byte	$48
	.byte	$49
	.byte	$4A
	.byte	$46
	.byte	$45
	.byte	$12
	.byte	$13
	.byte	$63
	.byte	$63
	.byte	$63
	.byte	$32
	.byte	$44
	.byte	$45
	.byte	$46
	.byte	$47
	.byte	$48
	.byte	$49
	.byte	$4A
	.byte	$46
	.byte	$30
	.byte	$31
	.byte	$22
	.byte	$47
_iceworld_18:
	.byte	$06
	.byte	$05
	.byte	$0D
	.byte	$05
	.byte	$0D
	.byte	$05
	.byte	$0D
	.byte	$05
	.byte	$0E
	.byte	$0F
	.byte	$05
	.byte	$02
	.byte	$0D
	.byte	$02
	.byte	$05
	.byte	$06
	.byte	$47
	.byte	$28
	.byte	$1D
	.byte	$BE
	.byte	$1D
	.byte	$BE
	.byte	$1D
	.byte	$BD
	.byte	$1E
	.byte	$1F
	.byte	$BE
	.byte	$BF
	.byte	$1D
	.byte	$CE
	.byte	$38
	.byte	$48
	.byte	$4C
	.byte	$CF
	.byte	$CD
	.byte	$CF
	.byte	$CF
	.byte	$CD
	.byte	$CF
	.byte	$CD
	.byte	$2E
	.byte	$2F
	.byte	$CE
	.byte	$CD
	.byte	$CE
	.byte	$60
	.byte	$60
	.byte	$30
	.byte	$29
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$CD
	.byte	$CE
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$CD
	.byte	$10
	.byte	$4B
	.byte	$BF
	.byte	$60
	.byte	$60
	.byte	$BD
	.byte	$BE
	.byte	$CF
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$20
	.byte	$CF
	.byte	$CF
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$CF
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$BD
	.byte	$BF
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$26
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$BD
	.byte	$BE
	.byte	$BE
	.byte	$CE
	.byte	$60
	.byte	$60
	.byte	$4A
	.byte	$60
	.byte	$BD
	.byte	$BF
	.byte	$BD
	.byte	$BE
	.byte	$BE
	.byte	$BF
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$CD
	.byte	$BE
	.byte	$CF
	.byte	$60
	.byte	$60
	.byte	$45
	.byte	$CD
	.byte	$CE
	.byte	$60
	.byte	$CF
	.byte	$CD
	.byte	$CF
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$CF
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$15
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$BD
	.byte	$BE
	.byte	$47
	.byte	$9B
	.byte	$9B
	.byte	$9C
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$BD
	.byte	$CE
	.byte	$60
	.byte	$60
	.byte	$CD
	.byte	$BF
	.byte	$BD
	.byte	$BE
	.byte	$BE
	.byte	$47
	.byte	$13
	.byte	$14
	.byte	$9E
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$CF
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$60
	.byte	$CD
	.byte	$CE
	.byte	$60
	.byte	$4A
	.byte	$4A
	.byte	$3B
	.byte	$AF
	.byte	$9B
	.byte	$9B
	.byte	$9B
	.byte	$9B
	.byte	$9B
	.byte	$9B
	.byte	$9B
	.byte	$9B
	.byte	$9B
	.byte	$9B
	.byte	$9B
	.byte	$9B
	.byte	$47
	.byte	$46
	.byte	$10
	.byte	$11
	.byte	$47
	.byte	$15
	.byte	$16
	.byte	$46
	.byte	$47
	.byte	$48
	.byte	$49
	.byte	$4A
	.byte	$49
	.byte	$47
	.byte	$4A
	.byte	$47
	.byte	$32
	.byte	$47
	.byte	$20
	.byte	$21
	.byte	$15
	.byte	$16
	.byte	$30
	.byte	$31
	.byte	$4A
	.byte	$4A
	.byte	$26
	.byte	$47
	.byte	$49
	.byte	$47
	.byte	$24
	.byte	$25
	.byte	$45
_iceworld_19:
	.byte	$02
	.byte	$2C
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$47
	.byte	$2D
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$4C
	.byte	$69
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$75
	.byte	$76
	.byte	$76
	.byte	$77
	.byte	$61
	.byte	$61
	.byte	$29
	.byte	$61
	.byte	$61
	.byte	$75
	.byte	$77
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$4B
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$6D
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$6D
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$98
	.byte	$99
	.byte	$9A
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$6D
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$A8
	.byte	$A9
	.byte	$AA
	.byte	$AB
	.byte	$AC
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$6D
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$B9
	.byte	$BA
	.byte	$BB
	.byte	$BC
	.byte	$BD
	.byte	$BE
	.byte	$BF
	.byte	$6D
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$C9
	.byte	$CA
	.byte	$CB
	.byte	$CC
	.byte	$CD
	.byte	$CE
	.byte	$CF
	.byte	$6D
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$D9
	.byte	$DA
	.byte	$DB
	.byte	$DC
	.byte	$DD
	.byte	$DE
	.byte	$DF
	.byte	$04
	.byte	$05
	.byte	$78
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$61
	.byte	$E9
	.byte	$EA
	.byte	$EB
	.byte	$EC
	.byte	$ED
	.byte	$EE
	.byte	$EF
	.byte	$4A
	.byte	$3B
	.byte	$79
	.byte	$05
	.byte	$02
	.byte	$05
	.byte	$05
	.byte	$02
	.byte	$05
	.byte	$06
	.byte	$02
	.byte	$05
	.byte	$04
	.byte	$05
	.byte	$05
	.byte	$04
	.byte	$46
	.byte	$10
	.byte	$11
	.byte	$47
	.byte	$15
	.byte	$16
	.byte	$46
	.byte	$47
	.byte	$48
	.byte	$49
	.byte	$4A
	.byte	$49
	.byte	$47
	.byte	$4A
	.byte	$47
	.byte	$32
	.byte	$47
	.byte	$20
	.byte	$21
	.byte	$15
	.byte	$16
	.byte	$30
	.byte	$31
	.byte	$4A
	.byte	$4A
	.byte	$26
	.byte	$47
	.byte	$49
	.byte	$47
	.byte	$24
	.byte	$25
	.byte	$45
_stage3_levels_list:
_stage3_max_rooms:
	.byte	$03
_stage3_offsets:
	.byte	$00
_stage3_bg_palette:
	.byte	$0F
	.byte	$0A
	.byte	$00
	.byte	$10
	.byte	$0F
	.byte	$02
	.byte	$21
	.byte	$30
	.byte	$0F
	.byte	$0F
	.byte	$17
	.byte	$07
	.byte	$0F
	.byte	$0F
	.byte	$08
	.byte	$0F
_stage3_palette_sp:
	.byte	$21
	.byte	$17
	.byte	$24
	.byte	$36
	.byte	$21
	.byte	$05
	.byte	$16
	.byte	$37
	.byte	$21
	.byte	$0F
	.byte	$11
	.byte	$30
	.byte	$21
	.byte	$17
	.byte	$3D
	.byte	$37
_stage3_metatiles:
	.byte	$80
	.byte	$80
	.byte	$90
	.byte	$91
	.byte	$01
	.byte	$81
	.byte	$81
	.byte	$92
	.byte	$93
	.byte	$01
	.byte	$82
	.byte	$82
	.byte	$94
	.byte	$93
	.byte	$01
	.byte	$82
	.byte	$82
	.byte	$92
	.byte	$91
	.byte	$01
	.byte	$82
	.byte	$82
	.byte	$93
	.byte	$90
	.byte	$01
	.byte	$83
	.byte	$83
	.byte	$91
	.byte	$93
	.byte	$01
	.byte	$84
	.byte	$84
	.byte	$90
	.byte	$91
	.byte	$01
	.byte	$85
	.byte	$85
	.byte	$92
	.byte	$93
	.byte	$01
	.byte	$86
	.byte	$86
	.byte	$90
	.byte	$91
	.byte	$01
	.byte	$82
	.byte	$87
	.byte	$90
	.byte	$97
	.byte	$01
	.byte	$A7
	.byte	$82
	.byte	$B7
	.byte	$93
	.byte	$01
	.byte	$82
	.byte	$B2
	.byte	$90
	.byte	$97
	.byte	$01
	.byte	$A2
	.byte	$82
	.byte	$B7
	.byte	$93
	.byte	$01
	.byte	$82
	.byte	$82
	.byte	$95
	.byte	$96
	.byte	$01
	.byte	$82
	.byte	$82
	.byte	$95
	.byte	$03
	.byte	$01
	.byte	$82
	.byte	$82
	.byte	$03
	.byte	$96
	.byte	$01
	.byte	$8C
	.byte	$8D
	.byte	$9C
	.byte	$9D
	.byte	$00
	.byte	$8E
	.byte	$D4
	.byte	$9E
	.byte	$9F
	.byte	$00
	.byte	$8C
	.byte	$8D
	.byte	$9C
	.byte	$9D
	.byte	$00
	.byte	$8E
	.byte	$8F
	.byte	$9E
	.byte	$DC
	.byte	$00
	.byte	$FC
	.byte	$D4
	.byte	$8E
	.byte	$8F
	.byte	$00
	.byte	$AC
	.byte	$8D
	.byte	$BC
	.byte	$9D
	.byte	$00
	.byte	$8E
	.byte	$8F
	.byte	$9E
	.byte	$BF
	.byte	$00
	.byte	$8E
	.byte	$EF
	.byte	$9E
	.byte	$FF
	.byte	$00
	.byte	$B1
	.byte	$A0
	.byte	$B1
	.byte	$FD
	.byte	$00
	.byte	$20
	.byte	$BF
	.byte	$CE
	.byte	$CF
	.byte	$00
	.byte	$EB
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$02
	.byte	$C1
	.byte	$C1
	.byte	$96
	.byte	$01
	.byte	$03
	.byte	$C0
	.byte	$91
	.byte	$97
	.byte	$01
	.byte	$A5
	.byte	$A6
	.byte	$B5
	.byte	$B6
	.byte	$01
	.byte	$A5
	.byte	$03
	.byte	$B5
	.byte	$03
	.byte	$01
	.byte	$A3
	.byte	$A6
	.byte	$B3
	.byte	$B4
	.byte	$01
	.byte	$AC
	.byte	$AD
	.byte	$BC
	.byte	$BD
	.byte	$00
	.byte	$AE
	.byte	$AF
	.byte	$BE
	.byte	$CC
	.byte	$00
	.byte	$EF
	.byte	$9F
	.byte	$FF
	.byte	$AF
	.byte	$00
	.byte	$AE
	.byte	$D5
	.byte	$BE
	.byte	$CC
	.byte	$00
	.byte	$DC
	.byte	$DD
	.byte	$8C
	.byte	$AC
	.byte	$00
	.byte	$9D
	.byte	$9E
	.byte	$AD
	.byte	$AE
	.byte	$00
	.byte	$8C
	.byte	$EF
	.byte	$9C
	.byte	$FF
	.byte	$00
	.byte	$EC
	.byte	$9F
	.byte	$FC
	.byte	$AF
	.byte	$00
	.byte	$EC
	.byte	$CA
	.byte	$CF
	.byte	$DA
	.byte	$00
	.byte	$ED
	.byte	$EA
	.byte	$FD
	.byte	$FA
	.byte	$00
	.byte	$EC
	.byte	$EB
	.byte	$FA
	.byte	$D4
	.byte	$00
	.byte	$A4
	.byte	$A6
	.byte	$C5
	.byte	$B6
	.byte	$01
	.byte	$E0
	.byte	$02
	.byte	$95
	.byte	$E0
	.byte	$01
	.byte	$A5
	.byte	$D1
	.byte	$B5
	.byte	$E1
	.byte	$01
	.byte	$B1
	.byte	$C2
	.byte	$D5
	.byte	$D2
	.byte	$01
	.byte	$C3
	.byte	$C4
	.byte	$D3
	.byte	$D4
	.byte	$01
	.byte	$CC
	.byte	$CD
	.byte	$DC
	.byte	$DD
	.byte	$00
	.byte	$CE
	.byte	$CF
	.byte	$DE
	.byte	$DF
	.byte	$00
	.byte	$ED
	.byte	$EE
	.byte	$FD
	.byte	$FE
	.byte	$00
	.byte	$AC
	.byte	$9F
	.byte	$BC
	.byte	$AF
	.byte	$00
	.byte	$A1
	.byte	$BC
	.byte	$00
	.byte	$A1
	.byte	$00
	.byte	$BD
	.byte	$BE
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$EB
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$CE
	.byte	$CF
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$CE
	.byte	$CF
	.byte	$00
	.byte	$CD
	.byte	$00
	.byte	$DC
	.byte	$8E
	.byte	$A1
	.byte	$BF
	.byte	$00
	.byte	$9F
	.byte	$CB
	.byte	$AF
	.byte	$DB
	.byte	$00
	.byte	$EF
	.byte	$CA
	.byte	$FF
	.byte	$DA
	.byte	$00
	.byte	$D0
	.byte	$03
	.byte	$B7
	.byte	$93
	.byte	$01
	.byte	$0A
	.byte	$0B
	.byte	$0A
	.byte	$0B
	.byte	$02
	.byte	$0A
	.byte	$0B
	.byte	$0A
	.byte	$0B
	.byte	$02
	.byte	$E9
	.byte	$E9
	.byte	$F9
	.byte	$F9
	.byte	$00
	.byte	$88
	.byte	$A8
	.byte	$98
	.byte	$03
	.byte	$01
	.byte	$A8
	.byte	$89
	.byte	$03
	.byte	$99
	.byte	$01
	.byte	$88
	.byte	$89
	.byte	$98
	.byte	$99
	.byte	$01
	.byte	$A8
	.byte	$A8
	.byte	$03
	.byte	$03
	.byte	$01
	.byte	$CD
	.byte	$20
	.byte	$DD
	.byte	$DF
	.byte	$00
	.byte	$9F
	.byte	$BF
	.byte	$AF
	.byte	$CD
	.byte	$00
	.byte	$BB
	.byte	$BF
	.byte	$EC
	.byte	$FC
	.byte	$00
	.byte	$ED
	.byte	$9E
	.byte	$FD
	.byte	$BE
	.byte	$00
	.byte	$CD
	.byte	$9F
	.byte	$BB
	.byte	$AF
	.byte	$00
	.byte	$CE
	.byte	$CF
	.byte	$DD
	.byte	$DF
	.byte	$00
	.byte	$EC
	.byte	$EF
	.byte	$BF
	.byte	$FF
	.byte	$00
	.byte	$9F
	.byte	$EB
	.byte	$AF
	.byte	$FB
	.byte	$00
	.byte	$9F
	.byte	$EB
	.byte	$AF
	.byte	$FB
	.byte	$00
	.byte	$CA
	.byte	$EB
	.byte	$DA
	.byte	$00
	.byte	$00
	.byte	$EF
	.byte	$FB
	.byte	$DB
	.byte	$00
	.byte	$00
	.byte	$EC
	.byte	$CD
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$F2
	.byte	$F6
	.byte	$CD
	.byte	$BF
	.byte	$00
	.byte	$F6
	.byte	$F5
	.byte	$BB
	.byte	$EB
	.byte	$00
	.byte	$F1
	.byte	$F6
	.byte	$CF
	.byte	$ED
	.byte	$00
	.byte	$F6
	.byte	$F7
	.byte	$EE
	.byte	$CE
	.byte	$00
	.byte	$F6
	.byte	$F6
	.byte	$CF
	.byte	$BF
	.byte	$00
	.byte	$F7
	.byte	$F7
	.byte	$BF
	.byte	$CF
	.byte	$00
	.byte	$F6
	.byte	$F3
	.byte	$9C
	.byte	$9D
	.byte	$00
	.byte	$F8
	.byte	$F6
	.byte	$9E
	.byte	$BB
	.byte	$00
	.byte	$F0
	.byte	$F6
	.byte	$CE
	.byte	$CF
	.byte	$00
	.byte	$F7
	.byte	$F0
	.byte	$CD
	.byte	$BC
	.byte	$00
	.byte	$F8
	.byte	$F7
	.byte	$BD
	.byte	$BE
	.byte	$00
	.byte	$F4
	.byte	$F6
	.byte	$BF
	.byte	$CE
	.byte	$00
	.byte	$F6
	.byte	$F6
	.byte	$DC
	.byte	$DD
	.byte	$00
	.byte	$F8
	.byte	$F6
	.byte	$DE
	.byte	$DF
	.byte	$00
	.byte	$F6
	.byte	$F6
	.byte	$9C
	.byte	$9D
	.byte	$00
	.byte	$F6
	.byte	$F6
	.byte	$9E
	.byte	$BF
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$02
	.byte	$02
	.byte	$02
	.byte	$02
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$04
	.byte	$04
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$60
	.byte	$61
	.byte	$02
	.byte	$02
	.byte	$01
	.byte	$62
	.byte	$60
	.byte	$02
	.byte	$02
	.byte	$01
	.byte	$61
	.byte	$62
	.byte	$02
	.byte	$02
	.byte	$01
	.byte	$63
	.byte	$64
	.byte	$02
	.byte	$05
	.byte	$01
	.byte	$64
	.byte	$64
	.byte	$06
	.byte	$06
	.byte	$01
	.byte	$60
	.byte	$02
	.byte	$02
	.byte	$02
	.byte	$01
	.byte	$06
	.byte	$06
	.byte	$06
	.byte	$06
	.byte	$01
	.byte	$07
	.byte	$00
	.byte	$06
	.byte	$07
	.byte	$01
	.byte	$00
	.byte	$E3
	.byte	$00
	.byte	$E5
	.byte	$01
	.byte	$00
	.byte	$E2
	.byte	$00
	.byte	$E2
	.byte	$01
	.byte	$00
	.byte	$E5
	.byte	$00
	.byte	$E4
	.byte	$01
	.byte	$7B
	.byte	$7C
	.byte	$02
	.byte	$02
	.byte	$01
	.byte	$0E
	.byte	$0E
	.byte	$30
	.byte	$30
	.byte	$01
	.byte	$0E
	.byte	$0D
	.byte	$0D
	.byte	$02
	.byte	$01
	.byte	$0E
	.byte	$0E
	.byte	$0E
	.byte	$0E
	.byte	$01
	.byte	$0C
	.byte	$0E
	.byte	$02
	.byte	$0C
	.byte	$01
	.byte	$0E
	.byte	$0E
	.byte	$0D
	.byte	$30
	.byte	$01
	.byte	$1C
	.byte	$1E
	.byte	$02
	.byte	$02
	.byte	$01
	.byte	$1E
	.byte	$1E
	.byte	$02
	.byte	$02
	.byte	$01
	.byte	$1E
	.byte	$1D
	.byte	$02
	.byte	$02
	.byte	$01
	.byte	$01
	.byte	$65
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$66
	.byte	$67
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$65
	.byte	$66
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$67
	.byte	$01
	.byte	$01
	.byte	$75
	.byte	$76
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$77
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$76
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$77
	.byte	$01
	.byte	$01
	.byte	$02
	.byte	$02
	.byte	$78
	.byte	$79
	.byte	$01
	.byte	$02
	.byte	$02
	.byte	$7A
	.byte	$78
	.byte	$01
	.byte	$02
	.byte	$02
	.byte	$79
	.byte	$7A
	.byte	$01
	.byte	$5E
	.byte	$30
	.byte	$68
	.byte	$69
	.byte	$01
	.byte	$30
	.byte	$30
	.byte	$6A
	.byte	$68
	.byte	$01
	.byte	$30
	.byte	$30
	.byte	$69
	.byte	$6A
	.byte	$01
	.byte	$30
	.byte	$30
	.byte	$68
	.byte	$69
	.byte	$01
	.byte	$30
	.byte	$33
	.byte	$68
	.byte	$69
	.byte	$01
	.byte	$7E
	.byte	$7E
	.byte	$69
	.byte	$6A
	.byte	$01
	.byte	$7E
	.byte	$7E
	.byte	$68
	.byte	$69
	.byte	$01
	.byte	$7E
	.byte	$7E
	.byte	$6A
	.byte	$68
	.byte	$01
	.byte	$34
	.byte	$30
	.byte	$69
	.byte	$6A
	.byte	$01
	.byte	$34
	.byte	$30
	.byte	$68
	.byte	$69
	.byte	$01
	.byte	$0F
	.byte	$02
	.byte	$68
	.byte	$69
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$8A
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$9A
	.byte	$01
	.byte	$01
	.byte	$02
	.byte	$02
	.byte	$02
	.byte	$5E
	.byte	$01
	.byte	$02
	.byte	$02
	.byte	$5F
	.byte	$02
	.byte	$01
	.byte	$32
	.byte	$5B
	.byte	$32
	.byte	$32
	.byte	$01
	.byte	$5C
	.byte	$6B
	.byte	$32
	.byte	$35
	.byte	$01
	.byte	$39
	.byte	$5D
	.byte	$36
	.byte	$30
	.byte	$01
	.byte	$31
	.byte	$32
	.byte	$38
	.byte	$38
	.byte	$01
	.byte	$02
	.byte	$02
	.byte	$6B
	.byte	$6C
	.byte	$01
	.byte	$5E
	.byte	$39
	.byte	$39
	.byte	$5D
	.byte	$01
	.byte	$31
	.byte	$5B
	.byte	$5D
	.byte	$32
	.byte	$01
	.byte	$5C
	.byte	$02
	.byte	$32
	.byte	$5F
	.byte	$01
	.byte	$00
	.byte	$BA
	.byte	$D8
	.byte	$D9
	.byte	$01
	.byte	$B9
	.byte	$B9
	.byte	$D6
	.byte	$D7
	.byte	$01
	.byte	$BA
	.byte	$00
	.byte	$C6
	.byte	$C7
	.byte	$01
	.byte	$00
	.byte	$00
	.byte	$B9
	.byte	$BA
	.byte	$01
	.byte	$E6
	.byte	$E7
	.byte	$E6
	.byte	$E7
	.byte	$01
	.byte	$70
	.byte	$71
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$02
	.byte	$02
	.byte	$02
	.byte	$6B
	.byte	$01
	.byte	$02
	.byte	$6B
	.byte	$6C
	.byte	$6F
	.byte	$01
	.byte	$39
	.byte	$5D
	.byte	$6D
	.byte	$5D
	.byte	$01
	.byte	$7F
	.byte	$5B
	.byte	$6D
	.byte	$31
	.byte	$01
	.byte	$5C
	.byte	$39
	.byte	$32
	.byte	$35
	.byte	$01
	.byte	$02
	.byte	$5E
	.byte	$5E
	.byte	$39
	.byte	$01
	.byte	$39
	.byte	$5D
	.byte	$5D
	.byte	$6F
	.byte	$01
	.byte	$5D
	.byte	$6D
	.byte	$6D
	.byte	$5D
	.byte	$01
	.byte	$6F
	.byte	$31
	.byte	$6D
	.byte	$6F
	.byte	$01
	.byte	$32
	.byte	$32
	.byte	$7F
	.byte	$32
	.byte	$01
	.byte	$5F
	.byte	$02
	.byte	$32
	.byte	$5B
	.byte	$01
	.byte	$02
	.byte	$02
	.byte	$5C
	.byte	$02
	.byte	$01
	.byte	$02
	.byte	$02
	.byte	$5B
	.byte	$5C
	.byte	$01
	.byte	$AA
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$00
	.byte	$A9
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$E6
	.byte	$E7
	.byte	$D6
	.byte	$D7
	.byte	$01
	.byte	$00
	.byte	$9F
	.byte	$00
	.byte	$AF
	.byte	$03
	.byte	$ED
	.byte	$EE
	.byte	$FD
	.byte	$FE
	.byte	$03
	.byte	$00
	.byte	$FD
	.byte	$D0
	.byte	$9F
	.byte	$03
	.byte	$FE
	.byte	$00
	.byte	$AF
	.byte	$9F
	.byte	$03
	.byte	$BF
	.byte	$FF
	.byte	$ED
	.byte	$EE
	.byte	$03
	.byte	$5D
	.byte	$6D
	.byte	$30
	.byte	$6F
	.byte	$01
	.byte	$6F
	.byte	$5D
	.byte	$6D
	.byte	$6F
	.byte	$01
	.byte	$6D
	.byte	$5D
	.byte	$6D
	.byte	$6D
	.byte	$01
	.byte	$6D
	.byte	$5D
	.byte	$5D
	.byte	$6D
	.byte	$01
	.byte	$38
	.byte	$7F
	.byte	$5D
	.byte	$6D
	.byte	$01
	.byte	$32
	.byte	$32
	.byte	$31
	.byte	$32
	.byte	$01
	.byte	$32
	.byte	$32
	.byte	$32
	.byte	$32
	.byte	$01
	.byte	$01
	.byte	$9B
	.byte	$01
	.byte	$AB
	.byte	$01
	.byte	$00
	.byte	$B1
	.byte	$B1
	.byte	$B0
	.byte	$01
	.byte	$A1
	.byte	$A0
	.byte	$A0
	.byte	$A0
	.byte	$01
	.byte	$D5
	.byte	$00
	.byte	$A0
	.byte	$D5
	.byte	$01
	.byte	$EC
	.byte	$9F
	.byte	$EE
	.byte	$AF
	.byte	$03
	.byte	$BF
	.byte	$9F
	.byte	$D0
	.byte	$9F
	.byte	$03
	.byte	$9F
	.byte	$AF
	.byte	$AF
	.byte	$BF
	.byte	$03
	.byte	$BF
	.byte	$AF
	.byte	$AF
	.byte	$00
	.byte	$03
	.byte	$EF
	.byte	$FF
	.byte	$FF
	.byte	$EF
	.byte	$03
	.byte	$00
	.byte	$DD
	.byte	$00
	.byte	$00
	.byte	$03
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$70
	.byte	$71
	.byte	$01
	.byte	$AB
	.byte	$01
	.byte	$B1
	.byte	$A0
	.byte	$00
	.byte	$A1
	.byte	$01
	.byte	$A0
	.byte	$D4
	.byte	$D5
	.byte	$00
	.byte	$01
	.byte	$A1
	.byte	$D4
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$FE
	.byte	$00
	.byte	$AF
	.byte	$9F
	.byte	$03
	.byte	$00
	.byte	$AF
	.byte	$ED
	.byte	$EE
	.byte	$03
	.byte	$FD
	.byte	$FE
	.byte	$AF
	.byte	$9F
	.byte	$03
	.byte	$00
	.byte	$00
	.byte	$DD
	.byte	$DE
	.byte	$03
	.byte	$ED
	.byte	$EE
	.byte	$8F
	.byte	$FE
	.byte	$03
	.byte	$EF
	.byte	$00
	.byte	$FF
	.byte	$9F
	.byte	$03
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$BF
	.byte	$AF
	.byte	$00
	.byte	$BF
	.byte	$03
	.byte	$FD
	.byte	$FE
	.byte	$00
	.byte	$BF
	.byte	$03
	.byte	$BF
	.byte	$FF
	.byte	$8E
	.byte	$8F
	.byte	$03
	.byte	$00
	.byte	$BF
	.byte	$BF
	.byte	$1B
	.byte	$03
	.byte	$00
	.byte	$BF
	.byte	$BF
	.byte	$00
	.byte	$03
	.byte	$EC
	.byte	$8F
	.byte	$CE
	.byte	$CF
	.byte	$03
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
_stage3_starting_x_place:
	.byte	$1E
_stage3_starting_y_place:
	.byte	$A0
_stage3_level_0_enemies:
	.byte	$FF
_stage3_Enemy_list:
	.addr	_stage3_level_0_enemies
_stage3_level_0_entities:
	.byte	$FF
_stage3_entity_list:
	.addr	_stage3_level_0_entities
_camelworld_0:
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$CE
	.byte	$EF
	.byte	$EF
	.byte	$DE
	.byte	$DD
	.byte	$EC
	.byte	$EF
	.byte	$EF
	.byte	$CF
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$ED
	.byte	$ED
	.byte	$EE
	.byte	$90
	.byte	$90
	.byte	$ED
	.byte	$EE
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$BC
	.byte	$BD
	.byte	$BD
	.byte	$BD
	.byte	$BE
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$B2
	.byte	$B3
	.byte	$B4
	.byte	$B6
	.byte	$B5
	.byte	$B0
	.byte	$B7
	.byte	$B9
	.byte	$B6
	.byte	$BB
	.byte	$B5
	.byte	$B5
	.byte	$B8
	.byte	$B5
	.byte	$B8
	.byte	$B7
	.byte	$C2
	.byte	$C3
	.byte	$C4
	.byte	$C1
	.byte	$C0
	.byte	$C1
	.byte	$C4
	.byte	$C1
	.byte	$C0
	.byte	$C1
	.byte	$C0
	.byte	$C5
	.byte	$C0
	.byte	$C1
	.byte	$C0
	.byte	$C1
	.byte	$D0
	.byte	$D1
	.byte	$D0
	.byte	$D1
	.byte	$D0
	.byte	$D1
	.byte	$D0
	.byte	$D1
	.byte	$D0
	.byte	$D1
	.byte	$D0
	.byte	$D1
	.byte	$D0
	.byte	$D1
	.byte	$06
	.byte	$05
	.byte	$E0
	.byte	$E1
	.byte	$E0
	.byte	$E1
	.byte	$E0
	.byte	$E1
	.byte	$E0
	.byte	$E1
	.byte	$E0
	.byte	$E1
	.byte	$E0
	.byte	$E1
	.byte	$06
	.byte	$3A
	.byte	$0F
	.byte	$0D
	.byte	$03
	.byte	$03
	.byte	$04
	.byte	$04
	.byte	$01
	.byte	$01
	.byte	$03
	.byte	$03
	.byte	$02
	.byte	$04
	.byte	$01
	.byte	$05
	.byte	$0F
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
_camelworld_1:
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$CE
	.byte	$EF
	.byte	$EF
	.byte	$EF
	.byte	$EF
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$BC
	.byte	$BD
	.byte	$BD
	.byte	$BE
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$ED
	.byte	$EC
	.byte	$DD
	.byte	$EC
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$BD
	.byte	$BE
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$BC
	.byte	$BD
	.byte	$BD
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$09
	.byte	$03
	.byte	$0A
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$B0
	.byte	$B1
	.byte	$37
	.byte	$05
	.byte	$0F
	.byte	$0D
	.byte	$1F
	.byte	$03
	.byte	$0A
	.byte	$B4
	.byte	$B7
	.byte	$B8
	.byte	$B5
	.byte	$B1
	.byte	$B4
	.byte	$B9
	.byte	$06
	.byte	$3A
	.byte	$0F
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$1F
	.byte	$03
	.byte	$08
	.byte	$C1
	.byte	$C0
	.byte	$C1
	.byte	$C0
	.byte	$C1
	.byte	$0F
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$1F
	.byte	$03
	.byte	$07
	.byte	$D1
	.byte	$D0
	.byte	$D1
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$1F
	.byte	$03
	.byte	$08
	.byte	$E1
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$1F
	.byte	$00
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
_camelworld_2:
	.byte	$EF
	.byte	$EF
	.byte	$EF
	.byte	$EF
	.byte	$EF
	.byte	$DE
	.byte	$CF
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$EF
	.byte	$DE
	.byte	$EC
	.byte	$EF
	.byte	$DE
	.byte	$DF
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$ED
	.byte	$EE
	.byte	$90
	.byte	$ED
	.byte	$EE
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$BC
	.byte	$BE
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$BC
	.byte	$BD
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$BD
	.byte	$BE
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$BC
	.byte	$BD
	.byte	$BE
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$B6
	.byte	$B1
	.byte	$B2
	.byte	$B3
	.byte	$B4
	.byte	$B5
	.byte	$B0
	.byte	$B5
	.byte	$B6
	.byte	$B5
	.byte	$BA
	.byte	$B8
	.byte	$B9
	.byte	$BB
	.byte	$B7
	.byte	$37
	.byte	$C0
	.byte	$C1
	.byte	$C2
	.byte	$C3
	.byte	$C0
	.byte	$C1
	.byte	$C0
	.byte	$C1
	.byte	$C0
	.byte	$C5
	.byte	$C4
	.byte	$C1
	.byte	$06
	.byte	$01
	.byte	$05
	.byte	$0F
	.byte	$D0
	.byte	$D1
	.byte	$D0
	.byte	$D1
	.byte	$D0
	.byte	$D1
	.byte	$D0
	.byte	$D1
	.byte	$D0
	.byte	$06
	.byte	$01
	.byte	$05
	.byte	$0F
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$E0
	.byte	$E1
	.byte	$E0
	.byte	$E1
	.byte	$E0
	.byte	$E1
	.byte	$06
	.byte	$01
	.byte	$3A
	.byte	$0F
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$01
	.byte	$04
	.byte	$03
	.byte	$03
	.byte	$04
	.byte	$05
	.byte	$0F
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
_camelworld_3:
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$DC
	.byte	$ED
	.byte	$CD
	.byte	$ED
	.byte	$DF
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$BD
	.byte	$BD
	.byte	$BE
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$09
	.byte	$00
	.byte	$00
	.byte	$0A
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$09
	.byte	$01
	.byte	$3A
	.byte	$0F
	.byte	$0D
	.byte	$0D
	.byte	$1F
	.byte	$03
	.byte	$01
	.byte	$0B
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$09
	.byte	$01
	.byte	$05
	.byte	$0F
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$1F
	.byte	$03
	.byte	$01
	.byte	$05
	.byte	$0F
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
_camelworld_4:
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$CE
	.byte	$EF
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$CD
	.byte	$90
	.byte	$BC
	.byte	$BD
	.byte	$BD
	.byte	$BE
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$BC
	.byte	$BD
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$BC
	.byte	$BD
	.byte	$BD
	.byte	$BD
	.byte	$BE
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$02
	.byte	$0B
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$0D
	.byte	$1F
	.byte	$03
	.byte	$04
	.byte	$0B
	.byte	$BA
	.byte	$B1
	.byte	$B1
	.byte	$B2
	.byte	$B3
	.byte	$B4
	.byte	$B7
	.byte	$B9
	.byte	$BA
	.byte	$B9
	.byte	$B7
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$1F
	.byte	$02
	.byte	$00
	.byte	$07
	.byte	$C2
	.byte	$C3
	.byte	$C0
	.byte	$C1
	.byte	$C0
	.byte	$C5
	.byte	$C0
	.byte	$C1
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$1F
	.byte	$03
	.byte	$03
	.byte	$07
	.byte	$D1
	.byte	$D0
	.byte	$D1
	.byte	$D0
	.byte	$D1
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$1F
	.byte	$04
	.byte	$01
	.byte	$07
	.byte	$E0
	.byte	$E1
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$1F
	.byte	$03
	.byte	$03
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
_camelworld_5:
	.byte	$DE
	.byte	$DD
	.byte	$DD
	.byte	$EC
	.byte	$EF
	.byte	$EF
	.byte	$EF
	.byte	$EF
	.byte	$EF
	.byte	$EF
	.byte	$DE
	.byte	$CD
	.byte	$CC
	.byte	$EE
	.byte	$90
	.byte	$90
	.byte	$EE
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$ED
	.byte	$DD
	.byte	$DD
	.byte	$DD
	.byte	$DD
	.byte	$DD
	.byte	$DF
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$BD
	.byte	$BE
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$BC
	.byte	$BD
	.byte	$BD
	.byte	$BD
	.byte	$BD
	.byte	$BE
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$BC
	.byte	$BD
	.byte	$BE
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$BC
	.byte	$BD
	.byte	$BD
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$B9
	.byte	$B9
	.byte	$B6
	.byte	$B1
	.byte	$B2
	.byte	$B3
	.byte	$B4
	.byte	$B7
	.byte	$BB
	.byte	$B9
	.byte	$BA
	.byte	$B8
	.byte	$B9
	.byte	$B9
	.byte	$BA
	.byte	$B1
	.byte	$C0
	.byte	$C1
	.byte	$C6
	.byte	$C1
	.byte	$C2
	.byte	$C3
	.byte	$C0
	.byte	$C1
	.byte	$C0
	.byte	$C1
	.byte	$C0
	.byte	$C1
	.byte	$C0
	.byte	$C5
	.byte	$C0
	.byte	$06
	.byte	$D0
	.byte	$D1
	.byte	$D0
	.byte	$D1
	.byte	$D0
	.byte	$D1
	.byte	$D0
	.byte	$D1
	.byte	$D0
	.byte	$D1
	.byte	$D0
	.byte	$D1
	.byte	$D0
	.byte	$D1
	.byte	$D0
	.byte	$0C
	.byte	$E0
	.byte	$E1
	.byte	$E0
	.byte	$E1
	.byte	$E0
	.byte	$E1
	.byte	$E0
	.byte	$E1
	.byte	$E0
	.byte	$E1
	.byte	$E0
	.byte	$E1
	.byte	$E0
	.byte	$E1
	.byte	$E0
	.byte	$0C
	.byte	$08
	.byte	$D7
	.byte	$D8
	.byte	$A0
	.byte	$D7
	.byte	$D8
	.byte	$A0
	.byte	$D7
	.byte	$D8
	.byte	$A0
	.byte	$D7
	.byte	$D8
	.byte	$A0
	.byte	$D7
	.byte	$D8
	.byte	$0C
	.byte	$1F
	.byte	$00
	.byte	$02
	.byte	$08
	.byte	$E7
	.byte	$A0
	.byte	$E6
	.byte	$E7
	.byte	$A0
	.byte	$E6
	.byte	$E7
	.byte	$A0
	.byte	$E6
	.byte	$E7
	.byte	$A0
	.byte	$0C
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$1F
	.byte	$03
	.byte	$03
	.byte	$03
	.byte	$03
	.byte	$03
	.byte	$03
	.byte	$08
	.byte	$E8
	.byte	$E8
	.byte	$E8
	.byte	$E8
	.byte	$0C
_camelworld_6:
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0E
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$0C
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0E
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$E9
	.byte	$C9
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0E
	.byte	$EA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0E
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$1F
	.byte	$02
	.byte	$04
	.byte	$01
	.byte	$47
	.byte	$DA
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0E
	.byte	$DA
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0E
	.byte	$E9
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0E
	.byte	$D9
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0E
	.byte	$D9
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$1F
	.byte	$03
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
_camelworld_7:
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$C9
	.byte	$C8
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$EA
	.byte	$DA
	.byte	$CA
	.byte	$CB
	.byte	$C9
	.byte	$C9
	.byte	$C8
	.byte	$CA
	.byte	$0C
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$DA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$E9
	.byte	$C9
	.byte	$C9
	.byte	$C8
	.byte	$CA
	.byte	$CA
	.byte	$CB
	.byte	$0C
	.byte	$0D
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$C9
	.byte	$C8
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$01
	.byte	$02
	.byte	$04
	.byte	$47
	.byte	$EA
	.byte	$DA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0E
	.byte	$DA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0E
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$DA
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0E
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$DA
	.byte	$E9
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0E
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$DA
	.byte	$E9
	.byte	$46
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0E
	.byte	$58
	.byte	$58
	.byte	$58
	.byte	$58
	.byte	$58
	.byte	$58
	.byte	$58
	.byte	$58
	.byte	$58
	.byte	$58
	.byte	$58
	.byte	$0C
_camelworld_8:
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0E
	.byte	$CA
	.byte	$CB
	.byte	$C9
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0E
	.byte	$CB
	.byte	$C9
	.byte	$C9
	.byte	$C8
	.byte	$CA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$0C
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0E
	.byte	$CA
	.byte	$CB
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$CB
	.byte	$0C
	.byte	$0D
	.byte	$0D
	.byte	$0E
	.byte	$CA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$D9
	.byte	$C9
	.byte	$C8
	.byte	$CA
	.byte	$CA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$DA
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$DA
	.byte	$E9
	.byte	$EA
	.byte	$DA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$DA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$D9
	.byte	$46
	.byte	$02
	.byte	$00
	.byte	$47
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$E9
	.byte	$05
	.byte	$0F
	.byte	$0D
	.byte	$0D
	.byte	$1F
	.byte	$04
	.byte	$47
	.byte	$DA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0E
	.byte	$58
	.byte	$58
	.byte	$58
	.byte	$58
	.byte	$58
	.byte	$58
	.byte	$58
	.byte	$58
	.byte	$58
_camelworld_9:
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$C9
	.byte	$C8
	.byte	$CA
	.byte	$CA
	.byte	$CB
	.byte	$C9
	.byte	$C9
	.byte	$C8
	.byte	$0C
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$EA
	.byte	$DA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$CB
	.byte	$C9
	.byte	$0C
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$DA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$C8
	.byte	$CA
	.byte	$0C
	.byte	$0D
	.byte	$0E
	.byte	$C9
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$DA
	.byte	$CB
	.byte	$C9
	.byte	$C9
	.byte	$EA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$DA
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$DA
	.byte	$E9
	.byte	$58
	.byte	$58
	.byte	$58
	.byte	$58
	.byte	$58
	.byte	$58
	.byte	$58
	.byte	$58
	.byte	$58
	.byte	$58
	.byte	$58
	.byte	$58
	.byte	$58
	.byte	$58
	.byte	$58
	.byte	$58
_camelworld_10:
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0E
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0E
	.byte	$CA
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0E
	.byte	$CA
	.byte	$E9
	.byte	$0E
	.byte	$C8
	.byte	$CA
	.byte	$CB
	.byte	$C9
	.byte	$C9
	.byte	$C8
	.byte	$CA
	.byte	$CA
	.byte	$CB
	.byte	$C9
	.byte	$C9
	.byte	$C8
	.byte	$CA
	.byte	$E9
	.byte	$D9
	.byte	$C8
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$DA
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$DA
	.byte	$E9
	.byte	$EA
	.byte	$DA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$DA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$46
	.byte	$05
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$46
	.byte	$02
	.byte	$02
	.byte	$04
	.byte	$05
	.byte	$0F
	.byte	$0D
	.byte	$58
	.byte	$58
	.byte	$58
	.byte	$58
	.byte	$58
	.byte	$58
	.byte	$58
	.byte	$58
	.byte	$58
	.byte	$0C
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
_camelworld_11:
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$DA
	.byte	$0C
	.byte	$0E
	.byte	$32
	.byte	$23
	.byte	$75
	.byte	$4D
	.byte	$4E
	.byte	$4F
	.byte	$22
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$46
	.byte	$05
	.byte	$0F
	.byte	$0E
	.byte	$20
	.byte	$23
	.byte	$81
	.byte	$4D
	.byte	$4E
	.byte	$4F
	.byte	$32
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$46
	.byte	$05
	.byte	$0F
	.byte	$0D
	.byte	$0D
	.byte	$0E
	.byte	$32
	.byte	$23
	.byte	$82
	.byte	$4D
	.byte	$4E
	.byte	$4F
	.byte	$22
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$5C
	.byte	$5B
	.byte	$5B
	.byte	$5B
	.byte	$5B
	.byte	$5D
	.byte	$22
	.byte	$23
	.byte	$98
	.byte	$4D
	.byte	$4E
	.byte	$4F
	.byte	$22
	.byte	$EA
	.byte	$DA
	.byte	$DA
	.byte	$50
	.byte	$51
	.byte	$51
	.byte	$51
	.byte	$51
	.byte	$51
	.byte	$51
	.byte	$52
	.byte	$78
	.byte	$4D
	.byte	$4E
	.byte	$4F
	.byte	$32
	.byte	$DA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$84
	.byte	$73
	.byte	$70
	.byte	$77
	.byte	$84
	.byte	$73
	.byte	$76
	.byte	$4D
	.byte	$4E
	.byte	$4F
	.byte	$20
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$85
	.byte	$98
	.byte	$82
	.byte	$76
	.byte	$85
	.byte	$98
	.byte	$77
	.byte	$4D
	.byte	$4E
	.byte	$4F
	.byte	$22
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$87
	.byte	$98
	.byte	$83
	.byte	$73
	.byte	$87
	.byte	$81
	.byte	$75
	.byte	$4D
	.byte	$4E
	.byte	$4F
	.byte	$22
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$E9
	.byte	$86
	.byte	$99
	.byte	$78
	.byte	$78
	.byte	$86
	.byte	$99
	.byte	$77
	.byte	$4D
	.byte	$4E
	.byte	$4F
	.byte	$32
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$E9
	.byte	$38
	.byte	$18
	.byte	$1B
	.byte	$18
	.byte	$19
	.byte	$18
	.byte	$19
	.byte	$3B
	.byte	$4D
	.byte	$4E
	.byte	$4F
	.byte	$22
	.byte	$EA
	.byte	$DA
	.byte	$46
	.byte	$05
	.byte	$22
	.byte	$3F
	.byte	$1C
	.byte	$3F
	.byte	$1C
	.byte	$3F
	.byte	$2C
	.byte	$2D
	.byte	$39
	.byte	$39
	.byte	$39
	.byte	$1E
	.byte	$46
	.byte	$05
	.byte	$0F
	.byte	$0E
	.byte	$32
	.byte	$2F
	.byte	$1C
	.byte	$2F
	.byte	$1C
	.byte	$2F
	.byte	$2C
	.byte	$3F
	.byte	$2C
	.byte	$3F
	.byte	$2C
	.byte	$3F
	.byte	$0F
	.byte	$0D
	.byte	$0D
	.byte	$0E
	.byte	$32
	.byte	$2F
	.byte	$1C
	.byte	$2F
	.byte	$2C
	.byte	$2F
	.byte	$1C
	.byte	$2F
	.byte	$2C
	.byte	$2F
	.byte	$2C
	.byte	$2F
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0E
	.byte	$25
	.byte	$29
	.byte	$2B
	.byte	$2A
	.byte	$2B
	.byte	$29
	.byte	$2B
	.byte	$29
	.byte	$2B
	.byte	$29
	.byte	$2B
	.byte	$29
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$5A
	.byte	$5A
	.byte	$5A
	.byte	$5A
	.byte	$5A
	.byte	$5A
	.byte	$5A
	.byte	$5A
	.byte	$5A
	.byte	$5A
	.byte	$5A
	.byte	$5A
_camelworld_12:
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$32
	.byte	$2F
	.byte	$3C
	.byte	$88
	.byte	$4D
	.byte	$4E
	.byte	$4F
	.byte	$32
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$32
	.byte	$2F
	.byte	$4C
	.byte	$79
	.byte	$4D
	.byte	$4E
	.byte	$4F
	.byte	$22
	.byte	$90
	.byte	$BC
	.byte	$BD
	.byte	$BD
	.byte	$BE
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$32
	.byte	$1D
	.byte	$33
	.byte	$99
	.byte	$4D
	.byte	$4E
	.byte	$4F
	.byte	$32
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$50
	.byte	$51
	.byte	$52
	.byte	$88
	.byte	$4D
	.byte	$4E
	.byte	$4F
	.byte	$22
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$9D
	.byte	$84
	.byte	$70
	.byte	$89
	.byte	$4D
	.byte	$4E
	.byte	$4F
	.byte	$32
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$87
	.byte	$72
	.byte	$76
	.byte	$4D
	.byte	$4E
	.byte	$4F
	.byte	$32
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$BC
	.byte	$BD
	.byte	$BE
	.byte	$90
	.byte	$90
	.byte	$85
	.byte	$72
	.byte	$79
	.byte	$4D
	.byte	$4E
	.byte	$4F
	.byte	$32
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$7E
	.byte	$86
	.byte	$72
	.byte	$81
	.byte	$4D
	.byte	$4E
	.byte	$4F
	.byte	$22
	.byte	$B6
	.byte	$B7
	.byte	$B6
	.byte	$B1
	.byte	$B2
	.byte	$B3
	.byte	$B4
	.byte	$BB
	.byte	$10
	.byte	$45
	.byte	$15
	.byte	$78
	.byte	$4D
	.byte	$4E
	.byte	$4F
	.byte	$32
	.byte	$C0
	.byte	$C1
	.byte	$C6
	.byte	$C1
	.byte	$C2
	.byte	$C3
	.byte	$C4
	.byte	$C1
	.byte	$22
	.byte	$2F
	.byte	$3C
	.byte	$76
	.byte	$4D
	.byte	$4E
	.byte	$4F
	.byte	$22
	.byte	$D0
	.byte	$D1
	.byte	$D0
	.byte	$D1
	.byte	$D0
	.byte	$D1
	.byte	$D0
	.byte	$D1
	.byte	$22
	.byte	$2F
	.byte	$3C
	.byte	$89
	.byte	$4D
	.byte	$4E
	.byte	$4F
	.byte	$32
	.byte	$E0
	.byte	$E1
	.byte	$E0
	.byte	$E1
	.byte	$E0
	.byte	$E1
	.byte	$E0
	.byte	$E1
	.byte	$32
	.byte	$2F
	.byte	$4C
	.byte	$78
	.byte	$4D
	.byte	$4E
	.byte	$4F
	.byte	$32
	.byte	$D7
	.byte	$D8
	.byte	$A0
	.byte	$D7
	.byte	$D8
	.byte	$A0
	.byte	$D7
	.byte	$D8
	.byte	$22
	.byte	$2F
	.byte	$3C
	.byte	$98
	.byte	$4D
	.byte	$4E
	.byte	$4F
	.byte	$22
	.byte	$E7
	.byte	$A0
	.byte	$E6
	.byte	$E7
	.byte	$A0
	.byte	$E6
	.byte	$E7
	.byte	$A0
	.byte	$22
	.byte	$2F
	.byte	$4C
	.byte	$79
	.byte	$4D
	.byte	$4E
	.byte	$4F
	.byte	$32
	.byte	$E8
	.byte	$E8
	.byte	$E8
	.byte	$E8
	.byte	$E8
	.byte	$E8
	.byte	$E8
	.byte	$E8
	.byte	$32
	.byte	$1D
	.byte	$23
	.byte	$78
	.byte	$4D
	.byte	$4E
	.byte	$4F
	.byte	$32
_camelworld_13:
	.byte	$32
	.byte	$33
	.byte	$4D
	.byte	$4E
	.byte	$4F
	.byte	$83
	.byte	$81
	.byte	$83
	.byte	$85
	.byte	$99
	.byte	$72
	.byte	$25
	.byte	$2A
	.byte	$2B
	.byte	$26
	.byte	$2D
	.byte	$32
	.byte	$23
	.byte	$4D
	.byte	$4E
	.byte	$4F
	.byte	$8C
	.byte	$82
	.byte	$72
	.byte	$85
	.byte	$72
	.byte	$83
	.byte	$73
	.byte	$70
	.byte	$71
	.byte	$73
	.byte	$22
	.byte	$22
	.byte	$23
	.byte	$4D
	.byte	$4E
	.byte	$4F
	.byte	$90
	.byte	$8A
	.byte	$8C
	.byte	$87
	.byte	$99
	.byte	$72
	.byte	$82
	.byte	$72
	.byte	$80
	.byte	$88
	.byte	$32
	.byte	$22
	.byte	$23
	.byte	$4D
	.byte	$4E
	.byte	$4F
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$85
	.byte	$72
	.byte	$72
	.byte	$80
	.byte	$72
	.byte	$72
	.byte	$99
	.byte	$32
	.byte	$32
	.byte	$23
	.byte	$4D
	.byte	$4E
	.byte	$4F
	.byte	$90
	.byte	$90
	.byte	$7E
	.byte	$85
	.byte	$98
	.byte	$81
	.byte	$72
	.byte	$98
	.byte	$72
	.byte	$78
	.byte	$32
	.byte	$22
	.byte	$33
	.byte	$4D
	.byte	$4E
	.byte	$4F
	.byte	$90
	.byte	$7E
	.byte	$89
	.byte	$87
	.byte	$75
	.byte	$80
	.byte	$99
	.byte	$73
	.byte	$72
	.byte	$88
	.byte	$32
	.byte	$22
	.byte	$23
	.byte	$4D
	.byte	$4E
	.byte	$4F
	.byte	$7D
	.byte	$83
	.byte	$94
	.byte	$86
	.byte	$95
	.byte	$81
	.byte	$78
	.byte	$93
	.byte	$95
	.byte	$94
	.byte	$22
	.byte	$32
	.byte	$33
	.byte	$4D
	.byte	$4E
	.byte	$4F
	.byte	$38
	.byte	$18
	.byte	$19
	.byte	$34
	.byte	$19
	.byte	$1B
	.byte	$79
	.byte	$4D
	.byte	$4E
	.byte	$4F
	.byte	$32
	.byte	$22
	.byte	$1D
	.byte	$39
	.byte	$39
	.byte	$39
	.byte	$2D
	.byte	$3F
	.byte	$2C
	.byte	$3F
	.byte	$1C
	.byte	$23
	.byte	$78
	.byte	$4D
	.byte	$4E
	.byte	$4F
	.byte	$32
	.byte	$32
	.byte	$1D
	.byte	$3F
	.byte	$1C
	.byte	$3F
	.byte	$2C
	.byte	$2F
	.byte	$1C
	.byte	$2F
	.byte	$2C
	.byte	$33
	.byte	$88
	.byte	$4D
	.byte	$4E
	.byte	$4F
	.byte	$32
	.byte	$32
	.byte	$1D
	.byte	$1D
	.byte	$1D
	.byte	$1D
	.byte	$1D
	.byte	$1D
	.byte	$1D
	.byte	$1D
	.byte	$1D
	.byte	$33
	.byte	$89
	.byte	$4D
	.byte	$4E
	.byte	$4F
	.byte	$32
	.byte	$50
	.byte	$51
	.byte	$51
	.byte	$51
	.byte	$51
	.byte	$51
	.byte	$51
	.byte	$51
	.byte	$51
	.byte	$2E
	.byte	$23
	.byte	$78
	.byte	$4D
	.byte	$4E
	.byte	$4F
	.byte	$22
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$22
	.byte	$2D
	.byte	$33
	.byte	$79
	.byte	$4D
	.byte	$4E
	.byte	$4F
	.byte	$22
	.byte	$90
	.byte	$BC
	.byte	$BD
	.byte	$BE
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$22
	.byte	$3F
	.byte	$3C
	.byte	$83
	.byte	$4D
	.byte	$4E
	.byte	$4F
	.byte	$32
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$22
	.byte	$2F
	.byte	$4C
	.byte	$78
	.byte	$4D
	.byte	$4E
	.byte	$4F
	.byte	$32
_camelworld_14:
	.byte	$EF
	.byte	$EF
	.byte	$EF
	.byte	$EF
	.byte	$EF
	.byte	$EF
	.byte	$EF
	.byte	$EF
	.byte	$CF
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$EF
	.byte	$EF
	.byte	$EF
	.byte	$EF
	.byte	$DE
	.byte	$CF
	.byte	$ED
	.byte	$ED
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$53
	.byte	$54
	.byte	$54
	.byte	$54
	.byte	$54
	.byte	$EC
	.byte	$DE
	.byte	$CD
	.byte	$ED
	.byte	$EE
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$50
	.byte	$51
	.byte	$51
	.byte	$51
	.byte	$51
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$60
	.byte	$84
	.byte	$98
	.byte	$73
	.byte	$71
	.byte	$7B
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$97
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$87
	.byte	$98
	.byte	$83
	.byte	$8F
	.byte	$83
	.byte	$7F
	.byte	$90
	.byte	$90
	.byte	$BC
	.byte	$BD
	.byte	$BD
	.byte	$87
	.byte	$BE
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$85
	.byte	$82
	.byte	$8D
	.byte	$90
	.byte	$15
	.byte	$9B
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$85
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$86
	.byte	$8E
	.byte	$90
	.byte	$90
	.byte	$2D
	.byte	$15
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$85
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$85
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$1D
	.byte	$23
	.byte	$7B
	.byte	$7E
	.byte	$7F
	.byte	$90
	.byte	$90
	.byte	$87
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$85
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$1E
	.byte	$23
	.byte	$81
	.byte	$72
	.byte	$81
	.byte	$7F
	.byte	$7C
	.byte	$85
	.byte	$7B
	.byte	$90
	.byte	$90
	.byte	$7C
	.byte	$86
	.byte	$7E
	.byte	$7F
	.byte	$7D
	.byte	$1D
	.byte	$23
	.byte	$81
	.byte	$81
	.byte	$81
	.byte	$81
	.byte	$81
	.byte	$85
	.byte	$73
	.byte	$7F
	.byte	$7D
	.byte	$78
	.byte	$85
	.byte	$98
	.byte	$81
	.byte	$82
	.byte	$1E
	.byte	$23
	.byte	$94
	.byte	$92
	.byte	$93
	.byte	$95
	.byte	$81
	.byte	$86
	.byte	$78
	.byte	$93
	.byte	$94
	.byte	$93
	.byte	$86
	.byte	$94
	.byte	$95
	.byte	$94
	.byte	$1D
	.byte	$23
	.byte	$4D
	.byte	$4E
	.byte	$4F
	.byte	$10
	.byte	$17
	.byte	$34
	.byte	$1B
	.byte	$5F
	.byte	$5F
	.byte	$38
	.byte	$34
	.byte	$1B
	.byte	$91
	.byte	$91
	.byte	$2D
	.byte	$23
	.byte	$4D
	.byte	$4E
	.byte	$4F
	.byte	$25
	.byte	$27
	.byte	$2D
	.byte	$28
	.byte	$26
	.byte	$26
	.byte	$1D
	.byte	$3F
	.byte	$3C
	.byte	$5F
	.byte	$5F
	.byte	$1D
	.byte	$23
	.byte	$4D
	.byte	$4E
	.byte	$4F
	.byte	$81
	.byte	$81
	.byte	$84
	.byte	$81
	.byte	$78
	.byte	$95
	.byte	$27
	.byte	$2F
	.byte	$2C
	.byte	$2D
	.byte	$2D
_camelworld_15:
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$DC
	.byte	$DD
	.byte	$EF
	.byte	$EF
	.byte	$EF
	.byte	$EF
	.byte	$EF
	.byte	$EF
	.byte	$EF
	.byte	$EF
	.byte	$54
	.byte	$54
	.byte	$55
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$ED
	.byte	$EC
	.byte	$DD
	.byte	$DD
	.byte	$DD
	.byte	$ED
	.byte	$EC
	.byte	$DE
	.byte	$51
	.byte	$51
	.byte	$52
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$DF
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$79
	.byte	$84
	.byte	$61
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$BC
	.byte	$BD
	.byte	$BD
	.byte	$BD
	.byte	$8E
	.byte	$86
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$85
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$BC
	.byte	$85
	.byte	$BD
	.byte	$BE
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$86
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$97
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$7B
	.byte	$85
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$7E
	.byte	$85
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$81
	.byte	$85
	.byte	$7F
	.byte	$7B
	.byte	$7D
	.byte	$79
	.byte	$86
	.byte	$7F
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$83
	.byte	$86
	.byte	$98
	.byte	$72
	.byte	$72
	.byte	$83
	.byte	$85
	.byte	$98
	.byte	$7F
	.byte	$7B
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$96
	.byte	$86
	.byte	$98
	.byte	$83
	.byte	$72
	.byte	$82
	.byte	$86
	.byte	$99
	.byte	$83
	.byte	$82
	.byte	$7F
	.byte	$90
	.byte	$A7
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$38
	.byte	$34
	.byte	$1B
	.byte	$95
	.byte	$94
	.byte	$10
	.byte	$45
	.byte	$15
	.byte	$96
	.byte	$91
	.byte	$94
	.byte	$10
	.byte	$45
	.byte	$44
	.byte	$90
	.byte	$90
	.byte	$22
	.byte	$3F
	.byte	$3C
	.byte	$5F
	.byte	$5F
	.byte	$2D
	.byte	$3F
	.byte	$1C
	.byte	$15
	.byte	$5F
	.byte	$5F
	.byte	$22
	.byte	$3F
	.byte	$1C
	.byte	$12
	.byte	$13
	.byte	$2E
	.byte	$2F
	.byte	$2C
	.byte	$1E
	.byte	$1E
	.byte	$2D
	.byte	$2F
	.byte	$2C
	.byte	$2D
	.byte	$1E
	.byte	$1E
	.byte	$2E
	.byte	$2F
	.byte	$1C
	.byte	$2D
	.byte	$2D
_camelworld_16:
	.byte	$DE
	.byte	$DF
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$DC
	.byte	$DD
	.byte	$EF
	.byte	$EF
	.byte	$EF
	.byte	$EF
	.byte	$EF
	.byte	$EF
	.byte	$CF
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$CE
	.byte	$EF
	.byte	$EF
	.byte	$EF
	.byte	$EF
	.byte	$CF
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$BC
	.byte	$BD
	.byte	$BD
	.byte	$BD
	.byte	$BE
	.byte	$90
	.byte	$ED
	.byte	$DD
	.byte	$CC
	.byte	$ED
	.byte	$90
	.byte	$90
	.byte	$BE
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$43
	.byte	$12
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$32
	.byte	$2D
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$32
	.byte	$2D
	.byte	$90
	.byte	$90
	.byte	$BC
	.byte	$BD
	.byte	$BD
	.byte	$BD
	.byte	$BE
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$22
	.byte	$1D
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$97
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$32
	.byte	$2E
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$87
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$32
	.byte	$2D
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$7B
	.byte	$86
	.byte	$7E
	.byte	$A7
	.byte	$7F
	.byte	$90
	.byte	$90
	.byte	$7C
	.byte	$7A
	.byte	$7A
	.byte	$22
	.byte	$2D
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$43
	.byte	$12
	.byte	$45
	.byte	$12
	.byte	$45
	.byte	$12
	.byte	$44
	.byte	$7D
	.byte	$73
	.byte	$81
	.byte	$73
	.byte	$22
	.byte	$1D
	.byte	$90
	.byte	$7B
	.byte	$7D
	.byte	$32
	.byte	$2D
	.byte	$2D
	.byte	$2D
	.byte	$2D
	.byte	$2D
	.byte	$23
	.byte	$98
	.byte	$83
	.byte	$72
	.byte	$80
	.byte	$22
	.byte	$1D
	.byte	$7E
	.byte	$81
	.byte	$10
	.byte	$2D
	.byte	$3F
	.byte	$3F
	.byte	$3F
	.byte	$3F
	.byte	$3F
	.byte	$3C
	.byte	$95
	.byte	$72
	.byte	$83
	.byte	$95
	.byte	$32
	.byte	$1D
	.byte	$13
	.byte	$13
	.byte	$2D
	.byte	$2D
	.byte	$2F
	.byte	$2F
	.byte	$2F
	.byte	$2F
	.byte	$2F
	.byte	$4C
	.byte	$91
	.byte	$95
	.byte	$72
	.byte	$73
	.byte	$32
	.byte	$2D
	.byte	$2D
	.byte	$2D
	.byte	$2D
	.byte	$2D
	.byte	$2F
	.byte	$2F
	.byte	$2F
	.byte	$2F
	.byte	$2F
	.byte	$2C
	.byte	$12
	.byte	$15
	.byte	$83
	.byte	$94
	.byte	$22
	.byte	$1D
_camelworld_17:
	.byte	$2D
	.byte	$2E
	.byte	$2E
	.byte	$2E
	.byte	$2F
	.byte	$2F
	.byte	$2F
	.byte	$2F
	.byte	$2F
	.byte	$1C
	.byte	$2E
	.byte	$33
	.byte	$80
	.byte	$80
	.byte	$32
	.byte	$2D
	.byte	$2E
	.byte	$2E
	.byte	$1E
	.byte	$2E
	.byte	$2F
	.byte	$2F
	.byte	$2F
	.byte	$2F
	.byte	$2F
	.byte	$2C
	.byte	$35
	.byte	$71
	.byte	$73
	.byte	$80
	.byte	$25
	.byte	$2D
	.byte	$2E
	.byte	$2E
	.byte	$2D
	.byte	$2E
	.byte	$26
	.byte	$28
	.byte	$28
	.byte	$27
	.byte	$28
	.byte	$35
	.byte	$70
	.byte	$80
	.byte	$82
	.byte	$80
	.byte	$70
	.byte	$32
	.byte	$2D
	.byte	$2E
	.byte	$2E
	.byte	$35
	.byte	$71
	.byte	$73
	.byte	$71
	.byte	$70
	.byte	$84
	.byte	$73
	.byte	$83
	.byte	$83
	.byte	$83
	.byte	$8D
	.byte	$8A
	.byte	$32
	.byte	$2E
	.byte	$2E
	.byte	$33
	.byte	$71
	.byte	$83
	.byte	$8F
	.byte	$8E
	.byte	$8C
	.byte	$85
	.byte	$8D
	.byte	$8C
	.byte	$8D
	.byte	$8B
	.byte	$90
	.byte	$90
	.byte	$32
	.byte	$2E
	.byte	$2D
	.byte	$33
	.byte	$83
	.byte	$9B
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$85
	.byte	$7B
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$32
	.byte	$2D
	.byte	$2D
	.byte	$23
	.byte	$82
	.byte	$8E
	.byte	$90
	.byte	$90
	.byte	$7E
	.byte	$86
	.byte	$99
	.byte	$7F
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$32
	.byte	$2E
	.byte	$2E
	.byte	$35
	.byte	$8E
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$10
	.byte	$45
	.byte	$12
	.byte	$15
	.byte	$90
	.byte	$90
	.byte	$BC
	.byte	$BD
	.byte	$32
	.byte	$2E
	.byte	$33
	.byte	$9A
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$25
	.byte	$2D
	.byte	$1D
	.byte	$35
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$32
	.byte	$2D
	.byte	$23
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$8C
	.byte	$25
	.byte	$35
	.byte	$8E
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$7E
	.byte	$32
	.byte	$2E
	.byte	$33
	.byte	$90
	.byte	$BC
	.byte	$BD
	.byte	$BE
	.byte	$90
	.byte	$90
	.byte	$8F
	.byte	$8E
	.byte	$90
	.byte	$90
	.byte	$7B
	.byte	$7E
	.byte	$82
	.byte	$32
	.byte	$2D
	.byte	$33
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$7D
	.byte	$72
	.byte	$73
	.byte	$83
	.byte	$32
	.byte	$2D
	.byte	$33
	.byte	$7B
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$BC
	.byte	$BE
	.byte	$9C
	.byte	$73
	.byte	$81
	.byte	$81
	.byte	$32
	.byte	$2E
	.byte	$33
	.byte	$72
	.byte	$7F
	.byte	$7B
	.byte	$7B
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$8C
	.byte	$73
	.byte	$83
	.byte	$82
	.byte	$32
	.byte	$2D
	.byte	$33
	.byte	$72
	.byte	$72
	.byte	$81
	.byte	$81
	.byte	$7F
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$7D
	.byte	$73
	.byte	$81
	.byte	$83
	.byte	$32
_camelworld_18:
	.byte	$2D
	.byte	$23
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$22
	.byte	$27
	.byte	$35
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$BC
	.byte	$BD
	.byte	$BD
	.byte	$BE
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$22
	.byte	$81
	.byte	$9A
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$32
	.byte	$9B
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$7B
	.byte	$7D
	.byte	$22
	.byte	$79
	.byte	$7F
	.byte	$90
	.byte	$BC
	.byte	$BD
	.byte	$BE
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$43
	.byte	$12
	.byte	$12
	.byte	$2E
	.byte	$12
	.byte	$15
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$BC
	.byte	$BE
	.byte	$90
	.byte	$25
	.byte	$28
	.byte	$27
	.byte	$2E
	.byte	$2E
	.byte	$33
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$9D
	.byte	$71
	.byte	$73
	.byte	$22
	.byte	$2E
	.byte	$33
	.byte	$7F
	.byte	$7B
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$8A
	.byte	$8C
	.byte	$22
	.byte	$2E
	.byte	$2E
	.byte	$12
	.byte	$12
	.byte	$11
	.byte	$12
	.byte	$14
	.byte	$44
	.byte	$B4
	.byte	$B5
	.byte	$B6
	.byte	$B7
	.byte	$B1
	.byte	$B2
	.byte	$B3
	.byte	$22
	.byte	$2E
	.byte	$1D
	.byte	$27
	.byte	$26
	.byte	$26
	.byte	$28
	.byte	$28
	.byte	$35
	.byte	$C0
	.byte	$C5
	.byte	$C6
	.byte	$C1
	.byte	$C0
	.byte	$C2
	.byte	$C3
	.byte	$25
	.byte	$2E
	.byte	$33
	.byte	$73
	.byte	$70
	.byte	$70
	.byte	$73
	.byte	$84
	.byte	$E1
	.byte	$D0
	.byte	$D1
	.byte	$D0
	.byte	$D1
	.byte	$D0
	.byte	$06
	.byte	$05
	.byte	$0F
	.byte	$2E
	.byte	$33
	.byte	$80
	.byte	$82
	.byte	$80
	.byte	$89
	.byte	$87
	.byte	$A0
	.byte	$E0
	.byte	$E1
	.byte	$E0
	.byte	$46
	.byte	$05
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$2E
	.byte	$33
	.byte	$82
	.byte	$80
	.byte	$78
	.byte	$77
	.byte	$86
	.byte	$D8
	.byte	$A0
	.byte	$06
	.byte	$05
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$2E
	.byte	$33
	.byte	$82
	.byte	$82
	.byte	$82
	.byte	$46
	.byte	$A8
	.byte	$04
	.byte	$05
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$2E
	.byte	$33
	.byte	$82
	.byte	$82
	.byte	$82
	.byte	$4D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
_camelworld_19:
	.byte	$2D
	.byte	$23
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$0C
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$1D
	.byte	$33
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$CA
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$2E
	.byte	$23
	.byte	$EA
	.byte	$DA
	.byte	$DA
	.byte	$E9
	.byte	$C9
	.byte	$C9
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$C9
	.byte	$C9
	.byte	$C8
	.byte	$CA
	.byte	$CA
	.byte	$2D
	.byte	$33
	.byte	$DA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$CA
	.byte	$CB
	.byte	$C9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$DA
	.byte	$E9
	.byte	$2E
	.byte	$23
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$2D
	.byte	$23
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$1D
	.byte	$23
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$2E
	.byte	$33
	.byte	$01
	.byte	$01
	.byte	$07
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$2D
	.byte	$23
	.byte	$0C
	.byte	$0D
	.byte	$1F
	.byte	$04
	.byte	$07
	.byte	$EA
	.byte	$DA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$E9
	.byte	$1D
	.byte	$33
	.byte	$0C
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$1F
	.byte	$01
	.byte	$04
	.byte	$00
	.byte	$07
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$2E
	.byte	$23
	.byte	$0C
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$1F
	.byte	$04
	.byte	$01
	.byte	$01
	.byte	$02
	.byte	$03
	.byte	$2D
	.byte	$23
	.byte	$0C
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$2E
	.byte	$23
	.byte	$0C
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$26
	.byte	$35
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$5A
	.byte	$5A
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
_camelworld_20:
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$CB
	.byte	$C9
	.byte	$C9
	.byte	$0C
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$0C
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$0C
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0E
	.byte	$CB
	.byte	$C9
	.byte	$EA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$0C
	.byte	$0D
	.byte	$0E
	.byte	$CB
	.byte	$C9
	.byte	$0C
	.byte	$0D
	.byte	$0D
	.byte	$0E
	.byte	$D9
	.byte	$D9
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$0C
	.byte	$0D
	.byte	$0E
	.byte	$D9
	.byte	$D9
	.byte	$0C
	.byte	$0D
	.byte	$0D
	.byte	$0E
	.byte	$D9
	.byte	$EA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$0C
	.byte	$0D
	.byte	$0E
	.byte	$D9
	.byte	$EA
	.byte	$0C
	.byte	$0D
	.byte	$0D
	.byte	$1F
	.byte	$04
	.byte	$05
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$DA
	.byte	$0C
	.byte	$0D
	.byte	$0E
	.byte	$EA
	.byte	$DA
	.byte	$0C
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$D9
	.byte	$EA
	.byte	$DA
	.byte	$DA
	.byte	$E9
	.byte	$0C
	.byte	$0D
	.byte	$1F
	.byte	$02
	.byte	$01
	.byte	$0F
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$01
	.byte	$03
	.byte	$03
	.byte	$01
	.byte	$05
	.byte	$0C
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
_camelworld_21:
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0E
	.byte	$CA
	.byte	$CA
	.byte	$0C
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0C
	.byte	$0E
	.byte	$CA
	.byte	$CA
	.byte	$0C
	.byte	$0D
	.byte	$0D
	.byte	$0E
	.byte	$DA
	.byte	$E9
	.byte	$0C
	.byte	$0D
	.byte	$0D
	.byte	$0E
	.byte	$CA
	.byte	$CB
	.byte	$0C
	.byte	$0E
	.byte	$DA
	.byte	$E9
	.byte	$0C
	.byte	$0D
	.byte	$0D
	.byte	$0E
	.byte	$E9
	.byte	$D9
	.byte	$0C
	.byte	$0D
	.byte	$0D
	.byte	$0E
	.byte	$E9
	.byte	$D9
	.byte	$0C
	.byte	$0E
	.byte	$E9
	.byte	$D9
	.byte	$0C
	.byte	$0D
	.byte	$0D
	.byte	$0E
	.byte	$D9
	.byte	$D9
	.byte	$0C
	.byte	$0D
	.byte	$0D
	.byte	$0E
	.byte	$D9
	.byte	$D9
	.byte	$0F
	.byte	$0E
	.byte	$D9
	.byte	$D9
	.byte	$0C
	.byte	$0D
	.byte	$0D
	.byte	$0E
	.byte	$D9
	.byte	$EA
	.byte	$0C
	.byte	$0D
	.byte	$0D
	.byte	$0E
	.byte	$D9
	.byte	$EA
	.byte	$0D
	.byte	$0E
	.byte	$D9
	.byte	$EA
	.byte	$0C
	.byte	$0D
	.byte	$0D
	.byte	$0E
	.byte	$EA
	.byte	$DA
	.byte	$0C
	.byte	$0D
	.byte	$0D
	.byte	$0E
	.byte	$EA
	.byte	$DA
	.byte	$0D
	.byte	$0E
	.byte	$EA
	.byte	$DA
	.byte	$0C
	.byte	$0D
	.byte	$0D
	.byte	$1F
	.byte	$03
	.byte	$3A
	.byte	$0F
	.byte	$0D
	.byte	$0D
	.byte	$0E
	.byte	$DA
	.byte	$E9
	.byte	$0D
	.byte	$1F
	.byte	$03
	.byte	$02
	.byte	$0F
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0E
	.byte	$E9
	.byte	$D9
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$1F
	.byte	$03
	.byte	$3A
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$0D
_camelworld_22:
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$22
	.byte	$2F
	.byte	$4C
	.byte	$4D
	.byte	$4E
	.byte	$4E
	.byte	$4F
	.byte	$22
	.byte	$1E
	.byte	$2E
	.byte	$2E
	.byte	$2D
	.byte	$2E
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$22
	.byte	$2F
	.byte	$4C
	.byte	$4D
	.byte	$4E
	.byte	$4E
	.byte	$4F
	.byte	$32
	.byte	$2E
	.byte	$2E
	.byte	$2E
	.byte	$1D
	.byte	$1E
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$22
	.byte	$2F
	.byte	$4C
	.byte	$4D
	.byte	$4E
	.byte	$4E
	.byte	$4F
	.byte	$22
	.byte	$2E
	.byte	$2D
	.byte	$2E
	.byte	$2E
	.byte	$2E
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$22
	.byte	$2E
	.byte	$23
	.byte	$4D
	.byte	$4E
	.byte	$4E
	.byte	$4F
	.byte	$22
	.byte	$1E
	.byte	$2E
	.byte	$2E
	.byte	$2D
	.byte	$2E
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$50
	.byte	$51
	.byte	$52
	.byte	$4D
	.byte	$4E
	.byte	$4E
	.byte	$4F
	.byte	$32
	.byte	$2E
	.byte	$2E
	.byte	$2E
	.byte	$2E
	.byte	$2E
	.byte	$C9
	.byte	$C8
	.byte	$CA
	.byte	$CB
	.byte	$84
	.byte	$71
	.byte	$4D
	.byte	$4E
	.byte	$4E
	.byte	$4F
	.byte	$22
	.byte	$2E
	.byte	$2E
	.byte	$2E
	.byte	$2D
	.byte	$2E
	.byte	$EA
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$87
	.byte	$78
	.byte	$4D
	.byte	$4E
	.byte	$4E
	.byte	$4F
	.byte	$22
	.byte	$2E
	.byte	$2D
	.byte	$2E
	.byte	$2E
	.byte	$2E
	.byte	$DA
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$87
	.byte	$81
	.byte	$4D
	.byte	$4E
	.byte	$4E
	.byte	$4F
	.byte	$32
	.byte	$2E
	.byte	$2D
	.byte	$2D
	.byte	$2E
	.byte	$2E
	.byte	$E9
	.byte	$D9
	.byte	$D9
	.byte	$EA
	.byte	$86
	.byte	$99
	.byte	$4D
	.byte	$4E
	.byte	$4E
	.byte	$4F
	.byte	$22
	.byte	$1E
	.byte	$2E
	.byte	$2E
	.byte	$2E
	.byte	$2E
	.byte	$46
	.byte	$01
	.byte	$05
	.byte	$38
	.byte	$34
	.byte	$3B
	.byte	$4D
	.byte	$4E
	.byte	$4E
	.byte	$4F
	.byte	$32
	.byte	$2E
	.byte	$2E
	.byte	$2E
	.byte	$2D
	.byte	$2E
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$22
	.byte	$3F
	.byte	$1C
	.byte	$39
	.byte	$39
	.byte	$39
	.byte	$39
	.byte	$2E
	.byte	$1D
	.byte	$2E
	.byte	$2E
	.byte	$2D
	.byte	$2E
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$2E
	.byte	$2F
	.byte	$2C
	.byte	$1D
	.byte	$2D
	.byte	$1D
	.byte	$1D
	.byte	$1D
	.byte	$2D
	.byte	$1D
	.byte	$2E
	.byte	$2E
	.byte	$2E
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$2E
	.byte	$2F
	.byte	$2C
	.byte	$1D
	.byte	$1D
	.byte	$1D
	.byte	$1D
	.byte	$1E
	.byte	$2E
	.byte	$2E
	.byte	$2E
	.byte	$2D
	.byte	$2E
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$25
	.byte	$28
	.byte	$27
	.byte	$26
	.byte	$26
	.byte	$28
	.byte	$26
	.byte	$26
	.byte	$26
	.byte	$28
	.byte	$26
	.byte	$26
	.byte	$35
	.byte	$0D
	.byte	$0D
	.byte	$0D
	.byte	$5A
	.byte	$5A
	.byte	$5A
	.byte	$5A
	.byte	$5A
	.byte	$5A
	.byte	$5A
	.byte	$5A
	.byte	$5A
	.byte	$5A
	.byte	$5A
	.byte	$5A
	.byte	$5A
_camelworld_23:
	.byte	$DC
	.byte	$DD
	.byte	$EF
	.byte	$DE
	.byte	$CF
	.byte	$90
	.byte	$4D
	.byte	$4E
	.byte	$4E
	.byte	$4F
	.byte	$90
	.byte	$DC
	.byte	$DD
	.byte	$DD
	.byte	$CF
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$ED
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$4D
	.byte	$4E
	.byte	$4E
	.byte	$4F
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$4D
	.byte	$4E
	.byte	$4E
	.byte	$4F
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$BC
	.byte	$BD
	.byte	$90
	.byte	$90
	.byte	$BC
	.byte	$BE
	.byte	$90
	.byte	$90
	.byte	$4D
	.byte	$4E
	.byte	$4E
	.byte	$4F
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$4D
	.byte	$4E
	.byte	$4E
	.byte	$4F
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$13
	.byte	$14
	.byte	$11
	.byte	$13
	.byte	$14
	.byte	$44
	.byte	$4D
	.byte	$4E
	.byte	$4E
	.byte	$4F
	.byte	$43
	.byte	$14
	.byte	$12
	.byte	$11
	.byte	$12
	.byte	$14
	.byte	$51
	.byte	$51
	.byte	$51
	.byte	$51
	.byte	$51
	.byte	$52
	.byte	$4D
	.byte	$4E
	.byte	$4E
	.byte	$4F
	.byte	$50
	.byte	$51
	.byte	$51
	.byte	$51
	.byte	$51
	.byte	$51
	.byte	$6B
	.byte	$84
	.byte	$6B
	.byte	$6B
	.byte	$84
	.byte	$90
	.byte	$4D
	.byte	$4E
	.byte	$4E
	.byte	$4F
	.byte	$6B
	.byte	$84
	.byte	$AC
	.byte	$AD
	.byte	$84
	.byte	$6B
	.byte	$B2
	.byte	$87
	.byte	$B4
	.byte	$B7
	.byte	$85
	.byte	$B9
	.byte	$4D
	.byte	$4E
	.byte	$4E
	.byte	$4F
	.byte	$B1
	.byte	$85
	.byte	$B3
	.byte	$B4
	.byte	$87
	.byte	$B6
	.byte	$C2
	.byte	$85
	.byte	$C0
	.byte	$C1
	.byte	$85
	.byte	$C1
	.byte	$4D
	.byte	$4E
	.byte	$4E
	.byte	$4F
	.byte	$C0
	.byte	$85
	.byte	$C0
	.byte	$C1
	.byte	$85
	.byte	$C1
	.byte	$D0
	.byte	$85
	.byte	$D0
	.byte	$D1
	.byte	$87
	.byte	$D1
	.byte	$4D
	.byte	$4E
	.byte	$4E
	.byte	$4F
	.byte	$D0
	.byte	$87
	.byte	$D0
	.byte	$D1
	.byte	$85
	.byte	$D1
	.byte	$E0
	.byte	$86
	.byte	$E0
	.byte	$E1
	.byte	$86
	.byte	$E1
	.byte	$4D
	.byte	$4E
	.byte	$4E
	.byte	$4F
	.byte	$E0
	.byte	$86
	.byte	$E0
	.byte	$E1
	.byte	$86
	.byte	$E1
	.byte	$02
	.byte	$A8
	.byte	$04
	.byte	$02
	.byte	$A8
	.byte	$07
	.byte	$4D
	.byte	$4E
	.byte	$4E
	.byte	$4F
	.byte	$06
	.byte	$A8
	.byte	$03
	.byte	$01
	.byte	$A8
	.byte	$01
	.byte	$1D
	.byte	$2E
	.byte	$1D
	.byte	$1D
	.byte	$2D
	.byte	$33
	.byte	$4D
	.byte	$4E
	.byte	$4E
	.byte	$4F
	.byte	$22
	.byte	$2D
	.byte	$2E
	.byte	$2E
	.byte	$1D
	.byte	$1D
	.byte	$1D
	.byte	$2D
	.byte	$1D
	.byte	$1D
	.byte	$1D
	.byte	$33
	.byte	$4D
	.byte	$4E
	.byte	$4E
	.byte	$4F
	.byte	$22
	.byte	$1D
	.byte	$1D
	.byte	$2E
	.byte	$1D
	.byte	$1D
_stage4_levels_list:
_stage4_max_rooms:
	.byte	$01
_stage4_offsets:
	.byte	$00
_stage4_bg_palette:
	.byte	$22
	.byte	$0F
	.byte	$28
	.byte	$38
	.byte	$22
	.byte	$0F
	.byte	$00
	.byte	$3D
	.byte	$22
	.byte	$17
	.byte	$28
	.byte	$38
	.byte	$22
	.byte	$0F
	.byte	$28
	.byte	$30
_stage4_palette_sp:
	.byte	$21
	.byte	$17
	.byte	$24
	.byte	$36
	.byte	$21
	.byte	$05
	.byte	$16
	.byte	$37
	.byte	$21
	.byte	$0F
	.byte	$11
	.byte	$30
	.byte	$21
	.byte	$17
	.byte	$3D
	.byte	$37
_stage4_metatiles:
	.byte	$B0
	.byte	$B1
	.byte	$C0
	.byte	$C1
	.byte	$02
	.byte	$B1
	.byte	$B1
	.byte	$C2
	.byte	$C5
	.byte	$02
	.byte	$B0
	.byte	$B1
	.byte	$C4
	.byte	$C5
	.byte	$02
	.byte	$B2
	.byte	$B3
	.byte	$C2
	.byte	$C3
	.byte	$02
	.byte	$B4
	.byte	$B4
	.byte	$C4
	.byte	$C5
	.byte	$02
	.byte	$B4
	.byte	$B5
	.byte	$C4
	.byte	$C5
	.byte	$02
	.byte	$B6
	.byte	$B2
	.byte	$C6
	.byte	$C3
	.byte	$02
	.byte	$B0
	.byte	$B7
	.byte	$C3
	.byte	$C7
	.byte	$02
	.byte	$B3
	.byte	$B7
	.byte	$C3
	.byte	$C7
	.byte	$02
	.byte	$B8
	.byte	$B2
	.byte	$C6
	.byte	$C3
	.byte	$02
	.byte	$B0
	.byte	$B9
	.byte	$C3
	.byte	$C7
	.byte	$02
	.byte	$B3
	.byte	$B9
	.byte	$C3
	.byte	$C7
	.byte	$02
	.byte	$70
	.byte	$71
	.byte	$80
	.byte	$81
	.byte	$02
	.byte	$71
	.byte	$72
	.byte	$81
	.byte	$82
	.byte	$02
	.byte	$72
	.byte	$73
	.byte	$82
	.byte	$83
	.byte	$02
	.byte	$70
	.byte	$71
	.byte	$81
	.byte	$81
	.byte	$02
	.byte	$9C
	.byte	$99
	.byte	$CC
	.byte	$CB
	.byte	$01
	.byte	$9B
	.byte	$9A
	.byte	$A9
	.byte	$CB
	.byte	$01
	.byte	$9B
	.byte	$9B
	.byte	$A8
	.byte	$DB
	.byte	$01
	.byte	$99
	.byte	$9B
	.byte	$A8
	.byte	$CB
	.byte	$01
	.byte	$9B
	.byte	$98
	.byte	$A8
	.byte	$A9
	.byte	$01
	.byte	$9B
	.byte	$97
	.byte	$DB
	.byte	$BA
	.byte	$01
	.byte	$9B
	.byte	$9B
	.byte	$CD
	.byte	$DD
	.byte	$01
	.byte	$8B
	.byte	$89
	.byte	$A8
	.byte	$CB
	.byte	$01
	.byte	$89
	.byte	$89
	.byte	$DB
	.byte	$A9
	.byte	$01
	.byte	$8A
	.byte	$89
	.byte	$DB
	.byte	$BB
	.byte	$01
	.byte	$89
	.byte	$89
	.byte	$CD
	.byte	$DD
	.byte	$01
	.byte	$89
	.byte	$88
	.byte	$BB
	.byte	$CA
	.byte	$01
	.byte	$AD
	.byte	$A8
	.byte	$BD
	.byte	$DB
	.byte	$01
	.byte	$A9
	.byte	$A8
	.byte	$A8
	.byte	$A8
	.byte	$01
	.byte	$A9
	.byte	$CB
	.byte	$A8
	.byte	$CB
	.byte	$01
	.byte	$72
	.byte	$73
	.byte	$82
	.byte	$82
	.byte	$02
	.byte	$DC
	.byte	$A9
	.byte	$DC
	.byte	$AE
	.byte	$01
	.byte	$CB
	.byte	$A9
	.byte	$11
	.byte	$13
	.byte	$01
	.byte	$DC
	.byte	$A9
	.byte	$DC
	.byte	$AE
	.byte	$01
	.byte	$A9
	.byte	$BA
	.byte	$A8
	.byte	$CA
	.byte	$01
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$CC
	.byte	$A9
	.byte	$BC
	.byte	$AB
	.byte	$01
	.byte	$A8
	.byte	$A8
	.byte	$AB
	.byte	$AB
	.byte	$01
	.byte	$A9
	.byte	$A9
	.byte	$AB
	.byte	$BC
	.byte	$01
	.byte	$BB
	.byte	$CB
	.byte	$AA
	.byte	$AA
	.byte	$01
	.byte	$11
	.byte	$11
	.byte	$AA
	.byte	$AB
	.byte	$01
	.byte	$11
	.byte	$11
	.byte	$BC
	.byte	$AA
	.byte	$01
	.byte	$BD
	.byte	$CB
	.byte	$AA
	.byte	$AB
	.byte	$01
	.byte	$BD
	.byte	$BB
	.byte	$AD
	.byte	$A9
	.byte	$01
	.byte	$BB
	.byte	$A8
	.byte	$A9
	.byte	$CB
	.byte	$01
	.byte	$A9
	.byte	$A9
	.byte	$A9
	.byte	$A8
	.byte	$01
	.byte	$11
	.byte	$11
	.byte	$11
	.byte	$11
	.byte	$01
	.byte	$6A
	.byte	$CE
	.byte	$9D
	.byte	$9E
	.byte	$01
	.byte	$CE
	.byte	$DE
	.byte	$9E
	.byte	$9E
	.byte	$01
	.byte	$CC
	.byte	$CB
	.byte	$DC
	.byte	$A9
	.byte	$01
	.byte	$A8
	.byte	$CA
	.byte	$A8
	.byte	$BA
	.byte	$01
	.byte	$A3
	.byte	$A6
	.byte	$A9
	.byte	$A8
	.byte	$01
	.byte	$A9
	.byte	$CA
	.byte	$AB
	.byte	$AA
	.byte	$01
	.byte	$02
	.byte	$AE
	.byte	$DB
	.byte	$AE
	.byte	$01
	.byte	$B8
	.byte	$B0
	.byte	$C6
	.byte	$C3
	.byte	$02
	.byte	$69
	.byte	$89
	.byte	$CC
	.byte	$AE
	.byte	$01
	.byte	$D6
	.byte	$D7
	.byte	$A9
	.byte	$A8
	.byte	$01
	.byte	$B0
	.byte	$B5
	.byte	$C4
	.byte	$C5
	.byte	$02
	.byte	$89
	.byte	$89
	.byte	$A9
	.byte	$CA
	.byte	$01
	.byte	$AD
	.byte	$BA
	.byte	$BD
	.byte	$CA
	.byte	$01
	.byte	$0A
	.byte	$0B
	.byte	$0A
	.byte	$0B
	.byte	$01
	.byte	$0A
	.byte	$0B
	.byte	$0A
	.byte	$0B
	.byte	$01
	.byte	$33
	.byte	$33
	.byte	$11
	.byte	$11
	.byte	$01
	.byte	$C8
	.byte	$79
	.byte	$C9
	.byte	$9E
	.byte	$01
	.byte	$78
	.byte	$79
	.byte	$9E
	.byte	$9E
	.byte	$01
	.byte	$DE
	.byte	$BA
	.byte	$9E
	.byte	$9E
	.byte	$01
	.byte	$3B
	.byte	$9B
	.byte	$CC
	.byte	$CB
	.byte	$01
	.byte	$9B
	.byte	$3C
	.byte	$DB
	.byte	$BA
	.byte	$01
	.byte	$93
	.byte	$94
	.byte	$CB
	.byte	$CB
	.byte	$01
	.byte	$D4
	.byte	$B2
	.byte	$C6
	.byte	$C3
	.byte	$02
	.byte	$B3
	.byte	$D5
	.byte	$C3
	.byte	$C7
	.byte	$02
	.byte	$68
	.byte	$89
	.byte	$CC
	.byte	$AE
	.byte	$01
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$BD
	.byte	$CA
	.byte	$AD
	.byte	$BA
	.byte	$01
	.byte	$04
	.byte	$05
	.byte	$14
	.byte	$15
	.byte	$02
	.byte	$05
	.byte	$06
	.byte	$15
	.byte	$16
	.byte	$02
	.byte	$06
	.byte	$07
	.byte	$16
	.byte	$17
	.byte	$02
	.byte	$2A
	.byte	$1B
	.byte	$C9
	.byte	$9E
	.byte	$01
	.byte	$1B
	.byte	$1B
	.byte	$9E
	.byte	$9E
	.byte	$01
	.byte	$1B
	.byte	$2B
	.byte	$9E
	.byte	$5E
	.byte	$01
	.byte	$00
	.byte	$00
	.byte	$9D
	.byte	$AC
	.byte	$01
	.byte	$00
	.byte	$00
	.byte	$AC
	.byte	$AC
	.byte	$01
	.byte	$00
	.byte	$00
	.byte	$AC
	.byte	$5B
	.byte	$01
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$11
	.byte	$11
	.byte	$01
	.byte	$01
	.byte	$00
	.byte	$11
	.byte	$11
	.byte	$11
	.byte	$11
	.byte	$00
	.byte	$60
	.byte	$61
	.byte	$81
	.byte	$82
	.byte	$02
	.byte	$71
	.byte	$72
	.byte	$62
	.byte	$63
	.byte	$02
	.byte	$70
	.byte	$72
	.byte	$62
	.byte	$63
	.byte	$02
	.byte	$71
	.byte	$73
	.byte	$62
	.byte	$63
	.byte	$02
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$1F
	.byte	$1F
	.byte	$2F
	.byte	$2F
	.byte	$01
	.byte	$00
	.byte	$32
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$32
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$12
	.byte	$12
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$22
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$22
	.byte	$12
	.byte	$00
	.byte	$22
	.byte	$01
	.byte	$22
	.byte	$12
	.byte	$00
	.byte	$12
	.byte	$01
	.byte	$00
	.byte	$12
	.byte	$00
	.byte	$12
	.byte	$01
	.byte	$00
	.byte	$12
	.byte	$00
	.byte	$22
	.byte	$01
	.byte	$12
	.byte	$12
	.byte	$12
	.byte	$12
	.byte	$01
	.byte	$12
	.byte	$23
	.byte	$23
	.byte	$00
	.byte	$01
	.byte	$23
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$32
	.byte	$32
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$ED
	.byte	$FD
	.byte	$EF
	.byte	$EF
	.byte	$01
	.byte	$ED
	.byte	$ED
	.byte	$EE
	.byte	$EF
	.byte	$01
	.byte	$FE
	.byte	$EF
	.byte	$FE
	.byte	$FE
	.byte	$01
	.byte	$FD
	.byte	$FE
	.byte	$FF
	.byte	$FE
	.byte	$01
	.byte	$00
	.byte	$00
	.byte	$5B
	.byte	$00
	.byte	$01
	.byte	$FE
	.byte	$01
	.byte	$FD
	.byte	$01
	.byte	$01
	.byte	$FF
	.byte	$01
	.byte	$FE
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$FC
	.byte	$01
	.byte	$01
	.byte	$FF
	.byte	$FC
	.byte	$FE
	.byte	$01
	.byte	$01
	.byte	$FF
	.byte	$FC
	.byte	$FE
	.byte	$FF
	.byte	$01
	.byte	$EA
	.byte	$EB
	.byte	$FE
	.byte	$FE
	.byte	$01
	.byte	$00
	.byte	$00
	.byte	$EA
	.byte	$EB
	.byte	$01
	.byte	$00
	.byte	$00
	.byte	$EC
	.byte	$EA
	.byte	$01
	.byte	$00
	.byte	$FB
	.byte	$EA
	.byte	$FF
	.byte	$01
	.byte	$00
	.byte	$EA
	.byte	$FB
	.byte	$FF
	.byte	$01
	.byte	$FA
	.byte	$00
	.byte	$FE
	.byte	$FA
	.byte	$01
	.byte	$FF
	.byte	$EE
	.byte	$EF
	.byte	$FE
	.byte	$01
	.byte	$EE
	.byte	$EF
	.byte	$FE
	.byte	$FF
	.byte	$01
	.byte	$FF
	.byte	$EE
	.byte	$FD
	.byte	$FF
	.byte	$01
	.byte	$EE
	.byte	$FF
	.byte	$FF
	.byte	$FE
	.byte	$01
	.byte	$6C
	.byte	$6D
	.byte	$7C
	.byte	$7D
	.byte	$01
	.byte	$7C
	.byte	$7D
	.byte	$7C
	.byte	$7D
	.byte	$01
	.byte	$7C
	.byte	$7D
	.byte	$8C
	.byte	$8D
	.byte	$01
	.byte	$7A
	.byte	$7B
	.byte	$7C
	.byte	$7D
	.byte	$01
	.byte	$EF
	.byte	$01
	.byte	$FC
	.byte	$01
	.byte	$01
	.byte	$EF
	.byte	$01
	.byte	$FE
	.byte	$01
	.byte	$01
	.byte	$7E
	.byte	$6E
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$6E
	.byte	$9F
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$8E
	.byte	$DF
	.byte	$00
	.byte	$8E
	.byte	$01
	.byte	$EF
	.byte	$8F
	.byte	$8F
	.byte	$00
	.byte	$01
	.byte	$FF
	.byte	$8F
	.byte	$9F
	.byte	$00
	.byte	$01
	.byte	$FF
	.byte	$FF
	.byte	$9F
	.byte	$6E
	.byte	$01
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$EF
	.byte	$FE
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$DF
	.byte	$CF
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$FE
	.byte	$CF
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$DF
	.byte	$FE
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$DF
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$5C
	.byte	$5D
	.byte	$7C
	.byte	$7D
	.byte	$01
	.byte	$5F
	.byte	$EF
	.byte	$6F
	.byte	$FF
	.byte	$01
	.byte	$6F
	.byte	$EE
	.byte	$5F
	.byte	$FE
	.byte	$01
	.byte	$EE
	.byte	$2D
	.byte	$8F
	.byte	$00
	.byte	$01
	.byte	$FF
	.byte	$3D
	.byte	$FF
	.byte	$3D
	.byte	$01
	.byte	$2E
	.byte	$EE
	.byte	$3E
	.byte	$EE
	.byte	$01
	.byte	$8E
	.byte	$BF
	.byte	$00
	.byte	$3E
	.byte	$01
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$03
	.byte	$03
	.byte	$03
	.byte	$03
	.byte	$02
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$5C
	.byte	$5D
	.byte	$8C
	.byte	$8D
	.byte	$01
	.byte	$D0
	.byte	$D1
	.byte	$C3
	.byte	$C4
	.byte	$02
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$02
	.byte	$32
	.byte	$32
	.byte	$1C
	.byte	$1E
	.byte	$03
	.byte	$32
	.byte	$32
	.byte	$1E
	.byte	$1E
	.byte	$03
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$02
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$00
	.byte	$00
	.byte	$E6
	.byte	$E7
	.byte	$02
	.byte	$00
	.byte	$00
	.byte	$E7
	.byte	$E0
	.byte	$02
	.byte	$00
	.byte	$D2
	.byte	$E1
	.byte	$E2
	.byte	$02
	.byte	$D3
	.byte	$00
	.byte	$E3
	.byte	$E4
	.byte	$02
	.byte	$00
	.byte	$00
	.byte	$E5
	.byte	$E6
	.byte	$02
	.byte	$00
	.byte	$00
	.byte	$E7
	.byte	$E6
	.byte	$02
	.byte	$00
	.byte	$00
	.byte	$D8
	.byte	$D9
	.byte	$02
	.byte	$00
	.byte	$00
	.byte	$E0
	.byte	$E5
	.byte	$02
	.byte	$00
	.byte	$00
	.byte	$DA
	.byte	$E6
	.byte	$02
	.byte	$00
	.byte	$00
	.byte	$F6
	.byte	$F7
	.byte	$02
	.byte	$00
	.byte	$00
	.byte	$F6
	.byte	$E7
	.byte	$02
	.byte	$00
	.byte	$00
	.byte	$E6
	.byte	$F7
	.byte	$02
	.byte	$00
	.byte	$00
	.byte	$1C
	.byte	$1E
	.byte	$03
	.byte	$00
	.byte	$00
	.byte	$1E
	.byte	$1E
	.byte	$03
	.byte	$00
	.byte	$00
	.byte	$1E
	.byte	$1D
	.byte	$03
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$92
	.byte	$03
	.byte	$03
	.byte	$A7
	.byte	$02
	.byte	$90
	.byte	$A1
	.byte	$03
	.byte	$77
	.byte	$02
	.byte	$F1
	.byte	$F2
	.byte	$A0
	.byte	$A7
	.byte	$02
	.byte	$F3
	.byte	$F4
	.byte	$A1
	.byte	$77
	.byte	$02
	.byte	$92
	.byte	$03
	.byte	$03
	.byte	$A7
	.byte	$02
	.byte	$F8
	.byte	$F9
	.byte	$03
	.byte	$77
	.byte	$02
	.byte	$E8
	.byte	$E9
	.byte	$A0
	.byte	$A7
	.byte	$02
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$02
	.byte	$39
	.byte	$3A
	.byte	$38
	.byte	$11
	.byte	$02
	.byte	$21
	.byte	$21
	.byte	$13
	.byte	$13
	.byte	$02
	.byte	$31
	.byte	$31
	.byte	$11
	.byte	$11
	.byte	$02
	.byte	$31
	.byte	$20
	.byte	$18
	.byte	$19
	.byte	$02
	.byte	$0F
	.byte	$0F
	.byte	$00
	.byte	$00
	.byte	$03
	.byte	$0D
	.byte	$0C
	.byte	$00
	.byte	$00
	.byte	$03
	.byte	$0C
	.byte	$0E
	.byte	$00
	.byte	$0C
	.byte	$03
	.byte	$0E
	.byte	$0D
	.byte	$0D
	.byte	$00
	.byte	$03
	.byte	$03
	.byte	$75
	.byte	$84
	.byte	$85
	.byte	$02
	.byte	$76
	.byte	$03
	.byte	$86
	.byte	$87
	.byte	$02
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$03
	.byte	$03
	.byte	$03
	.byte	$03
	.byte	$02
	.byte	$03
	.byte	$26
	.byte	$35
	.byte	$36
	.byte	$02
	.byte	$27
	.byte	$25
	.byte	$37
	.byte	$03
	.byte	$02
	.byte	$13
	.byte	$13
	.byte	$13
	.byte	$13
	.byte	$02
	.byte	$11
	.byte	$11
	.byte	$11
	.byte	$11
	.byte	$02
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$02
	.byte	$00
	.byte	$0C
	.byte	$00
	.byte	$00
	.byte	$03
	.byte	$0E
	.byte	$0E
	.byte	$0C
	.byte	$0D
	.byte	$03
	.byte	$0E
	.byte	$0E
	.byte	$0E
	.byte	$0D
	.byte	$03
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$02
	.byte	$03
	.byte	$95
	.byte	$A4
	.byte	$A5
	.byte	$02
	.byte	$96
	.byte	$03
	.byte	$03
	.byte	$03
	.byte	$02
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$02
	.byte	$03
	.byte	$65
	.byte	$24
	.byte	$34
	.byte	$02
	.byte	$66
	.byte	$67
	.byte	$64
	.byte	$03
	.byte	$02
	.byte	$03
	.byte	$03
	.byte	$13
	.byte	$13
	.byte	$02
	.byte	$11
	.byte	$09
	.byte	$18
	.byte	$19
	.byte	$02
	.byte	$28
	.byte	$29
	.byte	$38
	.byte	$11
	.byte	$02
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$02
	.byte	$0E
	.byte	$0E
	.byte	$0C
	.byte	$0F
	.byte	$03
	.byte	$0C
	.byte	$0D
	.byte	$00
	.byte	$00
	.byte	$03
	.byte	$0D
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$03
	.byte	$0E
	.byte	$0E
	.byte	$0E
	.byte	$0E
	.byte	$03
_stage4_starting_x_place:
	.byte	$1E
_stage4_starting_y_place:
	.byte	$A0
_stage4_level_0_enemies:
	.byte	$FF
_stage4_Enemy_list:
	.addr	_stage4_level_0_enemies
_stage4_level_0_entities:
	.byte	$FF
_stage4_entity_list:
	.addr	_stage4_level_0_entities
.segment	"BANK4"
_leviathanworld_0:
	.byte	$4A
	.byte	$45
	.byte	$42
	.byte	$44
	.byte	$27
	.byte	$47
	.byte	$6A
	.byte	$C0
	.byte	$C1
	.byte	$6B
	.byte	$41
	.byte	$41
	.byte	$44
	.byte	$80
	.byte	$71
	.byte	$72
	.byte	$3F
	.byte	$4A
	.byte	$26
	.byte	$2A
	.byte	$66
	.byte	$67
	.byte	$47
	.byte	$49
	.byte	$91
	.byte	$C1
	.byte	$77
	.byte	$52
	.byte	$45
	.byte	$41
	.byte	$42
	.byte	$44
	.byte	$5F
	.byte	$3F
	.byte	$4B
	.byte	$4C
	.byte	$4C
	.byte	$4C
	.byte	$76
	.byte	$39
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$C1
	.byte	$6B
	.byte	$46
	.byte	$66
	.byte	$45
	.byte	$5E
	.byte	$5E
	.byte	$3F
	.byte	$3F
	.byte	$3F
	.byte	$3F
	.byte	$BB
	.byte	$BC
	.byte	$AB
	.byte	$EA
	.byte	$DA
	.byte	$91
	.byte	$C1
	.byte	$78
	.byte	$7A
	.byte	$39
	.byte	$5F
	.byte	$5E
	.byte	$5E
	.byte	$6C
	.byte	$6E
	.byte	$6C
	.byte	$90
	.byte	$90
	.byte	$BB
	.byte	$BC
	.byte	$BA
	.byte	$B9
	.byte	$BC
	.byte	$AB
	.byte	$AF
	.byte	$91
	.byte	$5E
	.byte	$6C
	.byte	$6E
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$BB
	.byte	$BC
	.byte	$BC
	.byte	$6E
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$E7
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$E7
	.byte	$6F
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$5F
	.byte	$6D
	.byte	$6F
	.byte	$90
	.byte	$20
	.byte	$21
	.byte	$22
	.byte	$21
	.byte	$22
	.byte	$23
	.byte	$25
	.byte	$2E
	.byte	$DC
	.byte	$DD
	.byte	$CD
	.byte	$EE
	.byte	$5E
	.byte	$5F
	.byte	$5F
	.byte	$6D
	.byte	$6F
	.byte	$6D
	.byte	$6D
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$5B
	.byte	$2B
	.byte	$2F
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$5F
	.byte	$5E
	.byte	$4F
	.byte	$4F
	.byte	$4F
	.byte	$4F
	.byte	$4F
	.byte	$ED
	.byte	$CE
	.byte	$CD
	.byte	$EB
	.byte	$5C
	.byte	$2B
	.byte	$0D
	.byte	$0F
	.byte	$31
	.byte	$5F
	.byte	$4F
	.byte	$3A
	.byte	$3C
	.byte	$3C
	.byte	$3C
	.byte	$3C
	.byte	$6A
	.byte	$C0
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$5C
	.byte	$44
	.byte	$43
	.byte	$42
	.byte	$4F
	.byte	$3A
	.byte	$28
	.byte	$45
	.byte	$44
	.byte	$43
	.byte	$42
	.byte	$37
	.byte	$49
	.byte	$7D
	.byte	$7C
	.byte	$7D
	.byte	$7F
	.byte	$88
	.byte	$44
	.byte	$43
	.byte	$3B
	.byte	$28
	.byte	$70
	.byte	$71
	.byte	$72
	.byte	$43
	.byte	$41
	.byte	$41
	.byte	$82
	.byte	$8D
	.byte	$8E
	.byte	$8D
	.byte	$8F
	.byte	$89
	.byte	$40
	.byte	$44
_leviathanworld_1:
	.byte	$43
	.byte	$42
	.byte	$44
	.byte	$29
	.byte	$36
	.byte	$2A
	.byte	$45
	.byte	$42
	.byte	$46
	.byte	$29
	.byte	$2A
	.byte	$70
	.byte	$71
	.byte	$72
	.byte	$2A
	.byte	$45
	.byte	$43
	.byte	$79
	.byte	$7A
	.byte	$7A
	.byte	$41
	.byte	$42
	.byte	$79
	.byte	$52
	.byte	$47
	.byte	$37
	.byte	$28
	.byte	$45
	.byte	$42
	.byte	$46
	.byte	$66
	.byte	$67
	.byte	$39
	.byte	$D0
	.byte	$AF
	.byte	$C1
	.byte	$78
	.byte	$39
	.byte	$AF
	.byte	$C1
	.byte	$78
	.byte	$79
	.byte	$41
	.byte	$46
	.byte	$45
	.byte	$42
	.byte	$44
	.byte	$45
	.byte	$91
	.byte	$DA
	.byte	$DA
	.byte	$AD
	.byte	$AB
	.byte	$91
	.byte	$AD
	.byte	$BC
	.byte	$BC
	.byte	$BC
	.byte	$5A
	.byte	$54
	.byte	$7B
	.byte	$43
	.byte	$42
	.byte	$42
	.byte	$AD
	.byte	$BA
	.byte	$B9
	.byte	$BD
	.byte	$BB
	.byte	$BC
	.byte	$BD
	.byte	$C4
	.byte	$C5
	.byte	$C5
	.byte	$C6
	.byte	$9E
	.byte	$59
	.byte	$5A
	.byte	$45
	.byte	$44
	.byte	$BD
	.byte	$C4
	.byte	$C5
	.byte	$C6
	.byte	$C6
	.byte	$C4
	.byte	$C6
	.byte	$E4
	.byte	$D4
	.byte	$D4
	.byte	$D6
	.byte	$AE
	.byte	$B5
	.byte	$D3
	.byte	$87
	.byte	$42
	.byte	$C4
	.byte	$D4
	.byte	$D5
	.byte	$D4
	.byte	$D4
	.byte	$D4
	.byte	$D6
	.byte	$E7
	.byte	$E5
	.byte	$D5
	.byte	$D5
	.byte	$BE
	.byte	$D2
	.byte	$D2
	.byte	$86
	.byte	$42
	.byte	$C2
	.byte	$D4
	.byte	$D4
	.byte	$D5
	.byte	$D5
	.byte	$D4
	.byte	$E6
	.byte	$E7
	.byte	$D2
	.byte	$E4
	.byte	$E6
	.byte	$C4
	.byte	$C7
	.byte	$90
	.byte	$87
	.byte	$44
	.byte	$E7
	.byte	$C2
	.byte	$E6
	.byte	$E4
	.byte	$D4
	.byte	$E6
	.byte	$E7
	.byte	$C4
	.byte	$D6
	.byte	$C6
	.byte	$E7
	.byte	$D5
	.byte	$D6
	.byte	$C6
	.byte	$88
	.byte	$42
	.byte	$EF
	.byte	$DD
	.byte	$DE
	.byte	$E7
	.byte	$E5
	.byte	$E7
	.byte	$C2
	.byte	$E5
	.byte	$E6
	.byte	$E7
	.byte	$C4
	.byte	$D4
	.byte	$D5
	.byte	$C6
	.byte	$87
	.byte	$46
	.byte	$91
	.byte	$9A
	.byte	$DF
	.byte	$DD
	.byte	$DE
	.byte	$E4
	.byte	$E5
	.byte	$E7
	.byte	$E7
	.byte	$D3
	.byte	$D5
	.byte	$D4
	.byte	$D4
	.byte	$D6
	.byte	$86
	.byte	$42
	.byte	$1F
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$DF
	.byte	$EC
	.byte	$EE
	.byte	$30
	.byte	$31
	.byte	$35
	.byte	$E4
	.byte	$D5
	.byte	$E5
	.byte	$E6
	.byte	$87
	.byte	$44
	.byte	$81
	.byte	$31
	.byte	$32
	.byte	$31
	.byte	$32
	.byte	$32
	.byte	$32
	.byte	$8A
	.byte	$44
	.byte	$84
	.byte	$A5
	.byte	$A6
	.byte	$A7
	.byte	$A8
	.byte	$87
	.byte	$42
	.byte	$42
	.byte	$42
	.byte	$44
	.byte	$70
	.byte	$71
	.byte	$67
	.byte	$28
	.byte	$43
	.byte	$41
	.byte	$82
	.byte	$95
	.byte	$96
	.byte	$97
	.byte	$98
	.byte	$88
	.byte	$44
	.byte	$45
	.byte	$42
	.byte	$44
	.byte	$45
	.byte	$44
	.byte	$27
	.byte	$68
	.byte	$69
	.byte	$43
	.byte	$83
	.byte	$A5
	.byte	$A6
	.byte	$A7
	.byte	$A8
	.byte	$87
	.byte	$42
_leviathanworld_2:
	.byte	$41
	.byte	$41
	.byte	$42
	.byte	$42
	.byte	$44
	.byte	$45
	.byte	$44
	.byte	$45
	.byte	$41
	.byte	$83
	.byte	$95
	.byte	$A6
	.byte	$97
	.byte	$98
	.byte	$88
	.byte	$46
	.byte	$41
	.byte	$44
	.byte	$29
	.byte	$26
	.byte	$62
	.byte	$63
	.byte	$64
	.byte	$41
	.byte	$41
	.byte	$82
	.byte	$A5
	.byte	$96
	.byte	$A6
	.byte	$98
	.byte	$89
	.byte	$2A
	.byte	$41
	.byte	$40
	.byte	$42
	.byte	$40
	.byte	$42
	.byte	$42
	.byte	$41
	.byte	$46
	.byte	$43
	.byte	$83
	.byte	$95
	.byte	$A6
	.byte	$A7
	.byte	$98
	.byte	$88
	.byte	$41
	.byte	$41
	.byte	$44
	.byte	$57
	.byte	$55
	.byte	$55
	.byte	$56
	.byte	$55
	.byte	$58
	.byte	$55
	.byte	$61
	.byte	$A5
	.byte	$A6
	.byte	$A7
	.byte	$A8
	.byte	$89
	.byte	$2A
	.byte	$42
	.byte	$82
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$92
	.byte	$93
	.byte	$94
	.byte	$A2
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$88
	.byte	$41
	.byte	$43
	.byte	$83
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$A2
	.byte	$A3
	.byte	$A2
	.byte	$A2
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$88
	.byte	$46
	.byte	$42
	.byte	$83
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$B3
	.byte	$B4
	.byte	$B3
	.byte	$B3
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$89
	.byte	$2A
	.byte	$80
	.byte	$84
	.byte	$A5
	.byte	$A6
	.byte	$96
	.byte	$98
	.byte	$30
	.byte	$31
	.byte	$32
	.byte	$32
	.byte	$0E
	.byte	$0D
	.byte	$31
	.byte	$32
	.byte	$8A
	.byte	$46
	.byte	$42
	.byte	$83
	.byte	$A5
	.byte	$A6
	.byte	$A6
	.byte	$A8
	.byte	$88
	.byte	$44
	.byte	$70
	.byte	$71
	.byte	$67
	.byte	$68
	.byte	$69
	.byte	$53
	.byte	$43
	.byte	$46
	.byte	$45
	.byte	$82
	.byte	$95
	.byte	$A6
	.byte	$A6
	.byte	$98
	.byte	$65
	.byte	$55
	.byte	$56
	.byte	$56
	.byte	$56
	.byte	$58
	.byte	$56
	.byte	$58
	.byte	$41
	.byte	$41
	.byte	$43
	.byte	$83
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$92
	.byte	$93
	.byte	$94
	.byte	$93
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$87
	.byte	$41
	.byte	$42
	.byte	$82
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$A4
	.byte	$A2
	.byte	$A2
	.byte	$A2
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$89
	.byte	$26
	.byte	$43
	.byte	$83
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$B2
	.byte	$B3
	.byte	$B2
	.byte	$B4
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$88
	.byte	$41
	.byte	$41
	.byte	$81
	.byte	$32
	.byte	$31
	.byte	$32
	.byte	$31
	.byte	$32
	.byte	$0E
	.byte	$0D
	.byte	$35
	.byte	$A5
	.byte	$A6
	.byte	$A7
	.byte	$98
	.byte	$88
	.byte	$41
	.byte	$68
	.byte	$69
	.byte	$45
	.byte	$44
	.byte	$51
	.byte	$43
	.byte	$41
	.byte	$46
	.byte	$43
	.byte	$83
	.byte	$95
	.byte	$96
	.byte	$97
	.byte	$98
	.byte	$87
	.byte	$41
_leviathanworld_3:
	.byte	$41
	.byte	$46
	.byte	$54
	.byte	$57
	.byte	$56
	.byte	$55
	.byte	$58
	.byte	$55
	.byte	$55
	.byte	$61
	.byte	$95
	.byte	$96
	.byte	$97
	.byte	$98
	.byte	$87
	.byte	$41
	.byte	$41
	.byte	$83
	.byte	$C4
	.byte	$C5
	.byte	$C6
	.byte	$9F
	.byte	$C4
	.byte	$C5
	.byte	$E3
	.byte	$E7
	.byte	$90
	.byte	$90
	.byte	$E3
	.byte	$B5
	.byte	$88
	.byte	$46
	.byte	$45
	.byte	$83
	.byte	$D4
	.byte	$D4
	.byte	$D5
	.byte	$C5
	.byte	$D4
	.byte	$D5
	.byte	$D4
	.byte	$E6
	.byte	$90
	.byte	$C4
	.byte	$D4
	.byte	$E6
	.byte	$87
	.byte	$41
	.byte	$43
	.byte	$83
	.byte	$E4
	.byte	$E5
	.byte	$E5
	.byte	$E5
	.byte	$E6
	.byte	$E5
	.byte	$E5
	.byte	$E7
	.byte	$C4
	.byte	$E5
	.byte	$E5
	.byte	$D3
	.byte	$87
	.byte	$41
	.byte	$41
	.byte	$85
	.byte	$10
	.byte	$12
	.byte	$3E
	.byte	$13
	.byte	$15
	.byte	$16
	.byte	$15
	.byte	$14
	.byte	$15
	.byte	$14
	.byte	$14
	.byte	$11
	.byte	$86
	.byte	$40
	.byte	$41
	.byte	$83
	.byte	$C4
	.byte	$D6
	.byte	$3E
	.byte	$90
	.byte	$E3
	.byte	$C4
	.byte	$C6
	.byte	$C6
	.byte	$E3
	.byte	$C5
	.byte	$C5
	.byte	$C6
	.byte	$87
	.byte	$41
	.byte	$45
	.byte	$83
	.byte	$D5
	.byte	$E6
	.byte	$3E
	.byte	$C4
	.byte	$D4
	.byte	$D4
	.byte	$D4
	.byte	$D5
	.byte	$C5
	.byte	$E5
	.byte	$D4
	.byte	$D5
	.byte	$87
	.byte	$46
	.byte	$41
	.byte	$82
	.byte	$E6
	.byte	$E7
	.byte	$3E
	.byte	$E4
	.byte	$E5
	.byte	$E5
	.byte	$E6
	.byte	$E5
	.byte	$E7
	.byte	$90
	.byte	$E4
	.byte	$E6
	.byte	$88
	.byte	$41
	.byte	$80
	.byte	$84
	.byte	$10
	.byte	$1B
	.byte	$19
	.byte	$1B
	.byte	$1A
	.byte	$1B
	.byte	$1B
	.byte	$1A
	.byte	$12
	.byte	$3E
	.byte	$13
	.byte	$11
	.byte	$86
	.byte	$46
	.byte	$41
	.byte	$83
	.byte	$C4
	.byte	$C7
	.byte	$C5
	.byte	$C6
	.byte	$E4
	.byte	$C5
	.byte	$C4
	.byte	$C5
	.byte	$E3
	.byte	$3E
	.byte	$C5
	.byte	$C6
	.byte	$87
	.byte	$41
	.byte	$41
	.byte	$82
	.byte	$D4
	.byte	$C5
	.byte	$D4
	.byte	$D6
	.byte	$C6
	.byte	$E4
	.byte	$D4
	.byte	$D4
	.byte	$D5
	.byte	$3E
	.byte	$D4
	.byte	$D5
	.byte	$87
	.byte	$46
	.byte	$40
	.byte	$83
	.byte	$E5
	.byte	$E6
	.byte	$D3
	.byte	$E4
	.byte	$E5
	.byte	$C6
	.byte	$E4
	.byte	$E4
	.byte	$E5
	.byte	$3E
	.byte	$E6
	.byte	$E7
	.byte	$88
	.byte	$46
	.byte	$45
	.byte	$85
	.byte	$10
	.byte	$12
	.byte	$3E
	.byte	$13
	.byte	$15
	.byte	$14
	.byte	$14
	.byte	$15
	.byte	$14
	.byte	$16
	.byte	$15
	.byte	$11
	.byte	$86
	.byte	$41
	.byte	$41
	.byte	$82
	.byte	$C4
	.byte	$C5
	.byte	$3E
	.byte	$C5
	.byte	$E3
	.byte	$C5
	.byte	$C4
	.byte	$C6
	.byte	$9F
	.byte	$E7
	.byte	$C4
	.byte	$C6
	.byte	$88
	.byte	$41
	.byte	$41
	.byte	$83
	.byte	$E5
	.byte	$E6
	.byte	$3E
	.byte	$E4
	.byte	$E5
	.byte	$E5
	.byte	$D4
	.byte	$E6
	.byte	$90
	.byte	$90
	.byte	$D3
	.byte	$E5
	.byte	$89
	.byte	$2A
_leviathanworld_4:
	.byte	$41
	.byte	$83
	.byte	$D4
	.byte	$D4
	.byte	$3E
	.byte	$D4
	.byte	$D5
	.byte	$D4
	.byte	$D4
	.byte	$C6
	.byte	$E4
	.byte	$D5
	.byte	$D5
	.byte	$59
	.byte	$5A
	.byte	$57
	.byte	$45
	.byte	$85
	.byte	$D4
	.byte	$C3
	.byte	$3E
	.byte	$D5
	.byte	$D4
	.byte	$D4
	.byte	$D5
	.byte	$E6
	.byte	$B5
	.byte	$E4
	.byte	$E6
	.byte	$E7
	.byte	$90
	.byte	$9E
	.byte	$41
	.byte	$83
	.byte	$E6
	.byte	$C4
	.byte	$3E
	.byte	$D4
	.byte	$D4
	.byte	$D5
	.byte	$E6
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$AE
	.byte	$41
	.byte	$82
	.byte	$C3
	.byte	$E4
	.byte	$3E
	.byte	$D4
	.byte	$D4
	.byte	$D4
	.byte	$E6
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$BE
	.byte	$43
	.byte	$83
	.byte	$90
	.byte	$90
	.byte	$D7
	.byte	$D3
	.byte	$E5
	.byte	$E6
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$41
	.byte	$83
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$E7
	.byte	$B5
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$45
	.byte	$83
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$D7
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$41
	.byte	$82
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$DC
	.byte	$43
	.byte	$83
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$DC
	.byte	$CD
	.byte	$EE
	.byte	$CE
	.byte	$DE
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$DC
	.byte	$DD
	.byte	$EC
	.byte	$41
	.byte	$83
	.byte	$DC
	.byte	$DD
	.byte	$DD
	.byte	$EC
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$EE
	.byte	$EB
	.byte	$EF
	.byte	$DD
	.byte	$EC
	.byte	$EA
	.byte	$91
	.byte	$45
	.byte	$83
	.byte	$EC
	.byte	$EA
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$B1
	.byte	$91
	.byte	$91
	.byte	$B0
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$80
	.byte	$84
	.byte	$00
	.byte	$0A
	.byte	$0A
	.byte	$0A
	.byte	$0A
	.byte	$0A
	.byte	$91
	.byte	$91
	.byte	$08
	.byte	$0A
	.byte	$0A
	.byte	$0A
	.byte	$0A
	.byte	$0A
	.byte	$45
	.byte	$83
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$AF
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$AF
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$41
	.byte	$83
	.byte	$7C
	.byte	$7D
	.byte	$7E
	.byte	$7C
	.byte	$7D
	.byte	$7E
	.byte	$7E
	.byte	$7C
	.byte	$7D
	.byte	$7E
	.byte	$7F
	.byte	$7E
	.byte	$7F
	.byte	$7C
	.byte	$41
	.byte	$83
	.byte	$8C
	.byte	$8D
	.byte	$8E
	.byte	$8C
	.byte	$8D
	.byte	$8E
	.byte	$8E
	.byte	$8C
	.byte	$8D
	.byte	$8E
	.byte	$8F
	.byte	$8E
	.byte	$8F
	.byte	$8C
_leviathanworld_5:
	.byte	$76
	.byte	$39
	.byte	$9A
	.byte	$DA
	.byte	$AD
	.byte	$BD
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$E4
	.byte	$D5
	.byte	$D5
	.byte	$D5
	.byte	$C6
	.byte	$C2
	.byte	$E4
	.byte	$BB
	.byte	$BA
	.byte	$AA
	.byte	$B9
	.byte	$BD
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$E4
	.byte	$D5
	.byte	$D6
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$D7
	.byte	$D7
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$30
	.byte	$0E
	.byte	$0D
	.byte	$35
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$E0
	.byte	$90
	.byte	$87
	.byte	$44
	.byte	$80
	.byte	$84
	.byte	$10
	.byte	$1B
	.byte	$1B
	.byte	$1A
	.byte	$1A
	.byte	$1B
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$18
	.byte	$1C
	.byte	$86
	.byte	$41
	.byte	$56
	.byte	$61
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$DC
	.byte	$DD
	.byte	$DE
	.byte	$90
	.byte	$90
	.byte	$87
	.byte	$83
	.byte	$90
	.byte	$9F
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$DD
	.byte	$EC
	.byte	$B0
	.byte	$DF
	.byte	$ED
	.byte	$CE
	.byte	$88
	.byte	$82
	.byte	$90
	.byte	$90
	.byte	$DC
	.byte	$DD
	.byte	$DE
	.byte	$90
	.byte	$E0
	.byte	$90
	.byte	$91
	.byte	$91
	.byte	$08
	.byte	$0B
	.byte	$0A
	.byte	$0C
	.byte	$86
	.byte	$83
	.byte	$DC
	.byte	$DD
	.byte	$EC
	.byte	$9A
	.byte	$DF
	.byte	$DE
	.byte	$15
	.byte	$14
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$87
	.byte	$81
	.byte	$32
	.byte	$1F
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$DF
	.byte	$DE
	.byte	$90
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$88
	.byte	$41
	.byte	$42
	.byte	$82
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$EE
	.byte	$EF
	.byte	$0A
	.byte	$0A
	.byte	$0A
	.byte	$0A
	.byte	$0A
	.byte	$0C
	.byte	$86
	.byte	$44
	.byte	$43
	.byte	$81
	.byte	$0E
	.byte	$0D
	.byte	$1F
	.byte	$91
	.byte	$EA
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$87
	.byte	$44
	.byte	$70
	.byte	$72
	.byte	$43
	.byte	$41
	.byte	$85
	.byte	$00
	.byte	$0B
	.byte	$0B
	.byte	$7D
	.byte	$7E
	.byte	$7F
	.byte	$7E
	.byte	$7C
	.byte	$7D
	.byte	$74
	.byte	$45
	.byte	$41
	.byte	$41
	.byte	$42
	.byte	$41
	.byte	$83
	.byte	$7E
	.byte	$7F
	.byte	$7C
	.byte	$8D
	.byte	$8E
	.byte	$8F
	.byte	$8E
	.byte	$8C
	.byte	$8D
	.byte	$87
	.byte	$46
	.byte	$45
	.byte	$46
	.byte	$45
	.byte	$41
	.byte	$83
	.byte	$8E
	.byte	$8F
	.byte	$8C
_leviathanworld_6:
	.byte	$D5
	.byte	$D5
	.byte	$E5
	.byte	$E7
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$E1
	.byte	$48
	.byte	$45
	.byte	$42
	.byte	$44
	.byte	$45
	.byte	$E4
	.byte	$C3
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$E0
	.byte	$90
	.byte	$48
	.byte	$43
	.byte	$44
	.byte	$45
	.byte	$40
	.byte	$46
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$18
	.byte	$1C
	.byte	$86
	.byte	$41
	.byte	$40
	.byte	$41
	.byte	$79
	.byte	$39
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$88
	.byte	$46
	.byte	$45
	.byte	$39
	.byte	$DA
	.byte	$DA
	.byte	$1A
	.byte	$1B
	.byte	$1A
	.byte	$1B
	.byte	$12
	.byte	$3E
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$87
	.byte	$40
	.byte	$83
	.byte	$91
	.byte	$B1
	.byte	$91
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$3E
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$86
	.byte	$41
	.byte	$85
	.byte	$00
	.byte	$0A
	.byte	$DA
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$3E
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$DC
	.byte	$87
	.byte	$42
	.byte	$39
	.byte	$DA
	.byte	$AF
	.byte	$91
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$3E
	.byte	$90
	.byte	$DC
	.byte	$DD
	.byte	$EC
	.byte	$87
	.byte	$82
	.byte	$91
	.byte	$DA
	.byte	$91
	.byte	$AD
	.byte	$15
	.byte	$14
	.byte	$15
	.byte	$15
	.byte	$15
	.byte	$16
	.byte	$06
	.byte	$05
	.byte	$05
	.byte	$01
	.byte	$86
	.byte	$83
	.byte	$DA
	.byte	$AD
	.byte	$BC
	.byte	$BD
	.byte	$DC
	.byte	$CD
	.byte	$EF
	.byte	$DE
	.byte	$9F
	.byte	$DC
	.byte	$EC
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$78
	.byte	$39
	.byte	$C9
	.byte	$D9
	.byte	$D3
	.byte	$E7
	.byte	$EC
	.byte	$91
	.byte	$EA
	.byte	$EE
	.byte	$EF
	.byte	$EC
	.byte	$DA
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$AF
	.byte	$91
	.byte	$91
	.byte	$EE
	.byte	$EF
	.byte	$DE
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$EA
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$B1
	.byte	$91
	.byte	$EE
	.byte	$0A
	.byte	$0B
	.byte	$0B
	.byte	$0B
	.byte	$0A
	.byte	$0B
	.byte	$0B
	.byte	$0A
	.byte	$0A
	.byte	$0B
	.byte	$0B
	.byte	$0A
	.byte	$0B
	.byte	$0A
	.byte	$91
	.byte	$91
	.byte	$7D
	.byte	$7E
	.byte	$7F
	.byte	$7E
	.byte	$7F
	.byte	$7E
	.byte	$7C
	.byte	$7D
	.byte	$7E
	.byte	$7F
	.byte	$7C
	.byte	$7D
	.byte	$7E
	.byte	$7F
	.byte	$7C
	.byte	$7D
	.byte	$8D
	.byte	$8E
	.byte	$8F
	.byte	$8E
	.byte	$8F
	.byte	$8E
	.byte	$8C
	.byte	$8D
	.byte	$8E
	.byte	$8F
	.byte	$8C
	.byte	$8D
	.byte	$8E
	.byte	$8F
	.byte	$8C
	.byte	$8D
_leviathanworld_7:
	.byte	$42
	.byte	$44
	.byte	$45
	.byte	$41
	.byte	$41
	.byte	$46
	.byte	$45
	.byte	$44
	.byte	$66
	.byte	$67
	.byte	$68
	.byte	$69
	.byte	$50
	.byte	$51
	.byte	$53
	.byte	$29
	.byte	$51
	.byte	$45
	.byte	$41
	.byte	$42
	.byte	$56
	.byte	$56
	.byte	$56
	.byte	$55
	.byte	$55
	.byte	$55
	.byte	$55
	.byte	$55
	.byte	$58
	.byte	$55
	.byte	$55
	.byte	$76
	.byte	$77
	.byte	$7A
	.byte	$7A
	.byte	$39
	.byte	$DF
	.byte	$DD
	.byte	$DE
	.byte	$9E
	.byte	$9F
	.byte	$9E
	.byte	$90
	.byte	$90
	.byte	$9E
	.byte	$9F
	.byte	$9E
	.byte	$BB
	.byte	$91
	.byte	$AF
	.byte	$DA
	.byte	$91
	.byte	$91
	.byte	$C9
	.byte	$D9
	.byte	$8B
	.byte	$90
	.byte	$8B
	.byte	$90
	.byte	$90
	.byte	$8B
	.byte	$90
	.byte	$AE
	.byte	$90
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$DA
	.byte	$91
	.byte	$AD
	.byte	$BD
	.byte	$8B
	.byte	$90
	.byte	$9D
	.byte	$90
	.byte	$90
	.byte	$9D
	.byte	$90
	.byte	$BE
	.byte	$90
	.byte	$91
	.byte	$91
	.byte	$DA
	.byte	$AD
	.byte	$BC
	.byte	$BD
	.byte	$90
	.byte	$9D
	.byte	$90
	.byte	$9C
	.byte	$90
	.byte	$90
	.byte	$9D
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$AD
	.byte	$BA
	.byte	$B9
	.byte	$BD
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$9D
	.byte	$90
	.byte	$BE
	.byte	$90
	.byte	$90
	.byte	$9C
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$BD
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$9D
	.byte	$90
	.byte	$90
	.byte	$E3
	.byte	$90
	.byte	$BE
	.byte	$E3
	.byte	$E3
	.byte	$90
	.byte	$C4
	.byte	$D6
	.byte	$E3
	.byte	$C3
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$9C
	.byte	$90
	.byte	$C4
	.byte	$D6
	.byte	$C5
	.byte	$C4
	.byte	$E6
	.byte	$C3
	.byte	$DC
	.byte	$E5
	.byte	$E6
	.byte	$D7
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$BE
	.byte	$E7
	.byte	$D5
	.byte	$D6
	.byte	$D5
	.byte	$E6
	.byte	$90
	.byte	$DC
	.byte	$EC
	.byte	$20
	.byte	$21
	.byte	$22
	.byte	$22
	.byte	$33
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$E4
	.byte	$D5
	.byte	$E6
	.byte	$DC
	.byte	$DD
	.byte	$DD
	.byte	$EC
	.byte	$EA
	.byte	$EF
	.byte	$DD
	.byte	$DE
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$DC
	.byte	$DD
	.byte	$EC
	.byte	$DA
	.byte	$91
	.byte	$91
	.byte	$B0
	.byte	$91
	.byte	$91
	.byte	$EE
	.byte	$EF
	.byte	$DD
	.byte	$ED
	.byte	$EB
	.byte	$EF
	.byte	$ED
	.byte	$D8
	.byte	$EA
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$08
	.byte	$7E
	.byte	$7F
	.byte	$7C
	.byte	$7D
	.byte	$7E
	.byte	$7F
	.byte	$7C
	.byte	$7C
	.byte	$7D
	.byte	$7E
	.byte	$7C
	.byte	$7D
	.byte	$7E
	.byte	$7F
	.byte	$7C
	.byte	$7D
	.byte	$8E
	.byte	$8F
	.byte	$8C
	.byte	$8D
	.byte	$8E
	.byte	$8F
	.byte	$8C
	.byte	$8C
	.byte	$8D
	.byte	$8E
	.byte	$8C
	.byte	$8D
	.byte	$8E
	.byte	$8F
	.byte	$8C
	.byte	$8D
_leviathanworld_8:
	.byte	$26
	.byte	$2A
	.byte	$37
	.byte	$6A
	.byte	$C0
	.byte	$91
	.byte	$5C
	.byte	$7A
	.byte	$41
	.byte	$44
	.byte	$51
	.byte	$45
	.byte	$41
	.byte	$41
	.byte	$46
	.byte	$45
	.byte	$79
	.byte	$7A
	.byte	$79
	.byte	$39
	.byte	$D0
	.byte	$91
	.byte	$DA
	.byte	$C1
	.byte	$78
	.byte	$7A
	.byte	$79
	.byte	$7A
	.byte	$79
	.byte	$52
	.byte	$57
	.byte	$56
	.byte	$AB
	.byte	$DA
	.byte	$AF
	.byte	$91
	.byte	$91
	.byte	$9A
	.byte	$EA
	.byte	$DA
	.byte	$91
	.byte	$AF
	.byte	$DA
	.byte	$DA
	.byte	$CA
	.byte	$AD
	.byte	$90
	.byte	$9E
	.byte	$BB
	.byte	$BC
	.byte	$BC
	.byte	$AB
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$C9
	.byte	$D9
	.byte	$90
	.byte	$90
	.byte	$8B
	.byte	$90
	.byte	$DC
	.byte	$DD
	.byte	$EC
	.byte	$B0
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$B1
	.byte	$EE
	.byte	$EF
	.byte	$DE
	.byte	$9D
	.byte	$DC
	.byte	$EC
	.byte	$EA
	.byte	$91
	.byte	$08
	.byte	$02
	.byte	$4E
	.byte	$03
	.byte	$05
	.byte	$05
	.byte	$05
	.byte	$04
	.byte	$CA
	.byte	$B9
	.byte	$BD
	.byte	$9D
	.byte	$BB
	.byte	$BA
	.byte	$AA
	.byte	$AA
	.byte	$CF
	.byte	$91
	.byte	$4E
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$AD
	.byte	$BC
	.byte	$BD
	.byte	$D2
	.byte	$D2
	.byte	$9C
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$D3
	.byte	$BB
	.byte	$BA
	.byte	$4E
	.byte	$91
	.byte	$91
	.byte	$AD
	.byte	$BD
	.byte	$90
	.byte	$D3
	.byte	$D7
	.byte	$90
	.byte	$9C
	.byte	$DE
	.byte	$90
	.byte	$DC
	.byte	$DD
	.byte	$DD
	.byte	$CD
	.byte	$4E
	.byte	$9A
	.byte	$AD
	.byte	$BD
	.byte	$C4
	.byte	$C5
	.byte	$C6
	.byte	$90
	.byte	$90
	.byte	$BE
	.byte	$EE
	.byte	$EF
	.byte	$EC
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$4E
	.byte	$91
	.byte	$DF
	.byte	$DE
	.byte	$E4
	.byte	$E4
	.byte	$D5
	.byte	$D3
	.byte	$D2
	.byte	$90
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$38
	.byte	$31
	.byte	$31
	.byte	$1F
	.byte	$91
	.byte	$EE
	.byte	$EF
	.byte	$DE
	.byte	$E4
	.byte	$E7
	.byte	$90
	.byte	$90
	.byte	$91
	.byte	$38
	.byte	$0E
	.byte	$0D
	.byte	$8A
	.byte	$44
	.byte	$45
	.byte	$81
	.byte	$1F
	.byte	$91
	.byte	$EA
	.byte	$DF
	.byte	$DD
	.byte	$DD
	.byte	$DD
	.byte	$CD
	.byte	$0C
	.byte	$86
	.byte	$26
	.byte	$36
	.byte	$36
	.byte	$62
	.byte	$63
	.byte	$64
	.byte	$40
	.byte	$1F
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$7E
	.byte	$87
	.byte	$40
	.byte	$44
	.byte	$70
	.byte	$71
	.byte	$72
	.byte	$45
	.byte	$40
	.byte	$82
	.byte	$7F
	.byte	$7E
	.byte	$7E
	.byte	$7C
	.byte	$7D
	.byte	$7E
	.byte	$8E
	.byte	$88
	.byte	$44
	.byte	$43
	.byte	$40
	.byte	$41
	.byte	$44
	.byte	$53
	.byte	$45
	.byte	$83
	.byte	$8F
	.byte	$8C
	.byte	$8D
	.byte	$8E
	.byte	$8F
	.byte	$8D
_leviathanworld_9:
	.byte	$44
	.byte	$51
	.byte	$66
	.byte	$67
	.byte	$68
	.byte	$69
	.byte	$51
	.byte	$45
	.byte	$46
	.byte	$45
	.byte	$41
	.byte	$46
	.byte	$51
	.byte	$45
	.byte	$41
	.byte	$40
	.byte	$56
	.byte	$58
	.byte	$56
	.byte	$55
	.byte	$55
	.byte	$55
	.byte	$76
	.byte	$41
	.byte	$41
	.byte	$41
	.byte	$44
	.byte	$80
	.byte	$71
	.byte	$72
	.byte	$45
	.byte	$40
	.byte	$90
	.byte	$9E
	.byte	$9F
	.byte	$9E
	.byte	$90
	.byte	$9F
	.byte	$B6
	.byte	$5A
	.byte	$57
	.byte	$76
	.byte	$79
	.byte	$41
	.byte	$44
	.byte	$45
	.byte	$40
	.byte	$46
	.byte	$90
	.byte	$AE
	.byte	$90
	.byte	$8B
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$9F
	.byte	$BB
	.byte	$AB
	.byte	$5C
	.byte	$44
	.byte	$50
	.byte	$45
	.byte	$40
	.byte	$90
	.byte	$BE
	.byte	$90
	.byte	$9D
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$BB
	.byte	$BC
	.byte	$89
	.byte	$50
	.byte	$50
	.byte	$51
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$9C
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$5A
	.byte	$57
	.byte	$58
	.byte	$55
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$BE
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$D3
	.byte	$C5
	.byte	$C6
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$E7
	.byte	$E5
	.byte	$D5
	.byte	$D5
	.byte	$E3
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$20
	.byte	$21
	.byte	$21
	.byte	$22
	.byte	$60
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$D3
	.byte	$C5
	.byte	$E6
	.byte	$D7
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$DC
	.byte	$DE
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$D3
	.byte	$E5
	.byte	$E6
	.byte	$90
	.byte	$30
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$DC
	.byte	$EC
	.byte	$EE
	.byte	$EF
	.byte	$DE
	.byte	$90
	.byte	$DC
	.byte	$EC
	.byte	$EB
	.byte	$EB
	.byte	$EF
	.byte	$89
	.byte	$CE
	.byte	$DE
	.byte	$DC
	.byte	$DD
	.byte	$EC
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$DF
	.byte	$DD
	.byte	$EC
	.byte	$91
	.byte	$91
	.byte	$38
	.byte	$32
	.byte	$8A
	.byte	$EA
	.byte	$EE
	.byte	$D8
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$38
	.byte	$32
	.byte	$8A
	.byte	$40
	.byte	$42
	.byte	$7E
	.byte	$7F
	.byte	$7C
	.byte	$7D
	.byte	$7E
	.byte	$7F
	.byte	$7C
	.byte	$7D
	.byte	$7E
	.byte	$7F
	.byte	$7C
	.byte	$88
	.byte	$40
	.byte	$41
	.byte	$41
	.byte	$40
	.byte	$8E
	.byte	$8F
	.byte	$8C
	.byte	$8D
	.byte	$8E
	.byte	$8F
	.byte	$8C
	.byte	$8D
	.byte	$8E
	.byte	$8F
	.byte	$8C
	.byte	$89
	.byte	$42
	.byte	$41
	.byte	$40
	.byte	$46
_leviathanworld_10:
	.byte	$40
	.byte	$44
	.byte	$45
	.byte	$42
	.byte	$42
	.byte	$42
	.byte	$46
	.byte	$70
	.byte	$71
	.byte	$72
	.byte	$45
	.byte	$40
	.byte	$41
	.byte	$42
	.byte	$42
	.byte	$44
	.byte	$42
	.byte	$42
	.byte	$42
	.byte	$42
	.byte	$42
	.byte	$42
	.byte	$42
	.byte	$42
	.byte	$44
	.byte	$53
	.byte	$43
	.byte	$42
	.byte	$40
	.byte	$41
	.byte	$42
	.byte	$42
	.byte	$45
	.byte	$42
	.byte	$40
	.byte	$44
	.byte	$45
	.byte	$42
	.byte	$42
	.byte	$46
	.byte	$50
	.byte	$43
	.byte	$42
	.byte	$40
	.byte	$41
	.byte	$42
	.byte	$46
	.byte	$45
	.byte	$42
	.byte	$46
	.byte	$43
	.byte	$42
	.byte	$40
	.byte	$41
	.byte	$42
	.byte	$44
	.byte	$45
	.byte	$42
	.byte	$42
	.byte	$44
	.byte	$66
	.byte	$67
	.byte	$68
	.byte	$69
	.byte	$45
	.byte	$44
	.byte	$50
	.byte	$43
	.byte	$42
	.byte	$44
	.byte	$45
	.byte	$41
	.byte	$40
	.byte	$41
	.byte	$41
	.byte	$42
	.byte	$41
	.byte	$44
	.byte	$45
	.byte	$42
	.byte	$55
	.byte	$56
	.byte	$55
	.byte	$55
	.byte	$55
	.byte	$58
	.byte	$55
	.byte	$55
	.byte	$55
	.byte	$55
	.byte	$58
	.byte	$58
	.byte	$55
	.byte	$46
	.byte	$43
	.byte	$46
	.byte	$93
	.byte	$94
	.byte	$92
	.byte	$93
	.byte	$92
	.byte	$94
	.byte	$92
	.byte	$93
	.byte	$93
	.byte	$94
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$87
	.byte	$44
	.byte	$66
	.byte	$A2
	.byte	$A2
	.byte	$A2
	.byte	$A2
	.byte	$A2
	.byte	$A2
	.byte	$A2
	.byte	$A2
	.byte	$A2
	.byte	$A2
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$88
	.byte	$42
	.byte	$40
	.byte	$B2
	.byte	$B2
	.byte	$B3
	.byte	$B4
	.byte	$B2
	.byte	$B2
	.byte	$B3
	.byte	$B4
	.byte	$B3
	.byte	$B4
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$87
	.byte	$44
	.byte	$45
	.byte	$31
	.byte	$31
	.byte	$32
	.byte	$31
	.byte	$31
	.byte	$32
	.byte	$31
	.byte	$31
	.byte	$34
	.byte	$35
	.byte	$95
	.byte	$97
	.byte	$98
	.byte	$74
	.byte	$45
	.byte	$40
	.byte	$42
	.byte	$42
	.byte	$42
	.byte	$44
	.byte	$45
	.byte	$42
	.byte	$44
	.byte	$43
	.byte	$42
	.byte	$83
	.byte	$95
	.byte	$97
	.byte	$98
	.byte	$88
	.byte	$44
	.byte	$43
	.byte	$41
	.byte	$44
	.byte	$45
	.byte	$42
	.byte	$42
	.byte	$42
	.byte	$40
	.byte	$41
	.byte	$42
	.byte	$82
	.byte	$95
	.byte	$97
	.byte	$98
	.byte	$89
	.byte	$36
	.byte	$36
	.byte	$44
	.byte	$53
	.byte	$45
	.byte	$42
	.byte	$44
	.byte	$51
	.byte	$45
	.byte	$44
	.byte	$51
	.byte	$84
	.byte	$95
	.byte	$97
	.byte	$98
	.byte	$87
	.byte	$42
	.byte	$42
	.byte	$41
	.byte	$46
	.byte	$51
	.byte	$51
	.byte	$45
	.byte	$42
	.byte	$42
	.byte	$44
	.byte	$45
	.byte	$82
	.byte	$95
	.byte	$97
	.byte	$98
	.byte	$86
	.byte	$42
	.byte	$41
	.byte	$80
	.byte	$71
	.byte	$72
	.byte	$45
	.byte	$42
	.byte	$42
	.byte	$42
	.byte	$40
	.byte	$41
	.byte	$83
	.byte	$A5
	.byte	$A7
	.byte	$A8
	.byte	$87
	.byte	$40
	.byte	$44
_leviathanworld_11:
	.byte	$40
	.byte	$41
	.byte	$40
	.byte	$41
	.byte	$44
	.byte	$43
	.byte	$41
	.byte	$42
	.byte	$42
	.byte	$83
	.byte	$95
	.byte	$A7
	.byte	$98
	.byte	$88
	.byte	$46
	.byte	$45
	.byte	$42
	.byte	$44
	.byte	$29
	.byte	$4C
	.byte	$4C
	.byte	$4C
	.byte	$2A
	.byte	$43
	.byte	$42
	.byte	$83
	.byte	$A5
	.byte	$97
	.byte	$A8
	.byte	$87
	.byte	$40
	.byte	$41
	.byte	$44
	.byte	$43
	.byte	$83
	.byte	$3F
	.byte	$3F
	.byte	$3F
	.byte	$87
	.byte	$46
	.byte	$43
	.byte	$82
	.byte	$95
	.byte	$97
	.byte	$98
	.byte	$87
	.byte	$44
	.byte	$45
	.byte	$42
	.byte	$42
	.byte	$83
	.byte	$6E
	.byte	$6C
	.byte	$6C
	.byte	$65
	.byte	$55
	.byte	$56
	.byte	$61
	.byte	$95
	.byte	$97
	.byte	$A8
	.byte	$65
	.byte	$55
	.byte	$41
	.byte	$42
	.byte	$42
	.byte	$83
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$94
	.byte	$94
	.byte	$94
	.byte	$94
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$94
	.byte	$94
	.byte	$87
	.byte	$46
	.byte	$43
	.byte	$82
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$A2
	.byte	$A3
	.byte	$A2
	.byte	$A3
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$A3
	.byte	$A2
	.byte	$89
	.byte	$51
	.byte	$45
	.byte	$83
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$B3
	.byte	$B4
	.byte	$B4
	.byte	$B4
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$B4
	.byte	$B4
	.byte	$87
	.byte	$46
	.byte	$43
	.byte	$83
	.byte	$A5
	.byte	$97
	.byte	$A8
	.byte	$30
	.byte	$0E
	.byte	$0D
	.byte	$35
	.byte	$A5
	.byte	$97
	.byte	$A8
	.byte	$30
	.byte	$32
	.byte	$8A
	.byte	$45
	.byte	$42
	.byte	$83
	.byte	$A5
	.byte	$96
	.byte	$98
	.byte	$87
	.byte	$46
	.byte	$43
	.byte	$83
	.byte	$A5
	.byte	$97
	.byte	$A8
	.byte	$87
	.byte	$42
	.byte	$42
	.byte	$43
	.byte	$41
	.byte	$85
	.byte	$95
	.byte	$A7
	.byte	$98
	.byte	$89
	.byte	$26
	.byte	$2A
	.byte	$84
	.byte	$A5
	.byte	$A6
	.byte	$A8
	.byte	$88
	.byte	$40
	.byte	$46
	.byte	$40
	.byte	$41
	.byte	$83
	.byte	$A5
	.byte	$97
	.byte	$98
	.byte	$87
	.byte	$42
	.byte	$42
	.byte	$83
	.byte	$A5
	.byte	$97
	.byte	$A8
	.byte	$87
	.byte	$44
	.byte	$53
	.byte	$50
	.byte	$70
	.byte	$84
	.byte	$95
	.byte	$A7
	.byte	$98
	.byte	$88
	.byte	$41
	.byte	$42
	.byte	$82
	.byte	$A5
	.byte	$96
	.byte	$A8
	.byte	$88
	.byte	$42
	.byte	$46
	.byte	$44
	.byte	$43
	.byte	$83
	.byte	$A5
	.byte	$97
	.byte	$98
	.byte	$87
	.byte	$42
	.byte	$42
	.byte	$83
	.byte	$A5
	.byte	$97
	.byte	$A8
	.byte	$87
	.byte	$40
	.byte	$41
	.byte	$45
	.byte	$41
	.byte	$83
	.byte	$95
	.byte	$97
	.byte	$98
	.byte	$86
	.byte	$44
	.byte	$70
	.byte	$84
	.byte	$A5
	.byte	$97
	.byte	$A8
	.byte	$88
	.byte	$42
	.byte	$44
	.byte	$51
	.byte	$43
	.byte	$83
	.byte	$A5
	.byte	$A7
	.byte	$A8
	.byte	$88
	.byte	$41
	.byte	$42
	.byte	$83
	.byte	$A5
	.byte	$97
	.byte	$A8
	.byte	$86
	.byte	$40
	.byte	$41
_leviathanworld_12:
	.byte	$46
	.byte	$43
	.byte	$83
	.byte	$A5
	.byte	$A7
	.byte	$A8
	.byte	$88
	.byte	$46
	.byte	$45
	.byte	$83
	.byte	$A5
	.byte	$97
	.byte	$A8
	.byte	$87
	.byte	$42
	.byte	$46
	.byte	$29
	.byte	$55
	.byte	$75
	.byte	$E7
	.byte	$90
	.byte	$C6
	.byte	$87
	.byte	$41
	.byte	$41
	.byte	$83
	.byte	$C4
	.byte	$C5
	.byte	$C6
	.byte	$5A
	.byte	$75
	.byte	$88
	.byte	$83
	.byte	$90
	.byte	$C4
	.byte	$C5
	.byte	$C4
	.byte	$D7
	.byte	$86
	.byte	$46
	.byte	$43
	.byte	$83
	.byte	$E4
	.byte	$E5
	.byte	$E7
	.byte	$C6
	.byte	$9F
	.byte	$87
	.byte	$82
	.byte	$C4
	.byte	$D5
	.byte	$D4
	.byte	$E6
	.byte	$E7
	.byte	$87
	.byte	$46
	.byte	$43
	.byte	$82
	.byte	$C6
	.byte	$E4
	.byte	$C4
	.byte	$E5
	.byte	$E3
	.byte	$87
	.byte	$83
	.byte	$E4
	.byte	$E4
	.byte	$D5
	.byte	$C6
	.byte	$90
	.byte	$5A
	.byte	$54
	.byte	$54
	.byte	$75
	.byte	$E7
	.byte	$C4
	.byte	$D5
	.byte	$E6
	.byte	$E6
	.byte	$86
	.byte	$83
	.byte	$90
	.byte	$D7
	.byte	$E4
	.byte	$D5
	.byte	$B5
	.byte	$90
	.byte	$9F
	.byte	$D2
	.byte	$C4
	.byte	$E3
	.byte	$E4
	.byte	$D6
	.byte	$C6
	.byte	$90
	.byte	$74
	.byte	$83
	.byte	$90
	.byte	$73
	.byte	$60
	.byte	$E7
	.byte	$E7
	.byte	$B5
	.byte	$C5
	.byte	$C6
	.byte	$E7
	.byte	$D7
	.byte	$C4
	.byte	$D5
	.byte	$E6
	.byte	$C4
	.byte	$87
	.byte	$84
	.byte	$E7
	.byte	$C5
	.byte	$C6
	.byte	$90
	.byte	$E7
	.byte	$E4
	.byte	$D5
	.byte	$D6
	.byte	$C6
	.byte	$E4
	.byte	$D5
	.byte	$E6
	.byte	$E3
	.byte	$D3
	.byte	$87
	.byte	$83
	.byte	$6D
	.byte	$E4
	.byte	$E6
	.byte	$20
	.byte	$33
	.byte	$90
	.byte	$E4
	.byte	$D4
	.byte	$D6
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$E4
	.byte	$C4
	.byte	$88
	.byte	$82
	.byte	$5F
	.byte	$6D
	.byte	$6D
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$E7
	.byte	$D4
	.byte	$E6
	.byte	$90
	.byte	$73
	.byte	$33
	.byte	$E7
	.byte	$E4
	.byte	$87
	.byte	$83
	.byte	$4F
	.byte	$4F
	.byte	$4F
	.byte	$6D
	.byte	$90
	.byte	$E7
	.byte	$C4
	.byte	$D5
	.byte	$C6
	.byte	$90
	.byte	$90
	.byte	$9F
	.byte	$6D
	.byte	$90
	.byte	$87
	.byte	$27
	.byte	$3C
	.byte	$3C
	.byte	$5D
	.byte	$4F
	.byte	$6D
	.byte	$E4
	.byte	$D5
	.byte	$D5
	.byte	$E6
	.byte	$6D
	.byte	$6D
	.byte	$90
	.byte	$5E
	.byte	$6D
	.byte	$74
	.byte	$43
	.byte	$41
	.byte	$46
	.byte	$27
	.byte	$5D
	.byte	$4F
	.byte	$90
	.byte	$E4
	.byte	$E6
	.byte	$D2
	.byte	$4F
	.byte	$4F
	.byte	$6D
	.byte	$4F
	.byte	$4F
	.byte	$87
	.byte	$40
	.byte	$44
	.byte	$51
	.byte	$51
	.byte	$27
	.byte	$5D
	.byte	$95
	.byte	$96
	.byte	$96
	.byte	$98
	.byte	$3A
	.byte	$5D
	.byte	$4F
	.byte	$3B
	.byte	$3C
	.byte	$37
	.byte	$66
	.byte	$67
	.byte	$68
	.byte	$69
	.byte	$43
	.byte	$83
	.byte	$95
	.byte	$A6
	.byte	$A6
	.byte	$A8
	.byte	$74
	.byte	$27
	.byte	$3C
	.byte	$47
	.byte	$28
	.byte	$45
_leviathanworld_13:
	.byte	$62
	.byte	$63
	.byte	$2A
	.byte	$50
	.byte	$43
	.byte	$85
	.byte	$95
	.byte	$A6
	.byte	$A6
	.byte	$A8
	.byte	$87
	.byte	$40
	.byte	$40
	.byte	$46
	.byte	$51
	.byte	$45
	.byte	$41
	.byte	$41
	.byte	$44
	.byte	$45
	.byte	$41
	.byte	$82
	.byte	$95
	.byte	$96
	.byte	$97
	.byte	$98
	.byte	$88
	.byte	$40
	.byte	$40
	.byte	$44
	.byte	$70
	.byte	$71
	.byte	$41
	.byte	$44
	.byte	$29
	.byte	$55
	.byte	$56
	.byte	$75
	.byte	$A5
	.byte	$96
	.byte	$97
	.byte	$98
	.byte	$87
	.byte	$40
	.byte	$40
	.byte	$41
	.byte	$41
	.byte	$44
	.byte	$40
	.byte	$41
	.byte	$83
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$A5
	.byte	$A6
	.byte	$A7
	.byte	$98
	.byte	$86
	.byte	$41
	.byte	$44
	.byte	$45
	.byte	$41
	.byte	$40
	.byte	$41
	.byte	$41
	.byte	$83
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$A5
	.byte	$96
	.byte	$97
	.byte	$98
	.byte	$87
	.byte	$46
	.byte	$51
	.byte	$51
	.byte	$45
	.byte	$41
	.byte	$44
	.byte	$43
	.byte	$83
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$95
	.byte	$96
	.byte	$97
	.byte	$98
	.byte	$74
	.byte	$43
	.byte	$41
	.byte	$41
	.byte	$41
	.byte	$41
	.byte	$44
	.byte	$45
	.byte	$82
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$A5
	.byte	$A6
	.byte	$A7
	.byte	$98
	.byte	$87
	.byte	$41
	.byte	$41
	.byte	$46
	.byte	$45
	.byte	$41
	.byte	$41
	.byte	$41
	.byte	$81
	.byte	$0E
	.byte	$0D
	.byte	$35
	.byte	$95
	.byte	$A6
	.byte	$A7
	.byte	$98
	.byte	$86
	.byte	$40
	.byte	$41
	.byte	$40
	.byte	$41
	.byte	$41
	.byte	$62
	.byte	$63
	.byte	$64
	.byte	$41
	.byte	$40
	.byte	$83
	.byte	$95
	.byte	$96
	.byte	$97
	.byte	$98
	.byte	$5A
	.byte	$57
	.byte	$56
	.byte	$41
	.byte	$46
	.byte	$50
	.byte	$46
	.byte	$53
	.byte	$45
	.byte	$41
	.byte	$41
	.byte	$82
	.byte	$96
	.byte	$97
	.byte	$97
	.byte	$98
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$87
	.byte	$41
	.byte	$41
	.byte	$41
	.byte	$46
	.byte	$50
	.byte	$43
	.byte	$41
	.byte	$83
	.byte	$95
	.byte	$96
	.byte	$97
	.byte	$98
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$74
	.byte	$66
	.byte	$67
	.byte	$41
	.byte	$42
	.byte	$40
	.byte	$44
	.byte	$45
	.byte	$85
	.byte	$95
	.byte	$96
	.byte	$97
	.byte	$98
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$87
	.byte	$44
	.byte	$45
	.byte	$41
	.byte	$41
	.byte	$46
	.byte	$51
	.byte	$43
	.byte	$83
	.byte	$A5
	.byte	$96
	.byte	$97
	.byte	$A8
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$87
	.byte	$42
	.byte	$41
	.byte	$45
	.byte	$40
	.byte	$44
	.byte	$80
	.byte	$17
	.byte	$84
	.byte	$95
	.byte	$96
	.byte	$97
	.byte	$98
	.byte	$30
	.byte	$31
	.byte	$32
	.byte	$41
	.byte	$40
	.byte	$44
	.byte	$51
	.byte	$43
	.byte	$40
	.byte	$41
	.byte	$40
	.byte	$83
	.byte	$A5
	.byte	$A6
	.byte	$A7
	.byte	$98
	.byte	$87
	.byte	$46
	.byte	$45
	.byte	$41
	.byte	$46
	.byte	$51
_leviathanworld_14:
	.byte	$51
	.byte	$43
	.byte	$46
	.byte	$53
	.byte	$45
	.byte	$83
	.byte	$A5
	.byte	$A6
	.byte	$A7
	.byte	$98
	.byte	$87
	.byte	$44
	.byte	$29
	.byte	$55
	.byte	$2A
	.byte	$51
	.byte	$40
	.byte	$46
	.byte	$54
	.byte	$54
	.byte	$54
	.byte	$75
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$5A
	.byte	$54
	.byte	$75
	.byte	$90
	.byte	$5A
	.byte	$45
	.byte	$40
	.byte	$83
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$C4
	.byte	$C6
	.byte	$C6
	.byte	$90
	.byte	$87
	.byte	$40
	.byte	$82
	.byte	$EB
	.byte	$EF
	.byte	$DE
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$C4
	.byte	$D4
	.byte	$D4
	.byte	$D4
	.byte	$C6
	.byte	$87
	.byte	$45
	.byte	$83
	.byte	$91
	.byte	$91
	.byte	$EE
	.byte	$EF
	.byte	$DE
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$E4
	.byte	$E5
	.byte	$D4
	.byte	$D4
	.byte	$D6
	.byte	$87
	.byte	$40
	.byte	$81
	.byte	$32
	.byte	$0E
	.byte	$0D
	.byte	$31
	.byte	$31
	.byte	$0F
	.byte	$0E
	.byte	$0D
	.byte	$32
	.byte	$35
	.byte	$E4
	.byte	$D4
	.byte	$E6
	.byte	$88
	.byte	$44
	.byte	$29
	.byte	$26
	.byte	$4C
	.byte	$4C
	.byte	$4C
	.byte	$4C
	.byte	$4C
	.byte	$4C
	.byte	$4C
	.byte	$2A
	.byte	$84
	.byte	$90
	.byte	$E4
	.byte	$E7
	.byte	$87
	.byte	$50
	.byte	$43
	.byte	$83
	.byte	$3F
	.byte	$3F
	.byte	$3F
	.byte	$3F
	.byte	$3F
	.byte	$3F
	.byte	$3F
	.byte	$88
	.byte	$85
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$87
	.byte	$40
	.byte	$42
	.byte	$61
	.byte	$5E
	.byte	$6E
	.byte	$6E
	.byte	$6C
	.byte	$6E
	.byte	$6C
	.byte	$6E
	.byte	$65
	.byte	$61
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$87
	.byte	$45
	.byte	$40
	.byte	$90
	.byte	$6C
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$42
	.byte	$43
	.byte	$83
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$DC
	.byte	$DE
	.byte	$90
	.byte	$88
	.byte	$80
	.byte	$84
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$DC
	.byte	$EC
	.byte	$EE
	.byte	$EF
	.byte	$87
	.byte	$40
	.byte	$83
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$DC
	.byte	$ED
	.byte	$EF
	.byte	$EC
	.byte	$9A
	.byte	$91
	.byte	$91
	.byte	$87
	.byte	$42
	.byte	$82
	.byte	$90
	.byte	$90
	.byte	$3E
	.byte	$30
	.byte	$31
	.byte	$32
	.byte	$31
	.byte	$32
	.byte	$0E
	.byte	$31
	.byte	$31
	.byte	$32
	.byte	$31
	.byte	$8A
	.byte	$40
	.byte	$83
	.byte	$C4
	.byte	$C5
	.byte	$3E
	.byte	$87
	.byte	$46
	.byte	$29
	.byte	$62
	.byte	$63
	.byte	$64
	.byte	$40
	.byte	$41
	.byte	$40
	.byte	$44
	.byte	$50
_leviathanworld_15:
	.byte	$40
	.byte	$83
	.byte	$E4
	.byte	$D5
	.byte	$3E
	.byte	$87
	.byte	$42
	.byte	$42
	.byte	$44
	.byte	$70
	.byte	$71
	.byte	$72
	.byte	$45
	.byte	$7A
	.byte	$7A
	.byte	$79
	.byte	$41
	.byte	$82
	.byte	$C6
	.byte	$D3
	.byte	$3E
	.byte	$87
	.byte	$36
	.byte	$58
	.byte	$58
	.byte	$56
	.byte	$76
	.byte	$79
	.byte	$39
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$45
	.byte	$83
	.byte	$D6
	.byte	$C2
	.byte	$3E
	.byte	$5A
	.byte	$75
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$BB
	.byte	$AB
	.byte	$9A
	.byte	$EA
	.byte	$EA
	.byte	$91
	.byte	$41
	.byte	$85
	.byte	$D6
	.byte	$B5
	.byte	$3E
	.byte	$90
	.byte	$C4
	.byte	$C5
	.byte	$C6
	.byte	$C4
	.byte	$C5
	.byte	$BB
	.byte	$AB
	.byte	$CA
	.byte	$B9
	.byte	$BC
	.byte	$40
	.byte	$83
	.byte	$D4
	.byte	$D5
	.byte	$3E
	.byte	$C4
	.byte	$D5
	.byte	$D4
	.byte	$C6
	.byte	$C4
	.byte	$D5
	.byte	$D6
	.byte	$BB
	.byte	$BD
	.byte	$90
	.byte	$90
	.byte	$45
	.byte	$82
	.byte	$D4
	.byte	$D5
	.byte	$3E
	.byte	$D4
	.byte	$D5
	.byte	$D4
	.byte	$D4
	.byte	$D5
	.byte	$D4
	.byte	$D5
	.byte	$D6
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$42
	.byte	$83
	.byte	$D4
	.byte	$D5
	.byte	$3E
	.byte	$D4
	.byte	$D5
	.byte	$D4
	.byte	$D4
	.byte	$D4
	.byte	$D4
	.byte	$D5
	.byte	$D4
	.byte	$C6
	.byte	$D3
	.byte	$90
	.byte	$41
	.byte	$82
	.byte	$D4
	.byte	$D5
	.byte	$3E
	.byte	$D4
	.byte	$D4
	.byte	$D4
	.byte	$D4
	.byte	$E6
	.byte	$E4
	.byte	$D4
	.byte	$D5
	.byte	$D6
	.byte	$E7
	.byte	$C4
	.byte	$41
	.byte	$83
	.byte	$D4
	.byte	$D5
	.byte	$E6
	.byte	$E4
	.byte	$D5
	.byte	$D4
	.byte	$D4
	.byte	$C6
	.byte	$E7
	.byte	$E4
	.byte	$D5
	.byte	$E6
	.byte	$E4
	.byte	$E4
	.byte	$41
	.byte	$83
	.byte	$E4
	.byte	$E6
	.byte	$90
	.byte	$D2
	.byte	$E4
	.byte	$D4
	.byte	$D4
	.byte	$E6
	.byte	$E0
	.byte	$E4
	.byte	$E6
	.byte	$90
	.byte	$E2
	.byte	$90
	.byte	$43
	.byte	$83
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$E2
	.byte	$E4
	.byte	$E6
	.byte	$90
	.byte	$14
	.byte	$15
	.byte	$1D
	.byte	$1B
	.byte	$1A
	.byte	$90
	.byte	$17
	.byte	$84
	.byte	$10
	.byte	$1A
	.byte	$1B
	.byte	$1A
	.byte	$1B
	.byte	$DC
	.byte	$DD
	.byte	$DE
	.byte	$90
	.byte	$90
	.byte	$1E
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$45
	.byte	$83
	.byte	$DD
	.byte	$ED
	.byte	$EB
	.byte	$EB
	.byte	$EF
	.byte	$EC
	.byte	$91
	.byte	$DF
	.byte	$DD
	.byte	$ED
	.byte	$2C
	.byte	$EE
	.byte	$EF
	.byte	$DD
	.byte	$41
	.byte	$83
	.byte	$7E
	.byte	$7F
	.byte	$7F
	.byte	$7E
	.byte	$7E
	.byte	$7F
	.byte	$7C
	.byte	$7D
	.byte	$7E
	.byte	$7F
	.byte	$1E
	.byte	$7D
	.byte	$7E
	.byte	$7F
	.byte	$43
	.byte	$83
	.byte	$8E
	.byte	$8E
	.byte	$8C
	.byte	$8D
	.byte	$8E
	.byte	$8F
	.byte	$8C
	.byte	$8D
	.byte	$8E
	.byte	$8F
	.byte	$2D
	.byte	$8D
	.byte	$8E
	.byte	$8F
_leviathanworld_16:
	.byte	$39
	.byte	$9A
	.byte	$9B
	.byte	$1E
	.byte	$AD
	.byte	$BD
	.byte	$E4
	.byte	$E6
	.byte	$D5
	.byte	$D6
	.byte	$E6
	.byte	$BB
	.byte	$AB
	.byte	$2D
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$AD
	.byte	$BA
	.byte	$2D
	.byte	$BD
	.byte	$90
	.byte	$E3
	.byte	$C4
	.byte	$D5
	.byte	$E6
	.byte	$90
	.byte	$90
	.byte	$BB
	.byte	$1E
	.byte	$AB
	.byte	$EA
	.byte	$AD
	.byte	$BD
	.byte	$90
	.byte	$07
	.byte	$90
	.byte	$90
	.byte	$D7
	.byte	$D3
	.byte	$E6
	.byte	$E7
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$2C
	.byte	$BB
	.byte	$BC
	.byte	$BD
	.byte	$13
	.byte	$14
	.byte	$09
	.byte	$1A
	.byte	$12
	.byte	$3E
	.byte	$90
	.byte	$90
	.byte	$B5
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$07
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$3E
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$13
	.byte	$15
	.byte	$09
	.byte	$1A
	.byte	$12
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$3E
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$3E
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$C6
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$E6
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$20
	.byte	$21
	.byte	$22
	.byte	$23
	.byte	$33
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$DB
	.byte	$DC
	.byte	$DD
	.byte	$DE
	.byte	$DC
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$DC
	.byte	$ED
	.byte	$EB
	.byte	$EB
	.byte	$EF
	.byte	$CD
	.byte	$EB
	.byte	$CE
	.byte	$DD
	.byte	$EC
	.byte	$B0
	.byte	$DF
	.byte	$EC
	.byte	$DD
	.byte	$ED
	.byte	$EB
	.byte	$D8
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$05
	.byte	$05
	.byte	$05
	.byte	$7C
	.byte	$7D
	.byte	$7E
	.byte	$7F
	.byte	$7C
	.byte	$7D
	.byte	$7E
	.byte	$7F
	.byte	$7F
	.byte	$7E
	.byte	$7E
	.byte	$7F
	.byte	$7C
	.byte	$7D
	.byte	$7E
	.byte	$7C
	.byte	$8C
	.byte	$8D
	.byte	$8E
	.byte	$8F
	.byte	$8C
	.byte	$8D
	.byte	$8E
	.byte	$8E
	.byte	$8C
	.byte	$8D
	.byte	$8E
	.byte	$8F
	.byte	$8C
	.byte	$8D
	.byte	$8E
	.byte	$8C
_leviathanworld_17:
	.byte	$9A
	.byte	$9B
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$1E
	.byte	$AD
	.byte	$BC
	.byte	$AB
	.byte	$91
	.byte	$EA
	.byte	$9A
	.byte	$9B
	.byte	$91
	.byte	$EA
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$AD
	.byte	$BC
	.byte	$BA
	.byte	$2D
	.byte	$BD
	.byte	$E7
	.byte	$BB
	.byte	$AB
	.byte	$CA
	.byte	$B9
	.byte	$BA
	.byte	$B9
	.byte	$BC
	.byte	$AB
	.byte	$BC
	.byte	$BC
	.byte	$BD
	.byte	$E7
	.byte	$B5
	.byte	$1E
	.byte	$E7
	.byte	$C5
	.byte	$C6
	.byte	$BB
	.byte	$BD
	.byte	$90
	.byte	$C4
	.byte	$C6
	.byte	$D2
	.byte	$BB
	.byte	$90
	.byte	$B5
	.byte	$E7
	.byte	$C4
	.byte	$C5
	.byte	$2C
	.byte	$C4
	.byte	$D5
	.byte	$D5
	.byte	$D3
	.byte	$E7
	.byte	$C5
	.byte	$D4
	.byte	$D6
	.byte	$C5
	.byte	$E3
	.byte	$E7
	.byte	$E3
	.byte	$C4
	.byte	$D5
	.byte	$E5
	.byte	$1E
	.byte	$E4
	.byte	$B5
	.byte	$E4
	.byte	$B5
	.byte	$C5
	.byte	$E6
	.byte	$E4
	.byte	$D5
	.byte	$E5
	.byte	$E6
	.byte	$B5
	.byte	$E4
	.byte	$E5
	.byte	$E6
	.byte	$E7
	.byte	$2C
	.byte	$E7
	.byte	$E4
	.byte	$C6
	.byte	$E3
	.byte	$E5
	.byte	$E6
	.byte	$B5
	.byte	$A0
	.byte	$E7
	.byte	$DB
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$E3
	.byte	$C4
	.byte	$07
	.byte	$90
	.byte	$D7
	.byte	$90
	.byte	$C2
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$18
	.byte	$12
	.byte	$3E
	.byte	$90
	.byte	$90
	.byte	$13
	.byte	$14
	.byte	$15
	.byte	$09
	.byte	$1A
	.byte	$1B
	.byte	$12
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$3E
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$3E
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$CD
	.byte	$EE
	.byte	$EB
	.byte	$CE
	.byte	$CD
	.byte	$EB
	.byte	$EB
	.byte	$CE
	.byte	$DE
	.byte	$DC
	.byte	$DE
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$B1
	.byte	$91
	.byte	$91
	.byte	$B0
	.byte	$91
	.byte	$DF
	.byte	$EC
	.byte	$DF
	.byte	$DE
	.byte	$E2
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$1D
	.byte	$0A
	.byte	$0A
	.byte	$0B
	.byte	$91
	.byte	$91
	.byte	$05
	.byte	$05
	.byte	$05
	.byte	$1D
	.byte	$0A
	.byte	$0A
	.byte	$0B
	.byte	$CE
	.byte	$DD
	.byte	$CD
	.byte	$1E
	.byte	$7E
	.byte	$7F
	.byte	$7C
	.byte	$7D
	.byte	$7E
	.byte	$7F
	.byte	$7F
	.byte	$7F
	.byte	$2C
	.byte	$7F
	.byte	$7C
	.byte	$7D
	.byte	$7E
	.byte	$7F
	.byte	$7C
	.byte	$2D
	.byte	$8E
	.byte	$8F
	.byte	$8C
	.byte	$8D
	.byte	$8E
	.byte	$8F
	.byte	$8E
	.byte	$8C
	.byte	$1E
	.byte	$8F
	.byte	$8C
	.byte	$8D
	.byte	$8E
	.byte	$8F
	.byte	$8C
_leviathanworld_18:
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$9A
	.byte	$9B
	.byte	$AD
	.byte	$BD
	.byte	$90
	.byte	$E7
	.byte	$C4
	.byte	$C5
	.byte	$C6
	.byte	$90
	.byte	$90
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$EA
	.byte	$9A
	.byte	$AD
	.byte	$BD
	.byte	$90
	.byte	$90
	.byte	$C2
	.byte	$D6
	.byte	$D5
	.byte	$D5
	.byte	$E7
	.byte	$C4
	.byte	$AB
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$C9
	.byte	$D9
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$D3
	.byte	$E4
	.byte	$E7
	.byte	$E7
	.byte	$D5
	.byte	$BB
	.byte	$BC
	.byte	$BC
	.byte	$AB
	.byte	$91
	.byte	$91
	.byte	$DF
	.byte	$DE
	.byte	$90
	.byte	$90
	.byte	$E3
	.byte	$D2
	.byte	$E7
	.byte	$E6
	.byte	$E4
	.byte	$D5
	.byte	$E7
	.byte	$DC
	.byte	$DD
	.byte	$EC
	.byte	$9A
	.byte	$9B
	.byte	$91
	.byte	$DF
	.byte	$DE
	.byte	$90
	.byte	$C2
	.byte	$E6
	.byte	$90
	.byte	$E7
	.byte	$C4
	.byte	$E4
	.byte	$DC
	.byte	$EC
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$DF
	.byte	$DE
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$E6
	.byte	$90
	.byte	$30
	.byte	$0E
	.byte	$0D
	.byte	$31
	.byte	$32
	.byte	$31
	.byte	$0E
	.byte	$0D
	.byte	$31
	.byte	$31
	.byte	$35
	.byte	$90
	.byte	$90
	.byte	$E2
	.byte	$90
	.byte	$90
	.byte	$65
	.byte	$55
	.byte	$55
	.byte	$58
	.byte	$55
	.byte	$55
	.byte	$55
	.byte	$56
	.byte	$58
	.byte	$41
	.byte	$85
	.byte	$10
	.byte	$1A
	.byte	$1A
	.byte	$90
	.byte	$90
	.byte	$E4
	.byte	$E5
	.byte	$D7
	.byte	$C3
	.byte	$90
	.byte	$D7
	.byte	$E5
	.byte	$C3
	.byte	$B5
	.byte	$65
	.byte	$61
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$E3
	.byte	$C4
	.byte	$C5
	.byte	$C6
	.byte	$90
	.byte	$C5
	.byte	$C6
	.byte	$30
	.byte	$35
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$30
	.byte	$32
	.byte	$35
	.byte	$E4
	.byte	$D5
	.byte	$E6
	.byte	$C4
	.byte	$E6
	.byte	$30
	.byte	$8A
	.byte	$83
	.byte	$EE
	.byte	$EF
	.byte	$DE
	.byte	$90
	.byte	$90
	.byte	$88
	.byte	$41
	.byte	$81
	.byte	$35
	.byte	$E4
	.byte	$C3
	.byte	$C4
	.byte	$30
	.byte	$8A
	.byte	$41
	.byte	$82
	.byte	$91
	.byte	$91
	.byte	$EE
	.byte	$EF
	.byte	$DD
	.byte	$87
	.byte	$44
	.byte	$43
	.byte	$81
	.byte	$31
	.byte	$31
	.byte	$32
	.byte	$8A
	.byte	$46
	.byte	$17
	.byte	$84
	.byte	$7E
	.byte	$7F
	.byte	$7E
	.byte	$7F
	.byte	$7C
	.byte	$87
	.byte	$44
	.byte	$53
	.byte	$66
	.byte	$67
	.byte	$68
	.byte	$69
	.byte	$45
	.byte	$41
	.byte	$41
	.byte	$83
	.byte	$8C
	.byte	$8D
	.byte	$8E
	.byte	$8F
	.byte	$8C
_leviathanworld_19:
	.byte	$C5
	.byte	$D5
	.byte	$E6
	.byte	$C2
	.byte	$2D
	.byte	$90
	.byte	$90
	.byte	$D3
	.byte	$C5
	.byte	$C5
	.byte	$C6
	.byte	$C5
	.byte	$C4
	.byte	$C5
	.byte	$C6
	.byte	$90
	.byte	$D5
	.byte	$D5
	.byte	$D5
	.byte	$D2
	.byte	$2C
	.byte	$D3
	.byte	$C4
	.byte	$E7
	.byte	$E4
	.byte	$E4
	.byte	$D5
	.byte	$D5
	.byte	$D5
	.byte	$E5
	.byte	$D5
	.byte	$D2
	.byte	$D5
	.byte	$E6
	.byte	$C3
	.byte	$90
	.byte	$1E
	.byte	$E4
	.byte	$D5
	.byte	$D3
	.byte	$E7
	.byte	$C2
	.byte	$E4
	.byte	$C5
	.byte	$C6
	.byte	$C6
	.byte	$D3
	.byte	$C2
	.byte	$D5
	.byte	$E6
	.byte	$90
	.byte	$90
	.byte	$07
	.byte	$90
	.byte	$D3
	.byte	$E4
	.byte	$C6
	.byte	$E6
	.byte	$C4
	.byte	$D4
	.byte	$D5
	.byte	$E6
	.byte	$90
	.byte	$E3
	.byte	$D3
	.byte	$90
	.byte	$13
	.byte	$14
	.byte	$09
	.byte	$1A
	.byte	$12
	.byte	$90
	.byte	$C2
	.byte	$E4
	.byte	$D5
	.byte	$E6
	.byte	$E6
	.byte	$90
	.byte	$90
	.byte	$C2
	.byte	$90
	.byte	$90
	.byte	$9E
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$E4
	.byte	$D5
	.byte	$D3
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$AE
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$D7
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$D3
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$BE
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$DC
	.byte	$DE
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$DC
	.byte	$DD
	.byte	$EC
	.byte	$DF
	.byte	$ED
	.byte	$EB
	.byte	$EB
	.byte	$CE
	.byte	$CD
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$DC
	.byte	$EC
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$B0
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$B1
	.byte	$DC
	.byte	$DD
	.byte	$ED
	.byte	$EF
	.byte	$ED
	.byte	$EB
	.byte	$D8
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$05
	.byte	$05
	.byte	$1D
	.byte	$0A
	.byte	$0A
	.byte	$EC
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$1E
	.byte	$91
	.byte	$91
	.byte	$7D
	.byte	$7E
	.byte	$7F
	.byte	$7C
	.byte	$7D
	.byte	$7E
	.byte	$7F
	.byte	$7F
	.byte	$7E
	.byte	$7E
	.byte	$7F
	.byte	$7F
	.byte	$7E
	.byte	$1E
	.byte	$7F
	.byte	$7C
	.byte	$8D
	.byte	$8E
	.byte	$8F
	.byte	$8C
	.byte	$8D
	.byte	$8E
	.byte	$8F
	.byte	$8C
	.byte	$8D
	.byte	$8E
	.byte	$8F
	.byte	$8C
	.byte	$8D
	.byte	$1E
	.byte	$8F
	.byte	$8C
_leviathanworld_20:
	.byte	$BB
	.byte	$AB
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$6B
	.byte	$47
	.byte	$28
	.byte	$51
	.byte	$45
	.byte	$41
	.byte	$42
	.byte	$41
	.byte	$E7
	.byte	$BB
	.byte	$BC
	.byte	$BC
	.byte	$AB
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$C1
	.byte	$6B
	.byte	$46
	.byte	$66
	.byte	$67
	.byte	$68
	.byte	$69
	.byte	$51
	.byte	$90
	.byte	$E7
	.byte	$90
	.byte	$90
	.byte	$BB
	.byte	$BC
	.byte	$BC
	.byte	$BC
	.byte	$AB
	.byte	$C1
	.byte	$6B
	.byte	$44
	.byte	$53
	.byte	$50
	.byte	$51
	.byte	$45
	.byte	$C6
	.byte	$D3
	.byte	$E3
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$C4
	.byte	$BB
	.byte	$AB
	.byte	$C1
	.byte	$78
	.byte	$46
	.byte	$70
	.byte	$71
	.byte	$72
	.byte	$C3
	.byte	$90
	.byte	$D3
	.byte	$E6
	.byte	$90
	.byte	$C4
	.byte	$C6
	.byte	$E7
	.byte	$D2
	.byte	$BB
	.byte	$BC
	.byte	$BC
	.byte	$5A
	.byte	$54
	.byte	$57
	.byte	$58
	.byte	$90
	.byte	$C6
	.byte	$90
	.byte	$C4
	.byte	$C5
	.byte	$D5
	.byte	$D5
	.byte	$C5
	.byte	$C6
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$92
	.byte	$93
	.byte	$94
	.byte	$D3
	.byte	$C7
	.byte	$E3
	.byte	$E7
	.byte	$D5
	.byte	$E6
	.byte	$E4
	.byte	$D5
	.byte	$D5
	.byte	$C5
	.byte	$C5
	.byte	$E6
	.byte	$90
	.byte	$A2
	.byte	$A3
	.byte	$A4
	.byte	$90
	.byte	$E4
	.byte	$D5
	.byte	$D5
	.byte	$E5
	.byte	$D5
	.byte	$C6
	.byte	$E4
	.byte	$E4
	.byte	$D5
	.byte	$E6
	.byte	$90
	.byte	$90
	.byte	$B2
	.byte	$B3
	.byte	$B4
	.byte	$90
	.byte	$90
	.byte	$E4
	.byte	$E5
	.byte	$E7
	.byte	$E4
	.byte	$D5
	.byte	$C6
	.byte	$E4
	.byte	$E6
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$30
	.byte	$31
	.byte	$31
	.byte	$EB
	.byte	$CE
	.byte	$DE
	.byte	$90
	.byte	$90
	.byte	$90
	.byte	$E4
	.byte	$D5
	.byte	$E6
	.byte	$E6
	.byte	$90
	.byte	$30
	.byte	$31
	.byte	$8A
	.byte	$41
	.byte	$46
	.byte	$91
	.byte	$91
	.byte	$DF
	.byte	$DD
	.byte	$DD
	.byte	$DE
	.byte	$90
	.byte	$D7
	.byte	$90
	.byte	$30
	.byte	$31
	.byte	$8A
	.byte	$46
	.byte	$43
	.byte	$41
	.byte	$41
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$DF
	.byte	$DD
	.byte	$30
	.byte	$31
	.byte	$8A
	.byte	$46
	.byte	$70
	.byte	$71
	.byte	$29
	.byte	$63
	.byte	$64
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$38
	.byte	$31
	.byte	$31
	.byte	$8A
	.byte	$46
	.byte	$50
	.byte	$45
	.byte	$44
	.byte	$51
	.byte	$53
	.byte	$45
	.byte	$41
	.byte	$7D
	.byte	$7E
	.byte	$7F
	.byte	$7C
	.byte	$87
	.byte	$44
	.byte	$45
	.byte	$41
	.byte	$44
	.byte	$45
	.byte	$41
	.byte	$41
	.byte	$41
	.byte	$41
	.byte	$41
	.byte	$46
	.byte	$8D
	.byte	$8E
	.byte	$8F
	.byte	$8F
	.byte	$87
	.byte	$41
	.byte	$46
	.byte	$80
	.byte	$71
	.byte	$72
	.byte	$45
	.byte	$41
	.byte	$41
	.byte	$44
	.byte	$51
	.byte	$51
_leviathanworld_21:
	.byte	$41
	.byte	$40
	.byte	$41
	.byte	$44
	.byte	$50
	.byte	$29
	.byte	$36
	.byte	$36
	.byte	$36
	.byte	$2A
	.byte	$27
	.byte	$6A
	.byte	$C0
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$50
	.byte	$45
	.byte	$46
	.byte	$43
	.byte	$40
	.byte	$41
	.byte	$46
	.byte	$45
	.byte	$40
	.byte	$40
	.byte	$46
	.byte	$27
	.byte	$6A
	.byte	$C0
	.byte	$91
	.byte	$91
	.byte	$42
	.byte	$40
	.byte	$40
	.byte	$44
	.byte	$53
	.byte	$27
	.byte	$28
	.byte	$50
	.byte	$70
	.byte	$71
	.byte	$72
	.byte	$27
	.byte	$47
	.byte	$6A
	.byte	$C0
	.byte	$91
	.byte	$50
	.byte	$43
	.byte	$41
	.byte	$42
	.byte	$46
	.byte	$51
	.byte	$50
	.byte	$43
	.byte	$44
	.byte	$43
	.byte	$44
	.byte	$50
	.byte	$27
	.byte	$37
	.byte	$6A
	.byte	$C0
	.byte	$58
	.byte	$55
	.byte	$58
	.byte	$55
	.byte	$55
	.byte	$55
	.byte	$58
	.byte	$55
	.byte	$58
	.byte	$58
	.byte	$55
	.byte	$58
	.byte	$56
	.byte	$55
	.byte	$C8
	.byte	$B7
	.byte	$93
	.byte	$94
	.byte	$92
	.byte	$93
	.byte	$94
	.byte	$93
	.byte	$94
	.byte	$A4
	.byte	$92
	.byte	$93
	.byte	$94
	.byte	$A4
	.byte	$92
	.byte	$93
	.byte	$94
	.byte	$CB
	.byte	$A2
	.byte	$A3
	.byte	$A4
	.byte	$A2
	.byte	$A3
	.byte	$A4
	.byte	$A2
	.byte	$A2
	.byte	$A3
	.byte	$A4
	.byte	$A4
	.byte	$A4
	.byte	$A2
	.byte	$A3
	.byte	$A4
	.byte	$CB
	.byte	$B2
	.byte	$B3
	.byte	$B2
	.byte	$B2
	.byte	$B3
	.byte	$B4
	.byte	$B3
	.byte	$B4
	.byte	$B2
	.byte	$B3
	.byte	$B4
	.byte	$A4
	.byte	$B2
	.byte	$B2
	.byte	$B3
	.byte	$CB
	.byte	$31
	.byte	$31
	.byte	$32
	.byte	$31
	.byte	$31
	.byte	$0E
	.byte	$0D
	.byte	$31
	.byte	$31
	.byte	$0E
	.byte	$0D
	.byte	$31
	.byte	$31
	.byte	$0E
	.byte	$0D
	.byte	$B8
	.byte	$50
	.byte	$45
	.byte	$40
	.byte	$44
	.byte	$51
	.byte	$45
	.byte	$42
	.byte	$46
	.byte	$29
	.byte	$36
	.byte	$2A
	.byte	$45
	.byte	$40
	.byte	$42
	.byte	$39
	.byte	$D0
	.byte	$40
	.byte	$40
	.byte	$46
	.byte	$50
	.byte	$50
	.byte	$45
	.byte	$41
	.byte	$46
	.byte	$51
	.byte	$27
	.byte	$28
	.byte	$66
	.byte	$67
	.byte	$68
	.byte	$6A
	.byte	$C0
	.byte	$44
	.byte	$43
	.byte	$40
	.byte	$46
	.byte	$51
	.byte	$50
	.byte	$51
	.byte	$45
	.byte	$40
	.byte	$40
	.byte	$44
	.byte	$53
	.byte	$27
	.byte	$47
	.byte	$37
	.byte	$6A
	.byte	$42
	.byte	$41
	.byte	$41
	.byte	$42
	.byte	$44
	.byte	$45
	.byte	$46
	.byte	$27
	.byte	$28
	.byte	$43
	.byte	$42
	.byte	$44
	.byte	$43
	.byte	$40
	.byte	$40
	.byte	$39
	.byte	$50
	.byte	$80
	.byte	$71
	.byte	$72
	.byte	$43
	.byte	$42
	.byte	$40
	.byte	$46
	.byte	$50
	.byte	$45
	.byte	$79
	.byte	$7A
	.byte	$79
	.byte	$7A
	.byte	$39
	.byte	$D0
	.byte	$50
	.byte	$45
	.byte	$7A
	.byte	$7A
	.byte	$7A
	.byte	$7A
	.byte	$7A
	.byte	$7A
	.byte	$7A
	.byte	$39
	.byte	$D0
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
_leviathanworld_22:
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
	.byte	$91
_stage5_levels_list:
_stage5_max_rooms:
	.byte	$01
_stage5_offsets:
	.byte	$00
_stage5_bg_palette:
	.byte	$0F
	.byte	$05
	.byte	$07
	.byte	$29
	.byte	$0F
	.byte	$0C
	.byte	$36
	.byte	$0B
	.byte	$0F
	.byte	$0C
	.byte	$29
	.byte	$1A
	.byte	$0F
	.byte	$15
	.byte	$0C
	.byte	$05
_stage5_palette_sp:
	.byte	$21
	.byte	$17
	.byte	$24
	.byte	$36
	.byte	$21
	.byte	$05
	.byte	$16
	.byte	$37
	.byte	$21
	.byte	$0F
	.byte	$11
	.byte	$30
	.byte	$21
	.byte	$17
	.byte	$3D
	.byte	$37
_stage5_metatiles:
	.byte	$F3
	.byte	$B2
	.byte	$01
	.byte	$D1
	.byte	$01
	.byte	$B2
	.byte	$C3
	.byte	$D1
	.byte	$01
	.byte	$01
	.byte	$B2
	.byte	$F0
	.byte	$01
	.byte	$D1
	.byte	$01
	.byte	$F4
	.byte	$A1
	.byte	$D1
	.byte	$01
	.byte	$01
	.byte	$B1
	.byte	$D2
	.byte	$D1
	.byte	$01
	.byte	$01
	.byte	$B1
	.byte	$B3
	.byte	$D1
	.byte	$01
	.byte	$01
	.byte	$F1
	.byte	$A2
	.byte	$C1
	.byte	$87
	.byte	$01
	.byte	$32
	.byte	$33
	.byte	$38
	.byte	$39
	.byte	$01
	.byte	$D0
	.byte	$A2
	.byte	$01
	.byte	$D1
	.byte	$01
	.byte	$A2
	.byte	$A2
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$B3
	.byte	$B1
	.byte	$01
	.byte	$D1
	.byte	$01
	.byte	$A2
	.byte	$B2
	.byte	$01
	.byte	$D1
	.byte	$01
	.byte	$A2
	.byte	$C3
	.byte	$01
	.byte	$D1
	.byte	$01
	.byte	$61
	.byte	$80
	.byte	$E1
	.byte	$C8
	.byte	$03
	.byte	$91
	.byte	$90
	.byte	$C9
	.byte	$E0
	.byte	$03
	.byte	$93
	.byte	$93
	.byte	$D8
	.byte	$D8
	.byte	$03
	.byte	$F3
	.byte	$B2
	.byte	$00
	.byte	$C1
	.byte	$01
	.byte	$B2
	.byte	$C3
	.byte	$C1
	.byte	$00
	.byte	$01
	.byte	$B2
	.byte	$A3
	.byte	$00
	.byte	$C1
	.byte	$01
	.byte	$A4
	.byte	$A1
	.byte	$C1
	.byte	$00
	.byte	$01
	.byte	$B2
	.byte	$A2
	.byte	$C1
	.byte	$00
	.byte	$01
	.byte	$B1
	.byte	$B3
	.byte	$C1
	.byte	$00
	.byte	$01
	.byte	$F1
	.byte	$B4
	.byte	$C1
	.byte	$00
	.byte	$01
	.byte	$E1
	.byte	$E0
	.byte	$BA
	.byte	$BC
	.byte	$03
	.byte	$B0
	.byte	$A2
	.byte	$00
	.byte	$C1
	.byte	$01
	.byte	$B3
	.byte	$F2
	.byte	$00
	.byte	$C1
	.byte	$01
	.byte	$B3
	.byte	$B1
	.byte	$00
	.byte	$C1
	.byte	$01
	.byte	$A2
	.byte	$B2
	.byte	$00
	.byte	$C1
	.byte	$01
	.byte	$A1
	.byte	$C3
	.byte	$00
	.byte	$C1
	.byte	$01
	.byte	$A1
	.byte	$A2
	.byte	$32
	.byte	$33
	.byte	$01
	.byte	$32
	.byte	$33
	.byte	$32
	.byte	$33
	.byte	$01
	.byte	$61
	.byte	$85
	.byte	$D8
	.byte	$D6
	.byte	$03
	.byte	$60
	.byte	$61
	.byte	$70
	.byte	$71
	.byte	$03
	.byte	$90
	.byte	$94
	.byte	$72
	.byte	$71
	.byte	$03
	.byte	$92
	.byte	$91
	.byte	$72
	.byte	$71
	.byte	$03
	.byte	$90
	.byte	$91
	.byte	$72
	.byte	$71
	.byte	$03
	.byte	$92
	.byte	$94
	.byte	$72
	.byte	$71
	.byte	$03
	.byte	$94
	.byte	$91
	.byte	$FB
	.byte	$E2
	.byte	$03
	.byte	$D8
	.byte	$D8
	.byte	$C8
	.byte	$C9
	.byte	$03
	.byte	$E1
	.byte	$01
	.byte	$C8
	.byte	$D9
	.byte	$03
	.byte	$E0
	.byte	$01
	.byte	$D8
	.byte	$C9
	.byte	$03
	.byte	$C8
	.byte	$D8
	.byte	$01
	.byte	$E2
	.byte	$03
	.byte	$D8
	.byte	$C9
	.byte	$E2
	.byte	$E0
	.byte	$03
	.byte	$E1
	.byte	$73
	.byte	$E6
	.byte	$E2
	.byte	$03
	.byte	$32
	.byte	$33
	.byte	$36
	.byte	$37
	.byte	$01
	.byte	$34
	.byte	$33
	.byte	$32
	.byte	$35
	.byte	$01
	.byte	$61
	.byte	$63
	.byte	$E0
	.byte	$81
	.byte	$03
	.byte	$61
	.byte	$64
	.byte	$E0
	.byte	$74
	.byte	$03
	.byte	$60
	.byte	$61
	.byte	$CA
	.byte	$D8
	.byte	$03
	.byte	$91
	.byte	$80
	.byte	$D8
	.byte	$D9
	.byte	$03
	.byte	$62
	.byte	$61
	.byte	$D8
	.byte	$D8
	.byte	$03
	.byte	$94
	.byte	$84
	.byte	$71
	.byte	$9C
	.byte	$03
	.byte	$61
	.byte	$94
	.byte	$D8
	.byte	$D9
	.byte	$03
	.byte	$61
	.byte	$84
	.byte	$D8
	.byte	$D6
	.byte	$03
	.byte	$D8
	.byte	$D9
	.byte	$E2
	.byte	$E0
	.byte	$03
	.byte	$C8
	.byte	$C9
	.byte	$D8
	.byte	$D8
	.byte	$03
	.byte	$83
	.byte	$80
	.byte	$CA
	.byte	$D8
	.byte	$03
	.byte	$D8
	.byte	$E9
	.byte	$E9
	.byte	$EA
	.byte	$03
	.byte	$C5
	.byte	$E4
	.byte	$D5
	.byte	$C8
	.byte	$03
	.byte	$C5
	.byte	$E4
	.byte	$CA
	.byte	$D9
	.byte	$03
	.byte	$E4
	.byte	$E4
	.byte	$D8
	.byte	$D8
	.byte	$03
	.byte	$0A
	.byte	$0B
	.byte	$0A
	.byte	$0B
	.byte	$01
	.byte	$0A
	.byte	$0B
	.byte	$0A
	.byte	$0B
	.byte	$01
	.byte	$EC
	.byte	$EC
	.byte	$DB
	.byte	$DB
	.byte	$01
	.byte	$D8
	.byte	$D9
	.byte	$D8
	.byte	$D8
	.byte	$03
	.byte	$D8
	.byte	$D8
	.byte	$D9
	.byte	$D9
	.byte	$03
	.byte	$D8
	.byte	$D8
	.byte	$D8
	.byte	$D8
	.byte	$03
	.byte	$E1
	.byte	$C8
	.byte	$C8
	.byte	$D9
	.byte	$03
	.byte	$C9
	.byte	$E1
	.byte	$D8
	.byte	$C9
	.byte	$03
	.byte	$C8
	.byte	$D8
	.byte	$E0
	.byte	$C8
	.byte	$03
	.byte	$D9
	.byte	$C9
	.byte	$C9
	.byte	$E2
	.byte	$03
	.byte	$E0
	.byte	$E1
	.byte	$D9
	.byte	$D8
	.byte	$03
	.byte	$87
	.byte	$8B
	.byte	$A8
	.byte	$E1
	.byte	$03
	.byte	$96
	.byte	$99
	.byte	$D9
	.byte	$D6
	.byte	$03
	.byte	$D5
	.byte	$C8
	.byte	$E5
	.byte	$C4
	.byte	$03
	.byte	$D5
	.byte	$C8
	.byte	$E5
	.byte	$C4
	.byte	$03
	.byte	$D9
	.byte	$D9
	.byte	$C4
	.byte	$C4
	.byte	$03
	.byte	$1A
	.byte	$1B
	.byte	$1A
	.byte	$1B
	.byte	$01
	.byte	$1A
	.byte	$1B
	.byte	$1A
	.byte	$1B
	.byte	$01
	.byte	$DB
	.byte	$DB
	.byte	$DC
	.byte	$DC
	.byte	$01
	.byte	$E0
	.byte	$E1
	.byte	$01
	.byte	$E2
	.byte	$03
	.byte	$E0
	.byte	$E0
	.byte	$E1
	.byte	$E0
	.byte	$03
	.byte	$C9
	.byte	$E0
	.byte	$E8
	.byte	$E8
	.byte	$03
	.byte	$E0
	.byte	$9D
	.byte	$E2
	.byte	$E1
	.byte	$03
	.byte	$E0
	.byte	$E0
	.byte	$71
	.byte	$F8
	.byte	$03
	.byte	$D8
	.byte	$D8
	.byte	$71
	.byte	$F8
	.byte	$03
	.byte	$D8
	.byte	$D9
	.byte	$F5
	.byte	$72
	.byte	$03
	.byte	$C8
	.byte	$D8
	.byte	$F8
	.byte	$F8
	.byte	$03
	.byte	$C9
	.byte	$C8
	.byte	$71
	.byte	$F8
	.byte	$03
	.byte	$F7
	.byte	$F8
	.byte	$00
	.byte	$00
	.byte	$03
	.byte	$FB
	.byte	$E0
	.byte	$F7
	.byte	$F8
	.byte	$03
	.byte	$F7
	.byte	$FB
	.byte	$00
	.byte	$F7
	.byte	$03
	.byte	$E7
	.byte	$E6
	.byte	$02
	.byte	$E7
	.byte	$03
	.byte	$E4
	.byte	$30
	.byte	$D8
	.byte	$D6
	.byte	$03
	.byte	$DB
	.byte	$EB
	.byte	$EB
	.byte	$DB
	.byte	$01
	.byte	$DB
	.byte	$DB
	.byte	$DB
	.byte	$EB
	.byte	$01
	.byte	$62
	.byte	$84
	.byte	$72
	.byte	$2F
	.byte	$03
	.byte	$D8
	.byte	$D6
	.byte	$F8
	.byte	$F8
	.byte	$03
	.byte	$D8
	.byte	$D8
	.byte	$B6
	.byte	$B9
	.byte	$03
	.byte	$D9
	.byte	$D9
	.byte	$BA
	.byte	$BA
	.byte	$03
	.byte	$D8
	.byte	$D9
	.byte	$B9
	.byte	$C8
	.byte	$03
	.byte	$CA
	.byte	$D8
	.byte	$71
	.byte	$F8
	.byte	$03
	.byte	$B6
	.byte	$B9
	.byte	$E0
	.byte	$E1
	.byte	$03
	.byte	$BA
	.byte	$BB
	.byte	$C8
	.byte	$D8
	.byte	$03
	.byte	$BC
	.byte	$B6
	.byte	$D8
	.byte	$D8
	.byte	$03
	.byte	$BA
	.byte	$E2
	.byte	$D8
	.byte	$C9
	.byte	$03
	.byte	$96
	.byte	$9A
	.byte	$C9
	.byte	$A6
	.byte	$03
	.byte	$E8
	.byte	$D9
	.byte	$E7
	.byte	$D7
	.byte	$03
	.byte	$FC
	.byte	$DB
	.byte	$00
	.byte	$FC
	.byte	$01
	.byte	$00
	.byte	$CC
	.byte	$CC
	.byte	$DB
	.byte	$01
	.byte	$DB
	.byte	$EB
	.byte	$FC
	.byte	$FC
	.byte	$01
	.byte	$CC
	.byte	$CC
	.byte	$DB
	.byte	$DB
	.byte	$01
	.byte	$E1
	.byte	$B6
	.byte	$B6
	.byte	$BA
	.byte	$03
	.byte	$B7
	.byte	$B8
	.byte	$C7
	.byte	$E1
	.byte	$03
	.byte	$B9
	.byte	$BB
	.byte	$E1
	.byte	$E0
	.byte	$03
	.byte	$66
	.byte	$92
	.byte	$3F
	.byte	$F8
	.byte	$03
	.byte	$D5
	.byte	$E1
	.byte	$D5
	.byte	$E2
	.byte	$03
	.byte	$E1
	.byte	$7D
	.byte	$2F
	.byte	$FA
	.byte	$03
	.byte	$D8
	.byte	$D8
	.byte	$F8
	.byte	$AD
	.byte	$03
	.byte	$E8
	.byte	$E6
	.byte	$02
	.byte	$E7
	.byte	$03
	.byte	$D7
	.byte	$D8
	.byte	$E7
	.byte	$E8
	.byte	$03
	.byte	$D8
	.byte	$D8
	.byte	$E8
	.byte	$E8
	.byte	$03
	.byte	$D9
	.byte	$D8
	.byte	$E9
	.byte	$E8
	.byte	$03
	.byte	$E1
	.byte	$E1
	.byte	$FB
	.byte	$E2
	.byte	$03
	.byte	$DF
	.byte	$DD
	.byte	$FE
	.byte	$FF
	.byte	$02
	.byte	$DE
	.byte	$CE
	.byte	$EF
	.byte	$FF
	.byte	$02
	.byte	$CE
	.byte	$DF
	.byte	$EE
	.byte	$03
	.byte	$02
	.byte	$CE
	.byte	$DF
	.byte	$FD
	.byte	$EF
	.byte	$02
	.byte	$B6
	.byte	$B9
	.byte	$BB
	.byte	$BC
	.byte	$03
	.byte	$D8
	.byte	$D6
	.byte	$D8
	.byte	$D9
	.byte	$03
	.byte	$C9
	.byte	$D4
	.byte	$D8
	.byte	$D6
	.byte	$03
	.byte	$D8
	.byte	$D6
	.byte	$D8
	.byte	$D6
	.byte	$03
	.byte	$E0
	.byte	$E3
	.byte	$B9
	.byte	$BA
	.byte	$03
	.byte	$C9
	.byte	$E3
	.byte	$D8
	.byte	$D6
	.byte	$03
	.byte	$D3
	.byte	$C8
	.byte	$CA
	.byte	$D9
	.byte	$03
	.byte	$CA
	.byte	$D9
	.byte	$CA
	.byte	$D9
	.byte	$03
	.byte	$D5
	.byte	$C8
	.byte	$CA
	.byte	$D8
	.byte	$03
	.byte	$CA
	.byte	$D8
	.byte	$CA
	.byte	$C9
	.byte	$03
	.byte	$CA
	.byte	$D8
	.byte	$D8
	.byte	$D9
	.byte	$03
	.byte	$9E
	.byte	$00
	.byte	$9E
	.byte	$00
	.byte	$02
	.byte	$ED
	.byte	$FD
	.byte	$13
	.byte	$13
	.byte	$02
	.byte	$FD
	.byte	$FE
	.byte	$13
	.byte	$13
	.byte	$02
	.byte	$FD
	.byte	$FF
	.byte	$13
	.byte	$13
	.byte	$02
	.byte	$FD
	.byte	$EE
	.byte	$13
	.byte	$13
	.byte	$02
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$02
	.byte	$02
	.byte	$02
	.byte	$02
	.byte	$03
	.byte	$5E
	.byte	$5E
	.byte	$6E
	.byte	$6F
	.byte	$00
	.byte	$5F
	.byte	$5E
	.byte	$6E
	.byte	$6E
	.byte	$00
	.byte	$5E
	.byte	$5F
	.byte	$6E
	.byte	$6E
	.byte	$00
	.byte	$6B
	.byte	$5C
	.byte	$6B
	.byte	$5C
	.byte	$00
	.byte	$6C
	.byte	$5C
	.byte	$6C
	.byte	$5C
	.byte	$00
	.byte	$5C
	.byte	$6C
	.byte	$6C
	.byte	$6C
	.byte	$00
	.byte	$5C
	.byte	$6D
	.byte	$5C
	.byte	$5D
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$03
	.byte	$01
	.byte	$01
	.byte	$77
	.byte	$78
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$79
	.byte	$01
	.byte	$01
	.byte	$BE
	.byte	$00
	.byte	$BE
	.byte	$00
	.byte	$02
	.byte	$AE
	.byte	$00
	.byte	$AE
	.byte	$00
	.byte	$02
	.byte	$8F
	.byte	$00
	.byte	$9F
	.byte	$00
	.byte	$02
	.byte	$CD
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$02
	.byte	$17
	.byte	$17
	.byte	$A0
	.byte	$19
	.byte	$01
	.byte	$17
	.byte	$17
	.byte	$14
	.byte	$A5
	.byte	$01
	.byte	$6F
	.byte	$6E
	.byte	$6E
	.byte	$6E
	.byte	$00
	.byte	$6E
	.byte	$6F
	.byte	$6E
	.byte	$6E
	.byte	$00
	.byte	$6E
	.byte	$6E
	.byte	$6E
	.byte	$6E
	.byte	$00
	.byte	$6B
	.byte	$5C
	.byte	$5B
	.byte	$6C
	.byte	$00
	.byte	$6C
	.byte	$5C
	.byte	$5C
	.byte	$5C
	.byte	$00
	.byte	$5C
	.byte	$6C
	.byte	$6C
	.byte	$5C
	.byte	$00
	.byte	$5C
	.byte	$6D
	.byte	$5C
	.byte	$6D
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$03
	.byte	$01
	.byte	$01
	.byte	$F8
	.byte	$F8
	.byte	$01
	.byte	$FB
	.byte	$01
	.byte	$F7
	.byte	$FB
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$7D
	.byte	$7D
	.byte	$FA
	.byte	$01
	.byte	$AF
	.byte	$00
	.byte	$BF
	.byte	$00
	.byte	$02
	.byte	$01
	.byte	$BD
	.byte	$01
	.byte	$01
	.byte	$02
	.byte	$01
	.byte	$01
	.byte	$C0
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$C2
	.byte	$01
	.byte	$6E
	.byte	$6F
	.byte	$7F
	.byte	$7E
	.byte	$00
	.byte	$6E
	.byte	$6E
	.byte	$7E
	.byte	$7F
	.byte	$00
	.byte	$6E
	.byte	$6F
	.byte	$7E
	.byte	$7F
	.byte	$00
	.byte	$00
	.byte	$09
	.byte	$08
	.byte	$00
	.byte	$01
	.byte	$00
	.byte	$F7
	.byte	$00
	.byte	$00
	.byte	$03
	.byte	$A6
	.byte	$9A
	.byte	$E9
	.byte	$EA
	.byte	$03
	.byte	$85
	.byte	$9A
	.byte	$E9
	.byte	$EA
	.byte	$03
	.byte	$F6
	.byte	$F8
	.byte	$FA
	.byte	$00
	.byte	$01
	.byte	$F8
	.byte	$FB
	.byte	$00
	.byte	$F7
	.byte	$01
	.byte	$00
	.byte	$F7
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$F8
	.byte	$F9
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$FA
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$CF
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$02
	.byte	$7D
	.byte	$FA
	.byte	$FA
	.byte	$00
	.byte	$01
	.byte	$02
	.byte	$02
	.byte	$9A
	.byte	$02
	.byte	$03
	.byte	$02
	.byte	$E7
	.byte	$02
	.byte	$02
	.byte	$03
	.byte	$19
	.byte	$16
	.byte	$00
	.byte	$19
	.byte	$01
	.byte	$07
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$02
	.byte	$00
	.byte	$09
	.byte	$09
	.byte	$04
	.byte	$01
	.byte	$09
	.byte	$08
	.byte	$07
	.byte	$16
	.byte	$01
	.byte	$00
	.byte	$00
	.byte	$14
	.byte	$00
	.byte	$01
	.byte	$09
	.byte	$14
	.byte	$08
	.byte	$16
	.byte	$01
	.byte	$C9
	.byte	$E1
	.byte	$71
	.byte	$F8
	.byte	$03
	.byte	$F6
	.byte	$F9
	.byte	$A7
	.byte	$89
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$F6
	.byte	$F8
	.byte	$01
	.byte	$31
	.byte	$02
	.byte	$31
	.byte	$02
	.byte	$03
	.byte	$01
	.byte	$67
	.byte	$68
	.byte	$F6
	.byte	$01
	.byte	$87
	.byte	$88
	.byte	$8B
	.byte	$78
	.byte	$01
	.byte	$89
	.byte	$8A
	.byte	$01
	.byte	$A7
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$FB
	.byte	$01
	.byte	$01
	.byte	$EA
	.byte	$02
	.byte	$02
	.byte	$02
	.byte	$03
	.byte	$02
	.byte	$02
	.byte	$02
	.byte	$97
	.byte	$03
	.byte	$00
	.byte	$00
	.byte	$09
	.byte	$00
	.byte	$01
	.byte	$08
	.byte	$09
	.byte	$00
	.byte	$19
	.byte	$01
	.byte	$17
	.byte	$16
	.byte	$16
	.byte	$17
	.byte	$01
	.byte	$17
	.byte	$06
	.byte	$06
	.byte	$06
	.byte	$01
	.byte	$17
	.byte	$09
	.byte	$16
	.byte	$14
	.byte	$01
	.byte	$19
	.byte	$18
	.byte	$00
	.byte	$00
	.byte	$01
	.byte	$88
	.byte	$8B
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$FA
	.byte	$00
	.byte	$8A
	.byte	$00
	.byte	$01
	.byte	$68
	.byte	$69
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$03
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$87
	.byte	$01
	.byte	$00
	.byte	$00
	.byte	$88
	.byte	$88
	.byte	$01
	.byte	$00
	.byte	$00
	.byte	$8A
	.byte	$00
	.byte	$01
	.byte	$A7
	.byte	$8A
	.byte	$78
	.byte	$A7
	.byte	$01
	.byte	$00
	.byte	$00
	.byte	$A0
	.byte	$00
	.byte	$01
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$87
	.byte	$03
	.byte	$00
	.byte	$00
	.byte	$00
	.byte	$A5
	.byte	$01
	.byte	$00
	.byte	$00
	.byte	$09
	.byte	$08
	.byte	$01
	.byte	$19
	.byte	$06
	.byte	$00
	.byte	$18
	.byte	$01
	.byte	$06
	.byte	$06
	.byte	$19
	.byte	$18
	.byte	$01
	.byte	$06
	.byte	$09
	.byte	$18
	.byte	$00
	.byte	$01
	.byte	$18
	.byte	$00
	.byte	$00
	.byte	$08
	.byte	$01
	.byte	$01
	.byte	$F6
	.byte	$F6
	.byte	$FA
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$F6
	.byte	$01
	.byte	$78
	.byte	$79
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$88
	.byte	$88
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$87
	.byte	$8B
	.byte	$8B
	.byte	$01
	.byte	$01
	.byte	$00
	.byte	$87
	.byte	$88
	.byte	$8B
	.byte	$01
	.byte	$86
	.byte	$89
	.byte	$01
	.byte	$01
	.byte	$01
	.byte	$8A
	.byte	$00
	.byte	$86
	.byte	$89
	.byte	$01
_stage5_starting_x_place:
	.byte	$1E
_stage5_starting_y_place:
	.byte	$A0
_stage5_level_0_enemies:
	.byte	$FF
_stage5_Enemy_list:
	.addr	_stage5_level_0_enemies
_stage5_level_0_entities:
	.byte	$FF
_stage5_entity_list:
	.addr	_stage5_level_0_entities
.segment	"BANK5"
.segment	"BANK6"
.segment	"CODE"
_palette_bg:
	.byte	$21
	.byte	$0F
	.byte	$00
	.byte	$10
	.byte	$21
	.byte	$0F
	.byte	$30
	.byte	$08
	.byte	$21
	.byte	$0F
	.byte	$17
	.byte	$06
	.byte	$21
	.byte	$0F
	.byte	$19
	.byte	$29
_palette_sp:
	.byte	$21
	.byte	$17
	.byte	$24
	.byte	$36
	.byte	$21
	.byte	$05
	.byte	$16
	.byte	$37
	.byte	$21
	.byte	$0F
	.byte	$11
	.byte	$30
	.byte	$21
	.byte	$17
	.byte	$3D
	.byte	$37
L1C9C:
	.byte	$42,$52,$49,$41,$4E,$20,$26,$20,$41,$4C,$41,$4E,$20,$47,$41,$4D
	.byte	$45,$53,$00
L1C88:
	.byte	$4E,$4F,$41,$48,$20,$56,$53,$2E,$20,$41,$54,$52,$41,$48,$41,$53
	.byte	$49,$53,$00
L6C75:
	.byte	$53,$45,$4C,$45,$43,$54,$20,$53,$54,$41,$47,$45,$00
L6C84	:=	L6C75+0
L6CF7:
	.byte	$20,$20,$20,$4C,$45,$56,$49,$41,$54,$48,$41,$4E,$00
L6CEC:
	.byte	$58,$20,$20,$4C,$45,$56,$49,$41,$54,$48,$41,$4E,$00
L6CA7:
	.byte	$58,$20,$20,$47,$49,$52,$41,$46,$46,$45,$00
L6CB2:
	.byte	$20,$20,$20,$47,$49,$52,$41,$46,$46,$45,$00
L1C92:
	.byte	$4F,$4E,$45,$20,$4F,$4E,$20,$4F,$4E,$45,$00
L6CC9:
	.byte	$20,$20,$20,$57,$41,$4C,$52,$55,$53,$00
L6CBE:
	.byte	$58,$20,$20,$57,$41,$4C,$52,$55,$53,$00
L6CE0:
	.byte	$20,$20,$20,$43,$41,$4D,$45,$4C,$00
L6CD5:
	.byte	$58,$20,$20,$43,$41,$4D,$45,$4C,$00
L6C90:
	.byte	$58,$20,$20,$42,$45,$41,$52,$00
L6C9B:
	.byte	$20,$20,$20,$42,$45,$41,$52,$00

.segment	"BSS"

_bankLevel:
	.res	1,$00
_bankBuffer:
	.res	10,$00
.segment	"ZEROPAGE"
_direction:
	.res	1,$00
_direction_y:
	.res	1,$00
_arg1:
	.res	1,$00
_arg2:
	.res	1,$00
_pad1:
	.res	1,$00
_pad1_new:
	.res	1,$00
_pad1_state:
	.res	1,$00
_char_state:
	.res	1,$00
_song:
	.res	1,$00
_sound:
	.res	1,$00
_game_mode:
	.res	1,$00
_selected_stage:
	.res	1,$00
_temp_x:
	.res	2,$00
_temp_y:
	.res	2,$00
_temp_sprite:
	.res	2,$00
_old_x:
	.res	1,$00
_old_y:
	.res	1,$00
_player_in_air:
	.res	1,$00
_player_is_sliding:
	.res	1,$00
_player_is_running:
	.res	1,$00
_player_on_ladder:
	.res	1,$00
_player_on_ladder_pose:
	.res	1,$00
_player_on_ladder_top:
	.res	1,$00
_player_in_hitstun:
	.res	1,$00
_falling_down:
	.res	1,$00
_multi_jump:
	.res	1,$00
_multi_jump_max:
	.res	1,$00
_short_jump_count:
	.res	1,$00
_player_shooting:
	.res	1,$00
_projectile_cooldown:
	.res	1,$00
_projectile_index:
	.res	1,$00
_hit_direction:
	.res	1,$00
_animation_frame_counter:
	.res	1,$00
_current_animation_frame:
	.res	1,$00
_current_animation_ptr:
	.res	2,$00
_eject_L:
	.res	1,$00
_eject_R:
	.res	1,$00
_eject_D:
	.res	1,$00
_eject_U:
	.res	1,$00
_collision_L:
	.res	1,$00
_collision_R:
	.res	1,$00
_collision_U:
	.res	1,$00
_collision_D:
	.res	1,$00
_coordinates:
	.res	1,$00
_location_with_scroll:
	.res	2,$00
_collision:
	.res	1,$00
_map_loaded:
	.res	1,$00
_scrolling_direction:
	.res	1,$00
_nametable_to_load:
	.res	1,$00
_offset:
	.res	1,$00
_address:
	.res	2,$00
_x:
	.res	1,$00
_y:
	.res	1,$00
_nt:
	.res	1,$00
_index:
	.res	1,$00
_map:
	.res	1,$00
_scroll_x:
	.res	2,$00
_scroll_y:
	.res	2,$00
_max_rooms:
	.res	1,$00
_max_scroll:
	.res	2,$00
_temp_room:
	.res	1,$00
_scroll_count:
	.res	1,$00
_pseudo_scroll_x:
	.res	2,$00
.segment	"BSS"
_c_map:
	.res	240,$00
_c_map2:
	.res	240,$00
_temp1:
	.res	2,$00
_temp2:
	.res	2,$00
_temp3:
	.res	2,$00
_l_scroll_frames:
	.res	1,$00
_r_scroll_frames:
	.res	1,$00
_Generic:
	.res	4,$00
_Generic2:
	.res	4,$00
.segment	"XRAM"
_wram_array:
	.res	8192,$00

; ---------------------------------------------------------------
; void __near__ banked_call (unsigned char, __near__ __near__ function returning void *)
; ---------------------------------------------------------------

.segment	"CODE"

.proc	_banked_call: near

.segment	"CODE"

;
; void banked_call(unsigned char bankId, void (*method)(void)) {
;
	jsr     pushax
;
; bank_push(bankId);
;
	ldy     #$02
	lda     (sp),y
	jsr     _bank_push
;
; (*method)();
;
	ldy     #$01
	lda     (sp),y
	tax
	dey
	lda     (sp),y
	jsr     callax
;
; bank_pop();
;
	jsr     _bank_pop
;
; }
;
	jmp     incsp3

.endproc

; ---------------------------------------------------------------
; void __near__ bank_push (unsigned char)
; ---------------------------------------------------------------

.segment	"CODE"

.proc	_bank_push: near

.segment	"CODE"

;
; void bank_push(unsigned char bankId) {
;
	jsr     pusha
;
; bankBuffer[bankLevel] = get_prg_bank();
;
	lda     #<(_bankBuffer)
	ldx     #>(_bankBuffer)
	clc
	adc     _bankLevel
	bcc     L000B
	inx
L000B:	jsr     pushax
	jsr     _get_prg_bank
	ldy     #$00
	jsr     staspidx
;
; ++bankLevel;
;
	inc     _bankLevel
;
; set_prg_bank(bankId);
;
	ldy     #$00
	lda     (sp),y
	jsr     _set_prg_bank
;
; }
;
	jmp     incsp1

.endproc

; ---------------------------------------------------------------
; void __near__ bank_pop (void)
; ---------------------------------------------------------------

.segment	"CODE"

.proc	_bank_pop: near

.segment	"CODE"

;
; if (bankLevel != 0) {
;
	lda     _bankLevel
	beq     L0011
;
; --bankLevel;
;
	dec     _bankLevel
;
; set_prg_bank(bankBuffer[bankLevel]);
;
	ldy     _bankLevel
	lda     _bankBuffer,y
	jmp     _set_prg_bank
;
; }
;
L0011:	rts

.endproc

; ---------------------------------------------------------------
; unsigned char __near__ bg_collision_sub (void)
; ---------------------------------------------------------------

.segment	"BANK0"

.proc	_bg_collision_sub: near

.segment	"BANK0"

;
; if (temp_y >= 0xf0)
;
	lda     _temp_y
	cmp     #$F0
	lda     _temp_y+1
	sbc     #$00
	bcc     L0374
;
; return 0;
;
	ldx     #$00
	txa
	rts
;
; coordinates = (temp_x >> 4) + (temp_y & 0xf0);
;
L0374:	lda     _temp_x
	ldx     _temp_x+1
	jsr     shrax4
	sta     ptr1
	lda     _temp_y
	and     #$F0
	clc
	adc     ptr1
	sta     _coordinates
;
; map = temp_room & 1; // high byte
;
	lda     _temp_room
	and     #$01
	sta     _map
;
; if (!map)
;
	lda     _map
	bne     L037D
;
; collision = c_map[coordinates];
;
	ldy     _coordinates
	lda     _c_map,y
;
; else
;
	jmp     L6D74
;
; collision = c_map2[coordinates];
;
L037D:	ldy     _coordinates
	lda     _c_map2,y
L6D74:	sta     _collision
;
; return metatile_colision_map[collision];
;
	ldy     _collision
	ldx     #$00
	lda     _metatile_colision_map,y
;
; }
;
	rts

.endproc

; ---------------------------------------------------------------
; void __near__ bg_collision_fast (void)
; ---------------------------------------------------------------

.segment	"BANK0"

.proc	_bg_collision_fast: near

.segment	"BANK0"

;
; collision_L = 0;
;
	lda     #$00
	sta     _collision_L
;
; collision_R = 0;
;
	sta     _collision_R
;
; collision_D = 0;
;
	sta     _collision_D
;
; if (Generic.y >= 0xf0)
;
	lda     _Generic+1
	cmp     #$F0
;
; return;
;
	bcc     L6D78
;
; }
;
	rts
;
; location_with_scroll = Generic.x + scroll_x;
;
L6D78:	lda     _Generic
	clc
	adc     _scroll_x
	pha
	lda     #$00
	adc     _scroll_x+1
	sta     _location_with_scroll+1
	pla
	sta     _location_with_scroll
;
; temp_x = location_with_scroll & 0xff;  // low byte x
;
	ldx     #$00
	sta     _temp_x
	stx     _temp_x+1
;
; temp_room = location_with_scroll >> 8; // high byte x
;
	lda     _location_with_scroll+1
	sta     _temp_room
;
; temp_y = Generic.y + 6; // y middle
;
	lda     _Generic+1
	clc
	adc     #$06
	bcc     L039C
	inx
L039C:	sta     _temp_y
	stx     _temp_y+1
;
; bg_collision_sub();
;
	jsr     _bg_collision_sub
;
; if (bg_collision_sub() & COL_ALL)
;
	jsr     _bg_collision_sub
	and     #$40
	beq     L6D76
;
; ++collision_L;
;
	inc     _collision_L
;
; location_with_scroll += Generic.width;
;
L6D76:	lda     _Generic+2
	clc
	adc     _location_with_scroll
	sta     _location_with_scroll
	lda     #$00
	adc     _location_with_scroll+1
	sta     _location_with_scroll+1
;
; temp_x = location_with_scroll & 0xff;  // low byte x
;
	lda     #$00
	sta     _temp_x+1
	lda     _location_with_scroll
	sta     _temp_x
;
; temp_room = location_with_scroll >> 8; // high byte x
;
	lda     _location_with_scroll+1
	sta     _temp_room
;
; bg_collision_sub();
;
	jsr     _bg_collision_sub
;
; if (bg_collision_sub() & COL_ALL)
;
	jsr     _bg_collision_sub
	and     #$40
	beq     L6D77
;
; ++collision_R;
;
	inc     _collision_R
;
; location_with_scroll -= (Generic.width >> 1); // middle of character
;
L6D77:	lda     _Generic+2
	lsr     a
	eor     #$FF
	sec
	adc     _location_with_scroll
	sta     _location_with_scroll
	lda     #$FF
	adc     _location_with_scroll+1
	sta     _location_with_scroll+1
;
; temp_x = (char)location_with_scroll;          // low byte
;
	lda     #$00
	sta     _temp_x+1
	lda     _location_with_scroll
	sta     _temp_x
;
; temp_y += 6;
;
	lda     #$06
	clc
	adc     _temp_y
	sta     _temp_y
	bcc     L03B2
	inc     _temp_y+1
;
; if (bg_collision_sub() & COL_ALL)
;
L03B2:	jsr     _bg_collision_sub
	and     #$40
	beq     L03B3
;
; ++collision_D;
;
	inc     _collision_D
;
; }
;
L03B3:	rts

.endproc

; ---------------------------------------------------------------
; unsigned char __near__ bg_coll_L (void)
; ---------------------------------------------------------------

.segment	"BANK0"

.proc	_bg_coll_L: near

.segment	"BANK0"

;
; location_with_scroll = Generic.x + scroll_x;
;
	lda     _Generic
	clc
	adc     _scroll_x
	sta     _location_with_scroll
	lda     #$00
	adc     _scroll_x+1
	sta     _location_with_scroll+1
;
; temp_x = (char)location_with_scroll;   // low byte
;
	lda     #$00
	sta     _temp_x+1
	lda     _location_with_scroll
	sta     _temp_x
;
; temp_room = location_with_scroll >> 8; // high byte
;
	lda     _location_with_scroll+1
	sta     _temp_room
;
; eject_L = temp_x | 0xf0;
;
	lda     _temp_x
	ora     #$F0
	sta     _eject_L
;
; temp_y = Generic.y + 2;
;
	ldx     #$00
	lda     _Generic+1
	clc
	adc     #$02
	bcc     L03C1
	inx
L03C1:	sta     _temp_y
	stx     _temp_y+1
;
; if (bg_collision_sub() & COL_ALL)
;
	jsr     _bg_collision_sub
	ldx     #$00
	and     #$40
	beq     L6D7A
;
; return 1;
;
	lda     #$01
	rts
;
; temp_y = Generic.y + Generic.height;
;
L6D7A:	lda     _Generic+1
	clc
	adc     _Generic+3
	bcc     L6D79
	inx
L6D79:	sta     _temp_y
	stx     _temp_y+1
;
; temp_y -= 2;
;
	lda     _temp_y
	sec
	sbc     #$02
	sta     _temp_y
	bcs     L03C9
	dec     _temp_y+1
;
; if (bg_collision_sub() & COL_ALL)
;
L03C9:	jsr     _bg_collision_sub
	ldx     #$00
	and     #$40
	beq     L6D7C
;
; return 1;
;
	lda     #$01
	rts
;
; }
;
L6D7C:	rts

.endproc

; ---------------------------------------------------------------
; unsigned char __near__ bg_coll_R (void)
; ---------------------------------------------------------------

.segment	"BANK0"

.proc	_bg_coll_R: near

.segment	"BANK0"

;
; location_with_scroll = Generic.x + scroll_x + Generic.width;
;
	lda     _Generic
	clc
	adc     _scroll_x
	pha
	lda     #$00
	adc     _scroll_x+1
	tax
	pla
	clc
	adc     _Generic+2
	bcc     L6D7D
	inx
L6D7D:	sta     _location_with_scroll
	stx     _location_with_scroll+1
;
; temp_x = (char)location_with_scroll;   // low byte
;
	lda     #$00
	sta     _temp_x+1
	lda     _location_with_scroll
	sta     _temp_x
;
; temp_room = location_with_scroll >> 8; // high byte
;
	lda     _location_with_scroll+1
	sta     _temp_room
;
; eject_R = (temp_x + 1) & 0x0f;
;
	lda     _temp_x
	clc
	adc     #$01
	and     #$0F
	sta     _eject_R
;
; temp_y = Generic.y + 2;
;
	ldx     #$00
	lda     _Generic+1
	clc
	adc     #$02
	bcc     L03DB
	inx
L03DB:	sta     _temp_y
	stx     _temp_y+1
;
; if (bg_collision_sub() & COL_ALL)
;
	jsr     _bg_collision_sub
	ldx     #$00
	and     #$40
	beq     L6D7F
;
; return 1;
;
	lda     #$01
	rts
;
; temp_y = Generic.y + Generic.height;
;
L6D7F:	lda     _Generic+1
	clc
	adc     _Generic+3
	bcc     L6D7E
	inx
L6D7E:	sta     _temp_y
	stx     _temp_y+1
;
; temp_y -= 2;
;
	lda     _temp_y
	sec
	sbc     #$02
	sta     _temp_y
	bcs     L03E3
	dec     _temp_y+1
;
; if (bg_collision_sub() & COL_ALL)
;
L03E3:	jsr     _bg_collision_sub
	ldx     #$00
	and     #$40
	beq     L6D81
;
; return 1;
;
	lda     #$01
	rts
;
; }
;
L6D81:	rts

.endproc

; ---------------------------------------------------------------
; unsigned char __near__ bg_coll_ladder (void)
; ---------------------------------------------------------------

.segment	"BANK0"

.proc	_bg_coll_ladder: near

.segment	"BANK0"

;
; location_with_scroll = Generic.x + scroll_x + 7;
;
	lda     _Generic
	clc
	adc     _scroll_x
	pha
	lda     #$00
	adc     _scroll_x+1
	tax
	pla
	clc
	adc     #$07
	bcc     L03EB
	inx
L03EB:	sta     _location_with_scroll
	stx     _location_with_scroll+1
;
; temp_x = (char)location_with_scroll; // low byte
;
	lda     #$00
	sta     _temp_x+1
	lda     _location_with_scroll
	sta     _temp_x
;
; temp_y = Generic.y + Generic.height;
;
	ldx     #$00
	lda     _Generic+1
	clc
	adc     _Generic+3
	bcc     L6D82
	inx
L6D82:	sta     _temp_y
	stx     _temp_y+1
;
; temp_y -= 2;
;
	lda     _temp_y
	sec
	sbc     #$02
	sta     _temp_y
	bcs     L03F2
	dec     _temp_y+1
;
; if (bg_collision_sub() & COL_LADDER || bg_collision_sub() & COL_LADDER_TOP)
;
L03F2:	jsr     _bg_collision_sub
	and     #$01
	bne     L6D83
	jsr     _bg_collision_sub
	and     #$02
	bne     L6D83
	tax
	rts
;
; return location_with_scroll;
;
L6D83:	ldx     #$00
	lda     _location_with_scroll
	rts

.endproc

; ---------------------------------------------------------------
; unsigned char __near__ bg_coll_ladder_top_at_player (void)
; ---------------------------------------------------------------

.segment	"BANK0"

.proc	_bg_coll_ladder_top_at_player: near

.segment	"BANK0"

;
; location_with_scroll = Generic.x + scroll_x + 7;
;
	lda     _Generic
	clc
	adc     _scroll_x
	pha
	lda     #$00
	adc     _scroll_x+1
	tax
	pla
	clc
	adc     #$07
	bcc     L03FB
	inx
L03FB:	sta     _location_with_scroll
	stx     _location_with_scroll+1
;
; temp_x = (char)location_with_scroll; // low byte
;
	lda     #$00
	sta     _temp_x+1
	lda     _location_with_scroll
	sta     _temp_x
;
; temp_y = Generic.y + Generic.height;
;
	ldx     #$00
	lda     _Generic+1
	clc
	adc     _Generic+3
	bcc     L6D86
	inx
L6D86:	sta     _temp_y
	stx     _temp_y+1
;
; temp_y -= 2;
;
	lda     _temp_y
	sec
	sbc     #$02
	sta     _temp_y
	bcs     L0402
	dec     _temp_y+1
;
; if (bg_collision_sub() & COL_LADDER)
;
L0402:	jsr     _bg_collision_sub
	ldx     #$00
	and     #$01
	beq     L6D88
;
; return 1;
;
	lda     #$01
	rts
;
; }
;
L6D88:	rts

.endproc

; ---------------------------------------------------------------
; unsigned char __near__ bg_coll_ladder_top_under_player (void)
; ---------------------------------------------------------------

.segment	"BANK0"

.proc	_bg_coll_ladder_top_under_player: near

.segment	"BANK0"

;
; location_with_scroll = Generic.x + scroll_x + 7;
;
	lda     _Generic
	clc
	adc     _scroll_x
	pha
	lda     #$00
	adc     _scroll_x+1
	tax
	pla
	clc
	adc     #$07
	bcc     L040A
	inx
L040A:	sta     _location_with_scroll
	stx     _location_with_scroll+1
;
; temp_x = (char)location_with_scroll; // low byte
;
	lda     #$00
	sta     _temp_x+1
	lda     _location_with_scroll
	sta     _temp_x
;
; temp_y = Generic.y + Generic.height;
;
	ldx     #$00
	lda     _Generic+1
	clc
	adc     _Generic+3
	bcc     L6D89
	inx
L6D89:	sta     _temp_y
	stx     _temp_y+1
;
; temp_y += 4;
;
	lda     #$04
	clc
	adc     _temp_y
	sta     _temp_y
	bcc     L0411
	inc     _temp_y+1
;
; if (bg_collision_sub() & COL_LADDER_TOP)
;
L0411:	jsr     _bg_collision_sub
	ldx     #$00
	and     #$02
	beq     L6D8B
;
; return 1;
;
	lda     #$01
	rts
;
; }
;
L6D8B:	rts

.endproc

; ---------------------------------------------------------------
; unsigned char __near__ bg_coll_U (void)
; ---------------------------------------------------------------

.segment	"BANK0"

.proc	_bg_coll_U: near

.segment	"BANK0"

;
; location_with_scroll = Generic.x + scroll_x;
;
	lda     _Generic
	clc
	adc     _scroll_x
	sta     _location_with_scroll
	lda     #$00
	adc     _scroll_x+1
	sta     _location_with_scroll+1
;
; location_with_scroll += 2;
;
	lda     #$02
	clc
	adc     _location_with_scroll
	sta     _location_with_scroll
	bcc     L041B
	inc     _location_with_scroll+1
;
; temp_x = (char)location_with_scroll;   // low byte
;
L041B:	lda     #$00
	sta     _temp_x+1
	lda     _location_with_scroll
	sta     _temp_x
;
; temp_room = location_with_scroll >> 8; // high byte
;
	lda     _location_with_scroll+1
	sta     _temp_room
;
; temp_y = Generic.y;
;
	lda     #$00
	sta     _temp_y+1
	lda     _Generic+1
	sta     _temp_y
;
; eject_U = temp_y | 0xf0;
;
	ora     #$F0
	sta     _eject_U
;
; if (bg_collision_sub() & COL_ALL)
;
	jsr     _bg_collision_sub
	and     #$40
	beq     L6D8D
;
; return 1;
;
	ldx     #$00
	lda     #$01
	rts
;
; location_with_scroll = Generic.x + scroll_x + Generic.width;
;
L6D8D:	lda     _Generic
	clc
	adc     _scroll_x
	pha
	lda     #$00
	adc     _scroll_x+1
	tax
	pla
	clc
	adc     _Generic+2
	bcc     L6D8C
	inx
L6D8C:	sta     _location_with_scroll
	stx     _location_with_scroll+1
;
; location_with_scroll -= 2;
;
	lda     _location_with_scroll
	sec
	sbc     #$02
	sta     _location_with_scroll
	bcs     L042B
	dec     _location_with_scroll+1
;
; temp_x = (char)location_with_scroll;   // low byte
;
L042B:	lda     #$00
	sta     _temp_x+1
	lda     _location_with_scroll
	sta     _temp_x
;
; temp_room = location_with_scroll >> 8; // high byte
;
	lda     _location_with_scroll+1
	sta     _temp_room
;
; if (bg_collision_sub() & COL_ALL)
;
	jsr     _bg_collision_sub
	ldx     #$00
	and     #$40
	beq     L6D8F
;
; return 1;
;
	lda     #$01
	rts
;
; }
;
L6D8F:	rts

.endproc

; ---------------------------------------------------------------
; unsigned char __near__ bg_coll_D (void)
; ---------------------------------------------------------------

.segment	"BANK0"

.proc	_bg_coll_D: near

.segment	"BANK0"

;
; location_with_scroll = Generic.x + scroll_x;
;
	lda     _Generic
	clc
	adc     _scroll_x
	sta     _location_with_scroll
	lda     #$00
	adc     _scroll_x+1
	sta     _location_with_scroll+1
;
; temp_x = (char)location_with_scroll;   // low byte
;
	lda     #$00
	sta     _temp_x+1
	lda     _location_with_scroll
	sta     _temp_x
;
; temp_room = location_with_scroll >> 8; // high byte
;
	lda     _location_with_scroll+1
	sta     _temp_room
;
; temp_y = Generic.y + Generic.height;
;
	ldx     #$00
	lda     _Generic+1
	clc
	adc     _Generic+3
	bcc     L6D90
	inx
L6D90:	sta     _temp_y
	stx     _temp_y+1
;
; if ((temp_y & 0x0f) > 3)
;
	and     #$0F
	cmp     #$04
	lda     #$00
	bcc     L043D
;
; return 0; // bug fix
;
	tax
	rts
;
; eject_D = (temp_y + 1) & 0x0f;
;
L043D:	lda     _temp_y
	clc
	adc     #$01
	and     #$0F
	sta     _eject_D
;
; if (bg_collision_sub() & COL_ALL || ((bg_collision_sub() & COL_LADDER_TOP)))
;
	jsr     _bg_collision_sub
	and     #$40
	bne     L6D93
	jsr     _bg_collision_sub
	and     #$02
	beq     L6D94
;
; return 1;
;
L6D93:	ldx     #$00
	lda     #$01
	rts
;
; location_with_scroll = Generic.x + scroll_x + Generic.width;
;
L6D94:	lda     _Generic
	clc
	adc     _scroll_x
	pha
	lda     #$00
	adc     _scroll_x+1
	tax
	pla
	clc
	adc     _Generic+2
	bcc     L6D91
	inx
L6D91:	sta     _location_with_scroll
	stx     _location_with_scroll+1
;
; location_with_scroll -= 2;
;
	lda     _location_with_scroll
	sec
	sbc     #$02
	sta     _location_with_scroll
	bcs     L044F
	dec     _location_with_scroll+1
;
; temp_x = (char)location_with_scroll;   // low byte
;
L044F:	lda     #$00
	sta     _temp_x+1
	lda     _location_with_scroll
	sta     _temp_x
;
; temp_room = location_with_scroll >> 8; // high byte
;
	lda     _location_with_scroll+1
	sta     _temp_room
;
; if (bg_collision_sub() & COL_ALL || ((bg_collision_sub() & COL_LADDER_TOP)))
;
	jsr     _bg_collision_sub
	and     #$40
	bne     L6D95
	jsr     _bg_collision_sub
	and     #$02
	bne     L6D95
	tax
	rts
;
; return 1;
;
L6D95:	ldx     #$00
	lda     #$01
	rts

.endproc

; ---------------------------------------------------------------
; void __near__ movement (void)
; ---------------------------------------------------------------

.segment	"BANK0"

.proc	_movement: near

.segment	"BANK0"

;
; if (player_in_hitstun)
;
	lda     _player_in_hitstun
	beq     L045C
;
; --player_in_hitstun;
;
	dec     _player_in_hitstun
;
; if (player_in_hitstun > 0)
;
L045C:	lda     _player_in_hitstun
	beq     L6D9E
;
; if (hit_direction == RIGHT)
;
	lda     _hit_direction
	cmp     #$01
	bne     L0461
;
; Player1.vel_x += HITSTUN_DECEL;
;
	lda     #$2D
	clc
	adc     _Player1+4
	sta     _Player1+4
	bcc     L0465
	inc     _Player1+4+1
;
; if (Player1.vel_x >= MAX_SPEED)
;
L0465:	lda     _Player1+4
	cmp     #$60
	lda     _Player1+4+1
	sbc     #$01
	bvs     L0468
	eor     #$80
L0468:	bpl     L6D9E
;
; Player1.vel_x = MAX_SPEED;
;
	ldx     #$01
	lda     #$60
;
; else
;
	jmp     L6DD2
;
; Player1.vel_x -= HITSTUN_DECEL;
;
L0461:	lda     _Player1+4
	sec
	sbc     #$2D
	sta     _Player1+4
	bcs     L046E
	dec     _Player1+4+1
;
; if (Player1.vel_x < -MAX_SPEED)
;
L046E:	lda     _Player1+4
	cmp     #$A0
	lda     _Player1+4+1
	sbc     #$FE
	bvc     L0471
	eor     #$80
L0471:	bpl     L6D9E
;
; Player1.vel_x = -MAX_SPEED;
;
	ldx     #$FE
	lda     #$A0
L6DD2:	sta     _Player1+4
	stx     _Player1+4+1
;
; old_x = Player1.x;
;
L6D9E:	lda     _Player1
	sta     _old_x
;
; if (pad1 & PAD_LEFT && !player_in_hitstun && !player_is_sliding)
;
	lda     _pad1
	and     #$02
	beq     L6DA2
	lda     _player_in_hitstun
	bne     L6DA2
	lda     _player_is_sliding
	bne     L6DA2
;
; direction = LEFT;
;
	sta     _direction
;
; player_is_running = 1;
;
	lda     #$01
	sta     _player_is_running
;
; if (!player_on_ladder)
;
	lda     _player_on_ladder
	jne     L6DAA
;
; if (Player1.vel_x >= DECEL)
;
	lda     _Player1+4
	cmp     #$32
	lda     _Player1+4+1
	sbc     #$00
	bvs     L0482
	eor     #$80
L0482:	bpl     L0480
;
; Player1.vel_x -= DECEL;
;
	lda     _Player1+4
	sec
	sbc     #$32
	sta     _Player1+4
	jcs     L6DAA
	dec     _Player1+4+1
;
; else if (Player1.vel_x > 0)
;
	jmp     L6DAA
L0480:	lda     _Player1+4
	cmp     #$01
	lda     _Player1+4+1
	sbc     #$00
	bvs     L0489
	eor     #$80
L0489:	bpl     L0487
;
; Player1.vel_x = 0;
;
	ldx     #$00
	txa
;
; else
;
	jmp     L6DA9
;
; Player1.vel_x -= ACCEL;
;
L0487:	lda     _Player1+4
	sec
	sbc     #$1E
	sta     _Player1+4
	bcs     L048F
	dec     _Player1+4+1
;
; if (Player1.vel_x < -MAX_SPEED)
;
L048F:	lda     _Player1+4
	cmp     #$A0
	lda     _Player1+4+1
	sbc     #$FE
	bvc     L0492
	eor     #$80
L0492:	jpl     L6DAA
;
; Player1.vel_x = -MAX_SPEED;
;
	ldx     #$FE
	lda     #$A0
;
; else if (pad1 & PAD_RIGHT && !player_in_hitstun && !player_is_sliding)
;
	jmp     L6DA9
L6DA2:	lda     _pad1
	and     #$01
	beq     L6DA7
	lda     _player_in_hitstun
	bne     L6DA3
	lda     _player_is_sliding
	beq     L6DA5
L6DA3:	lda     #$00
	jmp     L6DA7
;
; direction = RIGHT;
;
L6DA5:	lda     #$01
	sta     _direction
;
; player_is_running = 1;
;
	sta     _player_is_running
;
; if (!player_on_ladder)
;
	lda     _player_on_ladder
	jne     L6DAA
;
; if (Player1.vel_x <= DECEL)
;
	lda     _Player1+4
	cmp     #$33
	lda     _Player1+4+1
	sbc     #$00
	bvc     L04A2
	eor     #$80
L04A2:	bpl     L04A0
;
; Player1.vel_x += DECEL;
;
	lda     #$32
	clc
	adc     _Player1+4
	sta     _Player1+4
	jcc     L6DAA
	inc     _Player1+4+1
;
; else if (Player1.vel_x < 0)
;
	jmp     L6DAA
L04A0:	ldx     _Player1+4+1
	cpx     #$80
	bcc     L04A7
;
; Player1.vel_x = 0;
;
	ldx     #$00
	txa
;
; else
;
	jmp     L6DA9
;
; Player1.vel_x += ACCEL;
;
L04A7:	lda     #$1E
	clc
	adc     _Player1+4
	sta     _Player1+4
	bcc     L04AE
	inc     _Player1+4+1
;
; if (Player1.vel_x >= MAX_SPEED)
;
L04AE:	lda     _Player1+4
	cmp     #$60
	lda     _Player1+4+1
	sbc     #$01
	bvs     L04B1
	eor     #$80
L04B1:	bpl     L6DAA
;
; Player1.vel_x = MAX_SPEED;
;
	ldx     #$01
	lda     #$60
;
; else
;
	jmp     L6DA9
;
; player_is_running = 0;
;
L6DA7:	sta     _player_is_running
;
; if (Player1.vel_x >= ACCEL)
;
	lda     _Player1+4
	cmp     #$1E
	lda     _Player1+4+1
	sbc     #$00
	bvs     L04B9
	eor     #$80
L04B9:	bpl     L04B7
;
; Player1.vel_x -= ACCEL;
;
	lda     _Player1+4
	sec
	sbc     #$1E
	sta     _Player1+4
	bcs     L6DAA
	dec     _Player1+4+1
;
; else if (Player1.vel_x < -ACCEL)
;
	jmp     L6DAA
L04B7:	lda     _Player1+4
	cmp     #$E2
	lda     _Player1+4+1
	sbc     #$FF
	bvc     L04C0
	eor     #$80
L04C0:	asl     a
	lda     #$00
	tax
	bcc     L6DA9
;
; Player1.vel_x += ACCEL;
;
	lda     #$1E
	clc
	adc     _Player1+4
	sta     _Player1+4
	bcc     L6DAA
	inc     _Player1+4+1
;
; else
;
	jmp     L6DAA
;
; Player1.vel_x = 0;
;
L6DA9:	sta     _Player1+4
	stx     _Player1+4+1
;
; if (player_is_sliding > 0)
;
L6DAA:	lda     _player_is_sliding
	beq     L04CE
;
; --player_is_sliding;
;
	dec     _player_is_sliding
;
; if (direction == LEFT)
;
	lda     _direction
	bne     L04CA
;
; Player1.vel_x = -MAX_SLIDE_SPEED;
;
	ldx     #$FE
;
; else
;
	jmp     L6DD6
;
; Player1.vel_x = MAX_SLIDE_SPEED;
;
L04CA:	ldx     #$01
L6DD6:	lda     #$80
	sta     _Player1+4
	stx     _Player1+4+1
;
; Player1.x += Player1.vel_x;
;
L04CE:	lda     _Player1+4
	clc
	adc     _Player1
	sta     _Player1
	lda     _Player1+4+1
	adc     _Player1+1
	sta     _Player1+1
;
; if (Player1.x > 0xf000)
;
	lda     _Player1
	cmp     #$01
	lda     _Player1+1
	sbc     #$F0
	bcc     L6DAC
;
; Player1.x = 0x0000; // max left
;
	lda     #$00
	sta     _Player1
	sta     _Player1+1
;
; Player1.vel_x = 0;
;
	sta     _Player1+4
	sta     _Player1+4+1
;
; Generic.x = high_byte(Player1.x);
;
L6DAC:	lda     _Player1+1
	sta     _Generic
;
; Generic.y = high_byte(Player1.y);
;
	lda     _Player1+3
	sta     _Generic+1
;
; Generic.width = HERO_WIDTH;
;
	lda     #$0B
	sta     _Generic+2
;
; Generic.height = HERO_HEIGHT;
;
	lda     #$14
	sta     _Generic+3
;
; if (Player1.vel_x < 0)
;
	ldx     _Player1+4+1
	cpx     #$80
	bcc     L04E8
;
; if (bg_coll_L())
;
	jsr     _bg_coll_L
	tax
	beq     L0502
;
; high_byte(Player1.x) = high_byte(Player1.x) - eject_L;
;
	lda     _Player1+1
	sec
	sbc     _eject_L
	sta     _Player1+1
;
; Player1.vel_x = 0;
;
	lda     #$00
	sta     _Player1+4
	sta     _Player1+4+1
;
; if (Player1.x > 0xf000)
;
	lda     _Player1
	cmp     #$01
	lda     _Player1+1
	sbc     #$F0
	bcc     L0502
;
; Player1.x = 0xf000;
;
	ldx     #$F0
;
; else if (Player1.vel_x > 0)
;
	jmp     L6DD7
L04E8:	lda     _Player1+4
	cmp     #$01
	lda     _Player1+4+1
	sbc     #$00
	bvs     L04F9
	eor     #$80
L04F9:	bpl     L0502
;
; if (bg_coll_R())
;
	jsr     _bg_coll_R
	tax
	beq     L0502
;
; high_byte(Player1.x) = high_byte(Player1.x) - eject_R;
;
	lda     _Player1+1
	sec
	sbc     _eject_R
	sta     _Player1+1
;
; Player1.vel_x = 0;
;
	lda     #$00
	sta     _Player1+4
	sta     _Player1+4+1
;
; if (Player1.x > 0xf000)
;
	lda     _Player1
	cmp     #$01
	lda     _Player1+1
	sbc     #$F0
	bcc     L0502
;
; Player1.x = 0x0000;
;
	ldx     #$00
L6DD7:	lda     #$00
	sta     _Player1
	stx     _Player1+1
;
; if (player_on_ladder && (bg_coll_ladder() || bg_coll_ladder_top_under_player()))
;
L0502:	lda     _player_on_ladder
	jeq     L0506
	jsr     _bg_coll_ladder
	tax
	bne     L6DB1
	jsr     _bg_coll_ladder_top_under_player
	tax
	jeq     L0506
;
; if (pad1 & PAD_DOWN)
;
L6DB1:	lda     _pad1
	and     #$04
	beq     L6DB4
;
; direction_y = DOWN;
;
	lda     #$04
	sta     _direction_y
;
; ++player_on_ladder_pose;
;
	inc     _player_on_ladder_pose
;
; Player1.vel_y += LADDER_ACCEL;
;
	lda     #$14
	clc
	adc     _Player1+6
	sta     _Player1+6
	bcc     L0513
	inc     _Player1+6+1
;
; if (Player1.vel_y > MAX_LADDER_SPEED)
;
L0513:	lda     _Player1+6
	cmp     #$01
	lda     _Player1+6+1
	sbc     #$01
	bvs     L0516
	eor     #$80
L0516:	asl     a
	lda     #$00
	bcc     L6DB3
;
; Player1.vel_y = MAX_LADDER_SPEED;
;
	ldx     #$01
	sta     _Player1+6
	stx     _Player1+6+1
;
; player_on_ladder_top = 0;
;
L6DB3:	sta     _player_on_ladder_top
;
; else if (pad1 & PAD_UP)
;
	jmp     L6DB9
L6DB4:	lda     _pad1
	and     #$08
	beq     L6DB6
;
; direction_y = UP;
;
	lda     #$03
	sta     _direction_y
;
; ++player_on_ladder_pose;
;
	inc     _player_on_ladder_pose
;
; Player1.vel_y -= LADDER_ACCEL;
;
	lda     _Player1+6
	sec
	sbc     #$14
	sta     _Player1+6
	bcs     L0523
	dec     _Player1+6+1
;
; if (Player1.vel_y < -MAX_LADDER_SPEED)
;
L0523:	lda     _Player1+6
	cmp     #$00
	lda     _Player1+6+1
	sbc     #$FF
	bvc     L0526
	eor     #$80
L0526:	bpl     L0524
;
; Player1.vel_y = -MAX_LADDER_SPEED;
;
	ldx     #$FF
	lda     #$00
	sta     _Player1+6
	stx     _Player1+6+1
;
; if (bg_coll_ladder_top_under_player())
;
L0524:	jsr     _bg_coll_ladder_top_under_player
	tax
	jeq     L6DB9
;
; ++player_on_ladder_top;
;
	inc     _player_on_ladder_top
;
; if (player_on_ladder_top > 10)
;
	lda     _player_on_ladder_top
	cmp     #$0B
	bcc     L6DB9
;
; if (Player1.vel_y == -MAX_LADDER_SPEED)
;
	lda     _Player1+6+1
	cmp     #$FF
	bne     L052E
	lda     _Player1+6
	bne     L052E
;
; Player1.y -= 0x200;
;
	lda     _Player1+2
	sec
	sbc     #$00
	sta     _Player1+2
	lda     _Player1+2+1
	sbc     #$02
;
; else
;
	jmp     L6DD5
;
; Player1.y -= 0x800;
;
L052E:	lda     _Player1+2
	sec
	sbc     #$00
	sta     _Player1+2
	lda     _Player1+2+1
	sbc     #$08
L6DD5:	sta     _Player1+2+1
;
; player_on_ladder_top = 0;
;
	lda     #$00
	sta     _player_on_ladder_top
;
; player_on_ladder = 0;
;
	sta     _player_on_ladder
;
; else
;
	jmp     L6DB9
;
; Player1.vel_y = 0;
;
L6DB6:	sta     _Player1+6
	sta     _Player1+6+1
;
; else
;
	jmp     L6DB9
;
; if (player_on_ladder && bg_coll_ladder_top_under_player())
;
L0506:	lda     _player_on_ladder
	beq     L053E
	jsr     _bg_coll_ladder_top_under_player
	tax
	beq     L053E
;
; player_on_ladder = 0;
;
	lda     #$00
	sta     _player_on_ladder
;
; if (Player1.vel_y < 0x300)
;
L053E:	lda     _Player1+6
	cmp     #$00
	lda     _Player1+6+1
	sbc     #$03
	bvc     L0546
	eor     #$80
L0546:	bpl     L0544
;
; Player1.vel_y += GRAVITY;
;
	lda     #$3C
	clc
	adc     _Player1+6
	sta     _Player1+6
	bcc     L6DB9
	inc     _Player1+6+1
;
; else
;
	jmp     L6DB9
;
; Player1.vel_y = 0x300; // Consistent max gravity
;
L0544:	ldx     #$03
	lda     #$00
	sta     _Player1+6
	stx     _Player1+6+1
;
; if (pad1 & PAD_DOWN)
;
L6DB9:	lda     _pad1
	and     #$04
	beq     L0551
;
; direction_y = DOWN;
;
	lda     #$04
	sta     _direction_y
;
; if (bg_coll_ladder_top_under_player())
;
	jsr     _bg_coll_ladder_top_under_player
	tax
	beq     L0551
;
; Player1.x = (Player1.x + 0x700) & ~0xF00;
;
	lda     _Player1
	sta     _Player1
	lda     _Player1+1
	clc
	adc     #$07
	and     #$F0
	sta     _Player1+1
;
; player_on_ladder_pose = 0;
;
	lda     #$00
	sta     _player_on_ladder_pose
;
; player_on_ladder = 1;
;
	lda     #$01
	sta     _player_on_ladder
;
; player_in_air = 0;
;
	lda     #$00
	sta     _player_in_air
;
; falling_down = 0;
;
	sta     _falling_down
;
; Player1.vel_y = 0;
;
	sta     _Player1+6
	sta     _Player1+6+1
;
; Player1.vel_x = 0;
;
	sta     _Player1+4
	sta     _Player1+4+1
;
; Player1.y += Player1.vel_y;
;
L0551:	lda     _Player1+6
	clc
	adc     _Player1+2
	sta     _Player1+2
	lda     _Player1+6+1
	adc     _Player1+2+1
	sta     _Player1+2+1
;
; if (Player1.y > 0xf000)
;
	lda     _Player1+2
	cmp     #$01
	lda     _Player1+2+1
	sbc     #$F0
	bcc     L6DBA
;
; Player1.y = 0x0000;
;
	lda     #$00
	sta     _Player1+2
	sta     _Player1+2+1
;
; Generic.x = high_byte(Player1.x);
;
L6DBA:	lda     _Player1+1
	sta     _Generic
;
; Generic.y = high_byte(Player1.y);
;
	lda     _Player1+3
	sta     _Generic+1
;
; if (Player1.vel_y > 0)
;
	lda     _Player1+6
	cmp     #$01
	lda     _Player1+6+1
	sbc     #$00
	bvs     L0570
	eor     #$80
L0570:	bpl     L056E
;
; ++falling_down;
;
	inc     _falling_down
;
; if (falling_down > 2 && !player_on_ladder)
;
	lda     _falling_down
	cmp     #$03
	bcc     L0572
	lda     _player_on_ladder
	bne     L0572
;
; player_in_air = 1;
;
	lda     #$01
	sta     _player_in_air
;
; if (bg_coll_D())
;
L0572:	jsr     _bg_coll_D
	tax
	beq     L6DBF
;
; player_in_air = 0;
;
	lda     #$00
	sta     _player_in_air
;
; falling_down = 0;
;
	sta     _falling_down
;
; multi_jump = 0;
;
	sta     _multi_jump
;
; player_on_ladder = 0;
;
	sta     _player_on_ladder
;
; high_byte(Player1.y) = high_byte(Player1.y) - eject_D;
;
	lda     _Player1+3
	sec
	sbc     _eject_D
	sta     _Player1+3
;
; Player1.y &= 0xff00;
;
	ldx     _Player1+2+1
	lda     #$00
	sta     _Player1+2
	stx     _Player1+2+1
;
; if (Player1.vel_y > 0)
;
	lda     _Player1+6
	cmp     #$01
	lda     _Player1+6+1
	sbc     #$00
	bvs     L058A
	eor     #$80
L058A:	bpl     L6DBE
;
; Player1.vel_y = 0;
;
	lda     #$00
	sta     _Player1+6
	sta     _Player1+6+1
;
; Player1.on_ground = 1;
;
L6DBE:	lda     #$01
	sta     _Player1+9
;
; else if (Player1.vel_y < 0)
;
	jmp     L6DBF
L056E:	ldx     _Player1+6+1
	cpx     #$80
	bcc     L6DBF
;
; if (bg_coll_U())
;
	jsr     _bg_coll_U
	tax
	beq     L6DBF
;
; high_byte(Player1.y) = high_byte(Player1.y) - eject_U;
;
	lda     _Player1+3
	sec
	sbc     _eject_U
	sta     _Player1+3
;
; Player1.vel_y = 0;
;
	lda     #$00
	sta     _Player1+6
	sta     _Player1+6+1
;
; Generic.y = high_byte(Player1.y);
;
L6DBF:	lda     _Player1+3
	sta     _Generic+1
;
; if (projectile_cooldown > 0)
;
	lda     _projectile_cooldown
	beq     L6DC0
;
; --projectile_cooldown;
;
	dec     _projectile_cooldown
;
; if (pad1_new & PAD_UP || pad1_state & PAD_UP)
;
L6DC0:	lda     _pad1_new
	and     #$08
	bne     L6DC1
	lda     _pad1_state
	and     #$08
	beq     L6DC3
;
; direction_y = UP;
;
L6DC1:	lda     #$03
	sta     _direction_y
;
; if (!player_on_ladder && bg_coll_ladder())
;
	lda     _player_on_ladder
	bne     L6DC3
	jsr     _bg_coll_ladder
	tax
	beq     L6DC3
;
; Player1.x = (Player1.x + 0x700) & ~0xF00;
;
	lda     _Player1
	sta     _Player1
	lda     _Player1+1
	clc
	adc     #$07
	and     #$F0
	sta     _Player1+1
;
; player_on_ladder_pose = 0;
;
	lda     #$00
	sta     _player_on_ladder_pose
;
; player_on_ladder = 1;
;
	lda     #$01
	sta     _player_on_ladder
;
; player_in_air = 0;
;
	lda     #$00
	sta     _player_in_air
;
; falling_down = 0;
;
	sta     _falling_down
;
; Player1.vel_y = 0;
;
	sta     _Player1+6
	sta     _Player1+6+1
;
; Player1.vel_x = 0;
;
	sta     _Player1+4
	sta     _Player1+4+1
;
; if (pad1_state & PAD_DOWN && pad1_new & PAD_A && !player_on_ladder)
;
L6DC3:	lda     _pad1_state
	and     #$04
	beq     L6DC7
	lda     _pad1_new
	and     #$80
	beq     L6DC7
	lda     _player_on_ladder
	bne     L6DC7
;
; player_is_sliding = 25;
;
	lda     #$19
	sta     _player_is_sliding
;
; else if (pad1_new & PAD_A && !player_in_hitstun && !player_is_sliding)
;
	jmp     L6DCC
L6DC7:	lda     _pad1_new
	and     #$80
	beq     L6DCC
	lda     _player_in_hitstun
	bne     L6DCC
	lda     _player_is_sliding
	bne     L6DCC
;
; if (player_on_ladder)
;
	lda     _player_on_ladder
	beq     L05C3
;
; player_on_ladder = 0;
;
	lda     #$00
	sta     _player_on_ladder
;
; player_on_ladder_top = 0;
;
	sta     _player_on_ladder_top
;
; player_in_air = 1;
;
	lda     #$01
;
; else if (bg_coll_D() || multi_jump < multi_jump_max)
;
	jmp     L6D9C
L05C3:	jsr     _bg_coll_D
	tax
	bne     L6DCB
	lda     _multi_jump
	cmp     _multi_jump_max
	bcs     L6DCC
;
; ++multi_jump;
;
L6DCB:	inc     _multi_jump
;
; Player1.vel_y = JUMP_VEL;
;
	ldx     #$FA
	lda     #$F0
	sta     _Player1+6
	stx     _Player1+6+1
;
; short_jump_count = 1;
;
	lda     #$01
	sta     _short_jump_count
;
; player_in_air = 1;
;
L6D9C:	sta     _player_in_air
;
; if (pad1_new & PAD_B && projectile_cooldown == 0 && !player_in_hitstun && !player_is_sliding)
;
L6DCC:	lda     _pad1_new
	and     #$40
	jeq     L05EB
	lda     _projectile_cooldown
	jne     L05EB
	lda     _player_in_hitstun
	jne     L05EB
	lda     _player_is_sliding
	jne     L05EB
;
; temp2 = 0;
;
	sta     _temp2
	sta     _temp2+1
;
; for (temp1 = 0; temp1 < MAX_PROJECTILES; ++temp1)
;
	sta     _temp1
	sta     _temp1+1
L05DC:	lda     _temp1+1
	cmp     #$00
	bne     L05E3
	lda     _temp1
	cmp     #$03
L05E3:	bcs     L05DD
;
; if (projectiles_list[temp1] == TURN_OFF)
;
	lda     #<(_projectiles_list)
	sta     ptr1
	lda     #>(_projectiles_list)
	clc
	adc     _temp1+1
	sta     ptr1+1
	ldy     _temp1
	lda     (ptr1),y
	cmp     #$FF
	bne     L05DE
;
; temp2 = 1;
;
	ldx     #$00
	lda     #$01
	sta     _temp2
	stx     _temp2+1
;
; break;
;
	jmp     L05DD
;
; for (temp1 = 0; temp1 < MAX_PROJECTILES; ++temp1)
;
L05DE:	inc     _temp1
	bne     L05DC
	inc     _temp1+1
	jmp     L05DC
;
; if (temp2)
;
L05DD:	lda     _temp2
	ora     _temp2+1
	beq     L05EB
;
; projectile_cooldown = PROJECTILE_COOLDOWN_FRAMES;
;
	lda     #$0A
	sta     _projectile_cooldown
;
; player_shooting = 10;
;
	sta     _player_shooting
;
; projectile_index = temp1;
;
	lda     _temp1
	sta     _projectile_index
;
; if (direction == RIGHT)
;
	lda     _direction
	cmp     #$01
	bne     L05F3
;
; projectiles_list[projectile_index] = RIGHT;
;
	ldy     _projectile_index
;
; else
;
	jmp     L6D9D
;
; projectiles_list[projectile_index] = LEFT;
;
L05F3:	ldy     _projectile_index
	lda     #$00
L6D9D:	sta     _projectiles_list,y
;
; projectiles_x[projectile_index] = high_byte(Player1.x) + 10;
;
	lda     #<(_projectiles_x)
	ldx     #>(_projectiles_x)
	clc
	adc     _projectile_index
	bcc     L0600
	inx
L0600:	sta     ptr1
	stx     ptr1+1
	lda     _Player1+1
	clc
	adc     #$0A
	ldy     #$00
	sta     (ptr1),y
;
; projectiles_y[projectile_index] = high_byte(Player1.y);
;
	ldy     _projectile_index
	lda     _Player1+3
	sta     _projectiles_y,y
;
; if (short_jump_count)
;
L05EB:	lda     _short_jump_count
	beq     L060C
;
; ++short_jump_count;
;
	inc     _short_jump_count
;
; if (short_jump_count > 30)
;
	lda     _short_jump_count
	cmp     #$1F
	bcc     L060C
;
; short_jump_count = 0;
;
	lda     #$00
	sta     _short_jump_count
;
; if ((short_jump_count) && ((pad1 & PAD_A) == 0) && (Player1.vel_y < -0x200))
;
L060C:	lda     _short_jump_count
	beq     L6DD0
	lda     _pad1
	and     #$80
	bne     L6DD0
	lda     _Player1+6
	cmp     #$00
	lda     _Player1+6+1
	sbc     #$FE
	bvc     L0618
	eor     #$80
L0618:	bmi     L6DD1
L6DD0:	rts
;
; Player1.vel_y = -0x200;
;
L6DD1:	ldx     #$FE
	lda     #$00
	sta     _Player1+6
	stx     _Player1+6+1
;
; short_jump_count = 0;
;
	sta     _short_jump_count
;
; }
;
	rts

.endproc

; ---------------------------------------------------------------
; void __near__ check_player_colision (void)
; ---------------------------------------------------------------

.segment	"BANK0"

.proc	_check_player_colision: near

.segment	"BANK0"

;
; }
;
	rts

.endproc

; ---------------------------------------------------------------
; void __near__ update_player_animation (void)
; ---------------------------------------------------------------

.segment	"BANK0"

.proc	_update_player_animation: near

.segment	"BANK0"

;
; if (player_is_sliding)
;
	lda     _player_is_sliding
	beq     L0620
;
; if (direction == LEFT)
;
	lda     _direction
	bne     L0622
;
; current_animation_ptr = animate_slideleft_data;
;
	lda     #>(_animate_slideleft_data)
	sta     _current_animation_ptr+1
	lda     #<(_animate_slideleft_data)
;
; else
;
	jmp     L6DDD
;
; current_animation_ptr = animate_slideright_data;
;
L0622:	lda     #>(_animate_slideright_data)
	sta     _current_animation_ptr+1
	lda     #<(_animate_slideright_data)
;
; else if (player_shooting > 0)
;
	jmp     L6DDD
L0620:	lda     _player_shooting
	beq     L062E
;
; --player_shooting;
;
	dec     _player_shooting
;
; if (direction == LEFT)
;
	lda     _direction
	bne     L0631
;
; current_animation_ptr = animate_playerstandshootleft_data;
;
	lda     #>(_animate_playerstandshootleft_data)
	sta     _current_animation_ptr+1
	lda     #<(_animate_playerstandshootleft_data)
;
; else
;
	jmp     L6DDD
;
; current_animation_ptr = animate_playerstandshootright_data;
;
L0631:	lda     #>(_animate_playerstandshootright_data)
	sta     _current_animation_ptr+1
	lda     #<(_animate_playerstandshootright_data)
;
; else if (player_in_air)
;
	jmp     L6DDD
L062E:	lda     _player_in_air
	beq     L063D
;
; if (direction == LEFT)
;
	lda     _direction
	bne     L063F
;
; current_animation_ptr = animate_playerjumpleft_data;
;
	lda     #>(_animate_playerjumpleft_data)
	sta     _current_animation_ptr+1
	lda     #<(_animate_playerjumpleft_data)
;
; else
;
	jmp     L6DDD
;
; current_animation_ptr = animate_playerjumpright_data;
;
L063F:	lda     #>(_animate_playerjumpright_data)
	sta     _current_animation_ptr+1
	lda     #<(_animate_playerjumpright_data)
;
; else if (player_on_ladder)
;
	jmp     L6DDD
L063D:	lda     _player_on_ladder
	beq     L064B
;
; if (player_on_ladder_pose > 15)
;
	lda     _player_on_ladder_pose
	cmp     #$10
	bcc     L6DDE
;
; current_animation_ptr = animate_playerclimb2_data;
;
	lda     #>(_animate_playerclimb2_data)
	sta     _current_animation_ptr+1
	lda     #<(_animate_playerclimb2_data)
	sta     _current_animation_ptr
;
; player_on_ladder_pose = 0;
;
	lda     #$00
	sta     _player_on_ladder_pose
;
; else if (player_on_ladder_pose > 8)
;
	rts
L6DDE:	lda     _player_on_ladder_pose
	cmp     #$09
	bcc     L0654
;
; current_animation_ptr = animate_playerclimb1_data;
;
	lda     #>(_animate_playerclimb1_data)
	sta     _current_animation_ptr+1
	lda     #<(_animate_playerclimb1_data)
	sta     _current_animation_ptr
;
; else
;
	rts
;
; current_animation_ptr = animate_playerclimb1_data;
;
L0654:	lda     #>(_animate_playerclimb1_data)
	sta     _current_animation_ptr+1
	lda     #<(_animate_playerclimb1_data)
	sta     _current_animation_ptr
;
; else if (player_is_running && (pad1 & PAD_LEFT || pad1 & PAD_RIGHT))
;
	rts
L064B:	lda     _player_is_running
	jeq     L6DEB
	lda     _pad1
	and     #$02
	bne     L6DE4
	lda     _pad1
	and     #$01
	jeq     L6DEB
;
; animation_frame_counter++;
;
L6DE4:	inc     _animation_frame_counter
;
; if (animation_frame_counter >= 6)
;
	lda     _animation_frame_counter
	cmp     #$06
	bcc     L6DE5
;
; animation_frame_counter = 0;
;
	lda     #$00
	sta     _animation_frame_counter
;
; current_animation_frame++;
;
	inc     _current_animation_frame
;
; if (direction == LEFT)
;
L6DE5:	lda     _direction
	bne     L6DE8
;
; if (current_animation_frame > 2)
;
	lda     _current_animation_frame
	cmp     #$03
	bcc     L6DE6
;
; current_animation_frame = 0;
;
	lda     #$00
	sta     _current_animation_frame
;
; if (current_animation_frame == 0)
;
L6DE6:	lda     _current_animation_frame
	bne     L6DE7
;
; current_animation_ptr = animate_playerrun1left_data;
;
	lda     #>(_animate_playerrun1left_data)
	sta     _current_animation_ptr+1
	lda     #<(_animate_playerrun1left_data)
;
; else if (current_animation_frame == 1)
;
	jmp     L6DDC
L6DE7:	lda     _current_animation_frame
	cmp     #$01
	bne     L0673
;
; current_animation_ptr = animate_playerrun2left_data;
;
	lda     #>(_animate_playerrun2left_data)
	sta     _current_animation_ptr+1
	lda     #<(_animate_playerrun2left_data)
;
; else
;
	jmp     L6DDC
;
; current_animation_ptr = animate_playerrun3left_data;
;
L0673:	lda     #>(_animate_playerrun3left_data)
	sta     _current_animation_ptr+1
	lda     #<(_animate_playerrun3left_data)
	sta     _current_animation_ptr
;
; else
;
	rts
;
; if (current_animation_frame > 2)
;
L6DE8:	lda     _current_animation_frame
	cmp     #$03
	bcc     L6DE9
;
; current_animation_frame = 0;
;
	lda     #$00
	sta     _current_animation_frame
;
; if (current_animation_frame == 0)
;
L6DE9:	lda     _current_animation_frame
	bne     L6DEA
;
; current_animation_ptr = animate_playerrun1right_data;
;
	lda     #>(_animate_playerrun1right_data)
	sta     _current_animation_ptr+1
	lda     #<(_animate_playerrun1right_data)
	sta     _current_animation_ptr
;
; else if (current_animation_frame == 1)
;
	rts
L6DEA:	lda     _current_animation_frame
	cmp     #$01
	bne     L0684
;
; current_animation_ptr = animate_playerrun2right_data;
;
	lda     #>(_animate_playerrun2right_data)
	sta     _current_animation_ptr+1
	lda     #<(_animate_playerrun2right_data)
	sta     _current_animation_ptr
;
; else
;
	rts
;
; current_animation_ptr = animate_playerrun3right_data;
;
L0684:	lda     #>(_animate_playerrun3right_data)
	sta     _current_animation_ptr+1
	lda     #<(_animate_playerrun3right_data)
L6DDC:	sta     _current_animation_ptr
;
; else
;
	rts
;
; if (direction == LEFT)
;
L6DEB:	lda     _direction
	bne     L068C
;
; current_animation_ptr = animate_playerstandleft_data;
;
	lda     #>(_animate_playerstandleft_data)
	sta     _current_animation_ptr+1
	lda     #<(_animate_playerstandleft_data)
;
; else
;
	jmp     L6DDD
;
; current_animation_ptr = animate_playerstandright_data;
;
L068C:	lda     #>(_animate_playerstandright_data)
	sta     _current_animation_ptr+1
	lda     #<(_animate_playerstandright_data)
L6DDD:	sta     _current_animation_ptr
;
; animation_frame_counter = 0;
;
	lda     #$00
	sta     _animation_frame_counter
;
; current_animation_frame = 0;
;
	sta     _current_animation_frame
;
; }
;
	rts

.endproc

; ---------------------------------------------------------------
; void __near__ bank0_player_movement (void)
; ---------------------------------------------------------------

.segment	"BANK0"

.proc	_bank0_player_movement: near

.segment	"BANK0"

;
; movement();
;
	jsr     _movement
;
; update_player_animation();
;
	jmp     _update_player_animation

.endproc

; ---------------------------------------------------------------
; void __near__ bank0_draw_player_sprite (void)
; ---------------------------------------------------------------

.segment	"BANK0"

.proc	_bank0_draw_player_sprite: near

.segment	"BANK0"

;
; temp_x = Player1.x >> 8;
;
	ldx     #$00
	lda     _Player1+1
	sta     _temp_x
	stx     _temp_x+1
;
; temp_y = Player1.y >> 8; 
;
	lda     _Player1+3
	sta     _temp_y
	stx     _temp_y+1
;
; oam_meta_spr(temp_x, temp_y, current_animation_ptr);
;
	jsr     decsp2
	lda     _temp_x
	ldy     #$01
	sta     (sp),y
	lda     _temp_y
	dey
	sta     (sp),y
	lda     _current_animation_ptr
	ldx     _current_animation_ptr+1
	jsr     _oam_meta_spr
;
; for (temp1 = 0; temp1 < MAX_PROJECTILES; ++temp1)
;
	lda     #$00
	sta     _temp1
	sta     _temp1+1
L06A3:	lda     _temp1+1
	cmp     #$00
	bne     L06AA
	lda     _temp1
	cmp     #$03
L06AA:	bcs     L06A4
;
; if (projectiles_list[temp1] != TURN_OFF)
;
	lda     #<(_projectiles_list)
	sta     ptr1
	lda     #>(_projectiles_list)
	clc
	adc     _temp1+1
	sta     ptr1+1
	ldy     _temp1
	lda     (ptr1),y
	cmp     #$FF
	beq     L06A5
;
; oam_meta_spr(projectiles_x[temp1], projectiles_y[temp1], projectile);
;
	jsr     decsp2
	lda     #<(_projectiles_x)
	sta     ptr1
	lda     #>(_projectiles_x)
	clc
	adc     _temp1+1
	sta     ptr1+1
	ldy     _temp1
	lda     (ptr1),y
	ldy     #$01
	sta     (sp),y
	lda     #<(_projectiles_y)
	sta     ptr1
	lda     #>(_projectiles_y)
	clc
	adc     _temp1+1
	sta     ptr1+1
	ldy     _temp1
	lda     (ptr1),y
	ldy     #$00
	sta     (sp),y
	lda     #<(_projectile)
	ldx     #>(_projectile)
	jsr     _oam_meta_spr
;
; for (temp1 = 0; temp1 < MAX_PROJECTILES; ++temp1)
;
L06A5:	inc     _temp1
	bne     L06A3
	inc     _temp1+1
	jmp     L06A3
;
; }
;
L06A4:	rts

.endproc

; ---------------------------------------------------------------
; void __near__ projectile_movement (void)
; ---------------------------------------------------------------

.segment	"BANK1"

.proc	_projectile_movement: near

.segment	"BANK1"

;
; for (temp1 = 0; temp1 < MAX_PROJECTILES; ++temp1)
;
	lda     #$00
	sta     _temp1
	sta     _temp1+1
L06B9:	lda     _temp1+1
	cmp     #$00
	bne     L06C0
	lda     _temp1
	cmp     #$03
L06C0:	bcc     L6DF4
;
; }
;
	rts
;
; if (projectiles_list[temp1] != TURN_OFF)
;
L6DF4:	lda     #<(_projectiles_list)
	sta     ptr1
	lda     #>(_projectiles_list)
	clc
	adc     _temp1+1
	sta     ptr1+1
	ldy     _temp1
	lda     (ptr1),y
	cmp     #$FF
	jeq     L06BB
;
; if (projectiles_x[temp1] > 250)
;
	lda     #<(_projectiles_x)
	sta     ptr1
	lda     #>(_projectiles_x)
	clc
	adc     _temp1+1
	sta     ptr1+1
	ldy     _temp1
	lda     (ptr1),y
	cmp     #$FB
	bcc     L06C6
;
; projectiles_list[temp1] = TURN_OFF;
;
	lda     #<(_projectiles_list)
	clc
	adc     _temp1
	sta     ptr1
	lda     #>(_projectiles_list)
	adc     _temp1+1
	sta     ptr1+1
	lda     #$FF
	ldy     #$00
;
; else
;
	jmp     L6DEE
;
; if (projectiles_list[temp1] == RIGHT)
;
L06C6:	lda     #<(_projectiles_list)
	sta     ptr1
	lda     #>(_projectiles_list)
	clc
	adc     _temp1+1
	sta     ptr1+1
	ldy     _temp1
	lda     (ptr1),y
	cmp     #$01
	bne     L06CD
;
; if ((Player1.x <= MAX_LEFT) && (pad1 & PAD_LEFT))
;
	lda     _Player1+1
	cmp     #$50
	bne     L06D3
	lda     _Player1
	cmp     #$01
L06D3:	bcs     L06D0
	lda     _pad1
	and     #$02
	beq     L06D0
;
; projectiles_x[temp1] += PROJECTILE_SPEED_WITH_SCROLL;
;
	lda     #<(_projectiles_x)
	clc
	adc     _temp1
	sta     ptr1
	lda     #>(_projectiles_x)
	adc     _temp1+1
	sta     ptr1+1
	ldy     #$00
	lda     (ptr1),y
	clc
	adc     #$05
;
; else
;
	jmp     L6DEE
;
; projectiles_x[temp1] += PROJECTILE_SPEED;
;
L06D0:	lda     #<(_projectiles_x)
	clc
	adc     _temp1
	sta     ptr1
	lda     #>(_projectiles_x)
	adc     _temp1+1
	sta     ptr1+1
	ldy     #$00
	lda     (ptr1),y
	clc
	adc     #$03
;
; else if (projectiles_list[temp1] == LEFT)
;
	jmp     L6DEE
L06CD:	lda     #<(_projectiles_list)
	sta     ptr1
	lda     #>(_projectiles_list)
	clc
	adc     _temp1+1
	sta     ptr1+1
	ldy     _temp1
	lda     (ptr1),y
	bne     L06BB
;
; if ((Player1.x >= MAX_RIGHT) && (pad1 & PAD_RIGHT))
;
	lda     _Player1
	cmp     #$00
	lda     _Player1+1
	sbc     #$90
	bcc     L06E2
	lda     _pad1
	and     #$01
	beq     L06E2
;
; projectiles_x[temp1] -= PROJECTILE_SPEED_WITH_SCROLL;
;
	lda     #<(_projectiles_x)
	clc
	adc     _temp1
	sta     ptr1
	lda     #>(_projectiles_x)
	adc     _temp1+1
	sta     ptr1+1
	ldy     #$00
	lda     (ptr1),y
	sec
	sbc     #$05
;
; else
;
	jmp     L6DEE
;
; projectiles_x[temp1] -= PROJECTILE_SPEED;
;
L06E2:	lda     #<(_projectiles_x)
	clc
	adc     _temp1
	sta     ptr1
	lda     #>(_projectiles_x)
	adc     _temp1+1
	sta     ptr1+1
	ldy     #$00
	lda     (ptr1),y
	sec
	sbc     #$03
L6DEE:	sta     (ptr1),y
;
; for (temp1 = 0; temp1 < MAX_PROJECTILES; ++temp1)
;
L06BB:	inc     _temp1
	jne     L06B9
	inc     _temp1+1
	jmp     L06B9

.endproc

; ---------------------------------------------------------------
; void __near__ function_bank1 (void)
; ---------------------------------------------------------------

.segment	"BANK1"

.proc	_function_bank1: near

.segment	"BANK1"

;
; }
;
	rts

.endproc

; ---------------------------------------------------------------
; void __near__ function_bank2 (void)
; ---------------------------------------------------------------

.segment	"BANK2"

.proc	_function_bank2: near

.segment	"BANK2"

;
; }
;
	rts

.endproc

; ---------------------------------------------------------------
; void __near__ drawMetatileBlock (void)
; ---------------------------------------------------------------

.segment	"BANK2"

.proc	_drawMetatileBlock: near

.segment	"BANK2"

;
; address = get_ppu_addr(nt, x, temp_y);
;
	jsr     decsp2
	lda     _nt
	ldy     #$01
	sta     (sp),y
	lda     _x
	dey
	sta     (sp),y
	lda     _temp_y
	jsr     _get_ppu_addr
	sta     _address
	stx     _address+1
;
; index = temp_y + (x >> 4);
;
	lda     _x
	lsr     a
	lsr     a
	lsr     a
	lsr     a
	clc
	adc     _temp_y
	sta     _index
	lda     #$00
	adc     _temp_y+1
;
; buffer_4_mt(address, index); // ppu_address, index to the data
;
	lda     _address
	ldx     _address+1
	jsr     pushax
	lda     _index
	jmp     _buffer_4_mt

.endproc

; ---------------------------------------------------------------
; void __near__ draw_screen_L (void)
; ---------------------------------------------------------------

.segment	"BANK2"

.proc	_draw_screen_L: near

.segment	"BANK2"

;
; pseudo_scroll_x = scroll_x - 0x20;
;
	lda     _scroll_x
	ldx     _scroll_x+1
	sec
	sbc     #$20
	bcs     L1A2F
	dex
L1A2F:	sta     _pseudo_scroll_x
	stx     _pseudo_scroll_x+1
;
; offset = (pseudo_scroll_x >> 8);
;
	lda     _pseudo_scroll_x+1
	sta     _offset
;
; if (offset >= levels_per_stage[current_stage] || offset < 0) {
;
	lda     _offset
	jsr     pusha0
	ldy     _current_stage
	lda     _levels_per_stage,y
	jsr     tosicmp0
	bcc     L6E00
;
; offset = 0;
;
	lda     #$00
	sta     _offset
;
; set_data_pointer(stage_table[current_stage][offset]);
;
L6E00:	ldx     #$00
	lda     _current_stage
	asl     a
	bcc     L6DF8
	inx
	clc
L6DF8:	adc     #<(_stage_table)
	sta     ptr1
	txa
	adc     #>(_stage_table)
	sta     ptr1+1
	ldy     #$01
	lda     (ptr1),y
	tax
	dey
	lda     (ptr1),y
	sta     ptr1
	stx     ptr1+1
	ldx     #$00
	lda     _offset
	asl     a
	bcc     L6DF9
	inx
	clc
L6DF9:	adc     ptr1
	sta     ptr1
	txa
	adc     ptr1+1
	sta     ptr1+1
	iny
	lda     (ptr1),y
	tax
	dey
	lda     (ptr1),y
	jsr     _set_data_pointer
;
; nt = ((pseudo_scroll_x >> 8) & 1);
;
	lda     _pseudo_scroll_x+1
	and     #$01
	sta     _nt
;
; x = pseudo_scroll_x & 0xff;
;
	lda     _pseudo_scroll_x
	sta     _x
;
; switch (scroll_count)
;
	ldx     #$00
	lda     _scroll_count
;
; }
;
	beq     L6DFC
	cmp     #$01
	beq     L6DFD
	cmp     #$02
	beq     L6DFE
	jmp     L6DFF
;
; temp_y = 0;
;
L6DFC:	sta     _temp_y
	sta     _temp_y+1
;
; drawMetatileBlock();
;
	jsr     _drawMetatileBlock
;
; temp_y = 0x20;
;
	ldx     #$00
	lda     #$20
;
; break;
;
	jmp     L6E02
;
; temp_y = 0x40;
;
L6DFD:	lda     #$40
	sta     _temp_y
	stx     _temp_y+1
;
; drawMetatileBlock();
;
	jsr     _drawMetatileBlock
;
; temp_y = 0x60;
;
	ldx     #$00
	lda     #$60
;
; break;
;
	jmp     L6E02
;
; temp_y = 0x80;
;
L6DFE:	lda     #$80
	sta     _temp_y
	stx     _temp_y+1
;
; drawMetatileBlock();
;
	jsr     _drawMetatileBlock
;
; temp_y = 0xa0;
;
	ldx     #$00
	lda     #$A0
;
; break;
;
	jmp     L6E02
;
; temp_y = 0xc0;
;
L6DFF:	lda     #$C0
	sta     _temp_y
	stx     _temp_y+1
;
; drawMetatileBlock();
;
	jsr     _drawMetatileBlock
;
; temp_y = 0xe0;
;
	ldx     #$00
	lda     #$E0
L6E02:	sta     _temp_y
	stx     _temp_y+1
;
; drawMetatileBlock();
;
	jsr     _drawMetatileBlock
;
; --scroll_count;   // Decrement scroll_count for left scrolling
;
	dec     _scroll_count
;
; scroll_count &= 3; // mask off top bits, keep it 0-3
;
	lda     _scroll_count
	and     #$03
	sta     _scroll_count
;
; }
;
	rts

.endproc

; ---------------------------------------------------------------
; void __near__ draw_screen_R (void)
; ---------------------------------------------------------------

.segment	"BANK2"

.proc	_draw_screen_R: near

.segment	"BANK2"

;
; pseudo_scroll_x = scroll_x + 0x120;
;
	lda     _scroll_x
	ldx     _scroll_x+1
	clc
	adc     #$20
	bcc     L1A6C
	inx
L1A6C:	inx
	sta     _pseudo_scroll_x
	stx     _pseudo_scroll_x+1
;
; offset = (pseudo_scroll_x >> 8);
;
	lda     _pseudo_scroll_x+1
	sta     _offset
;
; if (offset >= levels_per_stage[current_stage] || offset < 0) {
;
	lda     _offset
	jsr     pusha0
	ldy     _current_stage
	lda     _levels_per_stage,y
	jsr     tosicmp0
	bcc     L6E0E
;
; offset = 0;
;
	lda     #$00
	sta     _offset
;
; set_data_pointer(stage_table[current_stage][offset]);
;
L6E0E:	ldx     #$00
	lda     _current_stage
	asl     a
	bcc     L6E06
	inx
	clc
L6E06:	adc     #<(_stage_table)
	sta     ptr1
	txa
	adc     #>(_stage_table)
	sta     ptr1+1
	ldy     #$01
	lda     (ptr1),y
	tax
	dey
	lda     (ptr1),y
	sta     ptr1
	stx     ptr1+1
	ldx     #$00
	lda     _offset
	asl     a
	bcc     L6E07
	inx
	clc
L6E07:	adc     ptr1
	sta     ptr1
	txa
	adc     ptr1+1
	sta     ptr1+1
	iny
	lda     (ptr1),y
	tax
	dey
	lda     (ptr1),y
	jsr     _set_data_pointer
;
; nt = ((pseudo_scroll_x >> 8) & 1);
;
	lda     _pseudo_scroll_x+1
	and     #$01
	sta     _nt
;
; x = pseudo_scroll_x & 0xff;
;
	lda     _pseudo_scroll_x
	sta     _x
;
; switch (scroll_count)
;
	ldx     #$00
	lda     _scroll_count
;
; }
;
	beq     L6E0A
	cmp     #$01
	beq     L6E0B
	cmp     #$02
	beq     L6E0C
	jmp     L6E0D
;
; temp_y = 0;
;
L6E0A:	sta     _temp_y
	sta     _temp_y+1
;
; drawMetatileBlock();
;
	jsr     _drawMetatileBlock
;
; temp_y = 0x20;
;
	ldx     #$00
	lda     #$20
;
; break;
;
	jmp     L6E10
;
; temp_y = 0x40;
;
L6E0B:	lda     #$40
	sta     _temp_y
	stx     _temp_y+1
;
; drawMetatileBlock();
;
	jsr     _drawMetatileBlock
;
; temp_y = 0x60;
;
	ldx     #$00
	lda     #$60
;
; break;
;
	jmp     L6E10
;
; temp_y = 0x80;
;
L6E0C:	lda     #$80
	sta     _temp_y
	stx     _temp_y+1
;
; drawMetatileBlock();
;
	jsr     _drawMetatileBlock
;
; temp_y = 0xa0;
;
	ldx     #$00
	lda     #$A0
;
; break;
;
	jmp     L6E10
;
; temp_y = 0xc0;
;
L6E0D:	lda     #$C0
	sta     _temp_y
	stx     _temp_y+1
;
; drawMetatileBlock();
;
	jsr     _drawMetatileBlock
;
; temp_y = 0xe0;
;
	ldx     #$00
	lda     #$E0
L6E10:	sta     _temp_y
	stx     _temp_y+1
;
; drawMetatileBlock();
;
	jsr     _drawMetatileBlock
;
; ++scroll_count;   // Increment scroll_count for right scrolling
;
	inc     _scroll_count
;
; scroll_count &= 3; // mask off top bits, keep it 0-3
;
	lda     _scroll_count
	and     #$03
	sta     _scroll_count
;
; }
;
	rts

.endproc

; ---------------------------------------------------------------
; void __near__ new_cmap (void)
; ---------------------------------------------------------------

.segment	"BANK2"

.proc	_new_cmap: near

.segment	"BANK2"

;
; offset = room_to_load;
;
	lda     _room_to_load
	sta     _offset
;
; map = room_to_load & 1; // even or odd?
;
	lda     _room_to_load
	and     #$01
	sta     _map
;
; if (!map) 
;
	lda     _map
	jne     L1AAB
;
; memcpy(c_map, stage_table[current_stage][offset], 240); 
;
	lda     #<(_c_map)
	ldx     #>(_c_map)
	jsr     pushax
	ldx     #$00
	lda     _current_stage
	asl     a
	bcc     L6E19
	inx
	clc
L6E19:	adc     #<(_stage_table)
	sta     ptr1
	txa
	adc     #>(_stage_table)
	sta     ptr1+1
	ldy     #$01
	lda     (ptr1),y
	tax
	dey
	lda     (ptr1),y
	sta     ptr1
	stx     ptr1+1
	ldx     #$00
	lda     _offset
	asl     a
	bcc     L6E1A
	inx
	clc
L6E1A:	adc     ptr1
	sta     ptr1
	txa
	adc     ptr1+1
	sta     ptr1+1
	iny
	lda     (ptr1),y
	tax
	dey
	lda     (ptr1),y
	jsr     pushax
	ldx     #$00
	lda     #$F0
	jsr     _memcpy
;
; if (scrolling_direction) 
;
	lda     _scrolling_direction
	beq     L1AB3
;
; if (offset + 1 < levels_per_stage[current_stage])
;
	ldx     #$00
	lda     _offset
	clc
	adc     #$01
	bcc     L1AB7
	inx
L1AB7:	jsr     pushax
	ldy     _current_stage
	lda     _levels_per_stage,y
	jsr     tosicmp0
	bcc     L6E21
;
; else
;
	rts
;
; memcpy(c_map2, stage_table[current_stage][offset + 1], 240);
;
L6E21:	lda     #<(_c_map2)
	ldx     #>(_c_map2)
	jsr     pushax
	ldx     #$00
	lda     _current_stage
	asl     a
	bcc     L6E1B
	inx
	clc
L6E1B:	adc     #<(_stage_table)
	sta     ptr1
	txa
	adc     #>(_stage_table)
	sta     ptr1+1
	ldy     #$01
	lda     (ptr1),y
	tax
	dey
	lda     (ptr1),y
	sta     ptr1
	stx     ptr1+1
	ldx     #$00
	lda     _offset
	clc
	adc     #$01
	bcc     L1ABF
	inx
L1ABF:	stx     tmp1
	asl     a
	rol     tmp1
	clc
	adc     ptr1
	sta     ptr1
	lda     tmp1
	adc     ptr1+1
	sta     ptr1+1
	iny
	lda     (ptr1),y
	tax
	dey
	lda     (ptr1),y
	jsr     pushax
	ldx     #$00
	lda     #$F0
	jmp     _memcpy
;
; if (offset - 1 >= 0)
;
L1AB3:	lda     _offset
	sec
	sbc     #$01
	jsr     return1
	bne     L6E22
;
; }
;
	rts
;
; memcpy(c_map2, stage_table[current_stage][offset - 1], 240);
;
L6E22:	lda     #<(_c_map2)
	ldx     #>(_c_map2)
	jsr     pushax
	ldx     #$00
	lda     _current_stage
	asl     a
	bcc     L6E1C
	inx
	clc
L6E1C:	adc     #<(_stage_table)
	sta     ptr1
	txa
	adc     #>(_stage_table)
	sta     ptr1+1
	ldy     #$01
	lda     (ptr1),y
	tax
	dey
	lda     (ptr1),y
	sta     ptr1
	stx     ptr1+1
	ldx     #$00
	lda     _offset
	sec
	sbc     #$01
	bcs     L1ACA
	dex
L1ACA:	stx     tmp1
	asl     a
	rol     tmp1
	clc
	adc     ptr1
	sta     ptr1
	lda     tmp1
	adc     ptr1+1
	sta     ptr1+1
	iny
	lda     (ptr1),y
	tax
	dey
	lda     (ptr1),y
	jsr     pushax
	ldx     #$00
	lda     #$F0
	jmp     _memcpy
;
; memcpy(c_map2, stage_table[current_stage][offset], 240);
;
L1AAB:	lda     #<(_c_map2)
	ldx     #>(_c_map2)
	jsr     pushax
	ldx     #$00
	lda     _current_stage
	asl     a
	bcc     L6E1D
	inx
	clc
L6E1D:	adc     #<(_stage_table)
	sta     ptr1
	txa
	adc     #>(_stage_table)
	sta     ptr1+1
	ldy     #$01
	lda     (ptr1),y
	tax
	dey
	lda     (ptr1),y
	sta     ptr1
	stx     ptr1+1
	ldx     #$00
	lda     _offset
	asl     a
	bcc     L6E1E
	inx
	clc
L6E1E:	adc     ptr1
	sta     ptr1
	txa
	adc     ptr1+1
	sta     ptr1+1
	iny
	lda     (ptr1),y
	tax
	dey
	lda     (ptr1),y
	jsr     pushax
	ldx     #$00
	lda     #$F0
	jsr     _memcpy
;
; if (scrolling_direction)
;
	lda     _scrolling_direction
	beq     L1AD3
;
; if (offset + 1 < levels_per_stage[current_stage])
;
	ldx     #$00
	lda     _offset
	clc
	adc     #$01
	bcc     L1AD7
	inx
L1AD7:	jsr     pushax
	ldy     _current_stage
	lda     _levels_per_stage,y
	jsr     tosicmp0
	bcc     L6E23
;
; }
;
	rts
;
; memcpy(c_map, stage_table[current_stage][offset + 1], 240);
;
L6E23:	lda     #<(_c_map)
	ldx     #>(_c_map)
	jsr     pushax
	ldx     #$00
	lda     _current_stage
	asl     a
	bcc     L6E1F
	inx
	clc
L6E1F:	adc     #<(_stage_table)
	sta     ptr1
	txa
	adc     #>(_stage_table)
	sta     ptr1+1
	ldy     #$01
	lda     (ptr1),y
	tax
	dey
	lda     (ptr1),y
	sta     ptr1
	stx     ptr1+1
	ldx     #$00
	lda     _offset
	clc
	adc     #$01
	bcc     L1ADF
	inx
L1ADF:	stx     tmp1
	asl     a
	rol     tmp1
	clc
	adc     ptr1
	sta     ptr1
	lda     tmp1
	adc     ptr1+1
	sta     ptr1+1
	iny
	lda     (ptr1),y
	tax
	dey
	lda     (ptr1),y
	jsr     pushax
	ldx     #$00
	lda     #$F0
	jmp     _memcpy
;
; if (offset - 1 >= 0)
;
L1AD3:	lda     _offset
	sec
	sbc     #$01
	jsr     return1
	beq     L1AE2
;
; memcpy(c_map, stage_table[current_stage][offset - 1], 240);
;
	lda     #<(_c_map)
	ldx     #>(_c_map)
	jsr     pushax
	ldx     #$00
	lda     _current_stage
	asl     a
	bcc     L6E20
	inx
	clc
L6E20:	adc     #<(_stage_table)
	sta     ptr1
	txa
	adc     #>(_stage_table)
	sta     ptr1+1
	ldy     #$01
	lda     (ptr1),y
	tax
	dey
	lda     (ptr1),y
	sta     ptr1
	stx     ptr1+1
	ldx     #$00
	lda     _offset
	sec
	sbc     #$01
	bcs     L1AEA
	dex
L1AEA:	stx     tmp1
	asl     a
	rol     tmp1
	clc
	adc     ptr1
	sta     ptr1
	lda     tmp1
	adc     ptr1+1
	sta     ptr1+1
	iny
	lda     (ptr1),y
	tax
	dey
	lda     (ptr1),y
	jsr     pushax
	ldx     #$00
	lda     #$F0
	jmp     _memcpy
;
; }
;
L1AE2:	rts

.endproc

; ---------------------------------------------------------------
; void __near__ prep_scroll_screen (void)
; ---------------------------------------------------------------

.segment	"BANK2"

.proc	_prep_scroll_screen: near

.segment	"BANK2"

;
; temp1 = low_byte(scroll_x) + high_byte(Player1.x);
;
	ldx     #$00
	lda     _scroll_x
	clc
	adc     _Player1+1
	bcc     L6E24
	inx
L6E24:	sta     _temp1
	stx     _temp1+1
;
; if (temp1 > 0x98 && temp1 < 0xa4) // middle of the screen
;
	cmp     #$99
	txa
	sbc     #$00
	bcc     L1AF1
	lda     _temp1+1
	cmp     #$00
	bne     L1AF4
	lda     _temp1
	cmp     #$A4
L1AF4:	bcs     L1AF1
;
; map_loaded = 0;
;
	lda     #$00
	sta     _map_loaded
;
; temp2 = Player1.x; // store his x before we check the scrolling
;
L1AF1:	lda     _Player1+1
	sta     _temp2+1
	lda     _Player1
	sta     _temp2
;
; if (Player1.x < MAX_LEFT)
;
	ldx     _Player1+1
	cpx     #$50
	jcs     L1B23
;
; if (!map_loaded)
;
	lda     _map_loaded
	bne     L1AFC
;
; room_to_load = ((scroll_x >> 8) - 1); // high byte = room, one to the left
;
	lda     _scroll_x+1
	sec
	sbc     #$01
	sta     _room_to_load
;
; new_cmap();
;
	jsr     _new_cmap
;
; map_loaded = 1; // only do once
;
	lda     #$01
	sta     _map_loaded
;
; temp1 = (MAX_LEFT - Player1.x) >> 8;
;
L1AFC:	lda     #$00
	sec
	sbc     _Player1
	lda     #$50
	sbc     _Player1+1
	ldx     #$00
	sta     _temp1
	stx     _temp1+1
;
; if (temp1 > 3)
;
	cmp     #$04
	bcc     L1B09
;
; temp1 = 3; // max scroll change
;
	lda     #$03
	sta     _temp1
	stx     _temp1+1
;
; temp3 = scroll_x + high_byte(Player1.x);
;
L1B09:	lda     _scroll_x
	ldx     _scroll_x+1
	clc
	adc     _Player1+1
	bcc     L6E25
	inx
L6E25:	sta     _temp3
	stx     _temp3+1
;
; current_level = (temp3 >> 8);
;
	lda     _temp3+1
	sta     _current_level
;
; max_rooms = levels_per_stage[current_stage] - 1;
;
	ldy     _current_stage
	lda     _levels_per_stage,y
	sec
	sbc     #$01
	sta     _max_rooms
;
; max_scroll = (max_rooms * 0x100) - 1;
;
	tax
	lda     #$00
	sec
	sbc     #$01
	bcs     L1B1B
	dex
L1B1B:	sta     _max_scroll
	stx     _max_scroll+1
;
; if (max_rooms >= 1) // this is for the multi-room levels
;
	lda     _max_rooms
	beq     L1B23
;
; if ((scroll_x - temp1) > max_scroll) // if subtracting the scroll makes it overflow
;
	lda     _scroll_x
	sec
	sbc     _temp1
	pha
	lda     _scroll_x+1
	sbc     _temp1+1
	tax
	pla
	sec
	sbc     _max_scroll
	sta     tmp1
	txa
	sbc     _max_scroll+1
	ora     tmp1
	bcc     L1B1E
	beq     L1B1E
;
; scroll_x = 0; // just go to zero (and move the guy)
;
	lda     #$00
	sta     _scroll_x
	sta     _scroll_x+1
;
; else // otherwise scroll the window and offset the guy's movement
;
	jmp     L1B23
;
; scroll_x -= temp1;                  // scroll the window
;
L1B1E:	lda     _temp1
	eor     #$FF
	sec
	adc     _scroll_x
	sta     _scroll_x
	lda     _temp1+1
	eor     #$FF
	adc     _scroll_x+1
	sta     _scroll_x+1
;
; high_byte(Player1.x) = high_byte(Player1.x) + temp1; // add the offset to the guy
;
	lda     _Player1+1
	clc
	adc     _temp1
	sta     _Player1+1
	lda     #$00
	adc     _temp1+1
;
; if (Player1.x > MAX_RIGHT)
;
L1B23:	lda     _Player1
	cmp     #$01
	lda     _Player1+1
	sbc     #$90
	bcc     L1B3D
;
; if (!map_loaded) // gets reset whenever the player's in the middle of the level
;
	lda     _map_loaded
	bne     L1B2C
;
; room_to_load = ((scroll_x >> 8) + 1); // high byte = room, one to the left
;
	lda     _scroll_x+1
	clc
	adc     #$01
	sta     _room_to_load
;
; new_cmap();
;
	jsr     _new_cmap
;
; map_loaded = 1; // only do once
;
	lda     #$01
	sta     _map_loaded
;
; temp1 = (Player1.x - MAX_RIGHT) >> 8;
;
L1B2C:	lda     _Player1+1
	sec
	sbc     #$90
	ldx     #$00
	sta     _temp1
	stx     _temp1+1
;
; if (temp1 > 3)
;
	cmp     #$04
	bcc     L6E29
;
; temp1 = 3; // max scroll change
;
	lda     #$03
	sta     _temp1
	stx     _temp1+1
;
; if (max_rooms >= 1) // used for single room levels
;
L6E29:	lda     _max_rooms
	beq     L1B3D
;
; scroll_x += temp1;                  // scroll the window
;
	lda     _temp1
	clc
	adc     _scroll_x
	sta     _scroll_x
	lda     _temp1+1
	adc     _scroll_x+1
	sta     _scroll_x+1
;
; high_byte(Player1.x) = high_byte(Player1.x) - temp1; // sub the offet from the guy
;
	lda     _Player1+1
	sec
	sbc     _temp1
	sta     _Player1+1
	txa
	sbc     _temp1+1
;
; if (scroll_x >= max_scroll)
;
L1B3D:	lda     _scroll_x
	cmp     _max_scroll
	lda     _scroll_x+1
	sbc     _max_scroll+1
	bcc     L1B4B
;
; scroll_x = max_scroll;      // stop scrolling right, end of level
;
	lda     _max_scroll+1
	sta     _scroll_x+1
	lda     _max_scroll
	sta     _scroll_x
;
; Player1.x = temp2;        // but allow the x position to go all the way right
;
	lda     _temp2+1
	sta     _Player1+1
	lda     _temp2
	sta     _Player1
;
; if (high_byte(Player1.x) >= 0xe0) // but limit how far right he can go
;
	lda     _Player1+1
	cmp     #$E0
	bcc     L1B4B
;
; Player1.x = 0xe000;
;
	ldx     #$E0
	lda     #$00
	sta     _Player1
	stx     _Player1+1
;
; }
;
L1B4B:	rts

.endproc

; ---------------------------------------------------------------
; void __near__ handle_scrolling (void)
; ---------------------------------------------------------------

.segment	"BANK2"

.proc	_handle_scrolling: near

.segment	"BANK2"

;
; scrolling_direction = (Player1.vel_x >= 0) ? 0 : 1;
;
	ldx     _Player1+4+1
	bmi     L1B54
	lda     #$00
	jmp     L6E2A
L1B54:	lda     #$01
L6E2A:	sta     _scrolling_direction
;
; if (!r_scroll_frames && !l_scroll_frames)
;
	lda     _r_scroll_frames
	bne     L1B61
	lda     _l_scroll_frames
	bne     L1B61
;
; if (Player1.vel_x > 0)
;
	lda     _Player1+4
	cmp     #$01
	lda     _Player1+4+1
	sbc     #$00
	bvs     L1B5E
	eor     #$80
L1B5E:	bpl     L6E2D
;
; r_scroll_frames = 4;
;
	lda     #$04
	sta     _r_scroll_frames
;
; else
;
	jmp     L1B61
;
; l_scroll_frames = 4;
;
L6E2D:	lda     #$04
	sta     _l_scroll_frames
;
; if (r_scroll_frames)
;
L1B61:	lda     _r_scroll_frames
	beq     L1B64
;
; draw_screen_R();
;
	jsr     _draw_screen_R
;
; --r_scroll_frames;
;
	dec     _r_scroll_frames
;
; else if (l_scroll_frames)
;
	rts
L1B64:	lda     _l_scroll_frames
	beq     L1B69
;
; draw_screen_L();
;
	jsr     _draw_screen_L
;
; --l_scroll_frames;
;
	dec     _l_scroll_frames
;
; }
;
L1B69:	rts

.endproc

; ---------------------------------------------------------------
; void __near__ bank2_load_title (void)
; ---------------------------------------------------------------

.segment	"BANK2"

.proc	_bank2_load_title: near

.segment	"BANK2"

;
; ppu_off();
;
	jsr     _ppu_off
;
; set_data_pointer(titletiled_0);
;
	lda     #<(_titletiled_0)
	ldx     #>(_titletiled_0)
	jsr     _set_data_pointer
;
; set_mt_pointer(stage1_metatiles);
;
	lda     #<(_stage1_metatiles)
	ldx     #>(_stage1_metatiles)
	jsr     _set_mt_pointer
;
; for (y = 0;; y += 0x20)
;
	lda     #$00
L6E30:	sta     _y
;
; for (x = 0;; x += 0x20)
;
	lda     #$00
L6E2F:	sta     _x
;
; address = get_ppu_addr(0, x, y);
;
	jsr     decsp2
	lda     #$00
	ldy     #$01
	sta     (sp),y
	lda     _x
	dey
	sta     (sp),y
	lda     _y
	jsr     _get_ppu_addr
	sta     _address
	stx     _address+1
;
; index = (y & 0xf0) + (x >> 4);
;
	lda     _y
	and     #$F0
	sta     ptr1
	lda     _x
	lsr     a
	lsr     a
	lsr     a
	lsr     a
	clc
	adc     ptr1
	sta     _index
;
; buffer_4_mt(address, index); // ppu_address, index to the data
;
	lda     _address
	ldx     _address+1
	jsr     pushax
	lda     _index
	jsr     _buffer_4_mt
;
; flush_vram_update2();
;
	jsr     _flush_vram_update2
;
; if (x == 0xe0)
;
	lda     _x
	cmp     #$E0
;
; break;
;
	beq     L6E31
;
; for (x = 0;; x += 0x20)
;
	lda     #$20
	clc
	adc     _x
	jmp     L6E2F
;
; if (y == 0xe0)
;
L6E31:	lda     _y
	cmp     #$E0
;
; break;
;
	beq     L1C64
;
; for (y = 0;; y += 0x20)
;
	lda     #$20
	clc
	adc     _y
	jmp     L6E30
;
; ppu_on_all();
;
L1C64:	jsr     _ppu_on_all
;
; game_mode = MODE_TITLE;
;
	lda     #$00
	sta     _game_mode
;
; multi_vram_buffer_horz("NOAH VS. ATRAHASIS", 18, NTADR_A(12, 6));
;
	jsr     decsp3
	lda     #<(L1C88)
	ldy     #$01
	sta     (sp),y
	iny
	lda     #>(L1C88)
	sta     (sp),y
	lda     #$12
	ldy     #$00
	sta     (sp),y
	ldx     #$20
	lda     #$CC
	jsr     _multi_vram_buffer_horz
;
; multi_vram_buffer_horz("ONE ON ONE", 10, NTADR_A(14, 7));
;
	jsr     decsp3
	lda     #<(L1C92)
	ldy     #$01
	sta     (sp),y
	iny
	lda     #>(L1C92)
	sta     (sp),y
	lda     #$0A
	ldy     #$00
	sta     (sp),y
	ldx     #$20
	lda     #$EE
	jsr     _multi_vram_buffer_horz
;
; multi_vram_buffer_horz("BRIAN & ALAN GAMES", 18, NTADR_A(12, 10));
;
	jsr     decsp3
	lda     #<(L1C9C)
	ldy     #$01
	sta     (sp),y
	iny
	lda     #>(L1C9C)
	sta     (sp),y
	lda     #$12
	ldy     #$00
	sta     (sp),y
	ldx     #$21
	lda     #$4C
	jmp     _multi_vram_buffer_horz

.endproc

; ---------------------------------------------------------------
; void __near__ bank2_load_gameover (void)
; ---------------------------------------------------------------

.segment	"BANK2"

.proc	_bank2_load_gameover: near

.segment	"BANK2"

;
; ppu_off();
;
	jsr     _ppu_off
;
; set_data_pointer(gameovertiled_0);
;
	lda     #<(_gameovertiled_0)
	ldx     #>(_gameovertiled_0)
	jsr     _set_data_pointer
;
; set_mt_pointer(stage1_metatiles);
;
	lda     #<(_stage1_metatiles)
	ldx     #>(_stage1_metatiles)
	jsr     _set_mt_pointer
;
; for (y = 0;; y += 0x20)
;
	lda     #$00
L6E34:	sta     _y
;
; for (x = 0;; x += 0x20)
;
	lda     #$00
L6E33:	sta     _x
;
; address = get_ppu_addr(0, x, y);
;
	jsr     decsp2
	lda     #$00
	ldy     #$01
	sta     (sp),y
	lda     _x
	dey
	sta     (sp),y
	lda     _y
	jsr     _get_ppu_addr
	sta     _address
	stx     _address+1
;
; index = (y & 0xf0) + (x >> 4);
;
	lda     _y
	and     #$F0
	sta     ptr1
	lda     _x
	lsr     a
	lsr     a
	lsr     a
	lsr     a
	clc
	adc     ptr1
	sta     _index
;
; buffer_4_mt(address, index); // ppu_address, index to the data
;
	lda     _address
	ldx     _address+1
	jsr     pushax
	lda     _index
	jsr     _buffer_4_mt
;
; flush_vram_update2();
;
	jsr     _flush_vram_update2
;
; if (x == 0xe0)
;
	lda     _x
	cmp     #$E0
;
; break;
;
	beq     L6E35
;
; for (x = 0;; x += 0x20)
;
	lda     #$20
	clc
	adc     _x
	jmp     L6E33
;
; if (y == 0xe0)
;
L6E35:	lda     _y
	cmp     #$E0
;
; break;
;
	beq     L1D9C
;
; for (y = 0;; y += 0x20)
;
	lda     #$20
	clc
	adc     _y
	jmp     L6E34
;
; ppu_on_all();
;
L1D9C:	jsr     _ppu_on_all
;
; game_mode = MODE_GAMEOVER;
;
	lda     #$01
	sta     _game_mode
;
; }
;
	rts

.endproc

; ---------------------------------------------------------------
; void __near__ bank2_load_room (void)
; ---------------------------------------------------------------

.segment	"BANK2"

.proc	_bank2_load_room: near

.segment	"BANK2"

;
; if (current_stage == 0) {
;
	ldx     #$00
	lda     _current_stage
	bne     L6E4F
;
; set_chr_bank_0(CHR_STAGE_1_CHARACTER_CHR);
;
	jsr     _set_chr_bank_0
;
; set_chr_bank_1(CHR_STAGE_1_BACKGROUND_CHR);
;
	lda     #$01
;
; } else if (current_stage == 1) {
;
	jmp     L6E41
L6E4F:	lda     _current_stage
	cmp     #$01
	bne     L6E50
;
; set_chr_bank_0(CHR_STAGE_1_CHARACTER_CHR);
;
	txa
	jsr     _set_chr_bank_0
;
; set_chr_bank_1(CHR_STAGE_2_BACKGROUND_CHR);
;
	lda     #$03
;
; } else if (current_stage == 2) {
;
	jmp     L6E41
L6E50:	lda     _current_stage
	cmp     #$02
	bne     L6E51
;
; set_chr_bank_0(CHR_STAGE_1_CHARACTER_CHR);
;
	txa
	jsr     _set_chr_bank_0
;
; set_chr_bank_1(CHR_STAGE_3_BACKGROUND_CHR);
;
	lda     #$05
;
; } else if (current_stage == 3) {
;
	jmp     L6E41
L6E51:	lda     _current_stage
	cmp     #$03
	bne     L6E52
;
; set_chr_bank_0(CHR_STAGE_1_CHARACTER_CHR);
;
	txa
	jsr     _set_chr_bank_0
;
; set_chr_bank_1(CHR_STAGE_4_BACKGROUND_CHR);
;
	lda     #$07
;
; } else if (current_stage == 4) {
;
	jmp     L6E41
L6E52:	lda     _current_stage
	cmp     #$04
	bne     L6E53
;
; set_chr_bank_0(CHR_STAGE_1_CHARACTER_CHR);
;
	txa
	jsr     _set_chr_bank_0
;
; set_chr_bank_1(CHR_STAGE_5_BACKGROUND_CHR);
;
	lda     #$09
L6E41:	jsr     _set_chr_bank_1
;
; pal_bg(stage_bg_palettes[current_stage]);
;
	ldx     #$00
L6E53:	lda     _current_stage
	asl     a
	bcc     L6E46
	inx
	clc
L6E46:	adc     #<(_stage_bg_palettes)
	sta     ptr1
	txa
	adc     #>(_stage_bg_palettes)
	sta     ptr1+1
	ldy     #$01
	lda     (ptr1),y
	tax
	dey
	lda     (ptr1),y
	jsr     _pal_bg
;
; ppu_off(); 
;
	jsr     _ppu_off
;
; clear_vram_buffer();
;
	jsr     _clear_vram_buffer
;
; set_vram_buffer();
;
	jsr     _set_vram_buffer
;
; set_data_pointer(stage_table[current_stage][current_level]);
;
	ldx     #$00
	lda     _current_stage
	asl     a
	bcc     L6E47
	inx
	clc
L6E47:	adc     #<(_stage_table)
	sta     ptr1
	txa
	adc     #>(_stage_table)
	sta     ptr1+1
	ldy     #$01
	lda     (ptr1),y
	tax
	dey
	lda     (ptr1),y
	sta     ptr1
	stx     ptr1+1
	ldx     #$00
	lda     _current_level
	asl     a
	bcc     L6E48
	inx
	clc
L6E48:	adc     ptr1
	sta     ptr1
	txa
	adc     ptr1+1
	sta     ptr1+1
	iny
	lda     (ptr1),y
	tax
	dey
	lda     (ptr1),y
	jsr     _set_data_pointer
;
; set_mt_pointer(stage_metatiles[current_stage]);
;
	ldx     #$00
	lda     _current_stage
	asl     a
	bcc     L6E49
	inx
	clc
L6E49:	adc     #<(_stage_metatiles)
	sta     ptr1
	txa
	adc     #>(_stage_metatiles)
	sta     ptr1+1
	ldy     #$01
	lda     (ptr1),y
	tax
	dey
	lda     (ptr1),y
	jsr     _set_mt_pointer
;
; for (y = 0;; y += 0x20)
;
	lda     #$00
L6E43:	sta     _y
;
; for (x = 0;; x += 0x20)
;
	lda     #$00
L6E42:	sta     _x
;
; address = get_ppu_addr(nametable_to_load, x, y);
;
	jsr     decsp2
	lda     _nametable_to_load
	ldy     #$01
	sta     (sp),y
	lda     _x
	dey
	sta     (sp),y
	lda     _y
	jsr     _get_ppu_addr
	sta     _address
	stx     _address+1
;
; index = (y & 0xf0) + (x >> 4);
;
	lda     _y
	and     #$F0
	sta     ptr1
	lda     _x
	lsr     a
	lsr     a
	lsr     a
	lsr     a
	clc
	adc     ptr1
	sta     _index
;
; buffer_4_mt(address, index); // ppu_address, index to the data
;
	lda     _address
	ldx     _address+1
	jsr     pushax
	lda     _index
	jsr     _buffer_4_mt
;
; flush_vram_update2();
;
	jsr     _flush_vram_update2
;
; if (x == 0xe0)
;
	ldx     #$00
	lda     _x
	cmp     #$E0
;
; break;
;
	beq     L6E54
;
; for (x = 0;; x += 0x20)
;
	lda     #$20
	clc
	adc     _x
	jmp     L6E42
;
; if (y == 0xe0)
;
L6E54:	lda     _y
	cmp     #$E0
;
; break;
;
	beq     L6E55
;
; for (y = 0;; y += 0x20)
;
	lda     #$20
	clc
	adc     _y
	jmp     L6E43
;
; set_data_pointer(stage_table[current_stage][current_level+1]);
;
L6E55:	lda     _current_stage
	asl     a
	bcc     L6E4A
	inx
	clc
L6E4A:	adc     #<(_stage_table)
	sta     ptr1
	txa
	adc     #>(_stage_table)
	sta     ptr1+1
	ldy     #$01
	lda     (ptr1),y
	tax
	dey
	lda     (ptr1),y
	sta     ptr1
	stx     ptr1+1
	ldx     #$00
	lda     _current_level
	clc
	adc     #$01
	bcc     L1E14
	inx
L1E14:	stx     tmp1
	asl     a
	rol     tmp1
	clc
	adc     ptr1
	sta     ptr1
	lda     tmp1
	adc     ptr1+1
	sta     ptr1+1
	iny
	lda     (ptr1),y
	tax
	dey
	lda     (ptr1),y
	jsr     _set_data_pointer
;
; for (y = 0;; y += 0x20)
;
	lda     #$00
L6E44:	sta     _y
;
; x = 0;
;
	lda     #$00
	sta     _x
;
; nt = (nametable_to_load + 1) % 2;
;
	lda     _nametable_to_load
	clc
	adc     #$01
	and     #$01
	sta     _nt
;
; address = get_ppu_addr(1, x, y);
;
	jsr     decsp2
	lda     #$01
	tay
	sta     (sp),y
	lda     _x
	dey
	sta     (sp),y
	lda     _y
	jsr     _get_ppu_addr
	sta     _address
	stx     _address+1
;
; index = (y & 0xf0);
;
	lda     _y
	and     #$F0
	sta     _index
;
; buffer_4_mt(address, index); // ppu_address, index to the data
;
	lda     _address
	ldx     _address+1
	jsr     pushax
	lda     _index
	jsr     _buffer_4_mt
;
; flush_vram_update2();
;
	jsr     _flush_vram_update2
;
; if (y == 0xe0)
;
	lda     _y
	cmp     #$E0
;
; break;
;
	beq     L6E56
;
; for (y = 0;; y += 0x20)
;
	lda     #$20
	clc
	adc     _y
	jmp     L6E44
;
; if (current_level > 0) {
;
L6E56:	lda     _current_level
	jeq     L1E39
;
; set_data_pointer(stage_table[current_stage][current_level-1]);
;
	ldx     #$00
	lda     _current_stage
	asl     a
	bcc     L6E4B
	inx
	clc
L6E4B:	adc     #<(_stage_table)
	sta     ptr1
	txa
	adc     #>(_stage_table)
	sta     ptr1+1
	ldy     #$01
	lda     (ptr1),y
	tax
	dey
	lda     (ptr1),y
	sta     ptr1
	stx     ptr1+1
	ldx     #$00
	lda     _current_level
	sec
	sbc     #$01
	bcs     L1E37
	dex
L1E37:	stx     tmp1
	asl     a
	rol     tmp1
	clc
	adc     ptr1
	sta     ptr1
	lda     tmp1
	adc     ptr1+1
	sta     ptr1+1
	iny
	lda     (ptr1),y
	tax
	dey
	lda     (ptr1),y
	jsr     _set_data_pointer
;
; for (y = 0;; y += 0x20)
;
	lda     #$00
L6E45:	sta     _y
;
; x = 240;
;
	lda     #$F0
	sta     _x
;
; nt = nametable_to_load;
;
	lda     _nametable_to_load
	sta     _nt
;
; address = get_ppu_addr(nt, x, y);
;
	jsr     decsp2
	lda     _nt
	ldy     #$01
	sta     (sp),y
	lda     _x
	dey
	sta     (sp),y
	lda     _y
	jsr     _get_ppu_addr
	sta     _address
	stx     _address+1
;
; index = y + (x >> 4);
;
	lda     _x
	lsr     a
	lsr     a
	lsr     a
	lsr     a
	clc
	adc     _y
	sta     _index
;
; buffer_4_mt(address, index); // ppu_address, index to the data
;
	lda     _address
	ldx     _address+1
	jsr     pushax
	lda     _index
	jsr     _buffer_4_mt
;
; flush_vram_update2();
;
	jsr     _flush_vram_update2
;
; if (y == 0xe0)
;
	lda     _y
	cmp     #$E0
;
; break;
;
	beq     L1E39
;
; for (y = 0;; y += 0x20)
;
	lda     #$20
	clc
	adc     _y
	jmp     L6E45
;
; memcpy(c_map, stage_table[current_stage][current_level], 240); 
;
L1E39:	lda     #<(_c_map)
	ldx     #>(_c_map)
	jsr     pushax
	ldx     #$00
	lda     _current_stage
	asl     a
	bcc     L6E4C
	inx
	clc
L6E4C:	adc     #<(_stage_table)
	sta     ptr1
	txa
	adc     #>(_stage_table)
	sta     ptr1+1
	ldy     #$01
	lda     (ptr1),y
	tax
	dey
	lda     (ptr1),y
	sta     ptr1
	stx     ptr1+1
	ldx     #$00
	lda     _current_level
	asl     a
	bcc     L6E4D
	inx
	clc
L6E4D:	adc     ptr1
	sta     ptr1
	txa
	adc     ptr1+1
	sta     ptr1+1
	iny
	lda     (ptr1),y
	tax
	dey
	lda     (ptr1),y
	jsr     pushax
	ldx     #$00
	lda     #$F0
	jsr     _memcpy
;
; if (current_level + 1 < levels_per_stage[current_stage]) {
;
	ldx     #$00
	lda     _current_level
	clc
	adc     #$01
	bcc     L1E5A
	inx
L1E5A:	jsr     pushax
	ldy     _current_stage
	lda     _levels_per_stage,y
	jsr     tosicmp0
	bcs     L1E58
;
; memcpy(c_map2, stage_table[current_stage][current_level + 1], 240);
;
	lda     #<(_c_map2)
	ldx     #>(_c_map2)
	jsr     pushax
	ldx     #$00
	lda     _current_stage
	asl     a
	bcc     L6E4E
	inx
	clc
L6E4E:	adc     #<(_stage_table)
	sta     ptr1
	txa
	adc     #>(_stage_table)
	sta     ptr1+1
	ldy     #$01
	lda     (ptr1),y
	tax
	dey
	lda     (ptr1),y
	sta     ptr1
	stx     ptr1+1
	ldx     #$00
	lda     _current_level
	clc
	adc     #$01
	bcc     L1E62
	inx
L1E62:	stx     tmp1
	asl     a
	rol     tmp1
	clc
	adc     ptr1
	sta     ptr1
	lda     tmp1
	adc     ptr1+1
	sta     ptr1+1
	iny
	lda     (ptr1),y
	tax
	dey
	lda     (ptr1),y
	jsr     pushax
	ldx     #$00
	lda     #$F0
	jsr     _memcpy
;
; map_loaded = 1;
;
L1E58:	lda     #$01
	sta     _map_loaded
;
; ppu_on_all();
;
	jmp     _ppu_on_all

.endproc

; ---------------------------------------------------------------
; void __near__ bank2_scroll_screen (void)
; ---------------------------------------------------------------

.segment	"BANK2"

.proc	_bank2_scroll_screen: near

.segment	"BANK2"

;
; prep_scroll_screen();
;
	jsr     _prep_scroll_screen
;
; set_scroll_x(scroll_x);
;
	lda     _scroll_x
	ldx     _scroll_x+1
	jsr     _set_scroll_x
;
; set_scroll_y(scroll_y);
;
	lda     _scroll_y
	ldx     _scroll_y+1
	jsr     _set_scroll_y
;
; handle_scrolling();
;
	jmp     _handle_scrolling

.endproc

; ---------------------------------------------------------------
; void __near__ function_bank3 (void)
; ---------------------------------------------------------------

.segment	"BANK3"

.proc	_function_bank3: near

.segment	"BANK3"

;
; }
;
	rts

.endproc

; ---------------------------------------------------------------
; void __near__ function_bank4 (void)
; ---------------------------------------------------------------

.segment	"BANK4"

.proc	_function_bank4: near

.segment	"BANK4"

;
; }
;
	rts

.endproc

; ---------------------------------------------------------------
; void __near__ function_bank5 (void)
; ---------------------------------------------------------------

.segment	"BANK5"

.proc	_function_bank5: near

.segment	"BANK5"

;
; }
;
	rts

.endproc

; ---------------------------------------------------------------
; void __near__ function_bank6 (void)
; ---------------------------------------------------------------

.segment	"BANK6"

.proc	_function_bank6: near

.segment	"BANK6"

;
; }
;
	rts

.endproc

; ---------------------------------------------------------------
; void __near__ load_level_select (void)
; ---------------------------------------------------------------

.segment	"CODE"

.proc	_load_level_select: near

.segment	"CODE"

;
; ppu_off();
;
	jsr     _ppu_off
;
; clear_vram_buffer();
;
	jsr     _clear_vram_buffer
;
; oam_clear();
;
	jsr     _oam_clear
;
; vram_adr(NAMETABLE_A);
;
	ldx     #$20
	lda     #$00
	jsr     _vram_adr
;
; for (temp_x = 0; temp_x < 1024; ++temp_x)
;
	lda     #$00
	sta     _temp_x
	sta     _temp_x+1
L6C54:	ldx     _temp_x+1
	cpx     #$04
	bcs     L6C55
;
; vram_put(0x00);
;
	lda     #$00
	jsr     _vram_put
;
; flush_vram_update2();  
;
	jsr     _flush_vram_update2
;
; for (temp_x = 0; temp_x < 1024; ++temp_x)
;
	inc     _temp_x
	bne     L6C54
	inc     _temp_x+1
	jmp     L6C54
;
; multi_jump_max = 1;
;
L6C55:	lda     #$01
	sta     _multi_jump_max
;
; multi_jump = 0;
;
	lda     #$00
	sta     _multi_jump
;
; short_jump_count = 0;
;
	sta     _short_jump_count
;
; falling_down = 0;
;
	sta     _falling_down
;
; player_in_air = 0;
;
	sta     _player_in_air
;
; projectile_cooldown = 0;
;
	sta     _projectile_cooldown
;
; direction = RIGHT;
;
	lda     #$01
	sta     _direction
;
; current_animation_frame = 0;
;
	lda     #$00
	sta     _current_animation_frame
;
; animation_frame_counter = 0;
;
	sta     _animation_frame_counter
;
; current_animation_ptr = animate_playerstandright_data;
;
	lda     #>(_animate_playerstandright_data)
	sta     _current_animation_ptr+1
	lda     #<(_animate_playerstandright_data)
	sta     _current_animation_ptr
;
; multi_vram_buffer_horz("SELECT STAGE", 12, NTADR_A(8, 6));
;
	jsr     decsp3
	lda     #<(L6C75)
	ldy     #$01
	sta     (sp),y
	iny
	lda     #>(L6C75)
	sta     (sp),y
	lda     #$0C
	ldy     #$00
	sta     (sp),y
	ldx     #$20
	lda     #$C8
	jsr     _multi_vram_buffer_horz
;
; update_level_select_display();
;
	jsr     _update_level_select_display
;
; ppu_on_all();
;
	jmp     _ppu_on_all

.endproc

; ---------------------------------------------------------------
; void __near__ update_level_select_display (void)
; ---------------------------------------------------------------

.segment	"CODE"

.proc	_update_level_select_display: near

.segment	"CODE"

;
; ppu_off();
;
	jsr     _ppu_off
;
; clear_vram_buffer();
;
	jsr     _clear_vram_buffer
;
; multi_vram_buffer_horz("SELECT STAGE", 12, NTADR_A(8, 4));
;
	jsr     decsp3
	lda     #<(L6C84)
	ldy     #$01
	sta     (sp),y
	iny
	lda     #>(L6C84)
	sta     (sp),y
	lda     #$0C
	ldy     #$00
	sta     (sp),y
	ldx     #$20
	lda     #$88
	jsr     _multi_vram_buffer_horz
;
; if (selected_stage == 0)
;
	lda     _selected_stage
	bne     L6C8D
;
; multi_vram_buffer_horz("X  BEAR", 7, NTADR_A(8, 8));
;
	jsr     decsp3
	lda     #<(L6C90)
	ldy     #$01
	sta     (sp),y
	iny
	lda     #>(L6C90)
;
; else
;
	jmp     L6E75
;
; multi_vram_buffer_horz("   BEAR", 7, NTADR_A(8, 8));
;
L6C8D:	jsr     decsp3
	lda     #<(L6C9B)
	ldy     #$01
	sta     (sp),y
	iny
	lda     #>(L6C9B)
L6E75:	sta     (sp),y
	lda     #$07
	ldy     #$00
	sta     (sp),y
	ldx     #$21
	lda     #$08
	jsr     _multi_vram_buffer_horz
;
; if (selected_stage == 1)
;
	lda     _selected_stage
	cmp     #$01
	bne     L6CA4
;
; multi_vram_buffer_horz("X  GIRAFFE", 10, NTADR_A(8, 10));
;
	jsr     decsp3
	lda     #<(L6CA7)
	ldy     #$01
	sta     (sp),y
	iny
	lda     #>(L6CA7)
;
; else
;
	jmp     L6E76
;
; multi_vram_buffer_horz("   GIRAFFE", 10, NTADR_A(8, 10));
;
L6CA4:	jsr     decsp3
	lda     #<(L6CB2)
	ldy     #$01
	sta     (sp),y
	iny
	lda     #>(L6CB2)
L6E76:	sta     (sp),y
	lda     #$0A
	ldy     #$00
	sta     (sp),y
	ldx     #$21
	lda     #$48
	jsr     _multi_vram_buffer_horz
;
; if (selected_stage == 2)
;
	lda     _selected_stage
	cmp     #$02
	bne     L6CBB
;
; multi_vram_buffer_horz("X  WALRUS", 9, NTADR_A(8, 12));
;
	jsr     decsp3
	lda     #<(L6CBE)
	ldy     #$01
	sta     (sp),y
	iny
	lda     #>(L6CBE)
;
; else
;
	jmp     L6E77
;
; multi_vram_buffer_horz("   WALRUS", 9, NTADR_A(8, 12));
;
L6CBB:	jsr     decsp3
	lda     #<(L6CC9)
	ldy     #$01
	sta     (sp),y
	iny
	lda     #>(L6CC9)
L6E77:	sta     (sp),y
	lda     #$09
	ldy     #$00
	sta     (sp),y
	ldx     #$21
	lda     #$88
	jsr     _multi_vram_buffer_horz
;
; if (selected_stage == 3)
;
	lda     _selected_stage
	cmp     #$03
	bne     L6CD2
;
; multi_vram_buffer_horz("X  CAMEL", 8, NTADR_A(8, 14));
;
	jsr     decsp3
	lda     #<(L6CD5)
	ldy     #$01
	sta     (sp),y
	iny
	lda     #>(L6CD5)
;
; else
;
	jmp     L6E78
;
; multi_vram_buffer_horz("   CAMEL", 8, NTADR_A(8, 14));
;
L6CD2:	jsr     decsp3
	lda     #<(L6CE0)
	ldy     #$01
	sta     (sp),y
	iny
	lda     #>(L6CE0)
L6E78:	sta     (sp),y
	lda     #$08
	ldy     #$00
	sta     (sp),y
	ldx     #$21
	lda     #$C8
	jsr     _multi_vram_buffer_horz
;
; if (selected_stage == 4)
;
	lda     _selected_stage
	cmp     #$04
	bne     L6CE9
;
; multi_vram_buffer_horz("X  LEVIATHAN", 12, NTADR_A(8, 16));
;
	jsr     decsp3
	lda     #<(L6CEC)
	ldy     #$01
	sta     (sp),y
	iny
	lda     #>(L6CEC)
;
; else
;
	jmp     L6E79
;
; multi_vram_buffer_horz("   LEVIATHAN", 12, NTADR_A(8, 16));
;
L6CE9:	jsr     decsp3
	lda     #<(L6CF7)
	ldy     #$01
	sta     (sp),y
	iny
	lda     #>(L6CF7)
L6E79:	sta     (sp),y
	lda     #$0C
	ldy     #$00
	sta     (sp),y
	ldx     #$22
	lda     #$08
	jsr     _multi_vram_buffer_horz
;
; flush_vram_update2();
;
	jsr     _flush_vram_update2
;
; ppu_on_all();
;
	jmp     _ppu_on_all

.endproc

; ---------------------------------------------------------------
; void __near__ main (void)
; ---------------------------------------------------------------

.segment	"CODE"

.proc	_main: near

.segment	"CODE"

;
; bank_spr(0);
;
	lda     #$00
	jsr     _bank_spr
;
; bank_bg(1);
;
	lda     #$01
	jsr     _bank_bg
;
; ppu_off();    // screen off
;
	jsr     _ppu_off
;
; pal_bg(palette_bg); // load the BG palette
;
	lda     #<(_palette_bg)
	ldx     #>(_palette_bg)
	jsr     _pal_bg
;
; pal_spr(palette_sp); // load the sprite palette
;
	lda     #<(_palette_sp)
	ldx     #>(_palette_sp)
	jsr     _pal_spr
;
; set_vram_buffer(); // do at least once 
;
	jsr     _set_vram_buffer
;
; ppu_wait_nmi();
;
	jsr     _ppu_wait_nmi
;
; set_mirroring(MIRROR_VERTICAL);
;
	lda     #$02
	jsr     _set_mirroring
;
; nametable_to_load = 0;
;
	lda     #$00
	sta     _nametable_to_load
;
; current_level = 0;
;
	sta     _current_level
;
; current_stage = 0;
;
	sta     _current_stage
;
; selected_stage = 0;
;
	sta     _selected_stage
;
; scroll_x = 0;
;
	sta     _scroll_x
	sta     _scroll_x+1
;
; scroll_y = 0;
;
	sta     _scroll_y
	sta     _scroll_y+1
;
; game_mode = MODE_TITLE;
;
	sta     _game_mode
;
; banked_call(BANK_2, bank2_load_title);
;
	lda     #$02
	jsr     pusha
	lda     #<(_bank2_load_title)
	ldx     #>(_bank2_load_title)
	jsr     _banked_call
;
; ppu_on_all();   // turn on screen
;
	jsr     _ppu_on_all
;
; while (game_mode == MODE_TITLE)
;
	jmp     L6E7A
;
; ppu_wait_nmi();
;
L6D26:	jsr     _ppu_wait_nmi
;
; pad1 = pad_poll(0);
;
	lda     #$00
	jsr     _pad_poll
	sta     _pad1
;
; pad1_new = get_pad_new(0);
;
	lda     #$00
	jsr     _get_pad_new
	sta     _pad1_new
;
; if (pad1_new & PAD_START)
;
	and     #$10
	beq     L6E7A
;
; game_mode = MODE_LEVEL_SELECT;
;
	lda     #$02
	sta     _game_mode
;
; load_level_select();
;
	jsr     _load_level_select
;
; while (game_mode == MODE_TITLE)
;
L6E7A:	lda     _game_mode
	beq     L6D26
;
; while (game_mode == MODE_LEVEL_SELECT)
;
	jmp     L6E7D
;
; ppu_wait_nmi();
;
L6D36:	jsr     _ppu_wait_nmi
;
; pad1 = pad_poll(0);
;
	lda     #$00
	jsr     _pad_poll
	sta     _pad1
;
; pad1_new = get_pad_new(0);
;
	lda     #$00
	jsr     _get_pad_new
	sta     _pad1_new
;
; if (pad1_new & PAD_UP)
;
	and     #$08
	beq     L6E7B
;
; if (selected_stage > 0)
;
	lda     _selected_stage
	beq     L6D43
;
; selected_stage--;
;
	dec     _selected_stage
;
; update_level_select_display();
;
L6D43:	jsr     _update_level_select_display
;
; if (pad1_new & PAD_DOWN)
;
L6E7B:	lda     _pad1_new
	and     #$04
	beq     L6E7C
;
; if (selected_stage < 4)
;
	lda     _selected_stage
	cmp     #$04
	bcs     L6D49
;
; selected_stage++;
;
	inc     _selected_stage
;
; update_level_select_display();
;
L6D49:	jsr     _update_level_select_display
;
; if (pad1_new & PAD_START)
;
L6E7C:	lda     _pad1_new
	and     #$10
	beq     L6E7D
;
; current_stage = selected_stage;
;
	lda     _selected_stage
	sta     _current_stage
;
; current_level = 0;
;
	lda     #$00
	sta     _current_level
;
; game_mode = MODE_GAME;
;
	lda     #$03
	sta     _game_mode
;
; banked_call(BANK_2, bank2_load_room);
;
	lda     #$02
	jsr     pusha
	lda     #<(_bank2_load_room)
	ldx     #>(_bank2_load_room)
	jsr     _banked_call
;
; while (game_mode == MODE_LEVEL_SELECT)
;
L6E7D:	lda     _game_mode
	cmp     #$02
	beq     L6D36
;
; while (game_mode == MODE_GAME)
;
	jmp     L6E7E
;
; ppu_wait_nmi();      
;
L6D58:	jsr     _ppu_wait_nmi
;
; pad1 = pad_poll(0);  
;
	lda     #$00
	jsr     _pad_poll
	sta     _pad1
;
; pad1_new = get_pad_new(0);
;
	lda     #$00
	jsr     _get_pad_new
	sta     _pad1_new
;
; pad1_state = pad1;
;
	lda     _pad1
	sta     _pad1_state
;
; if (pad1_new & PAD_START)   
;
	lda     _pad1_new
	ldx     #$00
	and     #$10
	stx     tmp1
	ora     tmp1
;
; banked_call(BANK_0, bank0_player_movement);
;
	txa
	jsr     pusha
	lda     #<(_bank0_player_movement)
	ldx     #>(_bank0_player_movement)
	jsr     _banked_call
;
; banked_call(BANK_1, projectile_movement);
;
	lda     #$01
	jsr     pusha
	lda     #<(_projectile_movement)
	ldx     #>(_projectile_movement)
	jsr     _banked_call
;
; banked_call(BANK_2, bank2_scroll_screen);  
;
	lda     #$02
	jsr     pusha
	lda     #<(_bank2_scroll_screen)
	ldx     #>(_bank2_scroll_screen)
	jsr     _banked_call
;
; oam_clear(); 
;
	jsr     _oam_clear
;
; banked_call(BANK_0, bank0_draw_player_sprite);
;
	lda     #$00
	jsr     pusha
	lda     #<(_bank0_draw_player_sprite)
	ldx     #>(_bank0_draw_player_sprite)
	jsr     _banked_call
;
; while (game_mode == MODE_GAME)
;
L6E7E:	lda     _game_mode
	cmp     #$03
	beq     L6D58
;
; while (1)      
;
	jmp     L6E7A

.endproc

